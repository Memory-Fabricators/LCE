...
namespace
{
// Portable row-major 4x4 matrix helpers (Cramer's rule inverse), matching
// the row-major/translation-in-row-3 convention the DirectXMath branch below
// relies on, so behaviour is unchanged on platforms that don't have XMMATRIX.
...
#ifdef _SDL3
    float _modelview[16], _proj[16], _final[16], _invert[16];

    memcpy(_modelview, modelview->_getDataPointer(), 64);
    memcpy(_proj, projection->_getDataPointer(), 64);

    Mat4Multiply(_modelview, _proj, _final);
    Mat4Inverse(_final, _invert);

    xPlayerOffs = _invert[12] / _invert[15];
    yPlayerOffs = _invert[13] / _invert[15];
    zPlayerOffs = _invert[14] / _invert[15];
#else
