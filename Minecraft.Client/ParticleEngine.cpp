...
void ParticleEngine::add(shared_ptr<Particle> p)
{
    ...
    if ((t != DRAGON_BREATH_TEXTURE && particles[l][t].size() >= MAX_PARTICLES_PER_LAYER) || particles[l][t].size() >= MAX_DRAGON_BREATH_PARTICLES)
    {
        particles[l][t].pop_front(); // particles[l][t].erase(particles[l][t].begin());
    }
