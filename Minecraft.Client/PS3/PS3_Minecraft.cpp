...
int main()
{
    ...
    // Sony request that we have enough space for a save game, so people don't get
    // hit with this when they first save. Going to make the space 20MB
    ...
    {
    case e_sku_SCEE:
        // 4J-PB - need to be online to do this check, so let's stick with the 7+,
        // and move this
        ...
        if (StorageManager.GetBootTypeDisc())
        {
            ...
            ProfileManager.SetAustraliaMinimumAge(
                8); // PG rating has no age, but for some reason the testers are
                    // saying it's 8
    ...
    // and the number of actions Need to init Input before UI so we know what the
    // Circle/Cross state is
...
#pragma message(__LOC__ "Need to define the _FINAL_BUILD before submission")
    ...
    // 4J-PB - we really need to know at this point if we are playing the trial or
    // full game, so sleep until we know
    ...
    // 4J-PB - get the chat/UGC restriction flag here so we can stop online games,
    // and content rating and restriction It's ok to do this for the primary PSN
    // player here - it has this data locally. All other players need to do it on
    // PSN sign in.

    // 	bool bChatRestricted=false;
    // 	ProfileManager.GetChatAndContentRestrictions(0,&bChatRestricted,NULL,NULL);

    // 4J-PB - really want to wait until we've read the options, so we can set the
    // right language if they've chosen one other than the default
