...
class DmaData_SPU
{
  public:
    ...
    static void get(void *dest, uintptr_t ea, unsigned int dmaSize)
    {
        ...
        spu_assert((uint32_t(dest) % 0x10) ==
                   0); //  make sure we're 16 byte aligned
        spu_assert((dmaSize % 0x10) ==
                   0);               //  and that the transfer is a multiple of 16 bytes
        spu_assert(ea > 256 * 1024); //  and that we're not targetting SPU memory
        // start memory transfer
        if (sc_verbose)
        {
            spu_print("DMA PPU->SPU start: 0x%08x -> 0x%08x : size : %d bytes : tag "
                      "%d .... ",
                      (unsigned int)ea, (unsigned int)dest, dmaSize,
                      g_pSpursJobContext->dmaTag);
        }
    ...
    static void getAndWaitUnaligned(void *dest, uintptr_t ea,
    ...
    static void put(void *src, uintptr_t ea, unsigned int dmaSize)
    {
        if (sc_verbose)
        {
            spu_print("DMA SPU->PPU  start: 0x%08x -> 0x%08x : size : %d bytes .... ",
    ...
    static void putAndWaitUnaligned(void *src, uintptr_t ea,
                                    unsigned int dmaSize)
    {
        if (sc_verbose)
        {
