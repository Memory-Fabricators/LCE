...
class CompressedTileStorage_SPU
{
  ...
  private:
    ...
    static const unsigned int MM_PHYSICAL_4KB_BASE =
        0xE0000000; // Start of where 4KB page sized physical allocations are made
  public:
...
class TileCompressData_SPU
{
  ...
  public:
    ...
    int64_t
        m_lowerSkyLight[9]; // Contains packed-together data pointer (lower
                            // 32-bits), and count of lines used (upper 32-bits)
    int64_t
        m_lowerBlockLight[9]; // Contains packed-together data pointer (lower
                              // 32-bits), and count of lines used (upper 32-bits)
    int64_t
        m_upperSkyLight[9]; // Contains packed-together data pointer (lower
                            // 32-bits), and count of lines used (upper 32-bits)
    int64_t
        m_upperBlockLight[9]; // Contains packed-together data pointer (lower
                              // 32-bits), and count of lines used (upper 32-bits)

    int64_t m_lowerData[9]; // Contains packed-together data pointer (lower
                            // 32-bits), and count of lines used (upper 32-bits)
    int64_t m_upperData[9]; // Contains packed-together data pointer (lower
                            // 32-bits), and count of lines used (upper 32-bits)
    ...
    void uncompressLights(int x0, int z0, int x1, int z1, bool upper,
                          bool skyLight);
