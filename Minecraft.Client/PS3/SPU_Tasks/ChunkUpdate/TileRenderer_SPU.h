...
class TileRenderer_SPU
{
  ...
  public:
    ...
    void tesselateInWorldNoCulling(
        Tile_SPU *tile, int x, int y, int z, int forceData = -1,
        TileEntity *forceEntity = NULL); // 4J added forceData, forceEntity param
    bool tesselateInWorld(
        Tile_SPU *tt, int x, int y, int z, int forceData = -1,
        TileEntity *forceEntity = NULL); // 4J added forceData, forceEntity param
  ...
  private:
    ...
    bool tesselateBrewingStandInWorld(BrewingStandTile_SPU *tt, int x, int y,
                                      int z);
  ...
  private:
    bool tesselateAnvilInWorld(AnvilTile_SPU *tt, int x, int y, int z, int data,
                               bool render);
    float tesselateAnvilPiece(AnvilTile_SPU *tt, int x, int y, int z, int part,
  ...
  public:
    void
    tesselatePistonBaseForceExtended(Tile_SPU *tile, int x, int y, int z,
                                     int forceData = -1); // 4J added data param
  private:
    bool tesselatePistonBaseInWorld(Tile_SPU *tt, int x, int y, int z,
                                    bool forceExtended,
                                    int forceData = -1); // 4J added data param
    ...
    void renderPistonArmEastWest(float x0, float x1, float y0, float y1, float z0,
                                 float z1, float br, float armLengthPixels);

  public:
    void tesselatePistonArmNoCulling(Tile_SPU *tile, int x, int y, int z,
                                     bool fullArm,
                                     int forceData = -1); // 4J added data param
  private:
    bool
    tesselatePistonExtensionInWorld(Tile_SPU *tt, int x, int y, int z,
                                    bool fullArm,
                                    int forceData = -1); // 4J added data param
  public:
    ...
    void tesselateTorch(Tile_SPU *tt, float x, float y, float z, float xxa,
                        float zza, int data);
    void tesselateCrossTexture(Tile_SPU *tt, int data, float x, float y, float z,
                               float scale);
    void tesselateStemTexture(Tile_SPU *tt, int data, float h, float x, float y,
                              float z);
    bool tesselateLilypadInWorld(WaterlilyTile_SPU *tt, int x, int y, int z);
    void tesselateStemDirTexture(StemTile_SPU *tt, int data, int dir, float h,
  ...
  public:
    ...
    bool tesselateBlockInWorldWithAmbienceOcclusionTexLighting(
        ...
        float pBaseBlue);
    bool tesselateBlockInWorldWithAmbienceOcclusionOldLighting(
        Tile_SPU *tt, int pX, int pY, int pZ, float pBaseRed, float pBaseGreen,
        float pBaseBlue);
  ...
  public:
    bool tesselateBlockInWorld(Tile_SPU *tt, int x, int y, int z, float r,
                               float g, float b);
    bool tesselateCactusInWorld(Tile_SPU *tt, int x, int y, int z);
    bool tesselateCactusInWorld(Tile_SPU *tt, int x, int y, int z, float r,
                                float g, float b);
    ...
    // 	void renderTile( Tile_SPU* tile, int data, float brightness, float
    // fAlpha = 1.0f );
    static bool canRender(int renderShape);
    Icon_SPU *getTexture(Tile_SPU *tile, ChunkRebuildData *level, int x, int y,
                         int z, int face);
