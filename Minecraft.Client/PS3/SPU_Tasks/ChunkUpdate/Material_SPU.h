...
class Material_SPU
{
  ...
  public:
    ...
    bool isSolid()
    {
        return (flags & e_isSolid) != 0;
    }
    bool blocksLight()
    {
        return (flags & e_blocksLight) != 0;
    }
    bool blocksMotion()
    {
        return (flags & e_blocksMotion) != 0;
    }
    bool isFlammable()
    {
        return (flags & e_flammable) != 0;
    }
    bool isReplaceable()
    {
        return (flags & e_replaceable) != 0;
    }
    bool isSolidBlocking()
    {
        if (flags & e_neverBuildable)
        {
            return false;
        }
        return blocksMotion();
