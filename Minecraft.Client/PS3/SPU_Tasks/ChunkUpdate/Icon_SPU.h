...
class Icon_SPU
{
  ...
  public:
    ...
    // 	int getWidth() const		{ return (w<0) ?  -w : w; }	// can
    // be negative, to support flipped icons (only doors for now). 	int
    // getHeight() const		{ return h; }

    void set(int16_t _x, int16_t _y, int16_t _w, int16_t _h, int texWidth,
             int texHeight)
    {
    ...
    float getU0() const
    {
        return (float(x0) / 4096) + UVAdjust;
    } // sc_texWidth) + getUAdjust(); }
    float getU1() const
    {
        return (float(x1) / 4096.0f) - UVAdjust;
    } // sc_texWidth) - getUAdjust(); }
    float getU(double offset) const
    {
        float diff = getU1() - getU0();
        return getU0() + (diff * ((float)offset /
                                  16)); // SharedConstants::WORLD_RESOLUTION));
    }

    float getV0() const
    {
        return (float(y0) / 4096.0f) + UVAdjust;
    } // sc_texHeight) + getVAdjust(); }
    float getV1() const
    {
        return (float(y1) / 4096.0f) - UVAdjust;
    } // sc_texHeight) - getVAdjust(); }
    float getV(double offset) const
    {
        float diff = getV1() - getV0();
        return getV0() + (diff * ((float)offset /
                                  16)); // SharedConstants::WORLD_RESOLUTION));
    }
