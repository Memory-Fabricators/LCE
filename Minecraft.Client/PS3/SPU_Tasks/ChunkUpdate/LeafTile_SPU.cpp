...
// L"leaves", L"leaves_jungle"}, {L"leaves_opaque", L"leaves_spruce_opaque",
// L"leaves_opaque", L"leaves_jungle_opaque"},};

// from TransparentTile, since we're no longer inheriting
bool LeafTile_SPU::shouldRenderFace(ChunkRebuildData *level, int x, int y,
                                    int z, int face)
{
                           ...
                           int data)
{
    ...
    {
        return ms_pTileData
            ->foliageColor_evergreenColor; // FoliageColor::getEvergreenColor();
    }
    if ((data & LEAF_TYPE_MASK) == BIRCH_LEAF)
    {
        return ms_pTileData
            ->foliageColor_birchColor; // FoliageColor::getBirchColor();
    }

    // return level->getBiomeSource()->getBiome(x, z)->getFolageColor(level, x, y,
    // z);
...
Icon_SPU *LeafTile_SPU::getTexture(int face, int data)
{
    if ((data & LEAF_TYPE_MASK) == EVERGREEN_LEAF)
    {
        return &ms_pTileData->leafTile_icons[ms_pTileData->leafTile_fancyTextureSet]
                                            [EVERGREEN_LEAF];
    }
    ...
    return &ms_pTileData
                ->leafTile_icons[ms_pTileData->leafTile_fancyTextureSet][0];
}
