...
class StemTile_SPU : public Bush_SPU
{
  ...
  public:
        ...
        TileEntity *forceEntity = NULL) // 4J added forceData, forceEntity param
    {
        ...
        float ss = 0.125f;
        this->setShape(0.5f - ss, 0, 0.5f - ss, 0.5f + ss,
                       (float)ms_pTileData->yy1[id], 0.5f + ss);
    }
    ...
    int getConnectDir(ChunkRebuildData *level, int x, int y, int z)
    {
        ...
        if (level->getTile(x - 1, y, z) == fruitID)
        {
            return 0;
        }
        if (level->getTile(x + 1, y, z) == fruitID)
        ...
        if (level->getTile(x, y, z - 1) == fruitID)
        {
            return 2;
        }
        if (level->getTile(x, y, z + 1) == fruitID)
    ...
    {
        return Tile_SPU::SHAPE_STEM;
    }
    Icon_SPU *getAngledTexture()
    {
        return &ms_pTileData->stemTile_iconAngled;
    }
};
