...
Icon_SPU *GrassTile_SPU::getTexture(ChunkRebuildData *level, int x, int y,
                                    int z, int face)
{
    if (face == Facing::UP)
    {
    ...
    if (above->getID() == Material_SPU::topSnow_Id ||
        above->getID() == Material_SPU::snow_Id)
    {
...
int GrassTile_SPU::getColor(ChunkRebuildData *level, int x, int y, int z,
                            int data)
{
    // return level->getBiomeSource()->getBiome(x, z)->getGrassColor(level, x, y,
    // z);
