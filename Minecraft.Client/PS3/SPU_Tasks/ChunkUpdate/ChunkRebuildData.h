...
class ChunkRebuildData
{
  public:
    ...
    int getTileIdx(int x, int y, int z)
    {
        return ((x - m_x0) * sc_size * sc_size) + ((y - m_y0) * sc_size) +
               (z - m_z0);
    }
    int getTileIdx(int x, int z)
    {
        return ((x - m_x0) * sc_size) + (z - m_z0);
    }
  ...
  public:
    ...
    int getTile(int x, int y, int z)
    {
        return m_tileIds[getTileIdx(x, y, z)];
    }
    int getBrightnessSky(int x, int y, int z)
    {
        return m_brightness[getTileIdx(x, y, z)] & 0x0f;
    }
    int getBrightnessBlock(int x, int y, int z)
    {
        return m_brightness[getTileIdx(x, y, z)] >> 4;
    }
    int getData(int x, int y, int z)
    {
        return m_data_flags[getTileIdx(x, y, z)] & 0x0f;
    }
    int getFlags(int x, int y, int z)
    {
        return m_data_flags[getTileIdx(x, y, z)] >> 4;
    }

    void setFlag(int x, int y, int z, int flag)
    {
        m_data_flags[getTileIdx(x, y, z)] |= (flag << 4);
    }
    int getGrassColor(int x, int z)
    {
        return m_grassColor[getTileIdx(x, z)];
    }
    int getFoliageColor(int x, int z)
    {
        return m_foliageColor[getTileIdx(x, z)];
    }
    int getWaterColor(int x, int z)
    {
        return m_waterColor[getTileIdx(x, z)];
    }
#endif
    ...
    bool isEmptyChunk()
    {
        return m_flags & e_flag_EmptyChunk;
    }
    ...
    int getLightColor(int x, int y, int z,
                      int emitt); // 4J - change brought forward from 1.8.2
