...
class TopSnowTile_SPU : public Tile_SPU
{
  public:
    ...
    bool shouldRenderFace(ChunkRebuildData *level, int x, int y, int z,
                          int face)
    {
        ...
        if ((level->getTile(x, y, z) == Tile_SPU::topSnow_Id) && (face >= 2))
        {
            ...
            int yy = y;
            int zz = z;
            // Work out coords of tile who's face we're considering (rather than it's
            // neighbour which is passed in here as x,y,z already offsetting by the
            // face direction)
            switch (face)
            {
            case 2:
