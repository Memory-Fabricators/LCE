...
void TileRenderer_SPU::tesselateInWorldFixedTexture(
    ...
    Icon_SPU *fixedTexture) // 4J renamed to differentiate from tesselateInWorld
{
                                                ...
                                                int y, int z)
{
    ...
    const float cWidth =
        (2.0f / 16.0f) -
        (1.0f / 128.0f); // 4J - Moved by 1/128th (smallest movement possible with
    ...
    if (type != 0)
    {
        ...
        else
        {
            if (type == FlowerPotTile::TYPE_CACTUS)
            {
                ...
                setShape(0.5f - halfSize, 0.0f, 0.5f - halfSize, 0.5f + halfSize, 0.25f,
                         0.5f + halfSize);
                tesselateBlockInWorld(Tile::cactus, x, y, z);
                setShape(0.5f - halfSize, 0.25f, 0.5f - halfSize, 0.5f + halfSize, 0.5f,
                         0.5f + halfSize);
                tesselateBlockInWorld(Tile::cactus, x, y, z);
                setShape(0.5f - halfSize, 0.5f, 0.5f - halfSize, 0.5f + halfSize, 0.75f,
                         0.5f + halfSize);
                tesselateBlockInWorld(Tile::cactus, x, y, z);
            ...
            {
                tesselateCrossTexture(Tile::sapling, Sapling::TYPE_DEFAULT, x, y, z,
                                      0.75f);
            }
            else if (type == FlowerPotTile::TYPE_SAPLING_BIRCH)
            {
                tesselateCrossTexture(Tile::sapling, Sapling::TYPE_BIRCH, x, y, z,
                                      0.75f);
            }
            else if (type == FlowerPotTile::TYPE_SAPLING_EVERGREEN)
            {
                tesselateCrossTexture(Tile::sapling, Sapling::TYPE_EVERGREEN, x, y, z,
                                      0.75f);
            }
            else if (type == FlowerPotTile::TYPE_SAPLING_JUNGLE)
            {
                tesselateCrossTexture(Tile::sapling, Sapling::TYPE_JUNGLE, x, y, z,
                                      0.75f);
            }
                                             ...
                                             int z, int data, bool render)
{
    ...
    bottom = tesselateAnvilPiece(tt, x, y, z, AnvilTile::PART_COLUMN, bottom,
                                 4.0f / 16.0f, 5.0f / 16.0f, 8.0f / 16.0f, rotate,
                                 render, data);
                                             ...
                                             int z, int dir)
{
    ...
    // yet BEGIN TORCH SECTION
    {
        ...
        // render transmitter
        tesselateTorch(tt, x + transmitterX, y + h, z + transmitterZ, 0.0f, 0.0f,
                       0);
        // render receiver
    ...
    int forceData) // 4J added forceData param
{
    ...
    case Facing::DOWN:
        ...
        t->tex2(getLightColor(
            tt, level, x, y,
            z)); // 4J added - renderPistonArmDown doesn't set its own tex2 so just
        ...
        renderPistonArmUpDown(x + rightEdge, x + leftEdge, y + thickness,
                              y + thickness + armLength, z + leftEdge, z + leftEdge,
                              br * 0.8f, armLengthPixels);
    ...
    case Facing::UP:
        ...
        renderPistonArmUpDown(x + leftEdge, x + rightEdge,
                              ...
                              y - thickness + 1.0f, z + rightEdge, z + rightEdge,
                              br * 0.8f, armLengthPixels);
        renderPistonArmUpDown(x + rightEdge, x + leftEdge,
                              y - thickness + 1.0f - armLength,
                              y - thickness + 1.0f, z + leftEdge, z + leftEdge,
                              br * 0.8f, armLengthPixels);
        renderPistonArmUpDown(x + leftEdge, x + leftEdge,
                              y - thickness + 1.0f - armLength,
                              y - thickness + 1.0f, z + leftEdge, z + rightEdge,
                              br * 0.6f, armLengthPixels);
        renderPistonArmUpDown(x + rightEdge, x + rightEdge,
                              y - thickness + 1.0f - armLength,
                              y - thickness + 1.0f, z + rightEdge, z + leftEdge,
                              br * 0.6f, armLengthPixels);
        break;
    case Facing::NORTH:
        eastFlip = FLIP_CW;
        ...
        renderPistonArmNorthSouth(
            x + leftEdge, x + leftEdge, y + rightEdge, y + leftEdge, z + thickness,
            z + thickness + armLength, br * 0.6f, armLengthPixels);
        renderPistonArmNorthSouth(
            x + rightEdge, x + rightEdge, y + leftEdge, y + rightEdge,
            z + thickness, z + thickness + armLength, br * 0.6f, armLengthPixels);
        renderPistonArmNorthSouth(
            x + leftEdge, x + rightEdge, y + leftEdge, y + leftEdge, z + thickness,
            z + thickness + armLength, br * 0.5f, armLengthPixels);
        renderPistonArmNorthSouth(x + rightEdge, x + leftEdge, y + rightEdge,
                                  y + rightEdge, z + thickness,
    ...
    case Facing::SOUTH:
        ...
        renderPistonArmNorthSouth(x + leftEdge, x + leftEdge, y + rightEdge,
                                  y + leftEdge, z - thickness + 1.0f - armLength,
                                  z - thickness + 1.0f, br * 0.6f, armLengthPixels);
        renderPistonArmNorthSouth(x + rightEdge, x + rightEdge, y + leftEdge,
                                  y + rightEdge, z - thickness + 1.0f - armLength,
                                  z - thickness + 1.0f, br * 0.6f, armLengthPixels);
        renderPistonArmNorthSouth(x + leftEdge, x + rightEdge, y + leftEdge,
                                  y + leftEdge, z - thickness + 1.0f - armLength,
                                  z - thickness + 1.0f, br * 0.5f, armLengthPixels);
        renderPistonArmNorthSouth(x + rightEdge, x + leftEdge, y + rightEdge,
                                  y + rightEdge, z - thickness + 1.0f - armLength,
                                  z - thickness + 1.0f, br, armLengthPixels);
        break;
    case Facing::WEST:
        northFlip = FLIP_CW;
        southFlip = FLIP_CCW;
        ...
        tesselateBlockInWorld(
            tt, x, y, z); // 4J added - renderPistonArmDown doesn't set its own tex2
    ...
    else
    {
        ...
        if (FireTile_SPU::canBurn(level, x - 1, y, z))
        {
            ...
            t->vertexUV((float)(x + 0.0f), (float)(y + 0.0f + yo), (float)(z + 1.0f),
                        (float)(u1), (float)(v1));
            t->vertexUV((float)(x + 0.0f), (float)(y + 0.0f + yo), (float)(z + 0.0f),
                        (float)(u0), (float)(v1));
        ...
        if (FireTile_SPU::canBurn(level, x + 1, y, z))
        {
            t->vertexUV((float)(x + 1 - r), (float)(y + h + yo), (float)(z + 0.0f),
                        (float)(u0), (float)(v0));
            t->vertexUV((float)(x + 1 - 0), (float)(y + 0 + yo), (float)(z + 0.0f),
                        (float)(u0), (float)(v1));
            t->vertexUV((float)(x + 1 - 0), (float)(y + 0 + yo), (float)(z + 1.0f),
                        (float)(u1), (float)(v1));
            t->vertexUV((float)(x + 1 - r), (float)(y + h + yo), (float)(z + 1.0f),
                        (float)(u1), (float)(v0));

            t->vertexUV((float)(x + 1.0f - r), (float)(y + h + yo), (float)(z + 1.0f),
                        (float)(u1), (float)(v0));
            ...
            t->vertexUV((float)(x + 1.0f - r), (float)(y + h + yo), (float)(z + 0.0f),
                        (float)(u0), (float)(v0));
        }
        if (FireTile_SPU::canBurn(level, x, y, z - 1))
        {
            ...
            t->vertexUV((float)(x + 0.0f), (float)(y + 0.0f + yo), (float)(z + 0.0f),
                        (float)(u1), (float)(v1));
            t->vertexUV((float)(x + 1.0f), (float)(y + 0.0f + yo), (float)(z + 0.0f),
                        (float)(u0), (float)(v1));
        ...
        if (FireTile_SPU::canBurn(level, x, y, z + 1))
        {
            t->vertexUV((float)(x + 1.0f), (float)(y + h + yo), (float)(z + 1.0f - r),
                        (float)(u0), (float)(v0));
            ...
            t->vertexUV((float)(x + 0.0f), (float)(y + h + yo), (float)(z + 1.0f - r),
                        (float)(u1), (float)(v0));
        ...
        if (FireTile_SPU::canBurn(level, x, y + 1.0f, z))
        {
            ...
            if (((x + y + z) & 1) == 0)
            {
                t->vertexUV((float)(x0_), (float)(y + h), (float)(z + 0), (float)(u1),
                            (float)(v0));
                t->vertexUV((float)(x0), (float)(y + 0), (float)(z + 0), (float)(u1),
                            (float)(v1));
                t->vertexUV((float)(x0), (float)(y + 0), (float)(z + 1), (float)(u0),
                            (float)(v1));
                t->vertexUV((float)(x0_), (float)(y + h), (float)(z + 1), (float)(u0),
                            (float)(v0));
                ...
                t->vertexUV((float)(x1), (float)(y + 0.0f), (float)(z + 1.0f),
                            (float)(u1), (float)(v1));
                t->vertexUV((float)(x1), (float)(y + 0.0f), (float)(z + 0), (float)(u0),
                            (float)(v1));
                t->vertexUV((float)(x1_), (float)(y + h), (float)(z + 0), (float)(u0),
                            (float)(v0));
            }
            else
            {
...
bool TileRenderer_SPU::tesselateDustInWorld(Tile_SPU *tt, int x, int y, int z)
{
    ...
    if (pic == 0)
    {
        ...
        t->color(br, br, br);
        t->vertexUV((float)(x1), (float)(y + dustOffset), (float)(z1),
                    crossTextureOverlay->getU(u1), crossTextureOverlay->getV(v1));
        t->vertexUV((float)(x1), (float)(y + dustOffset), (float)(z0),
                    crossTextureOverlay->getU(u1), crossTextureOverlay->getV(v0));
        t->vertexUV((float)(x0), (float)(y + dustOffset), (float)(z0),
                    crossTextureOverlay->getU(u0), crossTextureOverlay->getV(v0));
        t->vertexUV((float)(x0), (float)(y + dustOffset), (float)(z1),
                    crossTextureOverlay->getU(u0), crossTextureOverlay->getV(v1));
    }
    else if (pic == 1)
    {
        t->vertexUV((float)(x1), (float)(y + dustOffset), (float)(z1),
    ...
    if (!level->isSolidBlockingTile(x, y + 1, z))
    {
        ...
        {
            t->color(br * red, br * green, br * blue);
            t->vertexUV((float)(x + 1 - dustOffset), (float)(y + 0), (float)(z + 1),
                        lineTexture->getU0(), lineTexture->getV1());
            t->vertexUV((float)(x + 1 - dustOffset), (float)(y + 1 + yStretch),
                        (float)(z + 1), lineTexture->getU1(), lineTexture->getV1());
            t->vertexUV((float)(x + 1 - dustOffset), (float)(y + 1 + yStretch),
                        (float)(z + 0), lineTexture->getU1(), lineTexture->getV0());
            t->vertexUV((float)(x + 1 - dustOffset), (float)(y + 0), (float)(z + 0),
                        lineTexture->getU0(), lineTexture->getV0());
            ...
            t->vertexUV((float)(x + 1 - overlayOffset), (float)(y + 0),
                        ...
                        lineTextureOverlay->getV1());
            t->vertexUV((float)(x + 1 - overlayOffset), (float)(y + 1 + yStretch),
                        (float)(z + 1), lineTextureOverlay->getU1(),
                        lineTextureOverlay->getV1());
            t->vertexUV((float)(x + 1 - overlayOffset), (float)(y + 1 + yStretch),
                        (float)(z + 0), lineTextureOverlay->getU1(),
            ...
            level->getTile(x, y + 1, z - 1) == Tile_SPU::redStoneDust_Id)
        {
            ...
            t->color(br, br, br);
            t->vertexUV((float)(x + 1), (float)(y + 0), (float)(z + overlayOffset),
                        lineTextureOverlay->getU0(), lineTextureOverlay->getV1());
            t->vertexUV((float)(x + 1), (float)(y + 1 + yStretch),
            ...
            t->vertexUV((float)(x + 0), (float)(y + 1 + yStretch),
                        (float)(z + overlayOffset), lineTextureOverlay->getU1(),
                        lineTextureOverlay->getV0());
            t->vertexUV((float)(x + 0), (float)(y + 0), (float)(z + overlayOffset),
                        lineTextureOverlay->getU0(), lineTextureOverlay->getV0());
        }
        if (level->isSolidBlockingTile(x, y, z + 1) &&
            level->getTile(x, y + 1, z + 1) == Tile_SPU::redStoneDust_Id)
        {
            ...
            t->vertexUV((float)(x + 1), (float)(y + 1 + yStretch),
                        ...
                        lineTexture->getV0());
            t->vertexUV((float)(x + 1), (float)(y + 0), (float)(z + 1 - dustOffset),
                        lineTexture->getU0(), lineTexture->getV0());
            t->vertexUV((float)(x + 0), (float)(y + 0), (float)(z + 1 - dustOffset),
                        lineTexture->getU0(), lineTexture->getV1());
            t->vertexUV((float)(x + 0), (float)(y + 1 + yStretch),
                        (float)(z + 1 - dustOffset), lineTexture->getU1(),
                        lineTexture->getV1());
            ...
            t->color(br, br, br);
            t->vertexUV((float)(x + 1), (float)(y + 1 + yStretch),
                        (float)(z + 1 - overlayOffset), lineTextureOverlay->getU1(),
                        lineTextureOverlay->getV0());
            t->vertexUV((float)(x + 1), (float)(y + 0),
                        (float)(z + 1 - overlayOffset), lineTextureOverlay->getU0(),
                        lineTextureOverlay->getV0());
                                            ...
                                            int z)
{
    ...
    {
        tesselateStemTexture(tt, level->getData(x, y, z), 0.5f, x, y - 1 / 16.0f,
                             z);
        tesselateStemDirTexture(tt, level->getData(x, y, z), dir, tileShapeY1, x,
                                y - 1 / 16.0f, z);
    }
    return true;
}
                                   ...
                                   int y, int z, int data)
{
    ...
    float center =
        SharedConstants::TEXTURE_LIGHTING ? 1 : tt->getBrightness(level, x, y, z);
                                             ...
                                             int z)
{
        ...
        0) // 4J - TODO/remove (Minecraft::useAmbientOcclusion())
    {
        if (SharedConstants::TEXTURE_LIGHTING)
        {
            return tesselateBlockInWorldWithAmbienceOcclusionTexLighting(tt, x, y, z,
                                                                         r, g, b);
        }
        else
        {
            return tesselateBlockInWorldWithAmbienceOcclusionOldLighting(tt, x, y, z,
                                                                         r, g, b);
        }
    }
    else
...
bool TileRenderer_SPU::tesselateBlockInWorldWithAmbienceOcclusionTexLighting(
    ...
    float pBaseBlue)
{
    // 4J - added these faceFlags so we can detect whether this block is going to
    // have no visible faces and early out the original code checked noCulling and
    // shouldRenderFace directly where faceFlags is used now
    int faceFlags = 0;
    if (noCulling)
    {
        faceFlags = 0x3f;
    ...
    else
    {
        ...
        1, pZ, 0 ) ? 0x01 : 0; faceFlags |= tt->shouldRenderFace( level, pX, pY + 1,
        pZ, 1 ) ? 0x02 : 0; faceFlags |= tt->shouldRenderFace( level, pX, pY, pZ -
        1, 2 ) ? 0x04 : 0; faceFlags |= tt->shouldRenderFace( level, pX, pY, pZ + 1,
        3 ) ? 0x08 : 0; faceFlags |= tt->shouldRenderFace( level, pX - 1, pY, pZ, 4
...
#pragma message(__LOC__ "ambientOcclusion NEEDS CHANGED FROM A BOOL ")
                                       ...
                                       // >= Options::AO_MAX)
            {
                ...
                tc1 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY1 * (1.0 - tileShapeX0),
                          tileShapeY1 * tileShapeX0, (1.0 - tileShapeY1) * tileShapeX0,
                          (1.0 - tileShapeY1) * (1.0 - tileShapeX0));
                tc2 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY1 * (1.0 - tileShapeX1),
                          tileShapeY1 * tileShapeX1, (1.0 - tileShapeY1) * tileShapeX1,
                          (1.0 - tileShapeY1) * (1.0 - tileShapeX1));
                tc3 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY0 * (1.0 - tileShapeX1),
                          tileShapeY0 * tileShapeX1, (1.0 - tileShapeY0) * tileShapeX1,
                          (1.0 - tileShapeY0) * (1.0 - tileShapeX1));
                tc4 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY0 * (1.0 - tileShapeX0),
                          tileShapeY0 * tileShapeX0, (1.0 - tileShapeY0) * tileShapeX0,
             ...
             !hasFixedTexture()))
        {
            ...
            bool prev = t->setMipmapEnable(
                false); // 4J added - this is rendering the little bit of grass at the
        ...
        if (blsmooth > 0)
        {
                                       ...
                                       // >= Options::AO_MAX)
            {
                ...
                tc1 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY1 * (1.0 - tileShapeX0),
                          (1.0 - tileShapeY1) * (1.0 - tileShapeX0),
                          (1.0 - tileShapeY1) * tileShapeX0, tileShapeY1 * tileShapeX0);
                tc2 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY0 * (1.0 - tileShapeX0),
                          (1.0 - tileShapeY0) * (1.0 - tileShapeX0),
                          (1.0 - tileShapeY0) * tileShapeX0, tileShapeY0 * tileShapeX0);
                tc3 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY0 * (1.0 - tileShapeX1),
                          (1.0 - tileShapeY0) * (1.0 - tileShapeX1),
                          (1.0 - tileShapeY0) * tileShapeX1, tileShapeY0 * tileShapeX1);
                tc4 =
                    blend(_tc1, _tc2, _tc3, _tc4, tileShapeY1 * (1.0 - tileShapeX1),
                          (1.0 - tileShapeY1) * (1.0 - tileShapeX1),
                          (1.0 - tileShapeY1) * tileShapeX1, tileShapeY1 * tileShapeX1);
            }
            else
            {
                ll1 = (llx0Z + llxYZ + ll00Z + ll0YZ) / 4.0f;
    ...
    if (faceFlags & 0x20)
    {
        if (blsmooth > 0)
        {
                                       ...
                                       // >= Options::AO_MAX)
            {
                ...
                tc1 =
                    blend(_tc1, _tc2, _tc3, _tc4, (1.0 - tileShapeY0) * tileShapeZ1,
                          (1.0 - tileShapeY0) * (1.0 - tileShapeZ1),
                          tileShapeY0 * (1.0 - tileShapeZ1), tileShapeY0 * tileShapeZ1);
                tc2 =
                    blend(_tc1, _tc2, _tc3, _tc4, (1.0 - tileShapeY0) * tileShapeZ0,
                          (1.0 - tileShapeY0) * (1.0 - tileShapeZ0),
                          tileShapeY0 * (1.0 - tileShapeZ0), tileShapeY0 * tileShapeZ0);
                tc3 =
                    blend(_tc1, _tc2, _tc3, _tc4, (1.0 - tileShapeY1) * tileShapeZ0,
                          (1.0 - tileShapeY1) * (1.0 - tileShapeZ0),
                          tileShapeY1 * (1.0 - tileShapeZ0), tileShapeY1 * tileShapeZ0);
                tc4 =
                    blend(_tc1, _tc2, _tc3, _tc4, (1.0 - tileShapeY1) * tileShapeZ1,
                          (1.0 - tileShapeY1) * (1.0 - tileShapeZ1),
                          tileShapeY1 * (1.0 - tileShapeZ1), tileShapeY1 * tileShapeZ1);
            }
            else
            {
    ...
    float pBaseBlue)
{
    ...
    // 0x01 : 0; 		faceFlags |= tt->shouldRenderFace( level, pX, pY + 1,
    // pZ, 1 ) ? 0x02 : 0; 		faceFlags |= tt->shouldRenderFace(
    // level, pX, pY, pZ - 1, 2 ) ? 0x04 : 0; 		faceFlags |=
    ...
    // pZ - 1 )]; 	llTransxY0 =
    // level->m_tileData.transculent[level->getTile( pX - 1, pY + 1, pZ )];
    // llTransxy0 = level->m_tileData.transculent[level->getTile( pX - 1, pY - 1,
    // pZ )]; 	llTransx0z = level->m_tileData.transculent[level->getTile( pX -
    // 1, pY, pZ - 1 )]; 	llTransx0Z =
    // level->m_tileData.transculent[level->getTile( pX - 1, pY, pZ + 1 )];
    // llTrans0YZ = level->m_tileData.transculent[level->getTile( pX, pY + 1, pZ +
    // 1 )]; 	llTrans0Yz = level->m_tileData.transculent[level->getTile( pX,
                                            ...
                                            int z)
{
    ...
    {
        // center post
        setShape(.5f - WallTile_SPU::POST_WIDTH, 0, .5f - WallTile_SPU::POST_WIDTH,
                 .5f + WallTile_SPU::POST_WIDTH, WallTile_SPU::POST_HEIGHT,
        ...
        if (e)
        {
            setShape(.5f + WallTile_SPU::POST_WIDTH, 0,
                     .5f - WallTile_SPU::WALL_WIDTH, 1, WallTile_SPU::WALL_HEIGHT,
                     .5f + WallTile_SPU::WALL_WIDTH);
            tesselateBlockInWorld(tt, x, y, z);
        }
        if (n)
        ...
        if (s)
        {
            setShape(.5f - WallTile_SPU::WALL_WIDTH, 0,
                     .5f + WallTile_SPU::POST_WIDTH, .5f + WallTile_SPU::WALL_WIDTH,
                     WallTile_SPU::WALL_HEIGHT, 1);
            tesselateBlockInWorld(tt, x, y, z);
        }
    }
    else if (vertical)
    ...
    {
        // west-east wall
        setShape(0, 0, .5f - WallTile_SPU::WALL_WIDTH, 1, WallTile_SPU::WALL_HEIGHT,
                 .5f + WallTile_SPU::WALL_WIDTH);
        tesselateBlockInWorld(tt, x, y, z);
    }
                                                 ...
                                                 int y, int z)
{
    ...
    // level->getTile(x - 1, y, z) == Tile_SPU::cobbleWall_Id && level->getTile(x
    // + 1, y, z) == Tile_SPU::cobbleWall_Id)
    //	|| ((direction == Direction::EAST || direction == Direction::WEST) &&
    // level->getTile(x, y, z - 1) == Tile_SPU::cobbleWall_Id && level->getTile(x,
    // y, z + 1) == Tile_SPU::cobbleWall_Id))
    //{
    //		h00 -= 3.0f / 16.0f;
    //		h01 -= 3.0f / 16.0f;
    //		h10 -= 3.0f / 16.0f;
