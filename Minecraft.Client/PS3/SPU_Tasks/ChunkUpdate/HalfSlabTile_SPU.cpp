...
void HalfSlabTile_SPU::updateShape(ChunkRebuildData *level, int x, int y, int z,
                                   int forceData /* = -1 */,
                                   TileEntity *forceEntity /* = NULL */)
{
    if (fullSize())
    {
...
bool HalfSlabTile_SPU::shouldRenderFace(ChunkRebuildData *level, int x, int y,
                                        int z, int face)
{
    if (fullSize())
    {
    ...
    if (face != Facing::UP && face != Facing::DOWN &&
        !Tile_SPU::shouldRenderFace(level, x, y, z, face))
    {
        return false;
    }
...
{
    return tileId == Tile_SPU::stoneSlabHalf_Id ||
           tileId == Tile_SPU::woodSlabHalf_Id;
}
