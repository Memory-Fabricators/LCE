...
Icon_SPU *DoorTile_SPU::getTexture(ChunkRebuildData *level, int x, int y, int z,
                                   int face)
{
    int texBase;
    if (getMaterial()->getID() == Material_SPU::metal_Id)
    ...
    return &ms_pTileData
                ->doorTile_Icons[texBase + (flip ? DOOR_TILE_TEXTURE_COUNT : 0) +
                                 (upper ? 1 : 0)];
}

void DoorTile_SPU::updateShape(
...
int DoorTile_SPU::getCompositeData(ChunkRebuildData *level, int x, int y,
                                   int z)
{
