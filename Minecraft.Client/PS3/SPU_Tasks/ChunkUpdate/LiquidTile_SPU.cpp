                             ...
                             int d)
{
    ...
    if (getMaterial()->getID() == Material_SPU::water_Id)
    {
        ...
        //            return level.getBiomeSource().getBiome(x,
        //            z).getGrassColor(level, x, y, z);
...
int LiquidTile_SPU::getRenderedDepth(ChunkRebuildData *level, int x, int y,
                                     int z)
{
    if (level->getMaterial(x, y, z)->getID() != getMaterial()->getID())
    {
...
bool LiquidTile_SPU::isSolidFace(ChunkRebuildData *level, int x, int y, int z,
                                 int face)
{
...
bool LiquidTile_SPU::shouldRenderFace(ChunkRebuildData *level, int x, int y,
                                      int z, int face)
{
    Material_SPU *m = level->getMaterial(x, y, z);
    if (m->getID() == this->getMaterial()->getID())
...
int LiquidTile_SPU::getLightColor(ChunkRebuildData *level, int x, int y,
                                  int z)
{
...
float LiquidTile_SPU::getBrightness(ChunkRebuildData *level, int x, int y,
                                    int z)
{
...
double LiquidTile_SPU::getSlopeAngle(ChunkRebuildData *level, int x, int y,
                                     int z, Material_SPU *m)
{
    Vec3_SPU flow = Vec3_SPU(0, 0, 0);
