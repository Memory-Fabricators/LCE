...
class VineTile_SPU : public Tile_SPU
{
  ...
  public:
    ...
    bool isAcceptableNeighbor(int id)
    {
        ...
        TileRef_SPU tile(id);
        if (tile->isCubeShaped() && tile->getMaterial()->blocksMotion())
        {
            return true;
        }
        return false;
  ...
  public:
    virtual int getColor(ChunkRebuildData *level, int x, int y, int z, int data)
    {
        return getColor(level, x, y, z);
    } // 4J added
    virtual int getColor(ChunkRebuildData *level, int x, int y, int z)
    {
        return level->getFoliageColor(x, z);
    }
};
