...
void Tesselator_SPU::end()
{
    ...
    // 		{
    //             // glDrawArrays(GL_TRIANGLES, 0, vertices); // 4J - changed for
    //             xbox
    // #ifdef _XBOX
...
void Tesselator_SPU::packCompactQuad()
{
    ...
    // If the min is 32, then this whole quad must be in that plane - make the min
    // 15 instead so we can still offset from that with our delta to get to the
    // exact edge
    if (basex == 32)
    {
        basex = 31;
    ...
    // Now get deltas to each vertex - these have an 8-bit range so they can span
    // a full unit range from the base position
    for (int i = 0; i < 4; i++)
    {
        m_ix[i] -= basex << 7;
        m_iy[i] -= basey << 7;
    ...
    if (axisAligned)
    {
        // Now go through each vertex, and work out which of the min/max should be
        // used for each dimension, and store
        for (int i = 0; i < 4; i++)
        {
            unsigned int code = 0;
            if (m_u[i] == maxu)
    ...
    {
        // The UVs aren't axis aligned - store them in the next 4 vertices. These
        // will be indexed from our base vertices because we'll set a special code
        // (4) for the UVs. They won't be drawn as actual verts when these extra
        // vertices go through the vertex shader, because we'll make sure that they
        // get interpreted as a zero area quad and so they'll be quickly eliminated
        // from rendering post-tranform

        for (int i = 0; i < 4; i++)
        {
            data[i * 2 + 1] |= (4); // The special code to indicate they need further
                                    // data to be fetched
            data[i * 2 + 1] |= m_t2[i] << 16;
            data[8 + i * 2] = 0; // This includes x/y coordinate of each vert as (0,0)
                                 // so they will be interpreted as a zero area quad
...
void Tesselator_SPU::vertex(float x, float y, float z)
{
    ...
    // Signal to pixel shader whether to use mipmapping or not, by putting u into
    // > 1 range if it is to be disabled
...
#define INT_ROUND(x) (int)(floorf(x + 0.5))
        ...
        if (vertices % 4 == 0 &&
            ((p >= size - 8 * 2) ||
             ((p / 2) >= 65532))) // Max 65535 verts in D3D, so 65532 is the last
                                  // point at the end of a quad to catch it
#else
        if (vertices % 4 == 0 &&
            ((p >= size - 4 * 4) ||
             ((p / 4) >= 65532))) // Max 65535 verts in D3D, so 65532 is the last
                                  // point at the end of a quad to catch it
#endif
        {
            // 			end();
    ...
    else
    {
        ...
        {
            // -512 each for u/v will mean that the renderer will use global settings
            // (set via RenderManager.StateSetVertexTextureUV) rather than these local
            // ones
...
void Tesselator_SPU::endData()
{
    ...
    if (sc_verbose)
    {
        spu_print("endData - Tesselator : DMA SPU->PPU complete : %d verts, %d "
                  "bytes in total\n",
