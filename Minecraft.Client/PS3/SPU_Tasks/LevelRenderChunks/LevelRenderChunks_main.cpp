...
CELL_SPU_LS_PARAM(128 * 1024, 16 * 1024); // can't use #defines here as it seems
                                          // to  create an asm instruction
...
static int32_t gcmReserveFailed(CellGcmContextData *context, uint32_t count)
{
    ...
    spu_assert(0);
    context->current =
        context->begin; // Back to the beginning so we don't trample memory.
    return CELL_OK;
}
...
int cellSpursTaskMain(qword argTask, uint64_t argTaskset)
{
    ...
    cellGcmSetupContextData(&gcmContext, pDataOut->m_commandBuffer,
                            sizeof(pDataOut->m_commandBuffer), gcmReserveFailed);
    ...
    while (1)
    {
        ...
        {
            spu_printf(
                "[Task#%02u][SPU#%u] waiting for event flag#0, mask = 0x%04x\n",
                idTask, idSpu, mask);
        }
        ...
        pDataOut->m_commandBufferSize =
            ((unsigned int)gcmContext.current) - ((unsigned int)gcmContext.begin);

        DmaData_SPU::putAndWait(pDataOut, eaDataOut,
                                sizeof(RenderChunksSpuDataOut));
