...
//
// CELL_SPU_LS_PARAM(128*1024, 16*1024);	// can't use #defines here as it
// seems to  create an asm instruction
  ...
  public:
    ...
    void flush()
    {
        if (m_cachePos > 0)
        {
            ...
            // 			spu_print("DMAing %d bytes from 0x%08x(SPU) to
            // 0x%08x(PPU)\n",(int)( m_cachePos*sizeof(int)), (int)m_localCache,
            // (int)&m_pDataPPU[m_ppuPos]);
            DmaData_SPU::put(m_localCache, (uintptr_t)&m_pDataPPU[m_ppuPos],
                             DmaData_SPU::roundUpDMASize(m_cachePos * sizeof(int)));
            m_ppuPos += m_cachePos;
            m_cachePos = 0;
        }
    }
...
bool LevelRenderer_FindNearestChunk_DataIn::MultiplayerChunkCache::
    getChunkEmpty(int lowerOffset, int upperOffset, int x, int y, int z)
{
    x >>= 4;
    z >>= 4;
    int ix = x + XZOFFSET;
    ...
    // 	spu_print("grabbing pointer idx %d from 0x%08x", idx,
    // (uintptr_t)&cache[idx]);
    ...
    else
    {
        ...
        uintptr_t pBlocks;
        // using a class structure offset here as we don't want to be compiling
        // LevelChunk on SPU
        int chunkY = y;
        if (y >= 128)
        {
            pBlocks =
                DmaData_SPU::getValue32((uintptr_t)(chunkPointer + upperOffset));
            chunkY -= 128;
        }
        else
        {
            pBlocks =
                DmaData_SPU::getValue32((uintptr_t)(chunkPointer + lowerOffset));
        }
        DmaData_SPU::getAndWaitUnaligned(&blocks, pBlocks,
                                         sizeof(CompressedTileStorage));
        return blocks.isRenderChunkEmpty(chunkY);
    }
}

bool LevelRenderer_FindNearestChunk_DataIn::CompressedTileStorage::
    isRenderChunkEmpty(
...
void LevelRenderer_FindNearestChunk_DataIn::findNearestChunk()
{
    unsigned char *globalChunkFlags =
        (unsigned char *)alloca(numGlobalChunks); // 164K !!!
    DmaData_SPU::getAndWait(globalChunkFlags, (uintptr_t)pGlobalChunkFlags,
                            sizeof(unsigned char) * numGlobalChunks);
    ...
    for (int p = 0; p < 4; p++)
    {
        // It's possible that the localplayers member can be set to NULL on the main
        // thread when a player chooses to exit the game So take a reference to the
        // player object now. As it is a shared_ptr it should live as long as we
        // need it
        ...
        {
            uintptr_t ClipChunkX_PPU =
                (uintptr_t)&chunks[p][(z * yChunks + 0) * xChunks + 0];
            DmaData_SPU::getAndWait(&clipChunk[0], ClipChunkX_PPU,
                                    sizeof(ClipChunk) * xChunks * CHUNK_Y_COUNT);
            for (int y = 0; y < CHUNK_Y_COUNT; y++)
            {
                for (int x = 0; x < xChunks; x++)
                {
                    ...
                    // Get distance to this chunk - deliberately not calling the chunk's
                    // method of doing this to avoid overheads (passing entitie, type
                    ...
                    int distSqWeighted =
                        xd * xd + yd * yd * 4 +
                        zd * zd; // Weighting against y to prioritise things in same x/z
                    ...
                    if (globalChunkFlags[pClipChunk->globalIdx] & CHUNK_FLAG_DIRTY)
                    {
                        if ((!onlyRebuild) ||
                            ...
                            (distSq <
                             20 * 20)) // Always rebuild really near things or else building
                                       // (say) at tower up into empty blocks when we are
                                       // low on memory will not create render data
                        {
                            // Is this chunk nearer than our nearest?
                            if (distSqWeighted < minDistSq)
                            {
                                // At this point we've got a chunk that we would like to
                                // consider for rendering, at least based on its proximity to
                                // the player(s). Its *quite* quick to generate empty render
                                // data for render chunks, but if we let the rebuilding do that
                                // then the after rebuilding we will have to start searching for
                                // the next nearest chunk from scratch again. Instead, its
                                // better to detect empty chunks at this stage, flag them up as
                                // not dirty (and empty), and carry on. The levelchunk's
                                // isRenderChunkEmpty method can be quite optimal as it can make
                                // use of the chunk's data compression to detect emptiness
                                // without actually testing as many data items as uncompressed
                                // data would.
                                Chunk chunk;
                                DmaData_SPU::getAndWait(&chunk, (uintptr_t)pClipChunk->chunk,
                                                        sizeof(Chunk));
                                if (!multiplayerChunkCache[p].getChunkEmpty(
                                        lowerOffset, upperOffset, chunk.x, y * 16, chunk.z))
                                {
                                    uintptr_t ClipChunkPPU =
                                        (uintptr_t)&chunks[p][(z * yChunks + y) * xChunks + x];
    ...
    DmaData_SPU::putAndWait(globalChunkFlags, (uintptr_t)pGlobalChunkFlags,
                            sizeof(unsigned char) * numGlobalChunks);
}

void cellSpursJobQueueMain(CellSpursJobContext2 *pContext,
                           CellSpursJob256 *pJob)
{
    ...
    DmaData_SPU::getAndWait(&dataIn, eaDataIn,
                            sizeof(LevelRenderer_FindNearestChunk_DataIn));

    dataIn.findNearestChunk();

    DmaData_SPU::putAndWait(&dataIn, eaDataIn,
                            sizeof(LevelRenderer_FindNearestChunk_DataIn));
