...
class LevelRenderer_FindNearestChunk_DataIn
{
  public:
    ...
    class CompressedTileStorage
    {
      ...
      private:
        static void getBlock(int *block, int x, int y, int z)
        {
    ...
    class Chunk
    {
      public:
        ...
        int padding[1];
        // public:
        //	vector<shared_ptr<TileEntity> > renderableTileEntities;		// 4J -
        // removed
    ...
    int lowerOffset; // offsets into the level class, we don't want to compile the
                     // entire class
    int upperOffset;
