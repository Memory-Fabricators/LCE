...
inline int CompressedTileStorage_SPU::getIndex(int block, int tile)
{
    ...
    int index =
        ((block & 0x180) << 6) | ((block & 0x060) << 4) | ((block & 0x01f) << 2);
...
inline void CompressedTileStorage_SPU::getBlockAndTile(int *block, int *tile,
                                                       int x, int y, int z)
{
...
int CompressedTileStorage_SPU::get(int x, int y, int z)
{
    ...
    if (indexType == INDEX_TYPE_0_OR_8_BIT)
    {
        ...
        {
            // 8-bit reads are just directly read from the 64 long array of values
    ...
    else
    {
        ...
        int tiletypecount =
            1 << bitspertile; // will be 2, 4 or 16 (from index values of 0, 1, 2)
        int tiletypemask =
            tiletypecount - 1; // will be 1, 3 or 15 (from index values of 0, 1, 2)
        int indexshift =
            3 - indexType; // will be 3, 2 or 1 (from index values of 0, 1, 2)
        int indexmask_bits =
            7 >> indexType; // will be 7, 3 or 1 (from index values of 0, 1, 2)
        int indexmask_bytes =
            62 >> indexshift; // will be 7, 15 or 31 (from index values of 0, 1, 2)
...
void TileCompressData_SPU::uncompressTiles(int x0, int z0, int x1, int z1,
                                           bool upper)
{
    int y0 = -2;
    int y1 = 18;
    for (int iY = y0; iY < y1; iY++)
    {
        if (validY(iY + m_y0, upper))
        {
            for (int iX = x0; iX < x1; iX++)
            {
                ...
                {
                    int index =
                        ((iX + 2) * sc_size * sc_size) + ((iY + 2) * sc_size) + (iZ + 2);
                    ...
                    {
                        m_OutputData.m_tileIds[index] =
                            m_pTileStorage->get(sectionX, sectionY, sectionZ);
                    }
                }
            }
        }
...
void TileCompressData_SPU::clearTiles(int x0, int z0, int x1, int z1,
                                      bool upper)
{
    int y0 = -2;
    int y1 = 18;
...
void TileCompressData_SPU::uncompressLights(int x0, int z0, int x1, int z1,
                                            bool upper, bool skyLight)
{
    int y0 = -2;
    int y1 = 18;
    for (int iY = y0; iY < y1; iY++)
        ...
        if (validY(iY + m_y0, upper))
        {
            for (int iX = x0; iX < x1; iX++)
            {
                for (int iZ = z0; iZ < z1; iZ++)
                {
                    ...
                    {
                        m_OutputData.m_brightness[index] =
                            m_pLightStorage->get(sectionX, sectionY, sectionZ);
                    }
                    else
                    {
                        m_OutputData.m_brightness[index] |=
                            m_pLightStorage->get(sectionX, sectionY, sectionZ) << 4;
                    }
                }
            }
        }
...
void TileCompressData_SPU::clearLights(int x0, int z0, int x1, int z1,
                                       bool upper, bool skyLight)
{
    int y0 = -2;
    int y1 = 18;
...
void TileCompressData_SPU::uncompressData(int x0, int z0, int x1, int z1,
                                          bool upper)
{
    int y0 = -2;
    int y1 = 18;
        ...
        if (validY(iY + m_y0, upper))
        {
            for (int iX = x0; iX < x1; iX++)
            {
                for (int iZ = z0; iZ < z1; iZ++)
                {
                    ...
                    {
                        m_OutputData.m_data_flags[index] =
                            m_pDataStorage->get(sectionX, sectionY, sectionZ);
                    }
                }
            }
        }
...
void TileCompressData_SPU::clearData(int x0, int z0, int x1, int z1,
                                     bool upper)
{
    int y0 = -2;
    int y1 = 18;
...
void TileCompressData_SPU::dmaSparseStorage(int64_t dataAndSize,
                                            unsigned char *pDest)
{
...
void TileCompressData_SPU::loadAndUncompressLowerSection(int block, int x0,
                                                         int z0, int x1,
                                                         int z1)
{
    ...
    if (m_lowerBlocks[block] != NULL)
    {
        ...
        DmaData_SPU::getAndWait(m_pTileStorage->getDataPtr(),
                                (uint32_t)m_lowerBlocks[block], dmaSize);
...
void TileCompressData_SPU::loadAndUncompressUpperSection(int block, int x0,
                                                         int z0, int x1,
                                                         int z1)
{
    if (m_upperBlocks[block] != NULL)
    {
        ...
        DmaData_SPU::getAndWait(m_pTileStorage->getDataPtr(),
                                (uint32_t)m_upperBlocks[block], dmaSize);
...
void TileCompressData_SPU::uncompress(uint32_t eaDataOut)
{
    ...
    // there may be an overlap here on the 128 boundary here we have to
    // uncompressed upper and lower
    if (m_y0 <= 127 + 16)
    {
...
void TileCompressData_SPU::setForChunk(Region *region, int x0, int y0, int z0)
{
    ...
    // we have to grab a 20x20x20 section, so we need 9 chunks in total, the
    // centre chunk and all neighbours in x and z
        ...
        for (int j = 0; j < 3; j++)
        {
            if (y0 <= 127 + 16)
            {
                LevelChunk *pLevelChunk =
                    region->getLevelChunk(m_x0 + offsets[i], 0, m_z0 + offsets[j]);
                if (pLevelChunk && !pLevelChunk->isEmpty())
                {
                    m_lowerBlocks[i * 3 + j] = pLevelChunk->lowerBlocks->indicesAndData;
                    m_lowerBlocksSize[i * 3 + j] =
                        pLevelChunk->lowerBlocks->allocatedSize;
                    m_lowerSkyLight[i * 3 + j] = pLevelChunk->lowerSkyLight->dataAndCount;
                    m_lowerBlockLight[i * 3 + j] =
                        pLevelChunk->lowerBlockLight->dataAndCount;
            ...
            {
                LevelChunk *pLevelChunk =
                    region->getLevelChunk(m_x0 + offsets[i], 128, m_z0 + offsets[j]);
                if (pLevelChunk && !pLevelChunk->isEmpty())
                {
                    m_upperBlocks[i * 3 + j] = pLevelChunk->upperBlocks->indicesAndData;
                    m_upperBlocksSize[i * 3 + j] =
                        pLevelChunk->upperBlocks->allocatedSize;
                    m_upperSkyLight[i * 3 + j] = pLevelChunk->upperSkyLight->dataAndCount;
                    m_upperBlockLight[i * 3 + j] =
                        pLevelChunk->upperBlockLight->dataAndCount;
