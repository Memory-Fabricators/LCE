...
class PPULoadArray
{
  ...
  public:
    ...
    {
        // dma data from PPU
        // 			spu_printf("DMAing %d bytes from 0x%08x(SPU) to
        ...
        DmaData_SPU::getAndWait(m_localCache, (uintptr_t)&m_pDataPPU[m_ppuPos],
                                dmaSize);
        m_cachePos = 0;
    }
};
  ...
  public:
    ...
    void flush()
    {
        if (m_cachePos > 0)
        {
            ...
            // 0x%08x(PPU)\n",(int)( m_cachePos*sizeof(int)), (int)m_localCache,
            // (int)&m_pDataPPU[m_ppuPos]);
            DmaData_SPU::putAndWait(m_localCache, (uintptr_t)&m_pDataPPU[m_ppuPos],
                                    DmaData_SPU::roundUpDMASize(m_cachePos));
            // 			if(m_ppuPos == 0)
...
void RLECompress(void *pPPUSrc, int srcSize, void *pPPUDst, int *pDstSize)
{
    ...
    do
    {
        ...
        while ((srcBuffer.getPos() != endPos) && (srcBuffer.getNext() == thisOne) &&
               (count < 256))
        {
            count++;
        }
