...
void cull(LevelRenderer_cull_DataIn *dataIn,
          LevelRenderer_cull_DataIn *eaDataIn)
{
    ClipChunk_SPU chunkDoubleBuffer[2];
    int chunkIdx = 0;
    unsigned char *globalChunkFlags =
        (unsigned char *)alloca(dataIn->numGlobalChunks); // 164K !!!
    DmaData_SPU::getAndWait(globalChunkFlags,
                            (uintptr_t)dataIn->pGlobalChunkFlags,
                            sizeof(unsigned char) * dataIn->numGlobalChunks);
    ...
    {
        DmaData_SPU::get(&chunkDoubleBuffer[chunkIdx],
                         (uintptr_t)&dataIn->pClipChunks[0], sizeof(ClipChunk_SPU));
    }
    ...
    for (int i = 0; i < dataIn->numClipChunks; i++)
    {
        ...
        DmaData_SPU::get(&chunkDoubleBuffer[chunkIdx],
                         (uintptr_t)&dataIn->pClipChunks[i + 1],
                         sizeof(ClipChunk_SPU));

        Vectormath::Aos::Vector4 pos(chunk.aabb[0] + xOff, chunk.aabb[1] + yOff,
                                     chunk.aabb[2] + zOff, 1.0f);
        Vectormath::Aos::Vector4 transPos = dataIn->clipMat * pos;
        float zDepth = -transPos.getZ();
        // 		spu_print("z val = %f : maxDepth = %f\n", zDepth,
        // dataIn->maxDepthRender);
        if (zDepth > dataIn->maxDepthRender ||
            chunk.aabb[1] < dataIn->maxHeightRender)
        {
            chunk.visible = false;
        }
        else
        {
            unsigned char flags =
                chunk.globalIdx == -1 ? 0 : globalChunkFlags[chunk.globalIdx];
            chunk.visible = false;
        ...
        DmaData_SPU::put(&chunk, (uintptr_t)&dataIn->pClipChunks[i],
                         16); // only DMA the first 16 bytes, as they contain the
                              // visible flag we need to pass back
        ...
        if ((globalChunkFlags[chunk.globalIdx] & CHUNK_FLAG_EMPTY0) ==
            0) // Check that this particular layer isn't empty
        {
        ...
        layer = 1;
        if ((globalChunkFlags[chunk.globalIdx] & CHUNK_FLAG_EMPTY1) ==
            0) // Check that this particular layer isn't empty
        {
    ...
    layer1List.flush();
    layer0ZDepth.flush();
    cellDmaPutUint32(layer0List.getSize(),
                     (uintptr_t)&eaDataIn->numToRender_layer0, 0, 0, 0);
    cellDmaPutUint32(layer1List.getSize(),
                     (uintptr_t)&eaDataIn->numToRender_layer1, 0, 0, 0);
}
                           ...
                           CellSpursJob256 *pJob)
{
    ...
    if (sc_verbose)
    {
        spu_print("LevelRenderer_cull [SPU#%u] DMA'd input data, ready to cull %d "
                  "chunks\n",
    ...
    // 	DmaData_SPU::putAndWait(outputData, eaDataOut, sizeof(char) *
    // numForDMA);
