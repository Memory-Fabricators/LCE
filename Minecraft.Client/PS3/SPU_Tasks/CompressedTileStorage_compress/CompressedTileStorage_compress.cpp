...
class CCompressedTileStorage_compress
{
  ...
  public:
    CCompressedTileStorage_compress(
        CompressedTileStorage_compress_dataIn &dataIn)
    {
        ...
        DmaData_SPU::getAndWait(indicesAndData, (uintptr_t)dataIn.indicesAndData,
                                DmaData_SPU::roundUpDMASize(allocatedSize));
    }
...
bool CCompressedTileStorage_compress::compress(int upgradeBlock /*=-1*/)
{
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        // First task is to find out what type of storage each block needs. Need to
        // unpack each where required. Note that we don't need to fully unpack the
        // data at this stage since we are only interested in working out how many
        // unique types of tiles are in each block, not what those actual tile ids
        // are.
        if (upgradeBlock == -1)
        {
            if (indexType == INDEX_TYPE_0_OR_8_BIT)
            {
                // Note that we are only interested in data that can be packed further,
                // so we don't need to consider things that are already at their most
                // compressed (ie with INDEX_TYPE_0_BIT_FLAG set)
                if ((blockIndices[i] & INDEX_TYPE_0_BIT_FLAG) == 0)
                {
                    unpacked_data = data + ((blockIndices[i] >> INDEX_OFFSET_SHIFT) &
                                            INDEX_OFFSET_MASK);
                }
            }
            else
            {
                ...
                for (int j = 0; j < 64; j++)
                {
                    ...
                    unpacked_data[j] = (packed_data[idx] >> bit) &
                                       tiletypemask; // Doesn't need the actual data for
            ...
            if (unpacked_data)
            {
                ...
                // compress any further and require no storage. Store flags for each
                // tile type used in an array of 4 64-bit flags.

                uint64_t usedFlags[4] = {0, 0, 0, 0};

                int64_t i64_1 = 1; // MGH - instead of 1i64, which is MS specific
                for (int j = 0; j < 64;
                     j++) // This loop of 64 is to go round the 4x4x4 tiles in the block
                {
                ...
                for (int tiletype = 0; tiletype < 256;
                     tiletype++) // This loop of 256 is to go round the 256 possible
                                 // values that the tiles might have had to find how
                ...
                if (count == 1)
                {
                    ...
                    // We'll need to compress if this isn't the same type as before. If it
                    // *was* a 0-bit one though, then unpacked_data wouldn't have been set
            ...
            {
                // Already will be 0 bits, so we can't do any further compression - just
                // copy the index over.
        ...
        else
        {
            if (i == upgradeBlock)
            {
                ...
                // INDEX_TYPE_4_BIT (2) -> INDEX_TYPE_0_OR_8_BIT (3)	(new will be
                // 8-bit) INDEX_TYPE_0_OR_8_BIT (3) -> INDEX_TYPE_1_BIT (0)
                // (assuming old was 0-bit)
                _blockIndices[i] =
            ...
            case INDEX_TYPE_0_OR_8_BIT:
                ...
                memToAlloc += 64;
                break;
                // Note that INDEX_TYPE_8_BIT|INDEX_TYPE_0_BIT_FLAG not in here as it
                // doesn't need any further allocation
            }
        }
    }
    ...
    if (needsCompressed)
    {
        ...
        for (int i = 0; i < 512; i++)
        {
            ...
            if (indexTypeOld == indexTypeNew)
            {
                ...
                if (indexTypeOld == INDEX_TYPE_0_OR_8_BIT)
                {
                        ...
                        (_blockIndices[i] & INDEX_TYPE_0_BIT_FLAG))
                    {
                        ...
                        {
                            packed_data = data + ((blockIndices[i] >> INDEX_OFFSET_SHIFT) &
                                                  INDEX_OFFSET_MASK);
                ...
                else
                {
                    ...
                    int dataSize =
                        8 << indexTypeOld;                // 8, 16 or 32 bytes of per-tile storage
                    dataSize += 1 << (1 << indexTypeOld); // 2, 4 or 16 bytes to store
                                                          // each tile type
                    newIndices[i] |= (usDataOffset & INDEX_OFFSET_MASK)
                                     << INDEX_OFFSET_SHIFT;
            ...
            // If we're not done, then we actually need to recompress this block.
            // First of all decompress from its current format.
            if (!done)
            {
                unsigned char *unpacked_data = NULL;
                ...
                {
                    int bitspertile = 1 << indexTypeOld;  // will be 1, 2 or 4 (from index
                                                          // values of 0, 1, 2)
                    int tiletypecount = 1 << bitspertile; // will be 2, 4 or 16
                    int tiletypemask = tiletypecount - 1; // will be 1, 3 or 15
                    int indexshift =
                        3 -
                        indexTypeOld; // will be 3, 2 or 1 (from index values of 0, 1, 2)
                    int indexmask_bits =
                        7 >>
                        indexTypeOld; // will be 7, 3 or 1 (from index values of 0, 1, 2)
                    int indexmask_bytes =
                        62 >>
                        indexshift; // will be 7, 15 or 31 (from index values of 0, 1, 2)

                    unpacked_data = tempdata;
                    tile_types = data + ((blockIndices[i] >> INDEX_OFFSET_SHIFT) &
                                         INDEX_OFFSET_MASK);
                    ...
                    for (int j = 0; j < 64; j++)
                    {
                        ...
                        unpacked_data[j] =
                            tile_types[(packed_data[idx] >> bit) & tiletypemask];
                    }
                }
                ...
                if (indexTypeNew == INDEX_TYPE_0_OR_8_BIT)
                {
                    if (_blockIndices[i] & INDEX_TYPE_0_BIT_FLAG)
                    {
                        newIndices[i] =
                            ...
                            (((unsigned short)unpacked_data[0]) << INDEX_TILE_SHIFT);
                    }
                    else
                    {
                        usDataOffset = (usDataOffset + 3) &
                ...
                {
                    int bitspertile = 1 << indexTypeNew;  // will be 1, 2 or 4 (from index
                                                          // values of 0, 1, 2)
                    ...
                    int indexshift =
                        3 -
                        indexTypeNew; // will be 3, 2 or 1 (from index values of 0, 1, 2)
                    int indexmask_bits =
                        7 >>
                        indexTypeNew; // will be 7, 3 or 1 (from index values of 0, 1, 2)
                    int indexmask_bytes =
                        62 >>
                        indexshift; // will be 7, 15 or 31 (from index values of 0, 1, 2)
        ...
        DmaData_SPU::putAndWait(&newIndicesAndData, newIndicesPPU,
                                DmaData_SPU::roundUpDMASize(newAllocatedSize));
        return true;
    }
    return false;
}
                           ...
                           CellSpursJob256 *pJob)
{
    ...
    DmaData_SPU::getAndWait(&compressDataIn, eaDataIn,
                            sizeof(CompressedTileStorage_compress_dataIn));
    ...
    DmaData_SPU::putAndWait(&compressDataIn, eaDataIn,
                            sizeof(CompressedTileStorage_compress_dataIn));
