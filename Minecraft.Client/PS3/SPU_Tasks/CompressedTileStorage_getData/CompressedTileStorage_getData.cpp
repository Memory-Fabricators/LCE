...
class CCompressedTileStorage_getData
{
  ...
  public:
    CCompressedTileStorage_getData(unsigned char *idxAndData, int dataSize)
    {
        ...
        DmaData_SPU::getAndWait(indicesAndData, (uintptr_t)idxAndData,
...
void CCompressedTileStorage_getData::getData(uint8_t *retArray,
                                             unsigned int retOffset)
{
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        else
        {
            ...
            int tiletypemask = tiletypecount -
            ...
            unsigned char *tile_types =
                data + ((blockIndices[i] >> INDEX_OFFSET_SHIFT) & INDEX_OFFSET_MASK);
