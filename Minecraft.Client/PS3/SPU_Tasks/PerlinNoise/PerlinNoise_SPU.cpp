...
//
// public:
// 	PPUStoreArray(uintptr_t pDataPPU) { m_pDataPPU = (double*)pDataPPU;
// m_cachePos = 0; m_ppuPos = 0; flush();}
//
// 	void store(int val)
// 	{
...
// 			// 			spu_print("DMAing %d bytes from
// 0x%08x(SPU) to 0x%08x(PPU)\n",(int)( m_cachePos*sizeof(int)),
// (int)m_localCache, (int)&m_pDataPPU[m_ppuPos]);
// 			DmaData_SPU::put(m_localCache,
// (uintptr_t)&m_pDataPPU[m_ppuPos],
// DmaData_SPU::roundUpDMASize(m_cachePos*sizeof(double)));
// m_ppuPos += m_cachePos; 			m_cachePos = 0;
// 		}
...
void PerlinNoise_SPU::getRegion(double *buffer, int x, int y, int z, int xSize,
                                ...
                                double yScale, double zScale)
{
    ...
    {
        //            value += noiseLevels[i].getValue(x * pow, y * pow, z * pow) /
        //            pow;
        ...
        xx += xb;
        zz += zb;
        noiseLevels[i].add(localBuffer, xx, yy, zz, xSize, ySize, zSize,
...
void PerlinNoise_SPU::getRegion(double *sr, int x, int z, int xSize, int zSize,
