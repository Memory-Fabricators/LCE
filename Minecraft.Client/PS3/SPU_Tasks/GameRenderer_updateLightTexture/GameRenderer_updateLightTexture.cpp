...
{
    // 4J - we've added separate light textures for all dimensions, and this loop
    // to update them all here
    for (int j = 0; j < 3; j++)
    {
        int lightPixels[16 * 16];
        GameRenderer_updateLightTexture_dataIn::LevelData &levelDataIn =
            dataIn.m_levelData[j];
        ...
        for (int i = 0; i < 256; i++)
        {
            ...
            float block =
                levelDataIn.m_brightnessRamp[i % 16] * (dataIn.blr * 0.1f + 1.5f);
            ...
            /*
             * float dr = darken; dr = dr - dr * dr; System.out.println(dr); if (dr >
             * 0) { gs += dr * 0.5f; rs += dr; }
             */
        ...
        DmaData_SPU::putAndWait(lightPixels,
                                (uintptr_t)levelDataIn.m_lightPixelsOutput,
                                16 * 16 * sizeof(int));
    }
}
                           ...
                           CellSpursJob256 *pJob)
{
    ...
    DmaData_SPU::getAndWait(&dataIn, eaDataIn,
                            sizeof(GameRenderer_updateLightTexture_dataIn));
