...
void ConsoleUIController::init(S32 w, S32 h)
{
  ...
  size we pass. */
    ...
    void *rt_mem =
        RenderManager.allocVRAM(rt_mem_size, CELL_GCM_TILE_ALIGN_OFFSET);
    ...
    gdraw_GCM_SetResourceMemory(GDRAW_GCM_RESOURCE_texture, 200,
                                RenderManager.allocVRAM(tex_mem_size, 128),
                                tex_mem_size);
    gdraw_GCM_SetResourceMemory(GDRAW_GCM_RESOURCE_vertexbuffer, 1000,
                                RenderManager.allocVRAM(vert_mem_size, 128),
                                vert_mem_size);

    // In this example, we allocate dynamic vertex buffer memory from main memory,
    // which is generally faster.
    S32 dyn_vert_size = 128 * 1024;
    gdraw_GCM_SetResourceMemory(GDRAW_GCM_RESOURCE_dyn_vertexbuffer, 0,
                                RenderManager.allocIOMem(dyn_vert_size, 128),
                                dyn_vert_size);
    ...
    // memory for GDraw to store its shaders etc. in (this is the last alloc!).
    // Unlike the rest of the memory configuration, the work area is reserved to
    // GDraw and can't be freed or relocated afterwards without shutting GDraw and
    // all attached Iggys down completely. But it's fairly small (64k as of this
    // writing) so this shouldn't be much of a problem.
    void *gdraw_work_mem =
        RenderManager.allocVRAM(GDRAW_GCM_LOCAL_WORKMEM_SIZE, 128);
    ...
    // Initialize audio
    // TODO: 4J Stu - Currently Iggy crashes if I have audio enabled. Disabling
    // for now.
    // IggyAudioUseDefault();
...
{
    gdraw_GCM_SetTileOrigin(RenderManager.getCurrentBackBufferSurface(), xPos,
                            yPos);
}
