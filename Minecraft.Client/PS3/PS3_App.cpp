...
BOOL CConsoleMinecraftApp::ReadProductCodes()
{
    ...
    HANDLE file = CreateFile("PS3/PS3ProductCodes.bin", GENERIC_READ, 0, NULL,
                             OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (file == INVALID_HANDLE_VALUE)
    {
        // DWORD error = GetLastError();
        app.DebugPrintf(
            "Failed to open ProductCodes.bin\n"); // with error code %d (%x)\n",
                                                  // error, error);
        return FALSE;
    }
    ...
    if (dwFileSize != 0)
    {
        ...
        WRAPPED_READFILE(file, ProductCodes.chProductCode, PRODUCT_CODE_SIZE,
                         &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chDiscProductCode, PRODUCT_CODE_SIZE,
                         &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chSaveFolderPrefix,
                         SAVEFOLDERPREFIX_SIZE, &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chDiscSaveFolderPrefix,
                         SAVEFOLDERPREFIX_SIZE, &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chCommerceCategory,
                         COMMERCE_CATEGORY_SIZE, &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chTexturePackID,
                         SCE_NP_COMMERCE2_CATEGORY_ID_LEN, &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chUpgradeKey, UPGRADE_KEY_SIZE,
                         &bytesRead, NULL);
        WRAPPED_READFILE(file, ProductCodes.chSkuPostfix, SKU_POSTFIX_SIZE,
                         &bytesRead, NULL);
        ...
        for (unsigned int i = 0; i < uiDLC; i++)
        {
            ...
            WRAPPED_READFILE(file, &pDLCInfo->eDLCType, sizeof(int), &bytesRead,
                             NULL);
            WRAPPED_READFILE(file, &pDLCInfo->iFirstSkin, sizeof(int), &bytesRead,
                             NULL);
...
void CConsoleMinecraftApp::FatalLoadError()
{
    ...
    case XC_LANGUAGE_GERMAN:
        ...
        aStrings[1] = L"Minecraft: PlayStation®3 Edition konnte nicht geladen "
                      L"werden und kann daher nicht fortgesetzt werden.";
        aStrings[2] = L"Spiel verlassen";
        break;
    case XC_LANGUAGE_SPANISH:
        aStrings[0] = L"Error al cargar";
        aStrings[1] = L"Se ha producido un error al cargar Minecraft: "
                      L"PlayStation®3 Edition y no es posible continuar.";
        aStrings[2] = L"Salir del juego";
        break;
    case XC_LANGUAGE_FRENCH:
        aStrings[0] = L"Échec du chargement";
        aStrings[1] = L"Le chargement de Minecraft: PlayStation®3 Edition a "
                      L"échoué : impossible de continuer.";
        aStrings[2] = L"Quitter le jeu";
        break;
    case XC_LANGUAGE_ITALIAN:
        aStrings[0] = L"Errore caricamento";
        aStrings[1] = L"Caricamento \"Minecraft: PlayStation®3 Edition\" non "
                      L"riuscito, impossibile continuare.";
        aStrings[2] = L"Esci dal gioco";
        break;
    case XC_LANGUAGE_JAPANESE:
        aStrings[0] = L"ロード エラー";
        aStrings[1] =
            L"Minecraft  PlayStation®3 版のロードに失敗しました。続行できません";
        aStrings[2] = L"ゲームを終了";
        break;
    case XC_LANGUAGE_KOREAN:
        aStrings[0] = L"불러오기 오류";
        aStrings[1] = L"\"Minecraft: PlayStation®3 Edition\"을 불러오는 중에 "
                      L"오류가 발생하여 계속할 수 없습니다.";
        aStrings[2] = L"게임 나가기";
        break;
    case XC_LANGUAGE_PORTUGUESE:
        // can only tell if it's brazilian when we've read the productcodes.bin,
        // which might have failed, so stick with Portuguese

        aStrings[0] = L"Erro de Carregamento";
        aStrings[1] = L"Não foi possível carregar Minecraft: PlayStation®3 Edition "
        ...
        /*	case XC_LANGUAGE_BRAZILIAN:
                                ...
                                aStrings[0] = L"Erro de carregamento";
                                aStrings[1] = L"\"Minecraft: PlayStation®3 Edition\"
           falhou ao carregar e não é possível continuar."; aStrings[2] = L"Sair do
           Jogo"; break;*/
    case XC_LANGUAGE_RUSSIAN:
        // Brazilian Portuguese
        aStrings[0] = L"Ошибка при загрузке";
        aStrings[1] = L"Не удалось загрузить игру \"Minecraft: PlayStation®3 "
                      L"Edition\". Продолжить загрузку невозможно.";
        aStrings[2] = L"Выйти из игры";
        break;
    case XC_LANGUAGE_TCHINESE:
        aStrings[0] = L"載入錯誤";
        aStrings[1] =
            L"無法載入「Minecraft: PlayStation®3 Edition」，因此無法繼續。";
        aStrings[2] = L"離開遊戲";
        break;
    ...
    case XC_LANGUAGE_FINISH:
        ...
        aStrings[1] = L"\"Minecraft: PlayStation®3 Edition\" -pelin lataaminen ei "
                      L"onnistunut, eikä sitä voi jatkaa.";
        aStrings[2] = L"Poistu pelistä";
        break;
    case XC_LANGUAGE_SWEDISH:
        aStrings[0] = L"Laddningsfel";
        aStrings[1] = L"\"Minecraft: PlayStation®3 Edition\" kunde inte laddas och "
                      L"kan inte fortsätta.";
        aStrings[2] = L"Avsluta";
        break;
    case XC_LANGUAGE_DANISH:
        aStrings[0] = L"Indlæsningsfejl";
        aStrings[1] = L"\"Minecraft: PlayStation®3 Edition\" kunne ikke blive "
                      L"indlæst og kan derfor ikke fortsætte.";
        aStrings[2] = L"Afslut spil";
        break;
    case XC_LANGUAGE_BNORWEGIAN:
        aStrings[0] = L"Innlastingsfeil";
        aStrings[1] = L"Feil under innlasting av Minecraft: PlayStation®3 Edition. "
                      L"Kan ikke fortsette.";
        aStrings[2] = L"Avslutt spill";
        break;
    case XC_LANGUAGE_POLISH:
        aStrings[0] = L"Błąd wczytywania";
        aStrings[1] = L"Wystąpił błąd podczas wczytywania Minecraft: Edycja "
                      L"PlayStation®3. Dalsze działanie jest niemożliwe.";
        aStrings[2] = L"Wyjdź z gry";
        break;
    case XC_LANGUAGE_TURKISH:
        aStrings[0] = L"Yükleme Hatası";
        aStrings[1] =
            L"Minecraft: PlayStation®3 Edition yüklenemedi ve devam edemiyor.";
        aStrings[2] = L"Oyundan Çık";
        break;
    case XC_LANGUAGE_LATINAMERICANSPANISH:
        aStrings[0] = L"Error al cargar";
        aStrings[1] = L"Se produjo un error al cargar Minecraft: PlayStation®3 "
                      L"Edition y no es posible continuar.";
        aStrings[2] = L"Salir de la partida";
        break;

    default:
        aStrings[0] = L"Loading Error";
        aStrings[1] = L"\"Minecraft: PlayStation®3 Edition\" has failed to load, "
                      L"and cannot continue.";
    ...
    dstlen = 256;
    L10nResult lres =
        UTF16stoUTF8s((uint16_t *)aStrings[1], &srclen, u8Message, &dstlen);

    // 4J-PB - let's have a sleep here, to give any disc eject system message time
    // to come in
    Sleep(250);
    ...
    if (ShutdownManager::ShouldRun(ShutdownManager::eMainThread) == false)
    {
        ...
        app.DebugPrintf(
            "The Fatal Error message box will possibly fail, so just exit\n");
        ShutdownManager::MainThreadHandleShutdown();
        _Exit(0);
    }

    app.DebugPrintf("Requesting Message Box for Fatal Error\n");
    EMsgBoxResult eResult = InputManager.RequestMessageBox(
        EMSgBoxType_None, 0, NULL, this, (char *)u8Message, 0);
    ...
    while ((eResult == EMsgBox_Busy) || (eResult == EMsgBox_SysUtilBusy))
    {
        ...
        app.DebugPrintf(
            "Requesting Message Box for Fatal Error again due to BUSY\n");
        eResult = InputManager.RequestMessageBox(EMSgBoxType_None, 0, NULL, this,
                                                 (char *)u8Message, 0);
    }
...
void CConsoleMinecraftApp::TemporaryCreateGameStart()
{
    ...
    __int64 seedValue =
        0; // BiomeSource::findSeed(isFlat?LevelType::lvl_flat:LevelType::lvl_normal);
           // // 4J - was (new Random())->nextLong() - now trying
           // to actually find a seed to suit our requirements
...
void CConsoleMinecraftApp::CommerceTick()
{
    ...
    if (ProfileManager.IsSignedInLive(0))
    {
        ...
        case eCommerce_State_Init:
            ...
            m_pCommerce->CreateSession(&CConsoleMinecraftApp::CommerceInitCallback,
                                       this);
            break;
        ...
        case eCommerce_State_GetProductList:
            {
                ...
                m_pCommerce->GetProductList(
                    ...
                    &m_ProductListA[m_ProductListRetrievedC], category.categoryId);
            }
        ...
        case eCommerce_State_AddProductInfoDetailed:
            {
                ...
                // for each of the products in the categories, get the detailed info. We
                // really only need the long description and price info.
                SonyCommerce::CategoryInfo *pCategories = app.GetCategoryInfo();
                std::list<SonyCommerce::CategoryInfoSub>::iterator iter =
                    pCategories->subCategories.begin();
                ...
                else
                {
                    ...
                    m_pCommerce->AddDetailedProductInfo(
                        &CConsoleMinecraftApp::CommerceAddDetailedProductInfoCallback, this,
                        pProductInfo, pProductInfo->productId, category.categoryId);
                }
            }
            break;
        case eCommerce_State_Checkout:
            m_pCommerce->CreateSession(
                &CConsoleMinecraftApp::CheckoutSessionStartedCallback, this);
        ...
        case eCommerce_State_Checkout_SessionStarted:
            ...
            m_pCommerce->Checkout(&CConsoleMinecraftApp::CommerceCheckoutCallback,
                                  this, m_pchSkuID);
            break;
        ...
        case eCommerce_State_DownloadAlreadyPurchased:
            m_pCommerce->CreateSession(
                &CConsoleMinecraftApp::DownloadAlreadyPurchasedSessionStartedCallback,
                this);
            m_eCommerce_State =
        ...
        case eCommerce_State_DownloadAlreadyPurchased_SessionStarted:
            ...
            m_pCommerce->DownloadAlreadyPurchased(
                &CConsoleMinecraftApp::CommerceCheckoutCallback, this, m_pchSkuID);
            break;

        case eCommerce_State_UpgradeTrial:
            m_pCommerce->CreateSession(
                &CConsoleMinecraftApp::UpgradeTrialSessionStartedCallback, this);
            m_eCommerce_State = eCommerce_State_UpgradeTrial_WaitingForSession;
            break;
        case eCommerce_State_UpgradeTrial_SessionStarted:
            m_pCommerce->UpgradeTrial(&CConsoleMinecraftApp::CommerceCheckoutCallback,
                                      this);
            m_eCommerce_State = eCommerce_State_UpgradeTrial_Pending;
            break;
        }

        // 4J-PB - bit of a hack to display the full version purchase after signing
        // in during a trial trophy popup
...
bool CConsoleMinecraftApp::UpgradeTrial()
{
    ...
    else if (m_eCommerce_State == eCommerce_State_Error)
    {
        ...
        C4JStorage::EMessageResult result =
            ui.RequestMessageBox(IDS_PRO_UNLOCKGAME_TITLE, IDS_NO_DLCOFFERS, uiIDA,
...
std::vector<SonyCommerce::ProductInfo> *
CConsoleMinecraftApp::GetProductList(int iIndex)
{
                                                          ...
                                                          int err)
{
    ...
    if (err == 0)
    {
        ...
        if (pClass->m_ProductListRetrievedC ==
            pClass->m_CategoryInfo.countOfSubCategories)
        {
            // we're done, so now retrieve the additional product details for each
            // product
    ...
    LPVOID lpParam, int err)
{
    ...
    if (err == 0)
    {
        // increment the current product counter. When this gets to the end of the
        // products, move to the next category
        pClass->m_iCurrentProduct++;

        std::vector<SonyCommerce::ProductInfo> *pvProductList =
            &pClass->m_ProductListA[pClass->m_iCurrentCategory];

        // if there are no more products in this category, move to the next category
        // (there may be no products in the category)
        if (pClass->m_iCurrentProduct >= pvProductList->size())
        {
            ...
            } while (pClass->m_ProductListA[pClass->m_iCurrentCategory].size() == 0 &&
...
char *PatchFilelist[] = {
    "PS3/PS3ProductCodes.bin", "Common/Media/MediaPS3.arc",
    // patch 7
    "PS3/Sound/Minecraft.msscmp", "font/Default.png",
    ...
    "music/music/creative1.binka", "music/music/creative2.binka",
    "music/music/creative3.binka", "music/music/creative4.binka",
    "music/music/creative5.binka", "music/music/creative6.binka",
    "music/music/menu1.binka", "music/music/menu2.binka",
    "music/music/menu3.binka", "music/music/menu4.binka",
...
bool CConsoleMinecraftApp::CheckForEmptyStore(int iPad)
{
    ...
    if (bEmptyStore)
    {
        ...
        C4JStorage::EMessageResult result = ui.RequestMessageBox(
            IDS_DOWNLOADABLE_CONTENT_OFFERS, IDS_NO_DLCOFFERS, uiIDA, 1, iPad);
    }
