...
LeaderboardManager *LeaderboardManager::m_instance =
    new PS3LeaderboardManager(); // Singleton instance of the LeaderboardManager
#endif
...
int PS3LeaderboardManager::scoreboardThreadEntry(LPVOID lpParam)
{
    ...
    PS3LeaderboardManager *self =
        reinterpret_cast<PS3LeaderboardManager *>(lpParam);
...
void PS3LeaderboardManager::scoreboardThreadInternal()
{
    ...
    if (m_titleContext == -1)
    {
        ...
        if (ret < 0)
        {
            ...
            {
                app.DebugPrintf(
                    "[LeaderboardManager] sceNpScoreInit() failed. ret = 0x%x\n", ret);
                return;
            }
            else
            {
                app.DebugPrintf("[LeaderboardManager] sceNpScoreInit() already "
                                "initialised, (0x%x)\n",
                                ret);
            }
        }
        ...
        if (ret < 0)
        {
            app.DebugPrintf("[LeaderboardManager] OpenSession(): sceNpManagerGetNpId "
                            "FAILED, ret == %X.\n",
                            ret);
            return;
        }

        ret = sceNpScoreCreateTitleCtx(&s_npCommunicationId,
                                       &s_npCommunicationPassphrase, &npId);

        if (ret < 0)
        {
            app.DebugPrintf("[LeaderboardManager] OpenSession(): "
                            "sceNpScoreCreateTitleCtx FAILED, ret == %X.\n",
                            ret);
            return;
        }
        else
...
bool PS3LeaderboardManager::getScoreByIds()
{
    ...
    else if (ret < 0)
    {
        ...
        app.DebugPrintf("[LeaderboardManager]\tgetScoreByIds() - createTransaction "
                        "failed, ret=0x%X\n",
                        ret);
    ...
    /* app.DebugPrintf("sceNpScoreGetRankingByNpId(\n\t transaction=%i,\n\t
       ...
       friendCount=%i,\n...\n", transaction, npId, friendCount, sizeof(SceNpId),
       friendCount*sizeof(SceNpId), rankData,
    ...
    {
        app.DebugPrintf(
            "[LeaderboardManager] getScoreByIds(): Game ranking not found.\n");
...
bool PS3LeaderboardManager::setScore()
{
    ...
    writeTitleContext = sceNpScoreCreateTitleCtx(
    ...
    ret = sceNpScoreRecordScore(m_transactionCtx, // transId,
                                boardId,          // boardId,
...
bool PS3LeaderboardManager::ReadStats_Friends(LeaderboardReadListener *listener,
...
// myUID ignored on PS3.
bool PS3LeaderboardManager::ReadStats_MyScore(LeaderboardReadListener *listener,
                                              int difficulty, EStatsType type,
                                              PlayerUID myUID,
                                              unsigned int readCount)
{
...
// myUID ignored on PS3.
bool PS3LeaderboardManager::ReadStats_TopRank(LeaderboardReadListener *listener,
                                              int difficulty, EStatsType type,
                                              unsigned int startIndex,
                                              unsigned int readCount)
{
...
/*
SceNpScoreRankData *PS3LeaderboardManager::filterJustScorers(unsigned int &num,
...
SceNpScorePlayerRankData *
PS3LeaderboardManager::addPadding(unsigned int num,
...
void PS3LeaderboardManager::convertToOutput(unsigned int &num, ReadScore *out,
...
void PS3LeaderboardManager::initReadScoreStruct(ReadScore &out,
                                                SceNpScoreRankData &rankData)
{
    ...
    wstring wstrName =
        convStringToWstring(string(rankData.onlineName.data)).c_str();
    // memcpy(&out.m_name, wstrName.c_str(), XUSER_NAME_SIZE);
    out.m_name = wstrName;
}

void PS3LeaderboardManager::fillReadScoreStruct(ReadScore &out,
                                                SceNpScoreComment &comment)
{
    StatsData statsData;
...
bool PS3LeaderboardManager::SortByRank(const ReadScore &lhs,
                                       const ReadScore &rhs)
{
