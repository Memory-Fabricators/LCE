...
int SonyRemoteStorage_PS3::initPreconditions()
{
    ...
    ret = sceNpManagerRequestTicket2(&npId, &ticketVersion, TICKETING_SERVICE_ID,
                                     NULL, 0, NULL, 0);
    if (ret < 0)
    {
        return ret;
...
void SonyRemoteStorage_PS3::staticInternalCallback(
...
void SonyRemoteStorage_PS3::internalCallback(const SceRemoteStorageEvent event,
                                             int32_t retCode)
{
    ...
    case ERROR_OCCURRED:
        ...
        // 		shutdown();				// removed, as the
        // remote storage lib now tries to reconnect if an error has occurred
        m_status = e_error;
        runCallback();
    ...
    case GET_DATA_RESULT:
        ...
        {
            app.DebugPrintf("An error occurred while Get Data was being processed. "
                            "retCode: 0x%x \n",
                            retCode);
            m_status = e_error;
        }
    ...
    case GET_STATUS_RESULT:
        ...
        {
            app.DebugPrintf("An error occurred while Get Status was being processed. "
                            "retCode: 0x%x \n",
                            retCode);
            m_status = e_error;
        }
    ...
    case SET_DATA_RESULT:
        ...
        {
            app.DebugPrintf("An error occurred while Set Data was being processed. "
                            "retCode: 0x%x \n",
                            retCode);
            m_status = e_error;
        }
    ...
    case WEB_BROWSER_RESULT:
        app.DebugPrintf(
            "This function is not used on PS Vita, as the account will be linked, "
            "it is not needed to open a browser to link it \n");
        assert(0);
        break;
...
bool SonyRemoteStorage_PS3::init(CallbackFunc cb, LPVOID lpParam)
{
    ...
    params.thread.threadPriority =
        1000; // Must be between [0-3071], being 0 the highest.
...
bool SonyRemoteStorage_PS3::getRemoteFileInfo(SceRemoteStorageStatus *pInfo,
...
bool SonyRemoteStorage_PS3::setDataInternal()
{
    ...
    if (m_thumbnailData)
    {
        ...
        app.GetImageTextData(m_thumbnailData, m_thumbnailDataSize,
                             (unsigned char *)seed, uiHostOptions, bHostOptionsRead,
                             uiTexturePack);
    ...
    memcpy(descData.m_saveNameUTF8, m_saveFileDesc,
           strlen(m_saveFileDesc) + 1); // plus null
...
bool SonyRemoteStorage_PS3::getData(const char *remotePath,
...
int SonyRemoteStorage_PS3::LoadCompressCallback(void *pParam, bool bIsCorrupt,
                                                bool bIsOwner)
{
...
void SonyRemoteStorage_PS3::CompressSaveData()
{
    ...
    C4JStorage::ESaveGameState eLoadStatus = StorageManager.LoadSaveData(
        m_setDataSaveInfo, &LoadCompressCallback, this);
    ...
    if (m_compressedSaveState == e_state_CompressedSave)
    {
        ...
        waitForStorageManagerIdle();
        StorageManager.SetDefaultSaveImage(); // we can't get the save image back to
                                              // overwrite, so set it to the default
        C4JStorage::ESaveGameState storageState = StorageManager.SaveSaveData(
            &SaveCompressCallback, this,
            true); // only save the data file, so we don't overwrite the icon
        ...
        {
            app.DebugPrintf(
                "StorageManager.SaveSaveData failed, save is left uncompressed\n");
        }
    }
    waitForStorageManagerIdle();
