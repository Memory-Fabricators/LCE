...
std::vector<SonyCommerce_PS3::ProductInfo>
    *SonyCommerce_PS3::m_pProductInfoList = NULL;
SonyCommerce_PS3::ProductInfoDetailed
    *SonyCommerce_PS3::m_pProductInfoDetailed = NULL;
...
SonyCommerce_PS3::DownloadListInputParams
    SonyCommerce_PS3::m_downloadInputParams;

SonyCommerce_PS3::CallbackFunc SonyCommerce_PS3::m_callbackFunc = NULL;
sys_memory_container_t SonyCommerce_PS3::m_memContainer =
...
bool SonyCommerce_PS3::m_contextCreated =
    false; ///< npcommerce2 context ID created?
SonyCommerce_PS3::Phase SonyCommerce_PS3::m_currentPhase =
...
bool SonyCommerce_PS3::m_bLicenseChecked =
...
void SonyCommerce_PS3::CheckForTrialUpgradeKey_Callback(LPVOID param,
                                                        bool bFullVersion)
{
...
int SonyCommerce_PS3::getProductList(std::vector<ProductInfo> *productList,
                                     char *categoryId)
{
    int ret = 0;
    uint32_t requestId;
    ...
    ret = sceNpCommerce2GetCategoryContentsGetResult(requestId, m_commercebuffer,
                                                     bufSize, &fillSize);
    if (ret < 0)
    {
        sceNpCommerce2DestroyReq(requestId);
    ...
    for (int i = 0; i < result.rangeOfContents.count; i++)
    {
        ...
        if (contentInfo.contentType == SCE_NP_COMMERCE2_CONTENT_TYPE_PRODUCT)
        {
            ...
            {
                strncpy(tempInfo.longDescription, productInfo.productLongDescription,
            ...
            if (productInfo.countOfSku == 1)
            {
                ...
                ret = sceNpCommerce2GetPrice(m_contextId, tempInfo.price,
                                             sizeof(tempInfo.price), skuInfo.price);
    ...
    if (categoryInfo.countOfSubCategory > 0)
    {
        ...
        for (int i = 0; i < result.rangeOfContents.count; i++)
        {
            ...
            if (contentInfo.contentType == SCE_NP_COMMERCE2_CONTENT_TYPE_CATEGORY)
            {
                ...
                ret = sceNpCommerce2GetCategoryInfoFromContentInfo(&contentInfo,
                                                                   &categoryInfo);
                if (ret < 0)
                {
                    sceNpCommerce2DestroyGetCategoryContentsResult(&result);
...
int SonyCommerce_PS3::getDetailedProductInfo(ProductInfoDetailed *pInfo,
...
int SonyCommerce_PS3::addDetailedProductInfo(ProductInfo *info,
                                             const char *productId,
                                             char *categoryId)
{
    int ret = 0;
    ...
    if (productInfo.countOfSku == 1)
    {
        ...
        ret = sceNpCommerce2GetPrice(m_contextId, info->price, sizeof(info->price),
                                     skuInfo.price);
        if (ret < 0)
        {
            sceNpCommerce2DestroyGetProductInfoResult(&result);
...
int SonyCommerce_PS3::downloadList(DownloadListInputParams &params)
{
    ...
    ret = sceNpCommerce2DoDlListStartAsync(m_contextId, app.GetCommerceCategory(),
                                           skuIdsTemp, params.skuIds.size(),
                                           *params.memContainer);
    if (ret < 0)
    {
        return ret;
...
void SonyCommerce_PS3::UpgradeTrialCallback1(LPVOID lpParam, int err)
{
    ...
    if (err == CELL_OK)
    {
            ...
            SCE_NP_COMMERCE2_SKU_PURCHASABILITY_FLAG_OFF)
        {
            ...
            pCommerce->DownloadAlreadyPurchased(UpgradeTrialCallback2, pCommerce,
                                                skuID);
        }
        else
        {
...
void SonyCommerce_PS3::commerce2Handler(uint32_t contextId, uint32_t subjectId,
...
void SonyCommerce_PS3::GetProductList(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce_PS3::GetDetailedProductInfo(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce_PS3::AddDetailedProductInfo(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce_PS3::GetCategoryInfo(CallbackFunc cb, LPVOID lpParam,
                                       CategoryInfo *info,
                                       const char *categoryId)
{
    EnterCriticalSection(&m_queueLock);
...
void SonyCommerce_PS3::Checkout(CallbackFunc cb, LPVOID lpParam,
                                const char *skuID)
{
    if (m_memContainer != SYS_MEMORY_CONTAINER_ID_INVALID)
    {
...
void SonyCommerce_PS3::DownloadAlreadyPurchased(CallbackFunc cb, LPVOID lpParam,
                                                const char *skuID)
{
    if (m_memContainer != SYS_MEMORY_CONTAINER_ID_INVALID)
    {
