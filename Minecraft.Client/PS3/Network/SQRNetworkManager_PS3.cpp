...
int (*SQRNetworkManager_PS3::s_SignInCompleteCallbackFn)(void *pParam,
                                                         bool bContinue,
                                                         int pad) = NULL;
...
SQRNetworkManager_PS3::PresenceSyncInfo
    SQRNetworkManager_PS3::s_lastPresenceSyncInfo = {0};
SQRNetworkManager_PS3::PresenceSyncInfo
    SQRNetworkManager_PS3::c_presenceSyncInfoNULL = {0};
SceNpBasicAttachmentDataId SQRNetworkManager_PS3::s_lastInviteIdToRetry =
...
const SQRNetworkManager_PS3::eSQRNetworkManagerState SQRNetworkManager_PS3::
    m_INTtoEXTStateMappings[SQRNetworkManager_PS3::SNM_INT_STATE_COUNT] = {
...
SQRNetworkManager_PS3::SQRNetworkManager_PS3(
    ISQRNetworkManagerListener *listener)
{
    ...
    sys_event_port_create(&m_basicEventPort, SYS_EVENT_PORT_LOCAL,
                          SYS_EVENT_PORT_NO_NAME);

    sys_event_queue_attribute_t queue_attr = {SYS_SYNC_PRIORITY, SYS_PPU_QUEUE};
    sys_event_queue_create(&m_basicEventQueue, &queue_attr, SYS_EVENT_QUEUE_LOCAL,
                           127);
...
#ifdef __PS3__
    ret = cellRudpEnableInternalIOThread(RUDP_THREAD_STACK_SIZE,
                                         RUDP_THREAD_PRIORITY);

    // Register for sys util callbacks... we need this for finding out what state
    // the network dialog stuff has got to

    ret = cellSysutilRegisterCallback(0, SysUtilCallback, this);

    // Kick off the Network Start Dialog Utility. This interacts with the user to
    // make sure they are signed in & ready for network play.
...
// ContextCallback. The SQRNetworkManager_PS3 is then finally moved into
...
void SQRNetworkManager_PS3::InitialiseAfterOnline()
{
    ...
    // stage (SQRNetworkManager_PS3::Initialise) or we've had a network disconnect
    // and are coming in from an offline state. Don't do anything otherwise - this
    // mainly to catch a bit of a corner case in the initialisation phase where
    // potentially we could register for the callback that would call this with
    // sceNpManagerRegisterCallback. and could then really quickly go online so
    // that the there becomes two paths (via the callback or
    // SQRNetworkManager_PS3::Initialise) by which this could be called
...
void SQRNetworkManager_PS3::CreateAndJoinRoom(int hostIndex,
                                              int localPlayerMask,
    ...
    if (m_searchResultCount)
    {
        ...
        for (unsigned int i = 0; i < m_searchResultCount;)
        {
            if (uniqueRooms.find(m_aSearchResultRoomId[i]) == uniqueRooms.end())
            {
                ...
                m_aSearchResultServerId[i] =
                    m_aSearchResultServerId[m_searchResultCount];
                m_aSearchResultRoomFound[i] =
                    m_aSearchResultRoomFound[m_searchResultCount];
                m_aSearchResultRoomExtDataReceived[i] =
                    m_aSearchResultRoomExtDataReceived[m_searchResultCount];
            }
            else
            {
        ...
        if (event.data1 == 0)
        {
            ...
            if (ret == 0)
            {
                if (iEvent == SCE_NP_BASIC_EVENT_INCOMING_BOOTABLE_INVITATION)
                {
                    ...
                    // gameplay. Players can bring this up from LoadOrJoinMenu, PauseMenu
                    // and InGameInfoMenu
...
int SQRNetworkManager_PS3::GetFriendsThreadProc(void *lpParameter)
{
    ...
    {
        // This is likely when friend list hasn't been received from the server yet
        // - will be returning SCE_NP_BASIC_ERROR_BUSY in this case
        manager->m_friendCount = 0;
    }

    // There shouldn't ever be more than 100 friends returned but limit here just
    // in case
    ...
    // just received, so only add in friends that we successfully get an entry for
    for (unsigned int i = 0; i < manager->m_friendCount; i++)
    {
        ...
        int ret =
            sceNpBasicGetFriendPresenceByIndex2(i, &userInfo, &presenceDetails, 0);

        if ((ret == 0) &&
            (!manager->ForceErrorPoint(SNM_FORCE_ERROR_GET_FRIEND_LIST_ENTRY)))
        {
            memcpy(&manager->m_aSearchResultNpId[manager->m_searchResultCount],
                   &userInfo.userId, sizeof(SceNpId));
            ...
            if (presenceDetails.size == sizeof(PresenceSyncInfo))
            {
                ...
                if (pso->netVersion == MINECRAFT_NET_VERSION)
                {
                    if (!pso->inviteOnly)
                    {
                        manager->m_aSearchResultRoomFound[manager->m_searchResultCount] =
                            true;
                        manager->m_aSearchResultRoomId[manager->m_searchResultCount] =
                            pso->m_RoomId;
                        manager->m_aSearchResultServerId[manager->m_searchResultCount] =
                            pso->m_ServerId;

                        CPlatformNetworkManagerSony::MallocAndSetExtDataFromSQRPresenceInfo(
                            &manager->m_aSearchResultRoomExtDataReceived
...
// returns false
void SQRNetworkManager_PS3::FriendRoomManagerGetRoomInfo(
    int idx, SQRNetworkManager_PS3::SessionSearchResult *searchResult)
{
...
SQRNetworkManager_PS3::eSQRNetworkManagerState
SQRNetworkManager_PS3::GetState()
{
...
SQRNetworkPlayer *
SQRNetworkManager_PS3::GetPlayerIfReady(SQRNetworkPlayer *player)
{
    if (player == NULL)
    {
        return NULL;
...
void SQRNetworkManager_PS3::SetState(
    SQRNetworkManager_PS3::eSQRNetworkManagerInternalState state)
{
...
// FriendRoomManagerGetCount(). Only valid when FriendRoomManagerIsBusy()
// returns false
bool SQRNetworkManager_PS3::JoinRoom(
    SQRNetworkManager_PS3::SessionSearchResult *searchResult,
    int localPlayerMask)
{
...
bool SQRNetworkManager_PS3::JoinRoom(
    ...
    int localPlayerMask,
    const SQRNetworkManager_PS3::PresenceSyncInfo *presence)
{
...
void SQRNetworkManager_PS3::SendInviteGUI()
{
    ...
    uint8_t *subject =
        mallocAndCreateUTF8ArrayFromString(IDS_INVITATION_SUBJECT_MAX_18_CHARS);
...
// persists forever.
void SQRNetworkManager_PS3::GetInviteDataAndProcess(
    SceNpBasicAttachmentDataId id)
{
    ...
    // If we fail ( which we might do if we aren't online at this point) then zero
    // the invite information and we'll try and get it later after (possibly)
    // signing in
    if (ret != 0)
    {
...
bool SQRNetworkManager_PS3::UpdateInviteData(
    SQRNetworkManager_PS3::PresenceSyncInfo *invite)
{
    size_t dataSize = sizeof(SQRNetworkManager_PS3::PresenceSyncInfo);
    int ret = sceNpBasicRecvMessageAttachmentLoad(s_lastInviteIdToRetry, invite,
                                                  &dataSize);
    return (ret == 0);
}
...
void SQRNetworkManager_PS3::FindOrCreateNonNetworkPlayer(
...
void SQRNetworkManager_PS3::LocalDataSend(SQRNetworkPlayer *playerFrom,
...
void SQRNetworkManager_PS3::MapRoomSlotPlayers(
...
bool SQRNetworkManager_PS3::AddRemotePlayersAndSync(
...
void SQRNetworkManager_PS3::RemoveRemotePlayersAndSync(
...
bool SQRNetworkManager_PS3::GetMatchingContext(
    eSQRNetworkManagerInternalState asyncState)
{
    ...
    {
        app.DebugPrintf("SQRNetworkManager::GetMatchingContext - "
                        "sceNpManagerGetNpId failed with code 0x%08x\n",
                        ret);
        return false;
    }

    //  Use Online Name and avatar to identify user
    int option = (SCE_NP_MATCHING2_CONTEXT_OPTION_USE_ONLINENAME |
                  SCE_NP_MATCHING2_CONTEXT_OPTION_USE_AVATARURL);
...
bool SQRNetworkManager_PS3::GetServerContext2()
{
    ...
    int serverCount =
        sceNpMatching2GetServerIdListLocal(m_matchingContext, NULL, 0);
    // If an error is returned here, we need to destroy and recerate our server -
    // if this goes ok we should come back through this path again
    if ((serverCount ==
         SCE_NP_MATCHING2_ERROR_CONTEXT_UNAVAILABLE) || // This error has been
                                                        // seen (occasionally) in
                                                        // a normal working
                                                        // environment
        (serverCount ==
         SCE_NP_MATCHING2_ERROR_CONTEXT_NOT_STARTED)) // Also checking for this as
                                                      // a means of simulating the
                                                      // previous error
    {
    ...
    m_aServerId = (SceNpMatching2ServerId *)realloc(
        m_aServerId, sizeof(SceNpMatching2ServerId) * serverCount);
    m_serverCount = sceNpMatching2GetServerIdListLocal(m_matchingContext,
                                                       m_aServerId, serverCount);
...
bool SQRNetworkManager_PS3::GetServerContext(SceNpMatching2ServerId serverId)
{
    ...
    {
        // Get list of server IDs of servers allocated to the application. We don't
        // actually need to do this, but it is as good a way as any to try a
        // matching2 service and check that the context *really* is valid.
        int serverCount =
            sceNpMatching2GetServerIdListLocal(m_matchingContext, NULL, 0);
        // If an error is returned here, we need to destroy and recerate our server
        // - if this goes ok we should come back through this path again
        if ((serverCount ==
            ...
            (serverCount ==
             SCE_NP_MATCHING2_ERROR_CONTEXT_NOT_STARTED)) // Also checking for this
                                                          // as a means of
                                                          // simulating the previous
                                                          // error
        {
            app.DebugPrintf(
                "SQRNetworkManager::GetServerContext - "
                "sceNpMatching2GetServerIdListLocal failed with error 0x%08x\n",
                serverCount);
            int ret = sceNpMatching2DestroyContext(m_matchingContext);
            app.DebugPrintf("SQRNetworkManager::GetServerContext - "
                            "sceNpMatching2DestroyContext returned 0x%08x\n",
                            ret);
            m_matchingContextValid = false;
            if (!GetMatchingContext(
                    SNM_INT_STATE_JOINING_STARTING_MATCHING_CONTEXT))
            {
                app.DebugPrintf("SQRNetworkManager::GetServerContext - Failed due to "
                                "no matching context after recreating\n");
                return false;
            }
        }
    }
    ...
    case SNM_INT_STATE_JOINING_SERVER_FOUND:
        {
            ...
            SetState((m_state == SNM_INT_STATE_HOSTING_SERVER_FOUND)
            ...
            int ret = sceNpMatching2CreateServerContext(
                m_matchingContext, &reqParam, NULL, &m_serverContextRequestId);
...
// registered callback.
void SQRNetworkManager_PS3::NetworkPlayerConnectionComplete(
...
void SQRNetworkManager_PS3::NetworkPlayerSmallIdAllocated(
...
void SQRNetworkManager_PS3::NetworkPlayerInitialDataReceived(
...
void SQRNetworkManager_PS3::NonNetworkPlayerComplete(SQRNetworkPlayer *player,
...
bool SQRNetworkManager_PS3::SelectRandomServer()
{
    ...
    // Remove this server from ones to try, if we need to try more than one to
    // find an available one
    m_serverCount--;
    m_aServerId[serverIdx] = m_aServerId[m_serverCount];

    // Kick off the search - we'll get a callback to DefaultRequestCallback with
    // the result from this
    app.DebugPrintf(CMinecraftApp::USER_RR,
                    "Kicking off sceNpMatching2GetServerInfo for server id %d\n",
                    reqParam.serverId);
    int ret = sceNpMatching2GetServerInfo(m_matchingContext, &reqParam, NULL,
    ...
    if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_GET_SERVER_INFO))
    {
        ...
        app.DebugPrintf(
            "SQRNetworkManager::SelectRandomServer - An error occured : 0x%08x\n",
            ret);
        return false;
    }
    return true;
...
void SQRNetworkManager_PS3::DeleteServerContext()
{
    if (m_serverContextValid)
    {
        ...
        int ret = sceNpMatching2DeleteServerContext(
            m_matchingContext, &reqParam, NULL, &m_serverContextRequestId);
...
bool SQRNetworkManager_PS3::CreateRudpConnections(
    ...
    int playerMask, SceNpMatching2RoomMemberId playersMemberId)
{
    ...
    app.DebugPrintf(CMinecraftApp::USER_RR,
                    ...
                    "connStatus %d peer add:0x%x peer port:0x%x\n",
    ...
    if (m_soc == -1)
    {
        ...
        ret =
            setsockopt(m_soc, SOL_SOCKET, SO_USESIGNATURE, &optval, sizeof(optval));
    ...
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++)
    {
        ...
        ret = cellRudpInitiate(rudpCtx, (struct sockaddr *)&sinp2pPeer,
                               sizeof(sinp2pPeer), 0);
...
SQRNetworkPlayer *
SQRNetworkManager_PS3::GetPlayerFromRoomMemberAndLocalIdx(int roomMember,
                                                          int localIdx)
{
    for (AUTO_VAR(it, m_RudpCtxToPlayerMap.begin());
...
bool SQRNetworkManager_PS3::RegisterCallbacks()
{
    ...
    ret = sceNpMatching2RegisterContextCallback(m_matchingContext,
    ...
    if (ret < 0)
    {
        app.DebugPrintf(
            "SQRNetworkManager::RegisterCallbacks - "
            "sceNpMatching2RegisterSignalingCallback failed with code 0x%08x\n",
            ret);
        return false;
    }
...
void SQRNetworkManager_PS3::ContextCallback(SceNpMatching2ContextId id,
    ...
    case SCE_NP_MATCHING2_CONTEXT_EVENT_Start:
        ...
        else
        {
            ...
            if (manager->m_doBootInviteCheck)
            {
                ...
                {
                    manager->GetInviteDataAndProcess(
                        SCE_NP_BASIC_SELECTED_INVITATION_DATA);
...
#ifdef __PS3__
void SQRNetworkManager_PS3::DefaultRequestCallback(
    SceNpMatching2ContextId id, SceNpMatching2RequestId reqId,
    SceNpMatching2Event event, SceNpMatching2EventKey eventKey, int errorCode,
    size_t dataSize, void *arg)
#else
void SQRNetworkManager_PS3::DefaultRequestCallback(
    SceNpMatching2ContextId id, SceNpMatching2RequestId reqId,
...
#endif
{
    ...
    // PS3 requires a call to get any data that is passed with the callback. On
    // PS3, the data is just passed in directly - do the getting of data here on
    // PS3 to get it into the same state as the PS4
#ifdef __PS3__
    void *data = 0;
    ...
    if (errorCode == 0)
    {
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_GetServerInfo:
            ...
            ret = sceNpMatching2GetEventData(
                id, eventKey, (void *)(manager->cServerInfoStorage), dataSize);
            if ((ret >= 0) &&
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_GET_SERVER_INFO_DATA))
            {
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_GetWorldInfoList:
            ...
            ret = sceNpMatching2GetEventData(
                ...
                (void *)(manager->cWorldListStorage),
                SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetWorldInfoList);
            if ((ret >= 0) &&
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_GET_WORLD_INFO_DATA))
            {
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_CreateJoinRoom:
            ...
            ret = sceNpMatching2GetEventData(
                manager->m_matchingContext, eventKey,
                (void *)(manager->cCreateJoinRoomStorage),
                SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_CreateJoinRoom);
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_GetUserInfoList:
            ...
            ret = sceNpMatching2GetEventData(
                manager->m_matchingContext, eventKey,
                (void *)(manager->cUserInfoListStorage),
                SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetUserInfoList);
            if ((ret >= 0) &&
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_GET_USER_INFO_LIST_DATA))
            {
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_JoinRoom:
            ...
            ret = sceNpMatching2GetEventData(
                manager->m_matchingContext, eventKey,
                (void *)(manager->cJoinRoomStorage),
                SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_JoinRoom);
            if ((ret >= 0) &&
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_GET_JOIN_ROOM_DATA))
            {
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_GetRoomMemberDataInternal:
            assert(dataSize <=
                   SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetRoomMemberDataInternal);
            ret = sceNpMatching2GetEventData(
                manager->m_matchingContext, eventKey,
                (void *)(manager->cRoomMemberDataInternal),
                SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetRoomMemberDataInternal);
            if ((ret >= 0) && !manager->ForceErrorPoint(
        ...
        case SCE_NP_MATCHING2_REQUEST_EVENT_GetRoomDataExternalList:
            assert(dataSize <=
                   SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetRoomDataExternalList);
            ret = sceNpMatching2GetEventData(
                manager->m_matchingContext, eventKey,
                (void *)(manager->cRoomDataExternalList),
                SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetRoomDataExternalList);
            if ((ret >= 0) &&
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_GET_ROOM_EXTERNAL_DATA2))
            {
    ...
    // If there was some problem getting data, then silently ignore as we don't
    // want to go any further with a NULL data pointer, and this will just be
    // indicating that the networking context is invalid which will be picked up
    // elsewhere to shut everything down properly
    if (dataError)
    {
        return;
    }

    // If we didn't get the event data, then we need to clear it, or the system
    // even queue will overflow
    if (data == 0)
    {
    ...
    switch (event)
    {
    // This is the responese to a call to sceNpMatching2GetServerInfo, used when
    // we are trying to establish an available server to get an explicit server
    // context for, which is the process kicked off by the GetServerContext
    // methods.
    case SCE_NP_MATCHING2_REQUEST_EVENT_GetServerInfo:
        ...
        assert((manager->m_state == SNM_INT_STATE_HOSTING_SEARCHING_FOR_SERVER) ||
               (manager->m_state == SNM_INT_STATE_JOINING_SEARCHING_FOR_SERVER));
        app.DebugPrintf(CMinecraftApp::USER_RR,
                        "Response to server search received, errorCode 0x%x\n",
                        errorCode);
        if (errorCode == 0)
        {
            if (data != 0)
            {
                SceNpMatching2GetServerInfoResponse *pServerInfo =
                    (SceNpMatching2GetServerInfoResponse *)data;
                if (pServerInfo->server.status ==
                    SCE_NP_MATCHING2_SERVER_STATUS_AVAILABLE)
                {
                    app.DebugPrintf(CMinecraftApp::USER_RR, "Server status available\n");
                    // This server is available
                    manager->SetState(
                        (manager->m_state == SNM_INT_STATE_HOSTING_SEARCHING_FOR_SERVER)
        ...
        // A problem of some kind with this particular server - let the tick decide
        // what to do
        manager->SetState(
            (manager->m_state == SNM_INT_STATE_HOSTING_SEARCHING_FOR_SERVER)
                ? SNM_INT_STATE_HOSTING_SERVER_SEARCH_SERVER_ERROR
    ...
    case SCE_NP_MATCHING2_REQUEST_EVENT_CreateJoinRoom:
        ...
        manager->SetState(SNM_INT_STATE_HOSTING_CREATE_ROOM_FAILED);
        break;
    // This is the response to sceNpMatching2CreateServerContext, which is the
    // final thing that happens in the chain of states kicked off by
    // GetServerContext, and happens after we've found an available server. If
    // this context creation is successful, then we can proceed to create or join
    // a room - we can determine which to do by virtue of the current state.
    ...
    case SCE_NP_MATCHING2_REQUEST_EVENT_GetRoomMemberDataInternal:
        if (errorCode == 0)
        {
            ...
            if (data != 0)
            {
                ...
                int playerMask = *((int *)(pRoomMemberData->roomMemberDataInternal
                ...
                // Something has gone wrong adding these players to the room - kick out
                // the player
                ...
                // Set flag to indicate whether we were kicked for being out of room or
                // not
...
#ifdef __PS3__
void SQRNetworkManager_PS3::RoomEventCallback(SceNpMatching2ContextId id,
                                              SceNpMatching2RoomId roomId,
                                              SceNpMatching2Event event,
                                              SceNpMatching2EventKey eventKey,
                                              int errorCode, size_t dataSize,
                                              void *arg)
#else
void SQRNetworkManager_PS3::RoomEventCallback(SceNpMatching2ContextId id,
                                              SceNpMatching2RoomId roomId,
                                              SceNpMatching2Event event,
                                              int errorCode, const void *data,
                                              void *arg)
#endif
{
    SQRNetworkManager_PS3 *manager = (SQRNetworkManager_PS3 *)arg;
    ...
    case SCE_NP_MATCHING2_ROOM_EVENT_Kickedout:
        {
            ...
            int ret = sceNpMatching2GetEventData(
                manager->m_matchingContext, eventKey, manager->cRoomDataUpdateInfo,
                ...
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_UPDATED_ROOM_DATA))
            {
                ...
                SceNpMatching2RoomUpdateInfo *pUpdateInfo =
                    (SceNpMatching2RoomUpdateInfo *)(manager->cRoomDataUpdateInfo);
    ...
    case SCE_NP_MATCHING2_ROOM_EVENT_UpdatedRoomDataInternal:
        ...
        if (!manager->m_isHosting)
        {
            ...
            int ret = sceNpMatching2GetEventData(
                manager->m_matchingContext, eventKey, manager->cRoomDataInternal,
                SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_RoomDataInternalUpdateInfo);
            if ((ret >= 0) &&
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_UPDATED_ROOM_DATA))
            {
...
void SQRNetworkManager_PS3::SignallingCallback(
    ...
    case SCE_NP_MATCHING2_SIGNALING_EVENT_Established:
        {
            if (manager->m_isHosting)
            {
                // If we're the host, then we need to get the data associated with the
                // connecting peer to know what connections we should be trying to match.
                // So the actual creation of connections happens when the response for
            ...
            {
                // If we are the client, then we locally know what Rupd connections we
                // need (from m_localPlayerJoinMask) and can kick this off.
                bool ret = manager->CreateRudpConnections(roomId, peerMemberId,
...
int SQRNetworkManager_PS3::BasicEventCallback(int event, int retCode,
                                              uint32_t reqId, void *arg)
{
    SQRNetworkManager_PS3 *manager = (SQRNetworkManager_PS3 *)arg;
    // We aren't allowed to actually get the event directly from this callback, so
    // send our own internal event to a thread dedicated to doing this
...
void SQRNetworkManager_PS3::SysUtilCallback(uint64_t status, uint64_t param,
                                            void *userdata)
{
    ...
    case CELL_SYSUTIL_NET_CTL_NETSTART_FINISHED:
        ...
        {
            // Failed, or user may have decided not to sign in - maybe need to
            // differentiate here
            manager->SetState(SNM_INT_STATE_INITIALISE_FAILED);
            if (s_SignInCompleteCallbackFn)
            {
...
void SQRNetworkManager_PS3::RudpContextCallback(int ctx_id, int event_id,
        ...
        if (manager->m_listener)
        {
            ...
            if (manager->m_isHosting)
            {
                ...
                if (playerFrom && !playerFrom->HasSmallIdConfirmed())
                {
                    if (dataSize >= sizeof(SQRNetworkPlayer::InitSendData))
                    {
                        ...
                        unsigned int bytesRead = cellRudpRead(
                            ctx_id, &ISD, sizeof(SQRNetworkPlayer::InitSendData), 0, NULL);
            ...
            if (dataSize > 0)
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        manager->m_listener->HandleDataReceived(playerFrom, playerTo, data,
                                                                bytesRead);
                    }
                }
                delete[] data;
...
#ifdef __PS3__
int SQRNetworkManager_PS3::RudpEventCallback(int event_id, int soc,
                                             uint8_t const *data,
                                             size_t datalen,
...
void SQRNetworkManager_PS3::NetCtlCallback(int prev_state, int new_state,
                                           int event, int error_code,
                                           void *arg)
{
    SQRNetworkManager_PS3 *manager = (SQRNetworkManager_PS3 *)arg;
...
void SQRNetworkManager_PS3::GetExtDataForRoom(
...
void SQRNetworkManager_PS3::AttemptPSNSignIn(
...
int SQRNetworkManager_PS3::SetRichPresence(const void *data,
                                           unsigned int options)
{
    const SceNpBasicPresenceDetails2 *newPresenceInfo =
        (const SceNpBasicPresenceDetails2 *)data;

    s_lastPresenceInfo.struct_size = sizeof(SceNpBasicPresenceDetails2);
    memcpy(s_lastPresenceInfo.status, newPresenceInfo->status,
           SCE_NP_BASIC_PRESENCE_EXTENDED_STATUS_SIZE_MAX);
...
void SQRNetworkManager_PS3::UpdateRichPresenceCustomData(
...
void SQRNetworkManager_PS3::SetPresenceDataStartHostingGame()
{
    if (m_offlineGame)
    {
        SQRNetworkManager_PS3::UpdateRichPresenceCustomData(
            &c_presenceSyncInfoNULL,
            sizeof(SQRNetworkManager_PS3::PresenceSyncInfo));
    }
    else
    {
        ...
        SQRNetworkManager_PS3::UpdateRichPresenceCustomData(
            &presenceInfo, sizeof(SQRNetworkManager_PS3::PresenceSyncInfo));
    }
}
