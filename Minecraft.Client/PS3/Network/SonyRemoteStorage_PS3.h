...
class SonyRemoteStorage_PS3 : public SonyRemoteStorage
{
  public:
    ...
    virtual bool getRemoteFileInfo(SceRemoteStorageStatus *pInfo, CallbackFunc cb,
                                   LPVOID lpParam);
    ...
    bool isWaitingForTicket()
    {
        return m_waitingForTicket;
    }
    void npauthhandler(int event, int result, void *arg);

    void SetSecureID(char *id)
    {
        memcpy(m_secureFileId, id, CELL_SAVEDATA_SECUREFILEID_SIZE);
    }

    void CompressSaveData();
    bool dataCompressDone()
    {
        return (m_compressedSaveState == e_state_Idle);
    }
