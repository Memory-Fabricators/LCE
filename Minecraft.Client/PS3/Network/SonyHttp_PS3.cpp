...
void *SonyHttp_PS3::getData(const char *url, int *pDataSize)
{
    ...
    if (0 > ret)
    {
        ...
        {
            app.DebugPrintf("Only supporting data that has a content length : "
    ...
    // 4J-PB - seems to be some limit to a read with cellHttpRecvResponse, even
    // though it claims it's read all the data, it hasn't reducing this to read
    // 10k at a time.
    int iMaxRead = 10240;
...
bool SonyHttp_PS3::getDataFromURL(const char *szURL, void **ppOutData,
                                  int *pDataSize)
{
    if (!bInitialised)
    {
