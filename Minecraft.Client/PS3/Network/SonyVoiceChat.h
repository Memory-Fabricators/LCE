...
class SonyVoiceChat
{
  public:
    ...
    static void
    mutePlayer(const SceNpMatching2RoomMemberId member_id,
    ...
    {
        return (sm_state == AVC_STATE_CHAT_LEAVE ||
                sm_state == AVC_STATE_CHAT_UNLOAD || AVC_STATE_CHAT_RESET);
    }
    static void signalRoomDestroyed()
    {
        if (!isShuttingDown())
        {
            sm_event = AVC_EVENT_NP2_ROOM_DESTROY_OR_KICKEDOUT;
        }
    }
    static void signalRoomKickedOut()
    {
        if (!isShuttingDown())
        {
            sm_event = AVC_EVENT_NP2_ROOM_DESTROY_OR_KICKEDOUT;
    ...
    static void signalDisconnected()
    {
        if (!isShuttingDown())
        {
            sm_event = AVC_EVENT_LAN_DISCONNECT;
        }
    }
  ...
  private:
    ...
    static bool sm_bCanStart;        // set to true on init, false on disconnect, used to
                                     // see if we should start after a re-enable
    static bool sm_isChatRestricted; // true if the parental controls have been
                                     // set on the main users account
    static int sm_currentBitrate;
    static int eventcb_load(CellSysutilAvc2EventId event_id,
                            CellSysutilAvc2EventParam event_param,
                            void *userdata);
    static int eventcb_join(CellSysutilAvc2EventId event_id,
                            CellSysutilAvc2EventParam event_param,
                            void *userdata);
    static int eventcb_leave(CellSysutilAvc2EventId event_id,
                             CellSysutilAvc2EventParam event_param,
                             void *userdata);
    static int eventcb_unload(CellSysutilAvc2EventId event_id,
                              CellSysutilAvc2EventParam event_param,
                              void *userdata);
    static int eventcb_voiceDetected(CellSysutilAvc2EventId event_id,
                                     CellSysutilAvc2EventParam event_param,
                                     void *userdata);
    static void eventcb(CellSysutilAvc2EventId event_id,
