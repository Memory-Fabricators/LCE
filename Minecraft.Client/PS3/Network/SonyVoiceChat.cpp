...
void SonyVoiceChat::init(SQRNetworkManager_PS3 *pNetMan)
{
    ...
    ProfileManager.GetChatAndContentRestrictions(0, false, &sm_isChatRestricted,
                                                 NULL, NULL);
}

void SonyVoiceChat::shutdown()
{
    if (sm_state == AVC_STATE_IDLE || sm_state == AVC_STATE_CHAT_LEAVE ||
        sm_state == AVC_STATE_CHAT_UNLOAD || sm_state == AVC_STATE_CHAT_RESET)
    {
...
int SonyVoiceChat::eventcb_load(CellSysutilAvc2EventId event_id,
                                ...
                                void *userdata)
{
    ...
    if (event_id == CELL_AVC2_EVENT_LOAD_SUCCEEDED)
    {
        ...
        if (ret != CELL_OK)
        {
            app.DebugPrintf("CELL_SYSUTIL_AVC2_ATTRIBUTE_VOICE_PACKET_CONTENTION "
                            "failed !!! 0x%08x\n",
                            ret);
        }
    }
...
int SonyVoiceChat::eventcb_join(CellSysutilAvc2EventId event_id,
                                CellSysutilAvc2EventParam event_param,
                                void *userdata)
{
    int ret = CELL_OK;
...
int SonyVoiceChat::eventcb_leave(CellSysutilAvc2EventId event_id,
                                 CellSysutilAvc2EventParam event_param,
                                 void *userdata)
{
    int ret = CELL_OK;
...
int SonyVoiceChat::eventcb_unload(CellSysutilAvc2EventId event_id,
                                  CellSysutilAvc2EventParam event_param,
                                  void *userdata)
{
    int ret = CELL_OK;
...
int SonyVoiceChat::eventcb_voiceDetected(CellSysutilAvc2EventId event_id,
                                         CellSysutilAvc2EventParam event_param,
                                         void *userdata)
{
    UNUSED_VARIABLE(userdata);
    ...
    // 	In the lower 32 bits, a value of 0 (mute) or a value between 1 (low
    // volume) 	and 10 (high volume) is passed as the audio signal value when
    // the notification 	method is the level method, or a value of 1 (start of
    // speaking) or 0 (end of speaking) 	is stored when the notification method
    // is the trigger method.

    SceNpMatching2RoomMemberId roomMemberID =
        (SceNpMatching2RoomMemberId)(event_param >> 32);
    uint32_t volume = (uint32_t)(event_param & 0xffffffff);

    // 	The precision of voice detection is not very high. Since the audio
    // signal values may 	always be relatively high depending on the audio input
    // device and the noise level in the 	room, you should set a large reference
    // value for determining whether or not a player is 	speaking. Relatively
    // good results can be obtained when an audio signal value of at 	least 9
    // is used to determine if a player is speaking.
    bool bTalking = false;
    if (volume >= 9)
    {
        bTalking = true;
...
void SonyVoiceChat::eventcb(CellSysutilAvc2EventId event_id,
                            CellSysutilAvc2EventParam event_param,
                            void *userdata)
{
    static struct _cb_func_tbl
    {
        ...
        int (*func)(CellSysutilAvc2EventId event_id,
                    CellSysutilAvc2EventParam event_param, void *userdata);
    } event_tbl[] = {
    ...
    int ret = 0;
    for (unsigned int i = 0; i < sizeof(event_tbl) / sizeof(struct _cb_func_tbl);
         ++i)
    {
...
int SonyVoiceChat::load()
{
    ...
    ret = cellSysutilAvc2LoadAsync(sm_pNetworkManager->m_matchingContext,
                                   SYS_MEMORY_CONTAINER_ID_INVALID, eventcb, NULL,
                                   &g_chat_avc2param);
    if (ret != CELL_OK)
    {
...
int SonyVoiceChat::initialize(void)
{
    ...
    /* Must use cellSysutilAvc2InitParam() for clearing CellSysutilAvc2InitParam
     * struct*/
    ret = cellSysutilAvc2InitParam(CELL_SYSUTIL_AVC2_INIT_PARAM_VERSION,
                                   &g_chat_avc2param);
    if (ret != CELL_OK)
    {
    ...
    g_chat_avc2param.voice_param.voice_quality =
        CELL_SYSUTIL_AVC2_VOICE_QUALITY_NORMAL;
    g_chat_avc2param.voice_param.max_speakers = AVC2_PARAM_DEFAULT_MAX_SPEAKERS;
    g_chat_avc2param.spu_load_average = 50;
    g_chat_avc2param.streaming_mode.mode =
...
void SonyVoiceChat::tick()
{
    ...
    static state_transition_table tbl[] = {
        /*  now state						event
           func					after the transition state */
        {AVC_STATE_CHAT_INIT, AVC_EVENT_EPSILON, initialize, AVC_STATE_CHAT_INIT},
        {AVC_STATE_CHAT_INIT, AVC_EVENT_EXIT_GAME, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_INIT, AVC_EVENT_ERROR, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_INIT, AVC_EVENT_LAN_DISCONNECT, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_INIT, AVC_EVENT_LAN_DISCONNECT, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_INIT, AVC_EVENT_CHAT_INIT_SUCCEEDED, invoke_epsilon_event,
         AVC_STATE_CHAT_LOAD},
        {AVC_STATE_CHAT_INIT, AVC_EVENT_NP2_ROOM_DESTROY_OR_KICKEDOUT,
         invoke_epsilon_event, AVC_STATE_CHAT_RESET},

        {AVC_STATE_CHAT_LOAD, AVC_EVENT_EPSILON, load, AVC_STATE_CHAT_LOAD},
        {AVC_STATE_CHAT_LOAD, AVC_EVENT_EXIT_GAME, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_LOAD, AVC_EVENT_ERROR, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_LOAD, AVC_EVENT_LAN_DISCONNECT, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_LOAD, AVC_EVENT_CHAT_LOAD_SUCCEEDED, invoke_epsilon_event,
         AVC_STATE_CHAT_JOIN},
        {AVC_STATE_CHAT_LOAD, AVC_EVENT_NP2_ROOM_DESTROY_OR_KICKEDOUT,
         invoke_epsilon_event, AVC_STATE_CHAT_RESET},

        {AVC_STATE_CHAT_JOIN, AVC_EVENT_EPSILON, join, AVC_STATE_CHAT_JOIN},
        {AVC_STATE_CHAT_JOIN, AVC_EVENT_EXIT_GAME, invoke_epsilon_event,
         AVC_STATE_CHAT_UNLOAD},
        {AVC_STATE_CHAT_JOIN, AVC_EVENT_ERROR, invoke_epsilon_event,
         AVC_STATE_CHAT_UNLOAD},
        {AVC_STATE_CHAT_JOIN, AVC_EVENT_LAN_DISCONNECT, invoke_epsilon_event,
         AVC_STATE_CHAT_UNLOAD},
        {AVC_STATE_CHAT_JOIN, AVC_EVENT_CHAT_JOIN_SUCCEEDED, invoke_epsilon_event,
         AVC_STATE_CHAT_SESSION_PROCESSING},
        {AVC_STATE_CHAT_JOIN, AVC_EVENT_NP2_ROOM_DESTROY_OR_KICKEDOUT,
         invoke_epsilon_event, AVC_STATE_CHAT_UNLOAD},

        {AVC_STATE_CHAT_SESSION_PROCESSING, AVC_EVENT_EPSILON, start,
         AVC_STATE_CHAT_SESSION_PROCESSING},
        {AVC_STATE_CHAT_SESSION_PROCESSING, AVC_EVENT_EXIT_GAME, stop,
         AVC_STATE_CHAT_SESSION_PROCESSING},
        {AVC_STATE_CHAT_SESSION_PROCESSING, AVC_EVENT_ERROR, stop,
         AVC_STATE_CHAT_SESSION_PROCESSING},
        {AVC_STATE_CHAT_SESSION_PROCESSING, AVC_EVENT_LAN_DISCONNECT, stop,
         AVC_STATE_CHAT_SESSION_PROCESSING},
        {AVC_STATE_CHAT_SESSION_PROCESSING, AVC_EVENT_CHAT_SESSION_STOPPED,
         invoke_epsilon_event, AVC_STATE_CHAT_LEAVE},
        {AVC_STATE_CHAT_SESSION_PROCESSING,
         AVC_EVENT_NP2_ROOM_DESTROY_OR_KICKEDOUT, stop,
         AVC_STATE_CHAT_SESSION_PROCESSING},

        {AVC_STATE_CHAT_LEAVE, AVC_EVENT_EPSILON, leave, AVC_STATE_CHAT_LEAVE},
        {AVC_STATE_CHAT_LEAVE, AVC_EVENT_ERROR, invoke_epsilon_event,
         AVC_STATE_CHAT_UNLOAD},
        {AVC_STATE_CHAT_LEAVE, AVC_EVENT_CHAT_LEAVE_SUCCEEDED,
         invoke_epsilon_event, AVC_STATE_CHAT_UNLOAD},

        {AVC_STATE_CHAT_UNLOAD, AVC_EVENT_EPSILON, unload, AVC_STATE_CHAT_UNLOAD},
        {AVC_STATE_CHAT_UNLOAD, AVC_EVENT_ERROR, invoke_epsilon_event,
         AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_UNLOAD, AVC_EVENT_CHAT_UNLOAD_SUCCEEDED,
         invoke_epsilon_event, AVC_STATE_CHAT_RESET},

        {AVC_STATE_CHAT_RESET, AVC_EVENT_EPSILON, finalize, AVC_STATE_CHAT_RESET},
        {AVC_STATE_CHAT_RESET, AVC_EVENT_ERROR, invoke_epsilon_event,
         AVC_STATE_CHAT_LEAVE},
        {AVC_STATE_CHAT_RESET, AVC_EVENT_CHAT_FINALIZE_SUCCEEDED,
         invoke_epsilon_event, AVC_STATE_IDLE},
...
void SonyVoiceChat::do_state_transition(state_transition_table *tbl,
                                        int tbl_size)
{
    int ret = CELL_OK;
...
bool SonyVoiceChat::hasMicConnected(
    const SceNpMatching2RoomMemberId *players_id)
{
...
void SonyVoiceChat::muteLocalPlayer(
...
void SonyVoiceChat::setBitRate()
{
    ...
    // 	This internal attribute represents the maximum voice bit rate.
    // attr_param
    ...
    static int bitRates[8] = {28000, 28000, 28000, 28000,
                              24000, 20000, 16000, 16000};
