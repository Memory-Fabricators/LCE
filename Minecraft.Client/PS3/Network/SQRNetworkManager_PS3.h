...
class SQRNetworkManager_PS3 : public SQRNetworkManager
{
  ...
  private:
    ...
    void UpdateOnlineStatus(int status)
    {
  ...
  private:
    ...
    SQRNetworkPlayer
        *m_aRoomSlotPlayers[MAX_ONLINE_PLAYER_COUNT]; // Maps from the players in
                                                      // m_roomSyncData, to
                                                      // SQRNetworkPlayers
    ...
    // arrays to store their NpIds, rooms, servers, whether a room was found, and
    // whether the external data had been received for the room. Also a count of
    // how many elements are used in this array.
    ...
    static void DefaultRequestCallback(SceNpMatching2ContextId id,
                                       ...
                                       int errorCode, size_t dataSize, void *arg);
    static void RoomEventCallback(SceNpMatching2ContextId id,
                                  SceNpMatching2RoomId roomId,
                                  SceNpMatching2Event event,
                                  SceNpMatching2EventKey eventKey, int errorCode,
    ...
    static int RudpEventCallback(int event_id, int soc, uint8_t const *data,
                                 size_t datalen, struct sockaddr const *addr,
    ...
    // Callback for netctl
    static void NetCtlCallback(int prev_state, int new_state, int event,
    ...
    // Response storage
    char
        cUserInfoListStorage[SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetUserInfoList]
        __attribute__((__aligned__(4)));
    char cServerInfoStorage[SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetServerInfo]
        __attribute__((__aligned__(4)));
    char cWorldListStorage[SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetWorldInfoList]
        __attribute__((__aligned__(4)));
    char cCreateJoinRoomStorage
        [SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_CreateJoinRoom]
        __attribute__((__aligned__(4)));
    char cJoinRoomStorage[SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_JoinRoom]
        __attribute__((__aligned__(4)));
    char cRoomMemberDataInternal
        [SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetRoomMemberDataInternal]
        __attribute__((__aligned__(4)));
    char cRoomDataInternal
        [SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_RoomDataInternalUpdateInfo]
        __attribute__((__aligned__(4)));
    char cRoomDataExternalList
        [SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_GetRoomDataExternalList]
        __attribute__((__aligned__(4)));
    char cRoomMemberDataInternalUpdate
        [SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_RoomMemberDataInternalUpdateInfo]
        __attribute__((__aligned__(4)));
    char cRoomDataUpdateInfo[SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_RoomUpdateInfo]
        __attribute__((__aligned__(4)));
#endif
  ...
  private:
    ...
    // 4J-PB - so we can stop the crash on PS3 when Iggy's LoadMovie is called
    // from the ContextCallback
