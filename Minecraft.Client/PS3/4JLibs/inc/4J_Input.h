...
class C_4JInput
{
  public:
    ...
    // *Func)(LPVOID,const bool),LPVOID lpParam,EKeyboardMode eMode,C4JStringTable
    // *pStringTable=NULL); 	EKeyboardResult		RequestKeyboard(UINT
    // uiTitle, LPCWSTR pwchDefault, UINT uiDesc, DWORD dwPad, WCHAR *pwchResult,
    // UINT uiResultSize,int( *Func)(LPVOID,const bool),LPVOID lpParam,
    // EKeyboardMode eMode,C4JStringTable *pStringTable=NULL);
    ...
    EMsgBoxResult RequestMessageBox(EMSgBoxType eType, DWORD dwPad,
                                    void (*Func)(int iButtonType,
                                                 void *pvUserData),
                                    LPVOID lpParam, const char *pchText,
                                    unsigned int uiFocusButton);
};
