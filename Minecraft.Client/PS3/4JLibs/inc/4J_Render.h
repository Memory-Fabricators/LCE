...
class C4JRender
{
  public:
    ...
    typedef enum
    {
        ...
        VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1_LIT,    // as VERTEX_TYPE_PF3_TF2_CB4_NB4_XW1
    ...
    void CBuffCallMultipleConditional(int *indexArray, int numIndices,
                                      int chunkLists, int condLists);
    void CBuffCallMultipleConditionalAlpha(int *indexArray, int numIndices,
                                           int chunkLists, int condLists);
    void CBuffTick();
    ...
    typedef enum
    {
        ...
        TEXTURE_FORMAT_R0G0B0Ax,		// One 8-bit component mapped to alpha
        channel, R=G=B=0 TEXTURE_FORMAT_R1G1B1Ax,		// One 8-bit component
        mapped to alpha channel, R=G=B=1 TEXTURE_FORMAT_RxGxBxAx,		// One
        8-bit component mapped to all channels
        */
        MAX_TEXTURE_FORMATS
    } eTextureFormat;
    ...
    HRESULT LoadTextureData(BYTE *pbData, DWORD dwBytes, D3DXIMAGE_INFO *pSrcInfo,
                            int **ppDataOut);
    HRESULT LoadTextureDataJPG(BYTE *pbData, DWORD dwBytes,
    ...
    void setTileInfo(const void *mem, uint32_t pitch, uint8_t comp,
                     uint8_t bank = 0);
