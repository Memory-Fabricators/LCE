...
class CONTENT_DATA
{
  public:
    ...
    // CHAR szHDDFileName[CELL_FS_MAX_FS_PATH_LENGTH]; // if we are running from
    // bluray, the keys for DLC will be in the HDD path, but the DLC will be on
    // the bluray
};
...
class C4JStorage
{
  ...
  public:
    ...
    // On PS3 the user profile data is stored in a file, so moving this to the
    // storage library rather than the profile library
    C4JStorage();
    ...
    void
    ResetSaveData(); // Call before a new save to clear out stored save file name
    ...
    void SetState(ESaveGameState eState, int (*Func)(LPVOID, const bool),
                  LPVOID lpParam);
    void SetSaveDisabled(bool bDisable);
    bool GetSaveDisabled(void);
    unsigned int GetSaveSize();
    void SetSaveImages(PBYTE pbThumbnail, DWORD dwThumbnailBytes, PBYTE pbImage,
                       DWORD dwImageBytes, PBYTE pbTextData,
                       DWORD dwTextDataBytes);
    void SetDefaultSaveImage(); // MGH - added for remote storage compress, we
                                // can't get the save image back to overwrite, so
    ...
    C4JStorage::ESaveGameState LoadSaveData(
        PSAVE_INFO pSaveInfo, int (*Func)(LPVOID lpParam, const bool, const bool),
        LPVOID lpParam,
        bool bIgnoreCRC = false); // MGH - added bIgnoreCRC for remote save stuff
    ...
    void FreeSaveData();
    void SetSaveDataSize(unsigned int uiBytes); // after a successful compression,
                                                // update the size of the gamedata

    // void						SaveSaveData(unsigned
    // int uiBytes,PBYTE pbThumbnail=NULL,DWORD cbThumbnail=0,PBYTE
    // pbTextData=NULL, DWORD dwTextLen=0);
    C4JStorage::ESaveGameState SaveSaveData(int (*Func)(LPVOID, const bool),
                                            LPVOID lpParam,
                                            bool bDataFileOnly = false);
    ...
    void
    SetSaveMessageVPosition(float fY); // The 'Saving' message will display at a
                                       // default position unless changed

    void SetMessageBoxCallback(int (*Func)(
    ...
    {
        static MARKETPLACE_CONTENTOFFER_INFO retval = {0};
        return retval;
    }
    int GetOfferCount()
    {
    ...
    DWORD InstallOffer(int iOfferIDC, ULONGLONG *ullOfferIDA,
    ...
    void SetDLCProductCode(const char *szProductCode,
                           const char *szDiscPatchProductCode);
    void SetProductUpgradeKey(const char *szKey);
    void SetBootTypeDisc(
        bool bDisc); // true if booting from disc, false if booting from HDD
    ...
    // PROFILE DATA
    int SetDefaultOptionsCallback(int (*Func)(LPVOID, PROFILESETTINGS *,
                                              const int iPad),
                                  LPVOID lpParam);
    ...
    void SetDefaultImages(PBYTE pbOptionsImage, DWORD dwOptionsImageBytes,
                          PBYTE pbSaveImage, DWORD dwSaveImageBytes,
                          PBYTE pbSaveThumbnail, DWORD dwSaveThumbnailBytes);
    void GetDefaultSaveImage(PBYTE *ppbSaveImage, DWORD *pdwSaveImageBytes);
    void GetDefaultSaveThumbnail(PBYTE *ppbSaveThumbnail,
                                 DWORD *pdwSaveThumbnailBytes);
    void SetOptionsDataCallback(int (*Func)(LPVOID, int iPad,
