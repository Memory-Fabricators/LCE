...
int SoundEngine::initAudioHardware(int minimum_chans)
{
    ...
    if (ch_pcm >= minimum_chans)
    {
        ...
        a_config.downMixer =
            CELL_AUDIO_OUT_DOWNMIXER_NONE; /* No downmixer is used */
    ...
    else
    {
        ...
        case 6:
            ...
            if (cellAudioOutConfigure(CELL_AUDIO_OUT_PRIMARY, &a_config, NULL, 0) !=
                CELL_OK)
            {
        ...
        case 2:
            ...
            if (ch_bit > 0)
            {
                ...
                {
                    // we don't have enough channels to support their minimum, so use the
                    // downmixer
                    a_config.downMixer = CELL_AUDIO_OUT_DOWNMIXER_TYPE_B;
                    ret = 8;
                }
                ...
                // if we got here the dolby encoder, didn't init, so fall through to
