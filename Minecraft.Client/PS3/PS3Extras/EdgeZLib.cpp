...
void EdgeZLib::Init(CellSpurs *pSpurs)
{
    ...
    //////////////////////////////////////////////////////////////////////////
    {
        ...
        int ret =
            cellSpursTasksetAttributeInitialize(&taskSetAttribute, 0, prios, 8);
        PS3_ASSERT_CELL_ERROR(ret);
        ret = cellSpursTasksetAttributeSetName(&taskSetAttribute,
                                               "edgeZlibDeflateTaskSet");
        PS3_ASSERT_CELL_ERROR(ret);
        ret = cellSpursCreateTasksetWithAttribute(pSpurs, &s_taskSetDeflate,
                                                  &taskSetAttribute);
        PS3_ASSERT_CELL_ERROR(ret);

        ret = cellSpursEventFlagInitializeIWL(pSpurs, &s_eventFlagDeflate,
        ...
        uint32_t deflateQueueBuffSize =
            edgeZlibGetDeflateQueueSize(kMaxNumDeflateQueueEntries);
        s_pDeflateQueueBuffer =
            memalign(EDGE_ZLIB_DEFLATE_QUEUE_ALIGN, deflateQueueBuffSize);
        s_deflateQueue =
            edgeZlibCreateDeflateQueue(s_pSpurs, kMaxNumDeflateQueueEntries,
                                       s_pDeflateQueueBuffer, deflateQueueBuffSize);
#ifndef _CONTENT_PACKAGE
        ...
        for (uint32_t taskNum = 0; taskNum < kNumDeflateTasks; taskNum++)
        {
            ...
            s_pDeflateTaskContext[taskNum] =
                memalign(CELL_SPURS_TASK_CONTEXT_ALIGN, contextSaveSize);
            edgeZlibCreateDeflateTask(&s_taskSetDeflate,
                                      s_pDeflateTaskContext[taskNum], s_deflateQueue);
        }
#ifndef _CONTENT_PACKAGE
...

    {
        ...
        PS3_ASSERT_CELL_ERROR(ret);
        ret = cellSpursTasksetAttributeSetName(&taskSetAttribute,
                                               "edgeZlibInflateTaskSet");
        PS3_ASSERT_CELL_ERROR(ret);
        ret = cellSpursCreateTasksetWithAttribute(s_pSpurs, &s_taskSetInflate,
                                                  &taskSetAttribute);
        PS3_ASSERT_CELL_ERROR(ret);
#ifndef _CONTENT_PACKAGE
        ...
        ret = cellSpursEventFlagInitializeIWL(s_pSpurs, &s_eventFlagInflate,
                                              CELL_SPURS_EVENT_FLAG_CLEAR_AUTO,
                                              CELL_SPURS_EVENT_FLAG_SPU2PPU);
        PS3_ASSERT_CELL_ERROR(ret);
        ...
        uint32_t inflateQueueBuffSize =
            edgeZlibGetInflateQueueSize(kMaxNumInflateQueueEntries);
        s_pInflateQueueBuffer =
            memalign(EDGE_ZLIB_INFLATE_QUEUE_ALIGN, inflateQueueBuffSize);
        s_inflateQueue =
            edgeZlibCreateInflateQueue(s_pSpurs, kMaxNumInflateQueueEntries,
                                       s_pInflateQueueBuffer, inflateQueueBuffSize);
#ifndef _CONTENT_PACKAGE
        ...
        for (uint32_t taskNum = 0; taskNum < kNumInflateTasks; taskNum++)
        {
            ...
            s_pInflateTaskContext[taskNum] =
                memalign(CELL_SPURS_TASK_CONTEXT_ALIGN, contextSaveSize);
            edgeZlibCreateInflateTask(&s_taskSetInflate,
                                      s_pInflateTaskContext[taskNum], s_inflateQueue);
        }
#ifndef _CONTENT_PACKAGE
...
bool EdgeZLib::Compress(void *pDestination, uint32_t *pDestSize,
                        const void *pSource, uint32_t SrcSize)
{
    ...
    s_numElementsToCompress =
        1; // Must be set correctly before any elements are added
    ...
    edgeZlibAddDeflateQueueElement(
        s_deflateQueue, pSource, SrcSize, &pDst[1], *pDestSize, &s_compressedSize,
        &s_numElementsToCompress, // This will be decremented by 1 when this
                                  // element is compressed
        &s_eventFlagDeflate,      // When s_numElementsToCompress decrements to zero,
                                  // this event will be triggered
        eventFlagBits, compressionLevel,
    ...
    int ret = cellSpursEventFlagWait(&s_eventFlagDeflate, &eventFlagBits,
                                     CELL_SPURS_EVENT_FLAG_AND);
    PS3_ASSERT_CELL_ERROR(ret);
    ...
    assert((s_compressedSize & kEdgeZlibDeflateTask_HowStoredMask) ==
    ...
    float compressionPercent =
        (1.f - (float)s_compressedSize / (float)SrcSize) * 100.f;

#ifndef _CONTENT_PACKAGE
    printf_verbose("   0x%05x    0x%05x   %8.2f%%         %d\n\n", (int)SrcSize,
                   (int)s_compressedSize, compressionPercent, kNumDeflateTasks);
#endif
...
bool EdgeZLib::Decompress(void *pDestination, uint32_t *pDestSize,
                          const void *pSource, uint32_t SrcSize)
{
    ...
    edgeZlibAddInflateQueueElement(
        s_inflateQueue, &pSrc[1], SrcSize, pDestination, uncompressedSize,
        &s_numElementsToDecompress, // This will be decremented by 1 when this
                                    // element is compressed
        &s_eventFlagInflate,        // When s_numElementsToDecompress decrements to zero,
                                    // this event will be triggered
        eventFlagBits, kEdgeZlibInflateTask_Inflate);
    ...
    int ret = cellSpursEventFlagWait(&s_eventFlagInflate, &eventFlagBits,
                                     CELL_SPURS_EVENT_FLAG_AND);
    PS3_ASSERT_CELL_ERROR(ret);
