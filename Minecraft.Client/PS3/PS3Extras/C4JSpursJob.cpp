...
C4JSpursJobQueue::Port *C4JSpursJobQueue::Port::s_allocatedPorts[16] = {
...
C4JSpursJobQueue::C4JSpursJobQueue()
{
    ...
    unsigned int sizeOfJobDescriptorPool =
        CELL_SPURS_JOBQUEUE_JOB_DESCRIPTOR_POOL_SIZE(0, 0, NUM_SUBMIT_JOBS, 0, 0,
                                                     0, 0, 0);

    pPool = (uint8_t *)memalign(DMA_ALIGNMENT, sizeOfJobDescriptorPool);

    // E create jobQueue
    pJobQueue = (JobQueue<JOB_QUEUE_DEPTH> *)memalign(
        CELL_SPURS_JOBQUEUE_ALIGN, sizeof(JobQueue<JOB_QUEUE_DEPTH>));
    assert(pJobQueue != NULL);

    ret = JobQueue<JOB_QUEUE_DEPTH>::create(
        pJobQueue, spurs, "SampleJobQueue",
        5, // numSpus
        8, // priority
        (void *)pPool, &poolDescriptor,
        4,     // maxGrab
        false, // submitWithEntryLock
        false, // doBusyWaiting
        false, // isHaltOnError
        true,  // autoPrxLoad
        1);    // uint8_t	maxNumJobsOnASpu =
               // CELL_SPURS_JOBQUEUE_DEFAULT_MAX_NUM_JOBS_ON_SPU,
...
C4JSpursJobQueue::Port::Port(const char *portName)
{
    ...
    m_pPort2 = (Port2 *)memalign(CELL_SPURS_JOBQUEUE_PORT2_ALIGN,
                                 CELL_SPURS_JOBQUEUE_PORT2_SIZE);
    assert(m_pPort2);
    int ret =
        Port2::create(m_pPort2, C4JSpursJobQueue::getMainJobQueue().pJobQueue);
    assert(ret == CELL_OK);
...
void C4JSpursJobQueue::Port::submitJob(C4JSpursJob *pJob)
{
    ...
    int ret;
    // E submit job
    ret = m_pPort2->copyPushJob(&pJob->m_job256.header, sizeof(CellSpursJob256),
                                sizeof(CellSpursJob256), m_jobTag,
...
{
    // bit hacky, but the texture blit port seems to stall on destroy if we don't
    // have this presumably because it uses a non-blocking sync
...
C4JSpursJob_CompressedTile::C4JSpursJob_CompressedTile(
    TileCompressData_SPU *pDataIn, unsigned char *pDataOut)
    : C4JSpursJob()
{
...
C4JSpursJob_ChunkUpdate::C4JSpursJob_ChunkUpdate(ChunkRebuildData *pDataIn,
                                                 ChunkRebuildData *pDataOut)
    : C4JSpursJob()
{
...
C4JSpursJob_LevelRenderer_cull::C4JSpursJob_LevelRenderer_cull(
    LevelRenderer_cull_DataIn *pDataIn)
    : C4JSpursJob()
{
    extern const CellSpursJobHeader JOBHEADER_SYMBOL(LevelRenderer_cull);
    // E create job
    m_job256.header = JOBHEADER_SYMBOL(LevelRenderer_cull);
    m_job256.header.sizeStack =
        (200 * 1024) >> 4; // this is extremely tight, global flags take up 164K
    m_job256.workArea.userData[0] = (uintptr_t)pDataIn;

    assert(cellSpursJobQueueCheckJob(&m_job256, 256, 256) == CELL_OK);
}

C4JSpursJob_LevelRenderer_zSort::C4JSpursJob_LevelRenderer_zSort(
    LevelRenderer_cull_DataIn *pDataIn)
    : C4JSpursJob()
{
...
C4JSpursJob_LevelRenderer_FindNearestChunk::
    C4JSpursJob_LevelRenderer_FindNearestChunk(
        LevelRenderer_FindNearestChunk_DataIn *pDataIn)
    : C4JSpursJob()
{
    extern const CellSpursJobHeader JOBHEADER_SYMBOL(
...
// C4JSpursJob_Renderer_TextureUpdate::C4JSpursJob_Renderer_TextureUpdate(Renderer_TextureUpdate_DataIn*
// pDataIn) 	: C4JSpursJob()
// {
// 	extern const CellSpursJobHeader
// JOBHEADER_SYMBOL(Renderer_TextureUpdate); 	int ret;
// 	//E create job
...
C4JSpursJob_CompressedTileStorage_compress::
    C4JSpursJob_CompressedTileStorage_compress(
        CompressedTileStorage_compress_dataIn *pDataIn)
    : C4JSpursJob()
{
    extern const CellSpursJobHeader JOBHEADER_SYMBOL(
...
/*
C4JSpursJob_CompressedTileStorage_getData::C4JSpursJob_CompressedTileStorage_getData(unsigned
char* pIdxAndData, int dataSize, unsigned char* pDst, unsigned int retOffset) :
C4JSpursJob()
{
        extern const CellSpursJobHeader
JOBHEADER_SYMBOL(CompressedTileStorage_getData); int ret;
        //E create job
...
// C4JSpursJob_GameRenderer_updateLightTexture::C4JSpursJob_GameRenderer_updateLightTexture(GameRenderer_updateLightTexture_dataIn*
// pDataIn) 	: C4JSpursJob()
// {
// 	extern const CellSpursJobHeader
// JOBHEADER_SYMBOL(GameRenderer_updateLightTexture); 	int ret;
// 	//E create job
...
// C4JSpursJob_RLECompress::C4JSpursJob_RLECompress(void* pSrc, int srcSize,
// void* pDst, int* pDstSize) 	: C4JSpursJob()
// {
