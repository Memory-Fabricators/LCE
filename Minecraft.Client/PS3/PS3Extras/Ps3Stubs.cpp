...
VOID GetSystemTime(LPSYSTEMTIME lpSystemTime)
{
    ...
    lpSystemTime->wDayOfWeek = cellRtcGetDayOfWeek(
...
{
    return cellAtomicCompareAndSwap32((uint32_t *)Destination,
                                      (uint32_t)Comperand, (uint32_t)Exchange);
}

LONG64 InterlockedCompareExchangeRelease64(LONG64 volatile *Destination,
                                           LONG64 Exchange, LONG64 Comperand)
{
    return cellAtomicCompareAndSwap64((uint64_t *)Destination,
                                      (uint64_t)Comperand, (uint64_t)Exchange);
}
...
VOID EnterCriticalSection(PCRITICAL_SECTION CriticalSection)
{
    ...
    // false) ) 		LeaveCriticalSection(&g_singleThreadCS);
    int err =
        sys_lwmutex_lock(CriticalSection, 0); // timeout, in microsecs - 0 here
    PS3_ASSERT_CELL_ERROR(err);
    // 	if(CriticalSection != &g_singleThreadCS && !C4JThread::isMainThread())
                    ...
                    DWORD flProtect)
{
    ...
    if (lpAddress == NULL)
    {
        ...
        err = sys_vm_memory_map(dwSize, commitSize, SYS_MEMORY_CONTAINER_ID_INVALID,
                                ...
                                SYS_VM_POLICY_AUTO_RECOMMENDED, &newAddress);
...
BOOL VirtualFree(LPVOID lpAddress, SIZE_T dwSize, DWORD dwFreeType)
{
    if (dwFreeType == MEM_DECOMMIT)
    {
        ...
        // From PS3 Docs:
        // The maximum amount of memory that can be returned is the difference of
        // the total amount of physical memory used by the virtual memory area minus
        // 1MB. When an amount exceeding this value is specified, EBUSY will return.
               ...
               LPDWORD lpNumberOfBytesWritten, LPOVERLAPPED lpOverlapped)
{
    ...
    CellFsErrno err =
        cellFsWrite(fd, lpBuffer, nNumberOfBytesToWrite, &bytesWritten);
                   ...
                   HANDLE hTemplateFile)
{
    ...
    // 4J-PB - If we are running from the blu ray patch , then we need to check if
    // we should load a file from the blu ray or the patch dir
    ...
    else
    {
        ...
        {
            sprintf(filePath, "%s/%s", usrdirPath,
    ...
    case CELL_FS_EBADF:
        ...
        // When cellFsFstat() returns CELL_FS_EBADF, it can be deduced that the
        // above error occurred because a disc was ejected. Explicitly call
        // cellFsClose() and close the applicable file. When using stream supporting
        // APIs, call cellFsStReadFinish() before calling cellFsClose().
