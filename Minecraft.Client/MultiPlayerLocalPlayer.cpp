...
void MultiplayerLocalPlayer::sendPosition()
{
    ...
    if (sprinting != lastSprinting)
    {
        if (sprinting)
        {
            connection->send(shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(shared_from_this(), PlayerCommandPacket::START_SPRINTING)));
        }
        else
        {
            connection->send(shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(shared_from_this(), PlayerCommandPacket::STOP_SPRINTING)));
        }
    ...
    if (sneaking != lastSneaked)
    {
        if (sneaking)
        {
            connection->send(shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(shared_from_this(), PlayerCommandPacket::START_SNEAKING)));
        }
        else
        {
            connection->send(shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(shared_from_this(), PlayerCommandPacket::STOP_SNEAKING)));
        }
    ...
    if (idle != lastIdle)
    {
        if (idle)
        {
            connection->send(shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(shared_from_this(), PlayerCommandPacket::START_IDLEANIM)));
        }
        else
        {
            connection->send(shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(shared_from_this(), PlayerCommandPacket::STOP_IDLEANIM)));
        }
...
void MultiplayerLocalPlayer::setAndBroadcastCustomSkin(DWORD skinId)
{
    ...
    {
        connection->send(shared_ptr<TextureAndGeometryChangePacket>(new TextureAndGeometryChangePacket(shared_from_this(), app.GetPlayerSkinName(GetXboxPad()))));
    }
}
...
void MultiplayerLocalPlayer::setAndBroadcastCustomCape(DWORD capeId)
{
    ...
    {
        connection->send(shared_ptr<TextureChangePacket>(new TextureChangePacket(shared_from_this(), TextureChangePacket::e_TextureChange_Cape, app.GetPlayerCapeName(GetXboxPad()))));
    }
}
