    ...
    else
    {
        if (e->riding == NULL)
        {
            ...
            if (tickCount++ % updateInterval == 0 || e->hasImpulse)
            {
                ...
                {
                    teleportDelay = 0;
                    packet = shared_ptr<TeleportEntityPacket>(new TeleportEntityPacket(e->entityId, xn, yn, zn, yRotn, xRotn));
                ...
                {
                    broadcast(shared_ptr<RotateHeadPacket>(new RotateHeadPacket(e->entityId, yHeadRot)));
                    yHeadRotp = yHeadRot;
                }
...
void TrackedEntity::updatePlayer(EntityTracker *tracker, shared_ptr<ServerPlayer> sp)
{
    ...
    if (visibility == eVisibility_SeenAndVisible && seenBy.find(sp) == seenBy.end())
    {
        seenBy.insert(sp);
    ...
    case eTYPE_MINECART:
        {
            ...
            {
                return shared_ptr<AddEntityPacket>(new AddEntityPacket(e, AddEntityPacket::MINECART_RIDEABLE, yRotp, xRotp, xp, yp, zp));
            }
            if (minecart->type == Minecart::CHEST)
            {
                return shared_ptr<AddEntityPacket>(new AddEntityPacket(e, AddEntityPacket::MINECART_CHEST, yRotp, xRotp, xp, yp, zp));
            }
            if (minecart->type == Minecart::FURNACE)
            {
                return shared_ptr<AddEntityPacket>(new AddEntityPacket(e, AddEntityPacket::MINECART_FURNACE, yRotp, xRotp, xp, yp, zp));
            }
        }
        break;
    case eTYPE_BOAT:
