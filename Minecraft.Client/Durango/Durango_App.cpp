...
void CConsoleMinecraftApp::HandleDLCLicenseChange()
{
    ...
    for (int i = 0; i < iOfferC; i++)
    {
        ...
        {
            DLCPack *pack =
                app.m_dlcManager.getPackFromProductID(xOffer.wszProductID);
            if (pack)
            {
                // Clear the DLC installed flag so the scenes will pick up the new dlc
                // (could be a full pack install)
                app.ClearDLCInstalled();
                app.DebugPrintf(
                    ">>> HandleDLCLicenseChange - Updating license for DLC [%ls]\n",
                    xOffer.wszOfferName);
                pack->updateLicenseMask(1);
            }
            else
            {
                app.DebugPrintf(">>> HandleDLCLicenseChange - Couldn't find licensed "
                                "DLC [%ls] in app.m_dlcManager\n",
                                xOffer.wszOfferName);
            }
        }
    }
...
void CConsoleMinecraftApp::SetRichPresenceContext(int iPad, int contextId)
{
    if (iPad < XUSER_MAX_COUNT && Minecraft::GetInstance()->localplayers[iPad])
    {
        ...
        if (uid != INVALID_XUID)
        {
            // 4J-JEV: Player has changed, update cached player and ensure this next
            // presence is sent.
            ...
            if (m_iLastPresenceContext[iPad] != contextId)
            {
                ...
                EventWriteRichPresenceState(uid.toString().c_str(),
...
int CConsoleMinecraftApp::LoadLocalDLCImages()
{
    ...
    for (AUTO_VAR(it, pDLCInfoA->begin()); it != pDLCInfoA->end(); it++)
    {
        ...
        LoadLocalDLCImage(pDLCInfo->wchBanner, &pDLCInfo->pbImageData,
...
int CConsoleMinecraftApp::LoadLocalDLCImage(WCHAR *wchName, PBYTE *ppbImageData,
                                            DWORD *pdwBytes)
{
    // load the local file
    WCHAR wchFilename[64];

    // 4J-PB - Read the file containing the product codes. This will be different
    // for the SCEE/SCEA/SCEJ builds
    swprintf(wchFilename, L"DLCImages/%s", wchName);
    HANDLE hFile = CreateFile(wchFilename, GENERIC_READ, 0, NULL, OPEN_EXISTING,
...
void CConsoleMinecraftApp::TemporaryCreateGameStart()
{
    ...
    bool isFlat = false;
    __int64 seedValue = BiomeSource::findSeed(
        isFlat ? LevelType::lvl_flat
               : LevelType::lvl_normal); // 4J - was (new Random())->nextLong() -
                                         // now trying to actually find a seed to
...
/*DLC_DETAILS DLCDetailsA[MAX_DLC_DETAILS] =
{
        ...
        // Skin Pack Festive
        { e_DLC_SkinPack,L"Festive Skin
Pack",L"6dffc4d6-a2d2-4c8c-9284-c607f77e431a",L"SPF.png",L"dlcskin00000600.png",0,1},
        // Skin Pack 1
        { e_DLC_SkinPack,L"Skin Pack
1",L"8ecf0f25-a119-4987-a32b-ee0a5925ad8d",L"SP1.png",L"dlcskin00000000.png",0,2},
        // Skin Pack 2
        { e_DLC_SkinPack,L"Skin Pack
2",L"cc59b688-7cfb-4fa0-a76e-84aa55b92cae",L"SP2.png",L"dlcskin00000900.png",0,3},
        // Skin Pack 6
        //{
e_DLC_SkinPack,L"0037a29f-876e-4709-8bb8-a388738e6f51","SP6.png","dlcskin00000900.png",0,3},
        // Skin Pack Battle And Beasts
        { e_DLC_SkinPack,L"Battle And
Beasts",L"eeeb6489-02a3-4c6e-a8c9-2ace2aa1094d",L"SPC.png",L"dlcskin00000800.png",0,4},
        // Skin Pack Battle And Beasts 2
        { e_DLC_SkinPack,L"Battle And Beasts
2",L"b858200a-59a8-4e1f-a049-f3e73db2d786",L"SPZ.png",L"dlcskin00001000.png",0,5},
        // Skin Pack Marvel Avengers
        { e_DLC_SkinPack,L"Marvel
Avengers",L"d59757dd-7757-4c5e-8499-dbe4743baa10",L"SPM.png",L"dlcskin00001700.png",0,6},
        // Skin Pack Marvel Spider-Man
        { e_DLC_SkinPack,L"Marvel
Spider-Man",L"cdca1bff-13d1-435a-8aee-e7a35002473f",L"SPI.png",L"dlcskin00001800.png",0,7},
        // Skin Pack Birthday 1
        { e_DLC_SkinPack,L"Birthday Skin
Pack",L"951e8062-3d4e-470c-8177-5eca91bc08b3",L"SPB.png",L"dlcskin00000700.png",0,8},
        // Skin Pack Birthday 2
        { e_DLC_SkinPack,L"2nd Birthday Skin
Pack",L"c7157788-468d-4ca8-9ecf-5d77a09850fc",L"SPB2.png",L"dlcskin00002200.png",0,9},

        // Texture Packs

        // Plastic Texture Pack
        { e_DLC_TexturePacks,L"Plastic Texture
Pack",L"52ecdcf1-d362-47a1-973b-1eeca0db0ea8",L"TP01.png",L"",2049,1},
        // Natural Texture Pack
        { e_DLC_TexturePacks,L"Natural Texture
Pack",L"1c56db0c-ff49-4bb1-b372-2122b0e813c1",L"TP02.png",L"",2053,2},
        // Halloween Texture Pack
        { e_DLC_TexturePacks,L"Halloween Texture
Pack",L"8cb331d1-8fa1-4367-a41a-d4830a80ce67",L"TP03.png",L"",2052,3},
        // Fantasy Texture Pack
        { e_DLC_TexturePacks,L"Fantasy Texture
Pack",L"375a1df4-5550-415b-b278-20f65b31a7a3",L"TP04.png",L"",2051,4},
        // City Texture Pack
        { e_DLC_TexturePacks,L"City Texture
Pack",L"ea5c7b40-e04d-4469-9382-8806467ca2c4",L"TP05.png",L"",2054,5},
        // Candy Texture Pack
        { e_DLC_TexturePacks,L"Candy Texture
Pack",L"94c75e45-0757-4886-916c-ab026ae27ca9",L"TP06.png",L"",2050,6},
        // Comic Texture Pack
        //{ e_DLC_TexturePacks,L"Comic Texture
Pack",L"3e14cf0f-26eb-40df-897d-7af905456e58",L"TP07.png",L"",2055,7},

        // Mash-up Packs

        // Mass Effect
        { e_DLC_MashupPacks,L"Mass
Effect",L"ead4f3bb-b388-42da-8fa9-f1f91570b5c7",L"MPMA.png",L"dlcskin00001100.png",1024,1},
        // Skyrim
        {
...
void CConsoleMinecraftApp::InitialiseDLCDetails()
{
    for (int i = 0; i < 18; i++)
    {
        // RegisterDLCData(DLCDetailsA[i].e_DLC_Type, DLCDetailsA[i].wchBannerName,
        // DLCDetailsA[i].wchDisplayName, DLCDetailsA[i].wchProductId,
        // DLCDetailsA[i].wchFirstSkin, DLCDetailsA[i].iConfig,
        // DLCDetailsA[i].uiSortIndex);
    }
}
...
int CConsoleMinecraftApp::Callback_TMSPPReadBannedList(void *pParam, int iPad,
                                                       int iUserData,
                                                       LPVOID lpvData,
                                                       WCHAR *wchFilename)
{
...
void CConsoleMinecraftApp::TMSPP_ReadDLCFile(int iPad, eTMSAction NextAction)
{
    ...
    if (m_bRead_TMS_DLCINFO_XML == false)
    {
        ...
        eResult = StorageManager.TMSPP_ReadFile(
            ...
            L"DLCXbox1.cmp", &CConsoleMinecraftApp::Callback_TMSPPReadDLCFile, this,
            NextAction);
        if (eResult != eTitleStorage_pending)
        {
            // something went wrong
    ...
    if (eStorageFacility == C4JStorage::eGlobalStorage_Title)
    {
        if (m_bTMSPP_GlobalFileListRead == false)
        {
            ...
            StorageManager.TMSPP_ReadFileList(
                iPad, eStorageFacility,
                &CConsoleMinecraftApp::Callback_TMSPPRetrieveFileList, this,
                NextAction);
        }
        else
        {
...
int CConsoleMinecraftApp::Callback_TMSPPRetrieveFileList(void *pParam, int iPad,
                                                         int iUserData,
                                                         LPVOID lpvData,
                                                         WCHAR *wchFilename)
{
    ...
    if (lpvData != NULL)
    {
        vector<C4JStorage::PTMSPP_FILE_DETAILS> *pvTmsFileDetails =
            (vector<C4JStorage::PTMSPP_FILE_DETAILS> *)lpvData;
...
int CConsoleMinecraftApp::Callback_TMSPPReadDLCFile(void *pParam, int iPad,
                                                    int iUserData,
                                                    LPVOID lpvData,
                                                    WCHAR *pwchFilename)
{
...
#endif
    {
        ...
        Compression::ECompressionTypes eOriginalCompressionType =
            Compression::getCompression()->GetDecompressionType();
    ...
    else
    {
        ...
        // open the local file
        file = CreateFile(L"DLCXbox1.cmp", GENERIC_READ, 0, NULL, OPEN_EXISTING,
                          FILE_ATTRIBUTE_NORMAL, NULL);
        ...
        if (dwFileSize != 0)
        {
            ...
            {
                // StorageManager.TMSPP_WriteFile(iPad,C4JStorage::eGlobalStorage_TitleUser,C4JStorage::TMS_FILETYPE_BINARY,L"DLCXbox1.cmp",(PBYTE)
                // pbData, dwFileSize,NULL,NULL, 0);
                StorageManager.TMSPP_WriteFile(
                    iPad, C4JStorage::eGlobalStorage_TitleUser,
                    C4JStorage::TMS_FILETYPE_BINARY, L"TP06.cmp", (PBYTE)pbData,
                    dwFileSize, NULL, NULL, 0);
            }
            Sleep(5000);
        }
        ...
        /*
        // now the icon
        file = CreateFile(L"TP06.png", GENERIC_READ, 0, NULL, OPEN_EXISTING,
        FILE_ATTRIBUTE_NORMAL, NULL); if( file == INVALID_HANDLE_VALUE )
        {
                DWORD error = GetLastError();
                app.DebugPrintf("Failed to open DLCXbox1.cmp with error code %d
        ...
        if(dwFileSize!=0)
        {
                ...
                if(bytesRead==dwFileSize)
                {
                        StorageManager.TMSPP_WriteFile(iPad,C4JStorage::eGlobalStorage_TitleUser,C4JStorage::TMS_FILETYPE_BINARY,L"TP06.png",(PBYTE)
        pbData, dwFileSize,NULL,NULL, 0);
                }
                Sleep(5000);
        }
    ...
    void *pParam, C4JStorage::ESaveIncompleteType saveIncompleteType)
{
        ...
        saveIncompleteType == C4JStorage::ESaveIncomplete_OutOfLocalStorage)
    {
        ...
        UINT uiIDA[3] = {IDS_SAVE_INCOMPLETE_RETRY_SAVING,
                         IDS_SAVE_INCOMPLETE_DISABLE_SAVING,
                         IDS_SAVE_INCOMPLETE_DELETE_SAVES};

        if (ui.RequestMessageBox(IDS_SAVE_INCOMPLETE_TITLE, message, uiIDA, 3, 0,
                                 Callback_SaveGameIncompleteMessageBoxReturned,
                                 pClass, app.GetStringTable()) ==
            C4JStorage::EMessage_Busy)
        {
            // If this failed to display, continue as if we cancelled. This isn't
            // ideal, but the user should already have had some system notification of
            // being out of memory, and if we instantly retry then they may not be
            // able to navigate whatever other error is blocking this from appearing
            Callback_SaveGameIncompleteMessageBoxReturned(
                pParam, 0, C4JStorage::EMessage_Cancelled);
        }
    }
    else
    {
...
int CConsoleMinecraftApp::Callback_SaveGameIncompleteMessageBoxReturned(
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    case C4JStorage::EMessage_ResultThirdOption:
        ui.NavigateToScene(iPad, eUIScene_InGameSaveManagementMenu, NULL,
                           eUILayer_Error, eUIGroup_Fullscreen);
        break;
    }
    return 0;
}
...
void CConsoleMinecraftApp::ReadLocalDLCList(void)
{
    ...
    if (fDLCList.exists())
    {
        ...
        Compression::getCompression()->SetDecompressionType(
            Compression::eCompressionType_LZXRLE);
        Compression::getCompression()->Decompress(pDecompressedData, &uiDecompSize,
