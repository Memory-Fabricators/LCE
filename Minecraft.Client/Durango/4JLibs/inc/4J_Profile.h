...
class C_4JProfile
{
  public:
    ...
    // 4 players have game defined data, puiGameDefinedDataChangedBitmask needs to
    // be checked by the game side to see if there's an update needed - it'll have
    // the bits set for players to be updated
    void Initialise(const std::wstring &serviceConfigId,
                    const std::wstring &titleProductId);
    // void				SetTrialTextStringTable(CXuiStringTable
    // *pStringTable,int iAccept,int iReject);
    ...
    UINT RequestSignInUI(bool bFromInvite, bool bLocalGame, bool bNoGuestsAllowed,
    ...
    void StartTrialGame(); // disables saves and leaderboard, and change state to
                           // readyforgame from pregame
    ...
    void CheckPrivilege(int iPad, bool thisQuadrantOnly,
                        WXAMS::KnownPrivileges privilege,
                        void (*Func)(LPVOID, bool, int), LPVOID lpParam);
    void CheckPrivileges(int iPad, bool thisQuadrantOnly,
                         const std::vector<WXAMS::KnownPrivileges> &privileges,
                         void (*Func)(LPVOID, bool, int), LPVOID lpParam);
    ...
    // PROFILE DATA
    // int					SetDefaultOptionsCallback(int(
    // *Func)(LPVOID,PROFILESETTINGS *, const int iPad),LPVOID lpParam); int
    // SetOldProfileVersionCallback(int( *Func)(LPVOID,unsigned char *, const
    // unsigned short,const int),LPVOID lpParam); PROFILESETTINGS *
    // GetDashboardProfileSettings(int iPad); void
    // WriteToProfile(int iQuadrant, bool bGameDefinedDataChanged=false, bool
    // bOverride5MinuteLimitOnProfileWrites=false); void
    // ForceQueuedProfileWrites(int iPad=XUSER_INDEX_ANY); void
    // *GetGameDefinedProfileData(int iQuadrant);
    void ResetProfileProcessState(); // after a sign out from the primary player,
                                     // call this
    void Tick(void);

    void
    GetProfile(PlayerUID xuid,
               void (*func)(LPVOID,
                            Microsoft::Xbox::Services::Social::XboxUserProfile ^),
               LPVOID param);
    void GetProfiles(
    ...
    // void				RegisterAward(int iAwardNumber,int
    // iGamerconfigID, eAwardType eType, bool bLeaderboardAffected=false,
    //									CXuiStringTable*pStringTable=NULL,
    // int iTitleStr=-1, int iTextStr=-1, int iAcceptStr=-1, char
    //*pszThemeName=NULL, unsigned int uiThemeSize=0L); int
    // GetAwardId(int iAwardNumber);
    ...
    // bool				IsAwardsFlagSet(int iQuadrant, int
    // iAward);

    // RICH PRESENCE

    // void				RichPresenceInit(int iPresenceCount, int
    // iContextCount); void
    // RegisterRichPresenceContext(int iGameConfigContextID); void
    // SetRichPresenceContextValue(int iPad,int iContextID, int iVal);
    ...
    void SetUpsellCallback(void (*Func)(LPVOID lpParam, eUpsellType type,
                                        eUpsellResponse response, int iUserData),
                           LPVOID lpParam);
