...
class C4JStorage
{
  public:
    ...
    void Init(unsigned int uiSaveVersion, LPCWSTR pwchDefaultSaveName,
              ...
              bool (*SetRetrieveProductIDFn)(XCONTENT_DATA &Data),
              WCHAR *pwchSCID, WCHAR *pwchTitleId);
    void SetGameSaveFolderTitle(
        WCHAR *wszGameSaveFolderTitle); // Sets the title to be set in the
    ...
    void SetDefaultImages(
        PBYTE pbSaveThumbnail,
        DWORD dwSaveThumbnailBytes); // Sets default save thumbnail, which can be
                                     // used when saving a game that hasn't
                                     // generated any yet
    void SetIncompleteSaveCallback(
        void (*Func)(LPVOID, const ESaveIncompleteType),
        LPVOID param); // Sets callback to be used in the event of a save method
    ...
    C4JStorage::ESaveGameState DoesSaveExist(
        bool *pbExists); // Determine if current savegame exists on storage device
    ...
    PSAVE_DETAILS
    ReturnSavesInfo();     // Returns result of search (or NULL if not yet received)
    ...
    C4JStorage::ESaveGameState LoadSaveDataThumbnail(
        ...
        LPVOID lpParam,
        bool force = false); // Get the thumbnail for an individual save
                             // referenced by pSaveInfo

    // Loading savedata & obtaining information from just-loaded file
    C4JStorage::ESaveGameState
    LoadSaveData(PSAVE_INFO pSaveInfo,
    ...
    void GetSaveData(void *pvData,
                     unsigned int *puiBytes); // Obtains pointer to, and size, of
                                              // just-loaded save
    bool
    GetSaveUniqueNumber(INT *piVal); // Gets the unique numeric portion of the
                                     // folder name used for the save (encodes m
    ...
    unsigned int
    GetSubfileCount();    // Get the number of sub-files in the just-loaded save
    void ResetSubfiles(); // Reset storage manager's references to sub-files, to
                          // be called when we are finished with the save game
    void GetSubfileDetails(
        int idx, unsigned int *subfileId, unsigned char **data,
        unsigned int *sizeOut); // Gets details for sub-file from 0 to
                                // GetSubfileCount() - 1. Caller is responsible
                                // for data allocation after this point.
    void
    UpdateSubfile(int idx, unsigned char *data,
                  unsigned int size);       // Update internal details for a sub-file,
                                            // from 0 to GetSubfileCount() - 1.
    int AddSubfile(unsigned int subfileId); // Adds an additional sub-file with a
                                            // given subfileId, returns index
    C4JStorage::ESaveGameState
    SaveSubfiles(int (*Func)(LPVOID, const bool),
                 LPVOID lpParam); // Writes all modified sub-files, calling
                                  // callback on completion

    // Saving savedata
    void SetSaveTitle(
        const wchar_t *UTF16String); // Sets the name which is used as a sub-title
                                     // in the savedata param.sfo
    ...
    void GetDefaultSaveImage(
        PBYTE *ppbSaveImage,
        DWORD *pdwSaveImageBytes); // Get the default save thumbnail (as set by
    ...
    C4JStorage::ESaveGameState SaveSaveData(
        ...
        LPVOID lpParam); // Save the actual data, calling callback on completion
    ...
    C4JStorage::ESaveGameState
    DeleteSaveData(PSAVE_INFO pSaveInfo, int (*Func)(LPVOID lpParam, const bool),
                   LPVOID lpParam); // Deletes savedata referenced by pSaveInfo,
                                    // calls callback when comple
    C4JStorage::ESaveGameState CopySaveData(
        PSAVE_INFO pSaveInfo,
        int (*Func)(LPVOID, const bool, C4JStorage::ESaveGameState state),
        bool (*FuncProg)(LPVOID, const int),
        LPVOID lpParam); // Copies savedata referenced by pSaveInfo, calls
                         // callback when complete
    C4JStorage::ESaveGameState
    RenameSaveData(int iRenameIndex, uint16_t *pui16NewName,
                   int (*Func)(LPVOID lpParam, const bool),
                   LPVOID lpParam); // Renamed savedata with index from last
                                    // established ReturnSavesInfo.
    ...
    void InitialiseProfileData(
        ...
        unsigned int *puiGameDefinedDataChangedBitmask); // General initialisation
    int SetDefaultOptionsCallback(
        int (*Func)(LPVOID, PROFILESETTINGS *, const int iPad),
        LPVOID lpParam); // Set a callback that can initialise a profile's storage
                         // to its default settings
    void
    SetOptionsDataCallback(int (*Func)(LPVOID, int iPad, unsigned short usVersion,
                                       C4JStorage::eOptionsCallback),
                           LPVOID lpParam); // Sets callback that is called when
                                            // status of any options has changed
    ...
    void *
    GetGameDefinedProfileData(int iQuadrant); // Get pointer to the game-defined
                                              // profile data for one user
    ...
    void ForceQueuedProfileWrites(
        int iPad = -1); // Force any queued profile writes to write now
    ...
    DWORD MountInstalledDLC(int iPad, DWORD dwDLC,
                            ...
                            LPVOID lpParam, LPWSTR szMountDrive = NULL);
    ...
    DWORD InstallOffer(int iOfferIDC, WCHAR *pwchProductId,
                       int (*Func)(LPVOID, int, int), LPVOID lpParam,
                       bool bTrial = false);

    void UpdateDLCProductIDs(); // once we have the dlc info, we can give local
                                // installed DLC their product ids

    // void
    // SetRetrieveProductIDFn(int( *Func)(XCONTENT_DATA&)); // Retrieve a product
    // id for the dlc from the game
...
#pragma warning(disable : 4100)
    ...
    void ClearDLCOffers();
    // 	C4JStorage::ETMSStatus		ReadTMSFile(int iQuadrant,eGlobalStorage
    // eStorageFacility,C4JStorage::eTMS_FileType eFileType, WCHAR
    // *pwchFilename,BYTE **ppBuffer,DWORD *pdwBufferSize,int( *Func)(LPVOID,
    // WCHAR *,int, bool, int),LPVOID lpParam, int iAction);// { return
    // C4JStorage::ETMSStatus_Idle; } 	bool
    // WriteTMSFile(int iQuadrant,eGlobalStorage eStorageFacility,LPWSTR
    // wszFilename,BYTE *pBuffer,DWORD dwBufferSize);
    bool DeleteTMSFile(int iQuadrant, eGlobalStorage eStorageFacility,
                       LPWSTR wszFilename);

    // TMS++
    C4JStorage::ETMSStatus TMSPP_GetUserQuotaInfo(
        C4JStorage::eGlobalStorage eStorageFacility,
        int iPad); //,TMSCLIENT_CALLBACK Func,LPVOID lpParam, int iUserData=0);
    eTitleStorageState
    TMSPP_WriteFile(int iQuadrant, C4JStorage::eGlobalStorage eStorageFacility,
                    C4JStorage::eTMS_FILETYPEVAL eFileTypeVal, LPWSTR wszFilename,
                    BYTE *pbBuffer, DWORD dwBufferSize,
                    int (*Func)(LPVOID, int, int) = NULL, LPVOID lpParam = NULL,
                    int iUserData = 0);
    eTitleStorageState
    TMSPP_ReadFile(int iQuadrant, C4JStorage::eGlobalStorage eStorageFacility,
                   C4JStorage::eTMS_FILETYPEVAL eFileTypeVal, LPWSTR wszFilename,
                   int (*Func)(LPVOID, int, int, LPVOID, WCHAR *), LPVOID lpParam,
                   int iUserData);
    eTitleStorageState
    TMSPP_DeleteFile(int iQuadrant, C4JStorage::eGlobalStorage eStorageFacility,
                     C4JStorage::eTMS_FILETYPEVAL eFileTypeVal,
                     LPWSTR wszFilename, int (*Func)(LPVOID, int, int),
                     LPVOID lpParam, int iUserData);
    eTitleStorageState
    TMSPP_ReadFileList(int iPad, C4JStorage::eGlobalStorage eStorageFacility,
                       int (*Func)(LPVOID, int, int, LPVOID, WCHAR *) = NULL,
                       LPVOID lpParam = NULL, int iUserData = 0);
    bool TMSPP_InFileList(eGlobalStorage eStorageFacility, int iPad,
    ...
    C4JStorage::eSaveTransferState SaveTransferGetDetails(
        int iPad, eGlobalStorage source, wchar_t *file,
        int (*Func)(LPVOID lpParam,
                    C4JStorage::SAVETRANSFER_FILE_DETAILS *pSaveTransferDetails),
        LPVOID lpParam);
    C4JStorage::eSaveTransferState SaveTransferGetData(
        int iPad, eGlobalStorage source, wchar_t *file,
        int (*Func)(LPVOID lpParam,
                    C4JStorage::SAVETRANSFER_FILE_DETAILS *pSaveTransferDetails),
        int (*ProgressFunc)(LPVOID lpParam, unsigned long), LPVOID lpParam,
        LPVOID lpProgressParam);
    void CancelSaveTransfer(int (*CancelCompleteFunc)(LPVOID lpParam),
                            LPVOID lpParam);
};
