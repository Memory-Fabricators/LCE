...
#define TITLE_PRODUCT_ID \
    L"582e7bcc-11bc-4702-ab1b-b31566f8e327" // Parent Title's ProductID
...
class CConsoleMinecraftApp : public CMinecraftApp
{
  ...
  public:
    ...
    static int Callback_TMSPPRetrieveFileList(void *pParam, int iPad,
                                              int iUserData, LPVOID lpvData,
                                              WCHAR *wchFilename);
    // 	static int Callback_TMSPPReadXuidsFile(void *pParam,int iPad, int
    // iUserData, C4JStorage::PTMSPP_FILEDATA pFileData,LPCSTR szFilename);
    // static int Callback_TMSPPReadConfigFile(void *pParam,int iPad, int
    // iUserData, C4JStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename);
    ...
    virtual bool GetTMSUserFileListRead()
    {
        return m_bTMSPP_UserFileListRead;
    }

    static void Callback_SaveGameIncomplete(
        void *pParam, C4JStorage::ESaveIncompleteType saveIncompleteType);
    static int Callback_SaveGameIncompleteMessageBoxReturned(
  ...
  private:
    ...
    bool m_bTMSPP_UserFileListRead;   // track whether we have already read the file
                                      // list from TMSPP
};
