...
PVOID WINAPI XMemAllocLog(_In_ SIZE_T dwSize, _In_ ULONGLONG dwAttributes)
{
    ...
    //	app.DebugPrintf("XMemAllocLog %p, Size: %d, AllocatorId: 0x%02x,
    // ObjectType: 0x%04x, PageSize: %d, Alignment: %d, MemoryType: %d\n", result,
    // dwSize, expanded_attributes.s.dwAllocatorId,
    // expanded_attributes.s.dwObjectType, expanded_attributes.s.dwPageSize,
    // expanded_attributes.s.dwAlignment, expanded_attributes.s.dwMemoryType  );
    return result;
}
...
VOID WINAPI XMemFreeLog(_In_ PVOID lpAddress, _In_ ULONGLONG dwAttributes)
{
    ...
    LeaveCriticalSection(&memTrackCS);
    XALLOC_ATTRIBUTES expanded_attributes = ExpandAllocAttributes(dwAttributes);
    //	app.DebugPrintf("XMemFreeLog %p, AllocatorId: 0x%02x, ObjectType:
    // 0x%04x, PageSize: %d, Alignment: %d, MemoryType: %d\n", lpAddress,
    // expanded_attributes.s.dwAllocatorId, expanded_attributes.s.dwObjectType,
    // expanded_attributes.s.dwPageSize, expanded_attributes.s.dwAlignment,
    // expanded_attributes.s.dwMemoryType  );
...
void ApplicationView::SetWindow(CoreWindow ^ window)
{
    window->Closed +=
        ref new TypedEventHandler<CoreWindow ^, CoreWindowEventArgs ^>(
            this, &ApplicationView::OnWindowClosed);
}
...
// Called when the application is activated.  For now, there is just one
// activation kind - Launch.
void ApplicationView::OnActivated(CoreApplicationView ^ applicationView,
                                  IActivatedEventArgs ^ args)
{
    CoreWindow::GetForCurrentThread()->Activate();

    Windows::Xbox::Multiplayer::PartyConfig::SuppressGameSessionReadyToast = true;
    Windows::Xbox::Multiplayer::PartyConfig::SuppressGameSessionReadyToastMode =
        Windows::Xbox::Multiplayer::SuppressGameSessionReadyToastMode::Always;

    // On starting up the game for the first time, we'll get one kind of
    // onActivated message through. This will also be called at other times
    // though, so don't rely on it only happening once for anything
    if (!m_activationComplete)
    {
        if (args->Kind ==
            Windows::ApplicationModel::Activation::ActivationKind::Launch)
        {
            // We get ActivationKind::Launch of we've just been started up from the
            // user selecting us in the system UI, or if we are run normally from the
            // debugger
            ILaunchActivatedEventArgs ^ newArgs =
                static_cast<ILaunchActivatedEventArgs ^>(args);
        }
        else if (args->Kind == Windows::ApplicationModel::Activation::
                                   ActivationKind::Protocol)
        {
            // We get Windows::ApplicationModel::Activation::ActivationKind::Protocol
            // if we have been started from accepting a party invite (including
            // joining an open party ourselves), or from a game session being ready
            IProtocolActivatedEventArgs ^ proArgs =
            ...
            {
                Platform::String ^ uriCommand =
                    ref new Platform::String(proArgs->Uri->RawUri->Data());
                ...
                DQRNetworkManager::SetPartyProcessJoinSession(
                    0, query->GetFirstValueByName(L"sessionName"),
                    query->GetFirstValueByName(L"serviceConfigurationId"),
                    query->GetFirstValueByName(L"sessionTemplateName"));
            }
        }
    ...
    {
        // This type of activation protocol can come in at any time during the game.
        // Not sure of the best way to handle both this and gamesessionready events,
        // so for now going to only process these (1) once (2) until the main menu
        // is reached To try and minimise any conflict of them coming in in the
        // middle of the game when we are already handling this gamesessionready.
        if (args->Kind ==
            Windows::ApplicationModel::Activation::ActivationKind::Protocol)
        {
            if (!app.HasReachedMainMenu())
            {
                ...
                // We get
                // Windows::ApplicationModel::Activation::ActivationKind::Protocol if we
                // have been started from accepting a party invite (including joining an
                // open party ourselves), or from a game session being ready
                IProtocolActivatedEventArgs ^ proArgs =
                    static_cast<IProtocolActivatedEventArgs ^>(args);
            ...
            {
                DQRNetworkManager::
                    SetInviteReceivedFlag(); // This just informs the network manager
                                             // that an invite has been received, not to
                                             // start processing it
            }
        }
    }
...
void ApplicationView::OnSuspending(Platform::Object ^ sender,
...
void ApplicationView::OnResuming(Platform::Object ^ sender,
                                 Platform::Object ^ args)
{
    ...
    {
        // Bit specific but we want to avoid switching to offline if the primary
        // player has signed out (and AFAIK there's no other way to find out until a
        // few ticks time)
        if (app.GetXuiAction(0) != eAppAction_PrimaryPlayerSignedOut)
        {
            // We've lost the party so switch to offline, previously we exited the
            // world here but this seems friendlier
            app.SetAction(ProfileManager.GetPrimaryPad(),
                          eAppAction_EthernetDisconnected);
        }
    }

    // 4J-PB - clear all DLC info, and recheck, since someone may have suspended
    // after starting a DLCinstall, and we won't have received any install/
    // license change event
    app.ClearDLCInstalled();
    ui.HandleDLCInstalled(ProfileManager.GetPrimaryPad());
}

void ApplicationView::OnResourceAvailabilityChanged(
    Platform::Object ^ /*sender*/, Platform::Object ^ /*args*/)
{
    app.DebugPrintf("###################################\n\n\n");
    if (Windows::ApplicationModel::Core::CoreApplication::ResourceAvailability ==
        Windows::ApplicationModel::Core::ResourceAvailability::Full)
    {
...
void ApplicationView::OnWindowClosed(CoreWindow ^ sender,
