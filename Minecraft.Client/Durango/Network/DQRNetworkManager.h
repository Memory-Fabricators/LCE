...
{
    internal : DQRNetworkManagerEventHandlers(DQRNetworkManager *pDQRNet);

  public:
    void Setup(WXNRs::Session ^ session);
    void Pulldown(WXNRs::Session ^ session);

    void DataReceivedHandler(Platform::Object ^ session,
                             WXNRs::DataReceivedEventArgs ^ args);
    void SessionAddressDataChangedHandler(
        Platform::Object ^ session,
        WXNRs::SessionAddressDataChangedEventArgs ^ args);
    void SessionStatusUpdateHandler(Platform::Object ^ session,
                                    WXNRs::SessionStatusUpdateEventArgs ^ args);
    void AddedSessionAddressHandler(Platform::Object ^ session,
                                    WXNRs::AddedSessionAddressEventArgs ^ args);
    void RemovedSessionAddressHandler(Platform::Object ^ session,
                                      WXNRs::RemovedSessionAddressEventArgs ^
                                          args);
    void GlobalSessionDataChangedHandler(
        Platform::Object ^ session,
...
class DQRNetworkManager
{
  ...
  public:
    ...
    static const int PRIMARY_PLAYER_LEAVING_PARTY_WAIT_MS =
        20000; // Time between primary player leaving and when we should respond
               // to it, to allow time for us to receive a new party for them to
               // be going to if they are just changing party rather than leaving
               // altogether

    class SessionInfo
    {
      public:
        SessionInfo(wstring &sessionName, wstring &serviceConfig,
    ...
    {
      public:
        wchar_t *m_XUID;           // XUID / XboxUserIds are passed round as decimal strings
                                   // on Xbox 1
        uint32_t m_sessionAddress; // XRNS session address for this player, ie can
                                   // identify which machine this player is on
        uint8_t m_smallId;         // Assigned by DQRNetworkManager, to attach a permanent
                                   // id to this player (until we have to wrap round), to
                                   // match a similar concept in qnet
        uint8_t m_channel;         // Local index / communication channel within session
                                   // address, of player on this machine
    ...
    void CreateAndJoinSession(int userMask, unsigned char *customSessionData,
                              unsigned int customSessionDataSize, bool offline);
    void JoinSession(int playerMask);
    void JoinSessionFromInviteInfo(int playerMask);
    bool AddUsersToSession(int playerMask,
    ...
    class SessionSearchResult
    {
      public:
        ...
        // These names/xuids reflect the server controlled list of who is actually
        // in the game
        wstring m_playerNames[MAX_ONLINE_PLAYER_COUNT];
        PlayerUID m_playerXuids[MAX_ONLINE_PLAYER_COUNT];
        int m_playerCount;

        // This count & set of xuids reflects the session document list of who is in
        // the game
  ...
  protected:
    ...
    // external state needs to be updated (m_INTtoEXTStateMappings, defined in the
    // cpp file)
    typedef enum
    {
    ...
    class StateChangeInfo
    {
      public:
        ...
        StateChangeInfo(eDQRNetworkManagerState oldState,
                        eDQRNetworkManagerState newState)
  ...
  private:
    ...
    static const eDQRNetworkManagerState
        m_INTtoEXTStateMappings[DNM_INT_STATE_COUNT];
  ...
  private:
    ...
    unordered_map<unsigned int, DQRConnectionInfo *>
        m_sessionAddressToConnectionInfoMapHost; // For host - there may be more
                                                 // than one remote session
                                                 // attached to this listening
                                                 // session
    unsigned int
        m_sessionAddressFromSmallId[256];     // For host - for mapping back from
                                              // small Id, to session address
    unsigned char m_channelFromSmallId[256];  // For host and client, for mapping
                                              // back from small Id, to channel
    ...
    MXSM::MultiplayerSession ^
        WriteSessionHelper(MXS::XboxLiveContext ^ xboxLiveContext,
                           MXSM::MultiplayerSession ^ multiplayerSession,
                           MXSM::MultiplayerSessionWriteMode writeMode,
                           HRESULT &hr);
    MXSM::MultiplayerSessionMember ^
        GetUserMemberInSession(Windows::Xbox::System::User ^ user,
                               MXSM::MultiplayerSession ^ session);
    bool IsPlayerInSession(Platform::String ^ xboxUserId,
                           MXSM::MultiplayerSession ^ session, int *smallId);
    ...
    void BytesReceivedInternal(DQRConnectionInfo *connectionInfo,
                               unsigned int sessionAddress, BYTE *bytes,
                               int byteCount);
    ...
    void SendBytesChat(unsigned int address, BYTE *bytes, int byteCount,
                       bool reliable, bool sequential, bool broadcast);

    bool AddRoomSyncPlayer(DQRNetworkPlayer *pPlayer, unsigned int sessionAddress,
                           int channel);
  ...
  public:
    ...
    static void SetPartyProcessJoinSession(int bootUserIndex,
                                           Platform::String ^ bootSessionName,
    ...
    void FriendPartyManagerGetSessionInfo(int idx,
                                          SessionSearchResult *searchResult);
    WFC::IVectorView<WXM::UserPartyAssociation ^> ^
        FilterPartiesByPermission(MXS::XboxLiveContext ^ context,
                                  WFC::IVectorView<WXM::UserPartyAssociation ^> ^
                                      partyResults);

    bool JoinPartyFromSearchResult(SessionSearchResult *searchResult,
                                   int playerMask);
  ...
  private:
    ...
    bool GetGameSessionData(MXSM::MultiplayerSession ^ session,
  ...
  private:
    ...
    static void GetProfileCallback(
        LPVOID pParam,
    ...
    // Incoming messages - to be called from the main thread, to get incoming
    // messages from the RTS work thread
    ...
    // Outgoing messages - to be called from the RTS work thread, to process
    ...
    // These methods are called from the main thread, to put an outgoing message
    // onto the queue to be processed by these previous methods
    void RTS_StartCient();
    void RTS_StartHost();
    void RTS_Terminate();
    void RTS_SendData(unsigned char *pucData, unsigned int dataSize,
                      unsigned int sessionAddress, bool reliable, bool sequential,
                      bool coalesce, bool includeMode, bool gameChannel);
};
...
{
  public:
    virtual void HandleDataReceived(DQRNetworkPlayer *playerFrom,
    ...
    virtual void
    HandleStateChange(DQRNetworkManager::eDQRNetworkManagerState oldState,
                      DQRNetworkManager::eDQRNetworkManagerState newState) = 0;
    ...
    HandleInviteReceived(int playerIndex,
                         DQRNetworkManager::SessionInfo *pInviteInfo) = 0;
