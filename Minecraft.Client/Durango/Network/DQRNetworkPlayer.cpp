...
DQRNetworkPlayer::DQRNetworkPlayer(DQRNetworkManager *manager,
                                   eDQRNetworkPlayerType playerType,
...
bool DQRNetworkPlayer::IsTalking()
{
    ...
    Microsoft::Xbox::GameChat::ChatUser ^ chatUser =
        m_manager->m_chat->GetChatUserByXboxUserId(
            ref new Platform::String(m_UID.toString().c_str()));

    if (chatUser == nullptr)
    {
        return false;
    }
    if (chatUser->TalkingMode ==
        Microsoft::Xbox::GameChat::ChatUserTalkingMode::NotTalking)
    {
        return false;
    }
    else
...
bool DQRNetworkPlayer::HasVoice()
{
    ...
    if (((int)chatUser->ParticipantType) &
        ((int)Windows::Xbox::Chat::ChatParticipantTypes::Talker))
    {
        return true;
    }
    else
...
void DQRNetworkPlayer::SendData(DQRNetworkPlayer *pPlayerTarget,
                                const void *data, unsigned int dataSize)
{
    // Our network is connected as a star. If we are the host, then we can send to
    // any remote player. If we're a client, we can send only to the host. The
    // host can also send to other local players, but this doesn't need to go
    // through Rudp.
    if (m_host)
    {
        if ((m_type == DNP_TYPE_HOST) &&
            (pPlayerTarget->m_type == DNP_TYPE_REMOTE))
        {
    ...
    {
        if ((m_type == DNP_TYPE_LOCAL) &&
            (pPlayerTarget->m_type == DNP_TYPE_HOST))
        {
