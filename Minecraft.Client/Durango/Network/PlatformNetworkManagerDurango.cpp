...
void CPlatformNetworkManagerDurango::HandleStateChange(
    ...
    DQRNetworkManager::eDQRNetworkManagerState newState)
{
    ...
    else if (newState == DQRNetworkManager::DNM_STATE_ENDING)
    {
        g_NetworkManager.StateChange_AnyToEnding(
            oldState == DQRNetworkManager::DNM_STATE_PLAYING);
...
void CPlatformNetworkManagerDurango::HandlePlayerJoined(
    DQRNetworkPlayer *pDQRPlayer)
{
    const char *pszDescription;

    // If this is a local player, we need to inform the chat system that it has
    // joined
    ...
    NetworkPlayerDurango *networkPlayer =
        (NetworkPlayerDurango *)addNetworkPlayer(pDQRPlayer);
    ...
    else
    {
        ...
        if (m_pDQRNet->IsHost() && !m_bHostChanged)
        {
            ...
            for (AUTO_VAR(it, m_machineDQRPrimaryPlayers.begin());
                 it < m_machineDQRPrimaryPlayers.end(); ++it)
            {
    ...
    app.DebugPrintf("Player 0x%p \"%ls\" joined; %s; voice %i; camera %i.\n",
                    pDQRPlayer, pDQRPlayer->GetGamertag(), pszDescription,
                    (int)pDQRPlayer->HasVoice(), (int)pDQRPlayer->HasCamera());
    ...
    if (m_pDQRNet->GetState() == QNET_STATE_GAME_PLAY)
    {
        ...
        {
            if (m_pDQRNet->GetLocalPlayerByUserIndex(idx) != NULL)
            {
                ++localPlayerCount;
            }
        }
...
void CPlatformNetworkManagerDurango::HandlePlayerLeaving(
    DQRNetworkPlayer *pDQRPlayer)
{
    //__debugbreak();
        ...
        if (m_pDQRNet->IsHost() && !m_bHostChanged)
        {
            if (isSystemPrimaryPlayer(pDQRPlayer))
            {
                ...
                for (unsigned int i = 0; i < m_pDQRNet->GetPlayerCount(); ++i)
                {
                    ...
                    if (pDQRPlayer2 != pDQRPlayer &&
                        pDQRPlayer2->IsSameSystem(pDQRPlayer))
                    {
                        pNewDQRPrimaryPlayer = pDQRPlayer2;
                        break;
                    }
                }
                AUTO_VAR(it, find(m_machineDQRPrimaryPlayers.begin(),
                                  m_machineDQRPrimaryPlayers.end(), pDQRPlayer));
...
void CPlatformNetworkManagerDurango::HandleDataReceived(
    DQRNetworkPlayer *playerFrom, DQRNetworkPlayer *playerTo,
...
void CPlatformNetworkManagerDurango::HandleInviteReceived(
    int playerIndex, DQRNetworkManager::SessionInfo *pInviteInfo)
{
    g_NetworkManager.GameInviteReceived(playerIndex, pInviteInfo);
}

bool CPlatformNetworkManagerDurango::Initialise(
    CGameNetworkManager *pGameNetworkManager, int flagIndexSize)
{
    ...
    Windows::Networking::Connectivity::NetworkInformation::NetworkStatusChanged +=
        ref new Windows::Networking::Connectivity::
            NetworkStatusChangedEventHandler([](Platform::Object ^ pxObject) {
                app.DebugPrintf("NetworkStatusChanged callback\n");
                auto internetProfile = Windows::Networking::Connectivity::
                    NetworkInformation::GetInternetConnectionProfile();
                if (internetProfile != nullptr)
                {
                    auto connectionLevel =
                    ...
                    // bool bConnected	= (connectionLevel ==
                    // Windows::Networking::Connectivity::NetworkConnectivityLevel::XboxLiveAccess)?true:false;
...
bool CPlatformNetworkManagerDurango::isSystemPrimaryPlayer(
    DQRNetworkPlayer *pDQRPlayer)
{
    bool playerIsSystemPrimary = false;
    for (auto it = m_machineDQRPrimaryPlayers.begin();
         it < m_machineDQRPrimaryPlayers.end(); ++it)
    {
...
bool CPlatformNetworkManagerDurango::RemoveLocalPlayerByUserIndex(
    int userIndex)
{
...
bool CPlatformNetworkManagerDurango::SessionHasSpace(
...
bool CPlatformNetworkManagerDurango::_LeaveGame(bool bMigrateHost,
                                                bool bLeaveRoom)
{
    m_bLeavingGame = true;
...
void CPlatformNetworkManagerDurango::HostGame(
...
void CPlatformNetworkManagerDurango::_HostGame(
    ...
    unsigned char privateSlots /*= 0*/)
{
    ...
    m_pDQRNet->CreateAndJoinSession(usersMask,
                                    (unsigned char *)&m_hostGameSessionData,
                                    sizeof(m_hostGameSessionData), IsLocalGame());
}
...
int CPlatformNetworkManagerDurango::JoinGame(FriendSessionInfo *searchResult,
                                             int localUsersMask,
                                             int primaryUserIndex)
{
    ...
    for (int i = 0; i < DQRNetworkManager::MAX_LOCAL_PLAYER_COUNT; i++)
    {
        ...
        {
            // Check if this joining user is already in the session, in which case we
            // don't need to count it
            bool isJoiningUser = false;
            for (int j = 0; j < searchResult->searchResult.m_usedSlotCount; j++)
            {
                Platform::String ^ xuid = ref new Platform::String(
                    searchResult->searchResult.m_sessionXuids[j].c_str());
                if (xuid == ProfileManager.GetUser(i)->XboxUserId)
                {
                    app.DebugPrintf("Joining user found to be already in session, so "
                                    "won't be counting to our total\n");
                    isJoiningUser = true;
                    break;
                }
            }
    ...
    app.DebugPrintf(
        "Used slots: %d, fully playing players: %d, trying to join %d\n",
        searchResult->searchResult.m_usedSlotCount,
        searchResult->searchResult.m_playerCount, joinPlayerCount);
    GameSessionData *gameSession = (GameSessionData *)(&searchResult->data);
    if ((searchResult->searchResult.m_usedSlotCount + joinPlayerCount) >
        DQRNetworkManager::MAX_ONLINE_PLAYER_COUNT)
    {
        return CGameNetworkManager::JOINGAME_FAIL_SERVER_FULL;
    }

    if (m_pDQRNet->JoinPartyFromSearchResult(&searchResult->searchResult,
                                             localUsersMask))
    {
...
void CPlatformNetworkManagerDurango::RegisterPlayerChangedCallback(
    int iPad,
...
void CPlatformNetworkManagerDurango::UnRegisterPlayerChangedCallback(
    int iPad,
    void (*callback)(void *callbackParam, INetworkPlayer *pPlayer,
                     bool leaving),
    void *callbackParam)
...
void CPlatformNetworkManagerDurango::HandleAddLocalPlayerFailed(
    int idx, bool serverFull)
{
...
int CPlatformNetworkManagerDurango::RemovePlayerOnSocketClosedThreadProc(
    void *lpParam)
{
...
bool CPlatformNetworkManagerDurango::RemoveLocalPlayer(
...
void CPlatformNetworkManagerDurango::SystemFlagAddPlayer(
    INetworkPlayer *pNetworkPlayer)
{
...
void CPlatformNetworkManagerDurango::SystemFlagRemovePlayer(
    INetworkPlayer *pNetworkPlayer)
{
    for (unsigned int i = 0; i < m_playerFlags.size(); i++)
    {
...
// shares that system
void CPlatformNetworkManagerDurango::SystemFlagSet(
...
// here
bool CPlatformNetworkManagerDurango::SystemFlagGet(
    INetworkPlayer *pNetworkPlayer, int index)
{
    if ((index < 0) || (index >= m_flagIndexSize))
    {
...
void CPlatformNetworkManagerDurango::TickSearch()
{
    if (m_bSearchPending)
    {
        if (!m_pDQRNet->FriendPartyManagerIsBusy())
        {
            ...
            m_pSearchResults =
                new DQRNetworkManager::SessionSearchResult[m_searchResultsCount];
        ...
        if (!m_pDQRNet->FriendPartyManagerIsBusy())
        {
            ...
            if (m_SessionsUpdatedCallback != NULL &&
                (m_lastSearchStartTime +
                 MINECRAFT_DURANGO_PARTY_SEARCH_DELAY_MILLISECONDS) <
                    GetTickCount())
            {
...
CPlatformNetworkManagerDurango::GetSessionList(int iPad, int localPlayers,
                                               bool partyOnly)
{
    ...
    for (int i = 0; i < m_searchResultsCount; i++)
    {
        GameSessionData *gameSessionData =
            (GameSessionData *)m_pSearchResults[i].m_extData;
            ...
            (gameSessionData->isReadyToJoin))
        {
            ...
            session->searchResult.m_extData =
                NULL; // We have another copy of the GameSessionData, so don't need to
                      // make another copy of this here
            session->displayLabelLength =
                session->searchResult.m_playerNames[0].size();
            session->displayLabel = new wchar_t[session->displayLabelLength + 1];
            memcpy(&session->data, gameSessionData, sizeof(GameSessionData));
            wcscpy_s(session->displayLabel, session->displayLabelLength + 1,
                     session->searchResult.m_playerNames[0].c_str());
...
bool CPlatformNetworkManagerDurango::GetGameSessionInfo(
    int iPad, SessionID sessionId, FriendSessionInfo *foundSessionInfo)
{
    return false;
}

void CPlatformNetworkManagerDurango::SetSessionsUpdatedCallback(
...
void CPlatformNetworkManagerDurango::GetFullFriendSessionInfo(
...
INetworkPlayer *
CPlatformNetworkManagerDurango::addNetworkPlayer(DQRNetworkPlayer *pDQRPlayer)
{
...
void CPlatformNetworkManagerDurango::removeNetworkPlayer(
    DQRNetworkPlayer *pDQRPlayer)
{
...
INetworkPlayer *
CPlatformNetworkManagerDurango::getNetworkPlayer(DQRNetworkPlayer *pDQRPlayer)
{
    return pDQRPlayer ? (INetworkPlayer *)(pDQRPlayer->GetCustomDataValue())
                      : NULL;
}

INetworkPlayer *
CPlatformNetworkManagerDurango::GetLocalPlayerByUserIndex(int userIndex)
{
    return getNetworkPlayer(m_pDQRNet->GetLocalPlayerByUserIndex(userIndex));
}

INetworkPlayer *
CPlatformNetworkManagerDurango::GetPlayerByIndex(int playerIndex)
{
    return getNetworkPlayer(m_pDQRNet->GetPlayerByIndex(playerIndex));
}

INetworkPlayer *
CPlatformNetworkManagerDurango::GetPlayerByXuid(PlayerUID xuid)
{
    return getNetworkPlayer(m_pDQRNet->GetPlayerByXuid(xuid));
}

INetworkPlayer *
CPlatformNetworkManagerDurango::GetPlayerBySmallId(unsigned char smallId)
{
    return getNetworkPlayer(m_pDQRNet->GetPlayerBySmallId(smallId));
}

wstring
CPlatformNetworkManagerDurango::GetDisplayNameByGamertag(wstring gamertag)
{
...
bool CPlatformNetworkManagerDurango::JoinGameFromInviteInfo(
