...
// This is the lowest level class for handling the concept of a player on
// Durango. This is managed by DQRNetworkManager. The game shouldn't directly
// communicate with this class, as it is wrapped by NetworkPlayerDurango which
// is an implementation of a platform-independent interface INetworkPlayer.
...
class DQRNetworkPlayer
{
  public:
    ...
    typedef enum
    {
        ...
        DNP_TYPE_LOCAL,  // On host - this player is a local player that needs
                         // communicated with specially not using rudp. On clients -
                         // this is a local player, where network communications can
                         // be used to communicate with the host
        DNP_TYPE_REMOTE, // On host - this player can be used to communicate from
                         // between the host and this player. On clients - this is a
                         // remote player that cannot be communicated with
    } eDQRNetworkPlayerType;

    DQRNetworkPlayer();
    DQRNetworkPlayer(DQRNetworkManager *manager, eDQRNetworkPlayerType playerType,
                     bool onHost, int localPlayerIdx,
    ...
    int GetSessionIndex();
    void SendData(DQRNetworkPlayer *pPlayerTarget, const void *data,
