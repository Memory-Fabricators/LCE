...
VOID XMLParser::FillBuffer()
{
    ...
    __int64 iProgress =
        m_dwCharsTotal
            ? (((__int64)m_dwCharsConsumed * 1000) / (__int64)m_dwCharsTotal)
            : 0;
...
HRESULT XMLParser::ConvertEscape()
{
    ...
    {
        Error(E_INVALID_XML_SYNTAX, "Unrecognized entity name after & - (should be "
                                    "lt, gt, amp, apos, or quot)");
...
HRESULT XMLParser::AdvanceElement()
{
    ...
    else if (m_Ch == '/')
    {
        ...
        if (FAILED(m_pISAXCallback->ElementEnd(
                pEntityRefVal, (UINT)(m_pWritePtr - pEntityRefVal))))
        {
            return E_ABORT;
        }
    ...
    {
        // just skip any xml header tag since not really important after identifying
        // character set
        for (;;)
        {
            if (FAILED(hr = AdvanceCharacter()))
    ...
    else
    {
        ...
        while ((m_Ch != '>') && (m_Ch != '/'))
        {
            ...
            Attributes[NumAttrs].NameLen =
                (UINT)(m_pWritePtr - Attributes[NumAttrs].strName);
            ...
            Attributes[NumAttrs].ValueLen =
                (UINT)(m_pWritePtr - Attributes[NumAttrs].strValue);
...
HRESULT XMLParser::AdvanceCDATA()
{
        ...
        *m_pWritePtr = m_Ch;
        ...
        if (m_pWritePtr - m_pWriteBuf >= XML_WRITE_BUFFER_SIZE)
        {
            if (FAILED(m_pISAXCallback->CDATAData(
                    m_pWriteBuf, (UINT)(m_pWritePtr - m_pWriteBuf), TRUE)))
            {
                return E_ABORT;
            }
            m_pWritePtr = m_pWriteBuf;
    ...
    if (FAILED(m_pISAXCallback->CDATAData(
            m_pWriteBuf, (UINT)(m_pWritePtr - m_pWriteBuf), FALSE)))
    {
        return E_ABORT;
    }
...
HRESULT XMLParser::MainParseLoop()
{
    ...
    {
        Error(E_INVALID_XML_SYNTAX, "Unrecognized encoding (parser does not "
                                    "support UTF-8 language encodings)");
        return E_INVALID_XML_SYNTAX;
    }
...
HRESULT XMLParser::ParseXMLFile(CONST CHAR *strFilename)
{
    ...
    m_hFile = CreateFile(wchFilename, GENERIC_READ, FILE_SHARE_READ, NULL,
...
VOID XMLParser::Error(HRESULT hErr,
