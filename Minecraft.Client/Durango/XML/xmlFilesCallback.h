...
public:
        ...
        virtual HRESULT  ElementBegin( CONST WCHAR* strName, UINT NameLen, CONST
XMLAttribute *pAttributes, UINT NumAttributes )
        {
                WCHAR wTemp[35] = L"";
                WCHAR wAttName[32] = L"";
...
pAttributes[i].NameLen); if (_wcsicmp(wAttName,L"name")==0)
                                {
...
pAttributes[i].strValue, pAttributes[i].ValueLen);
                        ...
                        // if the xuid hasn't been defined, then we can't use
the data if(xuid!=0LL)
                        {
                                return
CConsoleMinecraftApp::RegisterMojangData(wNameXUID , xuid, wNameSkin, wNameCloak
);
                        }
                        else return S_FALSE;
                }
...
return S_OK;    };
        ...
        virtual HRESULT  CDATAData( CONST WCHAR *strCDATA, UINT CDATALen, BOOL
bMore ){ return S_OK; };
                                 ...
                                 UINT NumAttributes)
    {
        ...
        else if (_wcsicmp(wAttName, L"data") == 0)
        {
            ...
            if (wUID[0] != 0)
            {
                ...
                return CConsoleMinecraftApp::RegisterDLCData(eDLCType, wNameBanner,
                                                             wUID, wName, wFirstSkin,
                                                             iConfig, uiSortIndex);
            }
            else
            {
