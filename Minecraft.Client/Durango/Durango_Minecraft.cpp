...
void CreateDevice()
{
    ...
    // than the API default. It is recommended usage, and is required for
    // compatibility with Direct2D.
...
void CreateResources()
{
    ...
    DX::ThrowIfFailed(
        g_d3dDevice->CreateTexture2D(&depthStencilDesc, nullptr, &depthStencil));
...
void oldWinMainInit()
{
    ...
    StorageManager.Init(0, app.GetString(IDS_DEFAULT_SAVENAME), "savegame.dat",
                        ...
                        (LPVOID)&app, app.UpdateProductId, SERVICE_CONFIG_ID,
                        TITLE_PRODUCT_ID);
    ...
    // 4J-PB - after the init, which will have read all the local DLC, update the
    // product ids by reading the local dlc file
...
void oldWinMainTick()
{
    ...
    else
    {
        ...
        {
            pMinecraft
                ->tickAllConnections(); // Added to stop timing out when we are
                                        // waiting after converting to an offline game
        }
    }
...
#endif
                // clear the stats first - there could have beena signout and sign back
                // in in the menus need to clear the player stats - can't assume it'll
    ...
    // 4J Stu - Added this back as it was causing horrendous lag (and eventually a
    // crash on a client) when playing with multiple clients
    g_NetworkManager
        .DoWork(); // TODO - consider putting this back in. Removed because we are
                   // having trouble with how long the tick is taking on Durango
    PIXEndNamedEvent();
    ...
    if (!ProfileManager.IsFullVersion())
    {
        ...
        if (app.GetGameStarted())
        {
            // 4J-PB - if the game is paused, add the elapsed time to the trial timer
            // count so it doesn't tick down
            if (app.IsAppPaused())
            {
    ...
    // A memory leak was caused because the icon renderer kept creating new Vec3's
    // because the pool wasn't reset
    Vec3::resetPool();
}
