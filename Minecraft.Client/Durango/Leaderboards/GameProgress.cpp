...
void GameProgress::updatePlayer(int iPad)
{
    ...
    CC::create_task(
        xlc->AchievementService->GetAchievementsForTitleIdAsync(
            ...
            0x149E11AE,                                       // TitleId
            MXSA::AchievementType::Persistent,                // Use regular achievements (not
                                                              // challenges)
            false,                                            // Unlocked only
        ...
        .then([this, iPad, uid,
               playerSession](CC::task<MXSA::AchievementsResult ^> resultTask) {
            try
            {
                ...
                if (result)
                {
                    for (unsigned int i = 0, iMax = result->Items->Size; i < iMax;
                         i++)
                    {
                        MXSA::Achievement ^ ach = result->Items->GetAt(i);
                        if (ach->ProgressState ==
                            MXSA::AchievementProgressState::Achieved)
                        {
                    ...
                    float gameprogress;
                    if (EventWriteGameProgress(uid.toString().c_str(), playerSession,
                                               gameprogress = calcGameProgress(
            ...
            {
                app.DebugPrintf("GameProgress:: Error, couldn't contact the "
                                "achievments service (?): %ls",
                                ex->Message->Data());
            }
        });
}
