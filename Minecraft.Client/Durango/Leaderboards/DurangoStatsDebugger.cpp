...
DurangoStatsDebugger *DurangoStatsDebugger::Initialize()
{
    ...
    StatParam *sp =
        new StatParam(L"McItemAcquired.AcquisitionMethodId.*.ItemId.*.ItemAux.*");
    sp->addArgs(1, Tile::dirt_Id, 0); // works
    sp->addArgs(2, Tile::dirt_Id, 0); // works

    sp->addArgs(DsItemEvent::eAcquisitionMethod_Pickedup, Tile::cloth_Id,
                0); // fixed (+ach 'Rainbow Collection')
    ...
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Crafted,
                Item::porkChop_cooked_Id); // BROKEN! (ach 'Pork Chop' configured
                                           // incorrectly)
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Crafted,
                Item::cake_Id); // works. (ach 'The Lie' configured incorrectly)
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Bought,
                Item::emerald_Id); // fixed (+ach)
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Crafted,
                Item::ironIngot_Id); // works. (+ach 'Acquired Hardware')
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Pickedup,
                Item::fish_raw_Id); // works. (+ach 'Delicious Fish')
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Crafted,
                Item::fish_cooked_Id); // works. (+ach 'Delicious Fish')
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Crafted, Item::sign_Id);
    sp->addArgs(DsItemEvent::eAcquisitionMethod_Crafted,
                Item::flowerPot_Id); // FIXING!
    out->m_stats.push_back(sp);

    sp = new StatParam(
        L"McItemAcquired.DifficultyLevelId.*.AcquisitionMethodId.*.ItemId.*");
    ...
    sp = new StatParam(
        L"MobKilled.KillTypeId.*.EnemyRoleId.*.PlayerWeaponId.*"); // BROKEN!
    ...
    sp = new StatParam(L"MaxKillDistance.KillTypeId.*.EnemyRoleId.*."
                       L"PlayerWeaponId.*"); // BROKEN!
    sp->addArgs(/*MELEE*/ 0, ioid_Cow, Item::sword_stone_Id);
    sp->addArgs(/*MELEE*/ 0, ioid_Pig, Item::sword_stone_Id);
    ...
    sp = new StatParam(
        L"MobKilledTotal.DifficultyLevelId.*.EnemyRoleId.*"); // BROKEN!
    ...
    sp->addArgs(
        eAward_stayinFrosty);    // works (achievement configured incorrectly)
...
void DurangoStatsDebugger::PrintStats(int iPad)
{
    ...
    instance->m_printQueue.insert(instance->m_printQueue.end(), tmp->begin(),
                                  tmp->end());
    ...
    app.DebugPrintf(
        "[DurangoStatsDebugger] (%i) Results returned, starting printing.\n",
    ...
    instance->m_retrievedStats.erase(instance->m_retrievedStats.begin(),
...
void DurangoStatsDebugger::retrieveStats(int iPad)
{
    ...
    try
    {
        WFC::IVectorView<Windows::Xbox::System::User ^> ^ userList =
        ...
        if (userList != nullptr)
        {
            ...
            {
                app.DebugPrintf("[DurangoStatsDebugger] Problem occured while creating "
                                "'XboxLiveContext'.\n");
                return;
            }
        }
    ...
    {
        app.DebugPrintf(
            "[DurangoStatsDebugger] 'XboxLiveContext' created successfully.\n");
    }
    else
    {
    ...
    app.DebugPrintf("[DurangoStatsDebugger] Retrieving (%i) stats for '%s'.\n",
                    m_printQueue.size(), plrname.c_str());

    static const unsigned short R_SIZE = 20;

    // Create Stat retrieval threads until there is no long any stats to start
    // retrieving.
    while (!instance->m_printQueue.empty())
    {
        ...
        {
            printing->insert(printing->end(), m_printQueue.begin(),
                             m_printQueue.begin() + R_SIZE);
        ...
        app.DebugPrintf(
            "[DurangoStatsDebugger] Starting retrieval operation (%i/%i stats).\n",
        ...
        auto asyncOp =
            xboxLiveContext->UserStatisticsService->GetMultipleUserStatisticsAsync(
                xboxUserIds->GetView(), // the collection of Xbox user IDs whose
                                        // stats we want to retrieve
                SERVICE_CONFIG_ID,      // the service config that contains the stats we
                                        // want
                statNames->GetView()    // a list of stat names we want
            );

        CC::create_task(asyncOp).then(
            [this, printing, iPad, r_runningThreads](
                CC::task<
                ...
                try
                {
                         ...
                         resultList)
                    {
                             ...
                             index++)
                        {
                            ...
                            app.DebugPrintf("[DurangoStatsDebugger] Retrieve complete, %i "
                                            "results returned.\n",
                                            configStat->Statistics->Size);

                            for (auto result : configStat->Statistics)
                            {
                                StatResult sr = {iPad, result->StatisticName->Data(),
                                                 result->Value->Data()};
                ...
                {
                    app.DebugPrintf("[DurangoStatsDebugger] resultListTask.get() "
