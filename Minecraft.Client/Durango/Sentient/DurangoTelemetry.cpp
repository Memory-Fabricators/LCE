...
bool CDurangoTelemetryManager::RecordPlayerSessionExit(int iPad,
                                                       int exitStatus)
{
    PlayerUID puid;
    ...
    if (plr != NULL && plr->level != NULL && plr->level->getLevelData() != NULL)
    {
        ULONG hr = EventWritePlayerSessionEnd(
            ...
            plr->level->difficulty, exitStatus);
        ...
        if (!g_NetworkManager.IsLocalGame())
        {
            float roundLength =
                app.getAppTime() -
                m_multiplayerRoundStartTimes[DurangoStats::getUserId(iPad)];

            hr = EventWriteMultiplayerRoundEnd(
                DurangoStats::getUserId(iPad), &ZERO_GUID, 0,
                DurangoStats::getPlayerSession(),
                DurangoStats::getMultiplayerCorrelationId(),
                plr->level->getLevelData()->getGameType()->isSurvival(), // GameMode
                0, plr->level->difficulty,
                roundLength, // Time (seconds)
                exitStatus);

            if (hr == 0) // Debug.
            {
                app.DebugPrintf("<%ls> MultiplayerRoundEnd(%ls,%ls,%i,%i,%.1f,%i).\n",
                                DurangoStats::getUserId(iPad),
                                guid2str(DurangoStats::getPlayerSession()).c_str(),
                                DurangoStats::getMultiplayerCorrelationId(),
                                plr->level->getLevelData()->getGameType()->isSurvival(),
                                plr->level->difficulty, roundLength, exitStatus);
            }
        }
    }
...
bool CDurangoTelemetryManager::RecordLevelStart(
...
{
    CTelemetryManager::RecordLevelStart(iPad, friendsOrMatch, competeOrCoop,
    ...
    if (plr != NULL && plr->level != NULL && plr->level->getLevelData() != NULL)
    {
        ...
        if (!g_NetworkManager.IsLocalGame())
        {
            ...
            if (hr == 0) // Debug.
            {
                app.DebugPrintf("<%ls> MultiplayerRoundStart(%ls,%ls,%i,%i).\n",
                                DurangoStats::getUserId(iPad),
                                guid2str(DurangoStats::getPlayerSession()).c_str(),
                                DurangoStats::getMultiplayerCorrelationId(),
                                plr->level->getLevelData()->getGameType()->isSurvival(),
    ...
    hr = EventWriteLevelStart(
        DurangoStats::getUserId(iPad), DurangoStats::getPlayerSession(),
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        friendsOrMatch, competeOrCoop, difficulty, numberOfLocalPlayers,
        numberOfOnlinePlayers, &ZERO_GUID);
...
bool CDurangoTelemetryManager::RecordLevelExit(
    int iPad, ESen_LevelExitStatus levelExitStatus)
{
...
bool CDurangoTelemetryManager::RecordLevelSaveOrCheckpoint(
    int iPad, int saveOrCheckPointID, int saveSizeInBytes)
{
    CTelemetryManager::RecordLevelSaveOrCheckpoint(iPad, saveOrCheckPointID,
                                                   saveSizeInBytes);
    ...
    ULONG hr = EventWriteLevelSaveOrCheckpoint(
        ...
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        &ZERO_GUID, saveOrCheckPointID, saveSizeInBytes);
...
bool CDurangoTelemetryManager::RecordLevelResume(
    int iPad, ESen_FriendOrMatch friendsOrMatch,
    ESen_CompeteOrCoop competeOrCoop, int difficulty, int numberOfLocalPlayers,
    int numberOfOnlinePlayers, int saveOrCheckPointID)
{
    CTelemetryManager::RecordLevelResume(
        iPad, friendsOrMatch, competeOrCoop, difficulty, numberOfLocalPlayers,
        numberOfOnlinePlayers, saveOrCheckPointID);

    ULONG hr = EventWriteLevelResume(
        DurangoStats::getUserId(iPad), DurangoStats::getPlayerSession(),
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        &ZERO_GUID, friendsOrMatch, competeOrCoop, difficulty,
        numberOfLocalPlayers, numberOfOnlinePlayers, saveOrCheckPointID);
...
bool CDurangoTelemetryManager::RecordMenuShown(int iPad, EUIScene menuID,
                                               int optionalMenuSubID)
{
    ...
    ULONG hr = EventWriteMenuShown(
        ...
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        &ZERO_GUID, menuID, optionalMenuSubID);
...
bool CDurangoTelemetryManager::RecordAchievementUnlocked(
    int iPad, int achievementID, int achievementGamerscore)
{
    CTelemetryManager::RecordAchievementUnlocked(iPad, achievementID,
    ...
    ULONG hr = EventWriteAchievemntUnlocked(
        ...
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        &ZERO_GUID, achievementID, achievementGamerscore);
...
bool CDurangoTelemetryManager::RecordMediaShareUpload(
...
bool CDurangoTelemetryManager::RecordUpsellPresented(int iPad,
...
bool CDurangoTelemetryManager::RecordUpsellResponded(
    ...
    ESen_UpsellOutcome upsellOutcome)
{
    ULONG hr = EventWriteUpsellResponded(
        ...
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        &ZERO_GUID, upsellId, marketplaceOfferID, upsellOutcome);
...
bool CDurangoTelemetryManager::RecordPlayerDiedOrFailed(
    ...
    int playerWeaponID, int enemyWeaponID, ETelemetryChallenges enemyTypeID)
{
    ULONG hr = EventWritePlayerDiedOrFailed(
        ...
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        &ZERO_GUID, lowResMapX, lowResMapY, lowResMapZ, mapID, playerWeaponID,
        enemyWeaponID, enemyTypeID);
...
bool CDurangoTelemetryManager::RecordEnemyKilledOrOvercome(
    int iPad, int lowResMapX, int lowResMapY, int lowResMapZ, int mapID,
    int playerWeaponID, int enemyWeaponID, ETelemetryChallenges enemyTypeID)
{
    ULONG hr = -1;
...
bool CDurangoTelemetryManager::RecordTexturePackLoaded(int iPad,
                                                       int texturePackId,
                                                       bool purchased)
{
    ULONG hr = EventWriteTexturePackLoaded(
        ...
        GetSecondsSinceInitialize(), GetMode(iPad), GetSubMode(iPad),
        GetLevelId(iPad), GetSubLevelId(iPad), GetLevelInstanceID(), &ZERO_GUID,
        &ZERO_GUID, texturePackId, purchased);
