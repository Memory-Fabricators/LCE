...
void UIControl_SpaceIndicatorBar::init(const wstring &label, int id, std::int64_t min, std::int64_t max)
{
    m_label = label;
    m_id = id;
    m_min = min;
...
void UIControl_SpaceIndicatorBar::addSave(std::int64_t size)
{
    ...
    m_sizeAndOffsets.push_back(pair<std::int64_t, float>(size, startPercent));
...
void UIControl_SpaceIndicatorBar::selectSave(int index)
{
    ...
    {
        pair<std::int64_t, float> values = m_sizeAndOffsets[index];
