...
void UIController::CloseUIScenes(int iPad, bool forceIPad)
{
    ...
    {
        m_groups[(int)group]->getTutorialPopup()->SetTutorialDescription(&popupInfo);
    }
...
void UIController::SetTooltips(unsigned int iPad, int iA, int iB, int iX, int iY, int iLT, int iRT, int iLB, int iRB, int iLS, int iRS, int iBack, bool forceUpdate)
{
    ...
    if (m_groups[(int)group]->getTooltips())
    {
        m_groups[(int)group]->getTooltips()->SetTooltips(iA, iB, iX, iY, iLT, iRT, iLB, iRB, iLS, iRS, iBack, forceUpdate);
    }
}
...
void UIController::AnimateKeyPress(int iPad, int iAction, bool bRepeat, bool bPressed, bool bReleased)
{
    ...
    if (m_groups[(int)group]->getTooltips())
    {
        m_groups[(int)group]->getTooltips()->handleInput(iPad, iAction, bRepeat, bPressed, bReleased, handled);
    }
}
...
void UIController::RemoveInteractSceneReference(int iPad, UIScene *scene)
{
    ...
    if (m_groups[(int)group]->getTutorialPopup())
    {
        m_groups[(int)group]->getTutorialPopup()->RemoveInteractSceneReference(scene);
    }
}
#endif
...
void UIController::ShowTrialTimer(bool show)
{
    ...
    {
        m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay()->showTrialTimer(show);
    }
}
...
void UIController::UpdateTrialTimer(unsigned int iPad)
{
...
#endif
    {
        ...
        if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
        {
            m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay()->setTrialTimer(wcTime);
        }
    }
    else
    {
        if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
        {
            m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay()->setTrialTimer(L"");
        }
    }
...
void UIController::UpdateAutosaveCountdownTimer(unsigned int uiSeconds)
{
    ...
    if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
    {
        m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay()->setTrialTimer(wcAutosaveCountdown);
    }
#endif
}
...
void UIController::ShowSavingMessage(unsigned int iPad, C4JStorage::ESavingMessage eVal)
{
    ...
    if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
    {
        m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay()->showSaveIcon(show);
    }
}
...
void UIController::ShowPressStart(unsigned int iPad)
{
    ...
    if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
    {
        m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay()->showPressStart(iPad, true);
    }
}
...
void UIController::HidePressStart()
{
    ...
    if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
    {
        m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay()->showPressStart(0, false);
    }
}
