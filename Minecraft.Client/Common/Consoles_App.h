...
#endif
{
  ...
  public:
    ...
    std::vector<wstring> m_vCreditText; // hold the credit text lines so we can
                                        // avoid duplicating them

    // In builds prior to TU5, the size of the GAME_SETTINGS struct was 204 bytes.
    // We added a few new values to the internal struct in TU5, and even though we
    // changed the size of the ucUnused array to be decreased by the size of the
    // values we added, the packing of the struct has introduced some extra
    // padding that resulted in the GAME_SETTINGS struct being 208 bytes. The
    // knock-on effect from this was that all the stats, which come after the game
    // settings in the profile data, we being read offset by 4 bytes. We need to
    // ensure that the GAME_SETTINGS struct does not grow larger than 204 bytes or
    // if we need it to then we need to rebuild the profile data completely and
    // increase the profile version. There should be enough free space to grow
    // larger for a few more updates as long as we take into account the padding
    // issues and check that settings are still stored at the same positions when
    // we read them
    static const int GAME_SETTINGS_PROFILE_DATA_BYTES = 204;

#ifdef _EXTENDED_ACHIEVEMENTS
    /* 4J-JEV:
     * We need more space in the profile data because of the new achievements and
     * statistics necessary for the new expanded achievement set.
     */
    ...
    bool IntroRunning()
    {
        return m_bIntroRunning;
    }
    void SetIntroRunning(bool bSet)
    {
        m_bIntroRunning = bSet;
    }
#ifdef _CONTENT_PACKAGE
#ifndef _FINAL_BUILD
    bool PartnernetPasswordRunning()
    {
        return m_bPartnernetPasswordRunning;
    }
    ...
    bool LoadInventoryMenu(int iPad, shared_ptr<LocalPlayer> player,
                           bool bNavigateBack = false);
    bool LoadCreativeMenu(int iPad, shared_ptr<LocalPlayer> player,
                          bool bNavigateBack = false);
    bool LoadEnchantingMenu(int iPad, shared_ptr<Inventory> inventory, int x,
                            int y, int z, Level *level);
    bool LoadFurnaceMenu(int iPad, shared_ptr<Inventory> inventory,
                         shared_ptr<FurnaceTileEntity> furnace);
    bool LoadBrewingStandMenu(int iPad, shared_ptr<Inventory> inventory,
                              shared_ptr<BrewingStandTileEntity> brewingStand);
    bool LoadContainerMenu(int iPad, shared_ptr<Container> inventory,
                           shared_ptr<Container> container);
    bool LoadTrapMenu(int iPad, shared_ptr<Container> inventory,
                      shared_ptr<DispenserTileEntity> trap);
    bool LoadCrafting2x2Menu(int iPad, shared_ptr<LocalPlayer> player);
    bool LoadCrafting3x3Menu(int iPad, shared_ptr<LocalPlayer> player, int x,
                             int y, int z);
    bool LoadSignEntryMenu(int iPad, shared_ptr<SignTileEntity> sign);
    bool LoadRepairingMenu(int iPad, shared_ptr<Inventory> inventory,
                           Level *level, int x, int y, int z);
    bool LoadTradingMenu(int iPad, shared_ptr<Inventory> inventory,
                         shared_ptr<Merchant> trader, Level *level);

    bool GetTutorialMode()
    {
        return m_bTutorialMode;
    }
    void SetTutorialMode(bool bSet)
    {
        m_bTutorialMode = bSet;
    }
    ...
    void SetGameMode(eGameMode eMode)
    {
        m_eGameMode = eMode;
    }
    ...
    {
        return m_eGlobalXuiAction;
    }
    void SetGlobalXuiAction(eXuiAction action)
    {
        m_eGlobalXuiAction = action;
    }
    eXuiAction GetXuiAction(int iPad)
    {
        return m_eXuiAction[iPad];
    }
    ...
    {
        return m_bChangingSessionType;
    }
    void SetChangingSessionType(bool bVal)
    {
        m_bChangingSessionType = bVal;
    }

    bool GetReallyChangingSessionType()
    {
        return m_bReallyChangingSessionType;
    }
    ...
    {
        return m_bMobsDontAttack;
    }
    void SetMobsDontAttackEnabled(bool bVal)
    {
        m_bMobsDontAttack = bVal;
    }
    bool GetUseDPadForDebug()
    {
        return m_bUseDPadForDebug;
    }
    void SetUseDPadForDebug(bool bVal)
    {
        m_bUseDPadForDebug = bVal;
    }
    bool GetMobsDontTickEnabled()
    {
        return m_bMobsDontTick;
    }
    void SetMobsDontTickEnabled(bool bVal)
    {
        m_bMobsDontTick = bVal;
    }

    bool GetFreezePlayers()
    {
        return m_bFreezePlayers;
    }
    void SetFreezePlayers(bool bVal)
    {
        m_bFreezePlayers = bVal;
    }
    ...
    static int DefaultOptionsCallback(LPVOID pParam,
                                      ...
                                      const int iPad);
    int SetDefaultOptions(C4JStorage::PROFILESETTINGS *pSettings, const int iPad,
    ...
    {
        if (m_uiOpacityCountDown[iPad] > 0)
        {
            m_uiOpacityCountDown[iPad]--;
        }
    }
  ...
  public:
    ...
    {
        return m_bLiveLinkRequired;
    }
    void SetLiveLinkRequired(bool required)
    {
        m_bLiveLinkRequired = required;
    }
    ...
    static int NowDisplayFullVersionPurchase(void *pParam, bool bContinue,
                                             int iPad);
    static int
    MustSignInFullVersionPurchaseReturned(void *pParam, int iPad,
    ...
    // C4JStorage::eGlobalStorage eStorageFacility, wstring *wsFile  );
    ...
    {
        return m_bDLCInstallPending;
    }
    bool DLCInstallProcessCompleted()
    {
        return m_bDLCInstallProcessCompleted;
    }
    void ClearDLCInstalled()
    {
        m_bDLCInstallProcessCompleted = false;
    }
    static int MarketplaceCountsCallback(LPVOID pParam,
                                         C4JStorage::DLC_TMS_DETAILS *, int iPad);
    ...
    bool GetNewDLCAvailable()
    {
        return m_bNewDLCAvailable;
    }
    void DisplayNewDLCTipAgain()
    {
    ...
    // void InstallDefaultCape(); // attempt  to install the default cape once per
    // game launch
  ...
  public:
    bool GetResourcesLoaded()
    {
        return m_bResourcesLoaded;
    }
    void SetResourcesLoaded(bool bVal)
    {
        m_bResourcesLoaded = bVal;
    }
  ...
  public:
    ...
    bool m_bDisplayFullVersionPurchase; // for after signing in during the trial,
                                        // and trying to unlock full version on an
                                        // upsell
  ...
  public:
    ...
    VNOTIFICATIONS *GetNotifications()
    {
        return &m_vNotifications;
    }

  private:
    // To avoid problems with threads being kicked off from xuis that alter things
    // that may be in progress within the run_middle, we'll action these at the
    ...
    static int ExitAndJoinFromInvite(void *pParam, int iPad,
                                     C4JStorage::EMessageResult result);
    static int
    ExitAndJoinFromInviteSaveDialogReturned(void *pParam, int iPad,
                                            C4JStorage::EMessageResult result);
    static int
    ExitAndJoinFromInviteAndSaveReturned(void *pParam, int iPad,
                                         C4JStorage::EMessageResult result);
    static int
    ExitAndJoinFromInviteDeclineSaveReturned(void *pParam, int iPad,
                                             C4JStorage::EMessageResult result);
    static int FatalErrorDialogReturned(void *pParam, int iPad,
                                        C4JStorage::EMessageResult result);
    static int WarningTrialTexturePackReturned(void *pParam, int iPad,
                                               C4JStorage::EMessageResult result);
  ...
  public:
    ...
    int GetHTMLColor(eMinecraftColour colour)
    {
        return GetHTMLColour(colour);
    }
    int GetHTMLFontSize(EHTMLFontSize size);
    wstring FormatHTMLString(int iPad, const wstring &desc,
  ...
  public:
    ...
    // bool GetDLCFullOfferIDForSkinID(const wstring &FirstSkin,WCHAR
    // *pwchProductId);
    bool GetDLCFullOfferIDForSkinID(const wstring &FirstSkin,
                                    wstring &wsProductId);
  ...
  private:
    ...
    static unordered_map<int, char *>
        DLCTextures_PackID; // for mash-up packs & texture packs
    static unordered_map<string, DLC_INFO *> DLCInfo;
    static unordered_map<wstring, ULONGLONG>
        DLCInfo_SkinName; // skin name, full offer id
#elif defined(_DURANGO)
    static unordered_map<PlayerUID, MOJANG_DATA *, PlayerUID::Hash> MojangData;
    static unordered_map<int, wstring>
        DLCTextures_PackID; // for mash-up packs & texture packs
    // static unordered_map<wstring,DLC_INFO * > DLCInfo_Trial; // full offerid,
    // dlc_info
    static unordered_map<wstring, DLC_INFO *>
        DLCInfo_Full; // full offerid, dlc_info
    static unordered_map<wstring, wstring>
        DLCInfo_SkinName; // skin name, full offer id
#else
    static unordered_map<PlayerUID, MOJANG_DATA *> MojangData;
    static unordered_map<int, ULONGLONG>
        DLCTextures_PackID; // for mash-up packs & texture packs
    static unordered_map<ULONGLONG, DLC_INFO *>
  ...
  public:
    ...
    // this is to check if their guest number has changed, so store the last known
    // one here 4J Stu - Now storing the whole XUSER_SIGNIN_INFO so we can detect
    // xuid changes
  ...
  public:
    ...
    {
        m_BanListCheck[iPad] = bVal;
    }
    bool GetBanListCheck(int iPad)
    {
        return m_BanListCheck[iPad];
    }
    // AUTOSAVE
  public:
  ...
  public:
    ...
    void SetResetNether(bool bResetNether)
    {
        m_bResetNether = bResetNether;
    }
    bool GetResetNether()
    {
        return m_bResetNether;
    }
  ...
  private:
    BYTE
        m_playerColours[MINECRAFT_NET_MAX_PLAYERS]; // An array of QNet small-id's
  ...
  public:
    ...
    void SetCorruptSaveDeleted(bool bVal)
    {
        m_bCorruptSaveDeleted = bVal;
    }
    bool GetCorruptSaveDeleted(void)
    {
        return m_bCorruptSaveDeleted;
    }
  ...
  public:
    ...
    void SetAdditionalSkinBoxes(DWORD dwSkinID, SKIN_BOX *SkinBoxA,
                                DWORD dwSkinBoxC);
    vector<ModelPart *> *SetAdditionalSkinBoxes(DWORD dwSkinID,
                                                vector<SKIN_BOX *> *pvSkinBoxA);
    ...
    virtual bool GetTMSGlobalFileListRead()
    {
        return true;
    }
    virtual bool GetTMSDLCInfoRead()
    {
        return true;
    }
    virtual bool GetTMSXUIDsFileRead()
    {
        return true;
    }
    ...
    {
        return m_bRead_BannedListA[iPad];
    }
    ...
    DWORD GetRequiredTexturePackID()
    {
