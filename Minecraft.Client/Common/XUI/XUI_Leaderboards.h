                            ...
                            public LeaderboardReadListener
{
  private:
    ...
    struct Leaderboard
    {
        DWORD m_totalEntryCount;   // Either total number of entries in leaderboard,
    ...
    unsigned int m_currentLeaderboard; // The current leaderboard selected for
                                       // view

#ifdef _XBOX
    LeaderboardManager::EFilterMode m_currentFilter; // The current filter
                                                     // selected
#endif
    ...
    PlayerUID *m_filteredFriends;      // List of all friend XUIDs (and player's), only
                                       // counting actual friends, not pending
    ...
    CXuiControl m_textLeaderboard; // The XUI text box showing the current
                                   // leaderboard name
    CXuiControl m_textInfo;        // The XUI text box showing info messages (loading, no
                                   // results, etc.)
    ...
    HRESULT OnGetSourceDataImage(XUIMessageGetSourceImage *pGetImage,
                                 BOOL &bHandled);
    HRESULT OnNotifySetFocus(HXUIOBJ hObjSource, XUINotifyFocus *pNotifyFocusData,
                             BOOL &bHandled);
