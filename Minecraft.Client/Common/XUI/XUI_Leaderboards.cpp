...
HRESULT CScene_Leaderboards::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    swprintf(entriesBuffer, 40, L"%ls%i", app.GetString(IDS_LEADERBOARD_ENTRIES),
             0);
...
void CScene_Leaderboards::UpdateTooltips()
{
    ...
    if (m_leaderboard.m_currentEntryCount > 0)
    {
        ...
        // if the selected user is me, don't show Send Friend Request, and show the
        // gamer profile, not the gamer card
                                       ...
                                       BOOL &bHandled)
{
    ...
    case VK_PAD_LSHOULDER:
        {
            // Do nothing if a stats read is currently in progress, otherwise the system
            // complains about to many read requests
                ...
                case LeaderboardManager::eFM_Friends:
                    {
                        ...
                        wchar_t filterBuffer[40];
                        swprintf_s(filterBuffer, 40, L"%ls%ls",
    ...
    case VK_PAD_Y:
        {
            ...
            if (m_leaderboard.m_currentEntryCount > 0)
            {
                                 ...
                                 m_leaderboard.m_currentEntryCount - 1))
                {
                    PlayerUID xuid =
                        m_leaderboard
                            .m_entries[selection - (m_leaderboard.m_entryStartIndex - 1)]
                            .m_xuid;
    ...
    case VK_PAD_A:
        {
            // Send friend request if the filter mode is not friend, and they're not a
            // friend or a pending friend
                ...
                if (m_leaderboard.m_currentEntryCount > 0)
                {
                    ...
                    if (selection >= m_leaderboard.m_entryStartIndex - 1 &&
                        selection < (m_leaderboard.m_entryStartIndex +
                                     m_leaderboard.m_currentEntryCount - 1))
                    {
                        // If not the player and neither currently a friend or requested to be
                        // a friend
                        if (!m_leaderboard
                                 .m_entries[selection - (m_leaderboard.m_entryStartIndex - 1)]
                                 .m_bPlayer &&
...
void CScene_Leaderboards::ReadStats(int startIndex)
{
    ...
    else
    {
        ...
        m_newReadSize = min((int)READ_SIZE, (int)m_leaderboard.m_totalEntryCount -
                                                (startIndex - 1));
    }
  ...
  */
    ...
    case LeaderboardManager::eFM_TopRank:
        LeaderboardManager::Instance()->ReadStats_TopRank(
            ...
            (LeaderboardManager::EStatsType)m_currentLeaderboard, m_newEntryIndex,
            m_newReadSize);
        break;
    case LeaderboardManager::eFM_MyScore:
...
bool CScene_Leaderboards::RetrieveStats()
{
                                  ...
                                  (1L << eDebugSetting_DebugLeaderboards)))
    {
             ...
             entryIndex < m_leaderboard.m_currentEntryCount; entryIndex++)
        {
            ...
            swprintf(m_leaderboard.m_entries[entryIndex].m_wcRank, 12,
            ...
            bool isDistanceLeaderboard =
                LEADERBOARD_DESCRIPTORS[m_currentLeaderboard][m_currentDifficulty]
                        .m_viewId == STATS_VIEW_TRAVELLING_PEACEFUL ||
                LEADERBOARD_DESCRIPTORS[m_currentLeaderboard][m_currentDifficulty]
                        .m_viewId == STATS_VIEW_TRAVELLING_EASY ||
                LEADERBOARD_DESCRIPTORS[m_currentLeaderboard][m_currentDifficulty]
                        .m_viewId == STATS_VIEW_TRAVELLING_NORMAL ||
                LEADERBOARD_DESCRIPTORS[m_currentLeaderboard][m_currentDifficulty]
                        .m_viewId == STATS_VIEW_TRAVELLING_HARD;
            ...
            for (unsigned int i = 0; i < m_leaderboard.m_numColumns; i++)
            {
                if (!isDistanceLeaderboard)
                {
                    ...
                    swprintf(m_leaderboard.m_entries[entryIndex].m_wcColumns[i], 12,
                             L"%u", m_leaderboard.m_entries[entryIndex].m_columns[i]);
                }
                else
                {
                    ...
                    swprintf(m_leaderboard.m_entries[entryIndex].m_wcColumns[i], 12,
                             L"%.1fkm",
                             ((float)m_leaderboard.m_entries[entryIndex].m_columns[i]) /
                                 100.f / 1000.f);
                }
            }
    ...
    if (m_leaderboard.m_totalEntryCount == 0)
    {
        ...
        // If the filter mode is "My Score" then centre the list around the entries
        // and select the player's score
        if (m_currentFilter == LeaderboardManager::eFM_MyScore)
        {
            ...
            for (unsigned int i = m_leaderboard.m_entryStartIndex;
                 i <
                 m_leaderboard.m_entryStartIndex + m_leaderboard.m_currentEntryCount;
                 ++i)
            {
    ...
    else
    {
        ...
        {
            // Calculate the overlap (this is by how much new entries would overhang
        ...
        // Otherwise we're inserting at the end of the array and there is plenty of
        // room
        else
        {
...
HRESULT
CScene_Leaderboards::OnGetSourceDataImage(XUIMessageGetSourceImage *pGetImage,
                                          BOOL &bHandled)
{
    if ((pGetImage->iData == 2) && (pGetImage->bItemData))
    {
        ...
        // m_leaderboard.m_entries[index].m_locale <= XC_LOCALE_RUSSIAN_FEDERATION )
        // 				pGetImage->szPath = FLAG_ICON_PATHS[
...
void CScene_Leaderboards::PopulateLeaderboard(bool noResults)
{
    ...
    if (!noResults && m_leaderboard.m_totalEntryCount > 0)
    {
        ...
        // 4J Stu - Fix for leaderboards taking forever to update. Was looping over
        // m_totalEntryCount which is ~2mil in some leaderboards in production atm.
        // Changed to loop over the range of cached values, although even that is
        // probably overkill. Really only the newly updated rows need changed, but
        // this shouldn't cause any performance issues
             ...
             ++i)
        {
            ...
            HXUIOBJ button;
            D3DXVECTOR3 vPos;
            // 4J-PB - fix for #13768 - Leaderboards: Player scores appear misaligned
            // when viewed under the "My Score" leaderboard filter
    ...
    bool isDistanceLeaderboard)
{
    ...
    // Is the player
    if (statsRow->xuid ==
        ((XboxLeaderboardManager *)LeaderboardManager::Instance())->GetMyXUID())
    {
             ...
             ++friendIndex)
        {
            if (m_friends[friendIndex].xuid == statsRow->xuid)
            {
                      ...
                      XONLINE_FRIENDSTATE_FLAG_RECEIVEDREQUEST)) == 0)
                {
                    ...
                    leaderboardEntry->m_bOnline = (m_friends[friendIndex].dwFriendState &
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        else
        {
            ...
            m_pHTitleIconSlots[i]->SetIcon(DEFAULT_XUI_MENU_USER,
                                           TitleIcons[m_currentLeaderboard][i], 0, 1,
                                           10, 31, false, FALSE);
        }
    }
