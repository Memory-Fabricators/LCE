...
HRESULT CScene_Win::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    {
        noNoiseString = replaceAll(noNoiseString, L"{*PLAYER*}",
    ...
    HRESULT hr = XuiHtmlControlSetSmoothScroll(
        m_htmlControl.m_hObj, XUI_SMOOTHSCROLL_VERTICAL, TRUE, AUTO_SCROLL_SPEED,
        1.0f, AUTO_SCROLL_SPEED);
    ...
    case VK_ESCAPE:
        {
            ...
            if (pMinecraft->localgameModes[ProfileManager.GetPrimaryPad()] != NULL)
            {
...
void CScene_Win::updateNoise()
{
    ...
    while (found != string::npos && it != m_noiseLengths.end())
    {
        ...
        // random->nextInt(2) + 8 + (darken ? 16 : 0)); ib.put(listPos + pos + 32);
