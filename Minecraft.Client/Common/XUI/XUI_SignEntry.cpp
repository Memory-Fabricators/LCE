...
HRESULT CScene_SignEntry::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    {
        // Have to have the Latin alphabet here, since that's what we have on the
        // sign in-game but because the JAP/KOR/CHN fonts don't have extended
        // European characters, let's restrict those languages to not having the
        // extended character set, since they can't see what they are typing
        switch (XGetLanguage())
        {
                                          ...
                                          BOOL &rfHandled)
{
    ...
    {
        // Set the sign text here so we on;y call the verify once it has been set,
        // not while we're typing in to it
        for (int i = 0; i < 4; i++)
        {
        ...
        if (pMinecraft->level->isClientSide)
        {
            ...
            {
                player->connection->send(shared_ptr<SignUpdatePacket>(
                    new SignUpdatePacket(m_sign->x, m_sign->y, m_sign->z,
                                         m_sign->IsVerified(), m_sign->IsCensored(),
                                         m_sign->GetMessages())));
            }
        }
