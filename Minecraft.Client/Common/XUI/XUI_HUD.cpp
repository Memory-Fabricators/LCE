...
HRESULT CXuiSceneHud::OnCustomMessage_TickScene()
{
    ...
    if (startFrame >= 0)
    {
        XuiElementPlayTimeline(m_hObj, startFrame, startFrame, startFrame, FALSE,
                               TRUE);
    }
    ...
    if (pMinecraft->localgameModes[m_iPad]->canHurtPlayer())
    {
        ...
        int progress = (int)(pMinecraft->localplayers[m_iPad]->experienceProgress *
    ...
    if (pMinecraft->localgameModes[m_iPad]->canHurtPlayer())
    {
        ...
        for (int icon = 0; icon < Player::MAX_HEALTH / 2; icon++)
        {
            if (blink)
            {
                if (icon * 2 + 1 < iLastHealth || icon * 2 + 1 < iHealth)
                {
                    ...
                    if (bHasPoison)
                    {
                        m_healthIcon[icon].PlayVisualRange(L"FullPoisonFlash", NULL,
                                                           L"FullPoisonFlash");
                    }
                    else
                    {
                ...
                else if (icon * 2 + 1 == iLastHealth || icon * 2 + 1 == iHealth)
                {
                    ...
                    if (bHasPoison)
                    {
                        m_healthIcon[icon].PlayVisualRange(L"HalfPoisonFlash", NULL,
                                                           L"HalfPoisonFlash");
                    }
                    else
                    {
            ...
            // TODO - 4J Stu - This should be scaled based on gui scale and overall
            // size (ie full, split or 480)
            pos.y = yo;
        ...
        int saturationLevel =
            pMinecraft->localplayers[m_iPad]->getFoodData()->getSaturationLevel();
        for (int icon = 0; icon < 10; icon++)
        {
            if (foodBlink)
            {
                if (icon * 2 + 1 < oldFood || icon * 2 + 1 < food)
                {
                    ...
                    if (hasHungerEffect)
                    {
                        m_foodIcon[icon].PlayVisualRange(L"FullPoisonFlash", NULL,
                                                         L"FullPoisonFlash");
                    }
                    else
                    {
                ...
                else if (icon * 2 + 1 == oldFood || icon * 2 + 1 == food)
                {
                    ...
                    if (hasHungerEffect)
                    {
                        m_foodIcon[icon].PlayVisualRange(L"HalfPoisonFlash", NULL,
                                                         L"HalfPoisonFlash");
                    }
                    else
                    {
        ...
        if (armor > 0)
        {
            ...
            for (int icon = 0; icon < 10; icon++)
            {
                ...
                {
                    m_armourIcon[icon].PlayVisualRange(L"Normal", NULL, L"Normal");
                }
            }
        }
        else
        ...
        if (pMinecraft->localplayers[m_iPad]->isUnderLiquid(Material::water))
        {
            ...
            int count =
                (int)ceil((pMinecraft->localplayers[m_iPad]->getAirSupply() - 2) *
                          10.0f / Player::TOTAL_AIR_SUPPLY);
            int extra = (int)ceil((pMinecraft->localplayers[m_iPad]->getAirSupply()) *
                                  10.0f / Player::TOTAL_AIR_SUPPLY) -
                        count;
            for (int icon = 0; icon < 10; icon++)
            {
...
HRESULT CXuiSceneHud::OnCustomMessage_DLCMountingComplete()
{
    ...
    // Need to switch to the texture pack if it's not being used yet, and turn off
