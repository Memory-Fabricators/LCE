...
HRESULT CScene_SkinSelect::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    else
    {
        ...
        {
            // Change to display the favorites if there are any. The current skin will
            // be in there (probably) - need to check for it
            ...
            if (m_currentPack != NULL)
            {
                m_packIndex = app.m_dlcManager.getPackIndex(
                                  m_currentPack, bFound, DLCManager::e_DLCType_Skin) +
    ...
    {
        ui.SetTooltips(m_iPad, IDS_TOOLTIPS_SELECT_SKIN, IDS_TOOLTIPS_CANCEL, -1,
        ...
        default:
            if (m_currentPack != NULL)
            {
                ...
                if (!skinFile->getParameterAsBool(DLCManager::e_DLCParamType_Free))
                {
                    ...
                    if (!m_currentPack->hasPurchasedFile(DLCManager::e_DLCType_Skin,
                                                         skinFile->getPath()))
                    {
                        // no
                        UINT uiIDA[1];
                        ...
                        else
                        {
                            ...
                            TelemetryManager->RecordUpsellPresented(
                                pInputData->UserIndex, eSet_UpsellID_Skin_DLC,
                            ...
                            StorageManager.RequestMessageBox(
                                IDS_UNLOCK_DLC_TITLE, IDS_UNLOCK_DLC_SKIN, uiIDA, 2,
                                pInputData->UserIndex, &CScene_SkinSelect::UnlockSkinReturned,
                    ...
                    {
                        app.SetPlayerSkin(pInputData->UserIndex, skinFile->getPath());
                        app.SetPlayerCape(pInputData->UserIndex,
            ...
            if (m_currentNavigation == eSkinNavigation_Skin)
            {
                if (!m_bAnimatingMove)
                {
                    ...
                    m_previewControl->SetFacing(
                        CXuiCtrlMinecraftSkinPreview::e_SkinPreviewFacing_Left, true);
                    m_previewPreviousControls[0]->SetFacing(
                        CXuiCtrlMinecraftSkinPreview::e_SkinPreviewFacing_Forward, true);

                    int startFrame, endFrame;
                    HRESULT hr =
                        m_charactersGroup.FindNamedFrame(L"CycleRight", &startFrame);
                    hr = m_charactersGroup.FindNamedFrame(L"EndCycleRight", &endFrame);
                    hr = m_charactersGroup.PlayTimeline(startFrame, startFrame, endFrame,
                                                        FALSE, FALSE);
                }
            }
    ...
    case VK_PAD_LTHUMB_RIGHT:
        {
            if (m_currentNavigation == eSkinNavigation_Skin)
            {
                if (!m_bAnimatingMove)
                {
                    ...
                    m_previewControl->SetFacing(
                        CXuiCtrlMinecraftSkinPreview::e_SkinPreviewFacing_Right, true);
                    m_previewNextControls[0]->SetFacing(
                        CXuiCtrlMinecraftSkinPreview::e_SkinPreviewFacing_Forward, true);

                    int startFrame, endFrame;
                    HRESULT hr =
                        m_charactersGroup.FindNamedFrame(L"CycleLeft", &startFrame);
...
HRESULT CScene_SkinSelect::OnTimelineEnd(HXUIOBJ hObjSource, BOOL &bHandled)
{
    if (hObjSource == m_charactersGroup)
    {
        ...
        {
            m_previewControl->SetFacing(
                CXuiCtrlMinecraftSkinPreview::e_SkinPreviewFacing_Forward, false);
            m_previewNextControls[0]->SetFacing(
...
void CScene_SkinSelect::handleSkinIndexChanged()
{
    ...
    if (m_currentPack != NULL)
    {
        ...
        skinName =
            skinFile->getParameterAsString(DLCManager::e_DLCParamType_DisplayName);
        skinOrigin =
            skinFile->getParameterAsString(DLCManager::e_DLCParamType_ThemeName);
        ...
        case SKIN_SELECT_PACK_FAVORITES:
            ...
            if (app.GetPlayerFavoriteSkinsCount(m_iPad) > 0)
            {
                ...
                if (Pack)
                {
                    ...
                    bLicensed = Pack->hasPurchasedFile(DLCManager::e_DLCType_Skin,
                                                       m_selectedSkinPath);
    ...
    if (bNoSkinsToShow)
    {
        if (bNotInGame)
        {
            ui.SetTooltips(DEFAULT_XUI_MENU_USER, -1, IDS_TOOLTIPS_CANCEL, -1, -1, -1,
                           -1, -1, -1, IDS_TOOLTIPS_NAVIGATE);
            CXuiSceneBase::ShowBackground(DEFAULT_XUI_MENU_USER, TRUE);
        }
        else
        {
...
void CScene_SkinSelect::updatePackDisplay()
{
    ...
    {
        DLCPack *thisPack = app.m_dlcManager.getPack(
            nextPackIndex - SKIN_SELECT_MAX_DEFAULTS, DLCManager::e_DLCType_Skin);
    ...
    {
        DLCPack *thisPack =
            app.m_dlcManager.getPack(previousPackIndex - SKIN_SELECT_MAX_DEFAULTS,
...
int CScene_SkinSelect::getPreviousPackIndex(DWORD sourceIndex)
{
    ...
    if (previousPack == SKIN_SELECT_PACK_DEFAULT)
    {
        if (app.m_dlcManager.getPackCount(DLCManager::e_DLCType_Skin) > 0)
        {
            previousPack = app.m_dlcManager.getPackCount(DLCManager::e_DLCType_Skin) -
...
void CScene_SkinSelect::updateClipping()
{
    ...
    propertyVal.Clear();
    HRESULT hr = XuiObjectGetPropertyId(m_charactersGroup.m_hObj, L"ClipChildren",
                                        &dwPropertyId);
                                          ...
                                          C4JStorage::EMessageResult result)
{
    ...
    if (result == C4JStorage::EMessage_ResultAccept)
    {
        if (ProfileManager.IsSignedIn(iPad))
        {
            ...
            // If we're in-game, then we need to enable DLC downloads. They'll be set
            // back to Auto on leaving the pause menu
            if (Minecraft::GetInstance()->level != NULL)
            {
                // need to allow downloads here, or the player would need to quit the
                // game to let the download of a skin pack happen. This might affect the
...
HRESULT CScene_SkinSelect::OnCustomMessage_DLCMountingComplete()
{
    ...
    if (app.m_dlcManager.getPackCount(DLCManager::e_DLCType_Skin) > 0)
    {
        ...
        {
            bool bFound = false;
            m_packIndex = app.m_dlcManager.getPackIndex(m_currentPack, bFound,
                                                        DLCManager::e_DLCType_Skin) +
                          SKIN_SELECT_MAX_DEFAULTS;
        }
    }
