                                           ...
                                           BOOL &bHandled)
{
        ...
        if (cheats)
        {
            ...
            else
            {
                bool canFly = Player::getPlayerGamePrivilege(
                    m_playerPrivileges, Player::ePlayerGamePrivilege_CanToggleFly);
                ...
                checked = canFly && Player::getPlayerGamePrivilege(
                                        m_playerPrivileges,
    ...
    else
    {
        if (localPlayer->IsHost())
        {
            ...
            m_checkboxes[eControl_Op].SetCheck(
                Player::getPlayerGamePrivilege(m_playerPrivileges,
                                               Player::ePlayerGamePrivilege_Op) != 0);
        }
        else
        {
            removeControl(m_checkboxes[eControl_Op], true);
        ...
        if (editingPlayer->IsLocal() != TRUE &&
            editingPlayer->IsSameSystem(g_NetworkManager.GetHostPlayer()) != TRUE)
        {
                                              ...
                                              BOOL &rfHandled)
{
    ...
    case VK_ESCAPE:
     ...
     defined(_FINAL_BUILD) && !defined(_DEBUG_MENUS_ENABLED))
#else
                Player::setPlayerGamePrivilege(m_playerPrivileges,
                ...
                if (cheats)
                {
                    ...
                    if (canBeInvisible)
                    {
                        Player::setPlayerGamePrivilege(
                            m_playerPrivileges, Player::ePlayerGamePrivilege_Invisible,
                            m_checkboxes[eControl_HostInvisible].IsChecked());
                    }
                    if (canBeInvisible)
                    {
                        Player::setPlayerGamePrivilege(
                            m_playerPrivileges, Player::ePlayerGamePrivilege_Invulnerable,
                            m_checkboxes[eControl_HostInvisible].IsChecked());
                    }
                    ...
                    if (!inCreativeMode)
                    {
                        ...
                        {
                            Player::setPlayerGamePrivilege(
                                m_playerPrivileges, Player::ePlayerGamePrivilege_CanFly,
                                m_checkboxes[eControl_HostFly].IsChecked());
                        }
                        if (canChangeHunger)
                        {
                            Player::setPlayerGamePrivilege(
                                m_playerPrivileges, Player::ePlayerGamePrivilege_ClassicHunger,
            ...
            else
            {
                ...
                {
                    Player::setPlayerGamePrivilege(
                        m_playerPrivileges, Player::ePlayerGamePrivilege_CannotMine,
                        !m_checkboxes[eControl_BuildAndMine].IsChecked());
                    Player::setPlayerGamePrivilege(
                        m_playerPrivileges, Player::ePlayerGamePrivilege_CannotBuild,
                        !m_checkboxes[eControl_BuildAndMine].IsChecked());
                    Player::setPlayerGamePrivilege(
                        m_playerPrivileges,
                    ...
                    Player::setPlayerGamePrivilege(
                        m_playerPrivileges, Player::ePlayerGamePrivilege_CanUseContainers,
                ...
                if (localPlayer->IsHost())
                {
                    if (cheats)
                    {
                        ...
                        Player::setPlayerGamePrivilege(
                            ...
                            m_checkboxes[eControl_HostHunger].IsChecked());
                        Player::setPlayerGamePrivilege(
                            m_playerPrivileges, Player::ePlayerGamePrivilege_CanTeleport,
            ...
            if (originalPrivileges != m_playerPrivileges)
            {
                ...
                {
                    player->connection->send(shared_ptr<PlayerInfoPacket>(
                        new PlayerInfoPacket(m_networkSmallId, -1, m_playerPrivileges)));
                }
            }
    ...
    if (hObjPressed == m_buttonKick)
    {
        ...
        *smallId = m_networkSmallId;
        ...
        StorageManager.RequestMessageBox(
            IDS_UNLOCK_KICK_PLAYER_TITLE, IDS_UNLOCK_KICK_PLAYER, uiIDA, 2, m_iPad,
            &CScene_InGamePlayerOptions::KickPlayerReturned, smallId,
            app.GetStringTable());
    }
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    if (result == C4JStorage::EMessage_ResultAccept)
    {
        ...
        // Fix for #61494 - [CRASH]: TU7: Code: Multiplayer: Title may crash while
        // kicking a player from an online game. We cannot do a navigate back here
        // is this actually occurs on a thread other than the main thread. On rare
        // occasions this can clash with the XUI render and causes a crash. The
        // OnPlayerChanged event should perform the navigate back on the main thread
...
HRESULT
CScene_InGamePlayerOptions::OnTransitionStart(XUIMessageTransition *pTransition,
                                              BOOL &bHandled)
{
    ...
    if (!m_editingSelf)
    {
        ...
        // NOT CREATIVE MODE.
        {
            ...
            m_checkboxes[eControl_HostFly].SetCheck(
                isModerator && (Player::getPlayerGamePrivilege(
                                    m_playerPrivileges,
                                    Player::ePlayerGamePrivilege_CanToggleFly) != 0));
