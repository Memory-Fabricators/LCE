                                         ...
                                         BOOL &bHandled)
{
    ...
    else
    {
        ...
        {
            memcpy(m_pSlotDataA[i].wchSaveTitle, app.GetString(IDS_XBONE_EMPTYSLOT),
    ...
    C4JStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename)
{
    ...
    unsigned int uiNewSlotsC = (pClass->m_MaxSlotC < uiSlotListFileSlots)
                                   ? pClass->m_MaxSlotC
                                   : uiSlotListFileSlots;
    memcpy(pClass->m_pSlotDataA, pClass->m_pbSlotListFile + sizeof(unsigned int),
    ...
    for (int i = 0; i < pClass->m_MaxSlotC; i++)
    {
        if (i < uiNewSlotsC)
        {
            ...
            {
                XuiCreateTextureBrushFromMemory(pbImageData,
                                                pClass->m_pSlotDataA[i].uiImageLength,
                                                &pClass->m_hXuiBrush);
                                                ...
                                                DWORD dwImageBytes)
{
    ...
    if (m_pbSlotListFile != NULL)
    {
        ...
        pbCurrentSlotDataPtr =
            m_pbSlotListFile + sizeof(unsigned int) + sizeof(SLOTDATA) * m_MaxSlotC;
    }
    ...
    *((unsigned int *)pbNewSlotFileData) = m_MaxSlotC;
    ...
    PBYTE pbNewSlotImageDataPtr =
        pbNewSlotFileData + sizeof(unsigned int) + sizeof(SLOTDATA) * m_MaxSlotC;
    ...
    LPVOID lpParameter)
{
    ...
    // now write the new slot data file to global storage, and then write the save
    // data
    ...
    else
    {
        ...
        C4JStorage::ELoadGameStatus eLoadStatus = StorageManager.LoadSaveData(
            &pClass->m_XContentData, CScene_TransferToXboxOne::LoadSaveDataReturned,
            lpParameter);
        ...
        for (int i = 0; i < (pClass->m_uiStorageLength / uiChunkSize) + 1; i++)
        {
            ...
            if (uiBytesLeft >= uiChunkSize)
            {
                eStatus = StorageManager.TMSPP_WriteFile(
                    pClass->m_iPad, C4JStorage::eGlobalStorage_TitleUser,
                    C4JStorage::TMS_FILETYPE_BINARY, C4JStorage::TMS_UGCTYPE_NONE,
                    szFilename, pchData, uiChunkSize,
                    &CScene_TransferToXboxOne::TMSPPWriteReturned, lpParameter, 0);
            ...
            else
            {
                ...
                eStatus = StorageManager.TMSPP_WriteFile(
                    pClass->m_iPad, C4JStorage::eGlobalStorage_TitleUser,
                    C4JStorage::TMS_FILETYPE_BINARY, C4JStorage::TMS_UGCTYPE_NONE,
                    szFilename, pchData, uiBytesLeft,
                    &CScene_TransferToXboxOne::TMSPPWriteReturned, lpParameter, 0);
            }
                                            ...
                                            BOOL &rfHandled)
{
    ...
    case VK_PAD_X:
        ...
        if (m_pbSlotListFile != NULL)
        {
            ...
            C4JStorage::ETMSStatus eStatus = StorageManager.TMSPP_DeleteFile(
                m_iPad, "XboxOne/SlotList", C4JStorage::TMS_FILETYPE_BINARY,
                &CScene_TransferToXboxOne::TMSPPDeleteReturned, this, 0);
        }
...
HRESULT
CScene_TransferToXboxOne::OnTransitionStart(XUIMessageTransition *pTransition,
                                            BOOL &bHandled)
{
