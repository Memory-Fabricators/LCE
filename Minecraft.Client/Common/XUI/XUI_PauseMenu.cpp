...
HRESULT UIScene_PauseMenu::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    // Display the tooltips, we are only allowed to display "SHARE" if we have the
    // capability (TCR).
    ...
    else if (StorageManager.GetSaveDisabled())
    {
        ...
        {
            ui.SetTooltips(m_iPad, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK,
                           bIsisPrimaryHost ? IDS_TOOLTIPS_SELECTDEVICE : -1, -1, -1,
                           -1, -1, bDisplayBanTip ? IDS_TOOLTIPS_BANLEVEL : -1);
        }
    }
    else
    {
        ...
        {
            ui.SetTooltips(m_iPad, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK,
                           bIsisPrimaryHost ? IDS_TOOLTIPS_CHANGEDEVICE : -1, -1, -1,
                           -1, -1, bDisplayBanTip ? IDS_TOOLTIPS_BANLEVEL : -1);
        }
    }
    ...
    // Pause Menu does not have a Leaderboards option. TCR # 128:  XLA Pause Menu:
    // When in a multiplayer game as a client the Pause Menu does not have an
    // Achievements option.
    ...
    case BUTTON_PAUSE_SAVEGAME:
        {
            ...
            if (result == C4JStorage::ELoadGame_DeviceRemoved)
            {
                ...
                StorageManager.RequestMessageBox(
                    IDS_STORAGEDEVICEPROBLEM_TITLE, IDS_FAILED_TO_LOADSAVE_TEXT, uiIDA, 2,
                    pNotifyPressData->UserIndex,
                    &UIScene_PauseMenu::DeviceRemovedDialogReturned, this);
            }
            else
            {
                // we need to ask if they are sure they want to overwrite the existing
                // game
                if (bSaveExists)
                {
                    UINT uiIDA[2];
    ...
    case BUTTON_PAUSE_EXITGAME:
        {
            ...
            if (ProfileManager.IsFullVersion())
            {
                ...
                if (pNotifyPressData->UserIndex == ProfileManager.GetPrimaryPad())
                {
                    ...
                    {
                        playTime = (int)pMinecraft->localplayers[pNotifyPressData->UserIndex]
                                       ->getSessionTimer();
                    }
                    ...
                    if (StorageManager.GetSaveDisabled())
                    {
                        ...
                        uiIDA[1] = IDS_CONFIRM_OK;
                        StorageManager.RequestMessageBox(
                            IDS_EXIT_GAME, IDS_CONFIRM_EXIT_GAME_PROGRESS_LOST, uiIDA, 2,
                            pNotifyPressData->UserIndex,
                    ...
                    else
                    {
                        if (g_NetworkManager.IsHost())
                        {
                            ...
                            {
                                StorageManager.RequestMessageBox(
                                    IDS_EXIT_GAME, IDS_CONFIRM_EXIT_GAME_CONFIRM_DISCONNECT_SAVE,
                                    uiIDA, 3, pNotifyPressData->UserIndex,
                                    &UIScene_PauseMenu::ExitGameSaveDialogReturned, this,
                                    app.GetStringTable());
                            }
                            else
                            {
                ...
                else
                {
                    ...
                    TelemetryManager->RecordLevelExit(pNotifyPressData->UserIndex,
                                     ...
                                     BOOL &rfHandled)
{
    ...
    case VK_PAD_START:
    case VK_ESCAPE:
        app.DebugPrintf("PAUSE PRESS PROCESSING - ipad = %d, "
    ...
    case VK_PAD_X:
        ...
        if (bIsisPrimaryHost)
        {
            // we need a function to deal with the return from this - if it changes,
            // we need to update the pause menu and tooltips Fix for #12531 - TCR 001:
            // BAS Game Stability: When a player selects to change a storage device,
            // and repeatedly backs out of the SD screen, disconnects from LIVE, and
            // then selects a SD, the title crashes.
            m_bIgnoreInput = true;

            StorageManager.SetSaveDevice(&UIScene_PauseMenu::DeviceSelectReturned,
                                         this, true);
        }
        rfHandled = TRUE;
        break;
    ...
    case VK_PAD_RSHOULDER:
        if (bDisplayBanTip)
        {
            ...
            uiIDA[1] = IDS_CONFIRM_OK;
            StorageManager.RequestMessageBox(
                IDS_ACTION_BAN_LEVEL_TITLE, IDS_ACTION_BAN_LEVEL_DESCRIPTION, uiIDA,
                2, pInputData->UserIndex, &UIScene_PauseMenu::BanGameDialogReturned,
        ...
        // handle a keyboard Return specifically, because we've turned off the
        // VK_A_OR_START for the pause menu, since this should exit the menu, rather
        // than cause the button to be activated
    case VK_RETURN:
...
HRESULT UIScene_PauseMenu::OnNavReturn(HXUIOBJ hObj, BOOL &rfHandled)
{
    ...
    if (StorageManager.GetSaveDisabled())
    {
        ...
        {
            ui.SetTooltips(m_iPad, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK, -1, -1, -1,
                           -1, -1, bDisplayBanTip ? IDS_TOOLTIPS_BANLEVEL : -1);
        }
    }
    else
    ...
    if (app.GetLocalPlayerCount() == 1)
    {
        // If we were created as a splitscreen scene, then it's now going to be in
        // the wrong place. Get rid of this scene.
        if (m_bSplitscreen)
        {
            CXuiSceneBase::ShowLogo(m_iPad, FALSE);
            isWrongSize = true;
...
int UIScene_PauseMenu::DeviceSelectReturned(void *pParam, bool bContinue)
{
    ...
    if (StorageManager.GetSaveDisabled())
    {
        if (ProfileManager.IsFullVersion() &&
            ...
            bUserisClientSide)
        {
            ui.SetTooltips(pClass->m_iPad, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK,
                           bIsisPrimaryHost ? IDS_TOOLTIPS_SELECTDEVICE : -1,
                           IDS_TOOLTIPS_SHARE, -1, -1, -1,
                           bDisplayBanTip ? IDS_TOOLTIPS_BANLEVEL : -1);
        }
        ...
        {
            ui.SetTooltips(pClass->m_iPad, IDS_TOOLTIPS_SELECT, IDS_TOOLTIPS_BACK, -1,
                           -1, -1, -1, -1,
                           bDisplayBanTip ? IDS_TOOLTIPS_BANLEVEL : -1);
        }
        // disable save button
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    if (result == C4JStorage::EMessage_ResultAccept)
    {
        if (ProfileManager.IsSignedIn(iPad))
        {
            ...
            // need to allow downloads here, or the player would need to quit the game
            // to let the download of a texture pack happen. This might affect the
            // network traffic, since the download could take all the bandwidth...
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
        ...
        result == C4JStorage::EMessage_ResultThirdOption)
    {
        if (result == C4JStorage::EMessage_ResultDecline) // Save
        {
            ...
            if (!Minecraft::GetInstance()->skins->isUsingDefaultSkin())
            {
                ...
                if (!pDLCPack->hasPurchasedFile(DLCManager::e_DLCType_Texture, L""))
                {
                    ...
                    TelemetryManager->RecordUpsellPresented(
                        iPad, eSet_UpsellID_Texture_DLC, ullOfferID_Full & 0xFFFFFFFF);
#endif
                    ...
                    ui.RequestMessageBox(
                        ...
                        &UIScene_PauseMenu::WarningTrialTexturePackReturned, pClass,
                        app.GetStringTable());
            ...
            if (bSaveExists)
            {
                ...
                ui.RequestMessageBox(IDS_TITLE_SAVE_GAME, IDS_CONFIRM_SAVE_GAME, uiIDA,
                                     ...
                                     &UIScene_PauseMenu::ExitGameAndSaveReturned,
        ...
        else
        {
            ...
            uiIDA[1] = IDS_CONFIRM_OK;
            ui.RequestMessageBox(
                IDS_TITLE_DECLINE_SAVE_GAME, IDS_CONFIRM_DECLINE_SAVE_GAME, uiIDA, 2,
                ProfileManager.GetPrimaryPad(),
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    else
    {
        ...
        if (ui.IsPauseMenuDisplayed(ProfileManager.GetPrimaryPad()))
        {
            ...
            UINT uiIDA[3];
            // you cancelled the save on exit after choosing exit and save? You go
            // back to the Exit choices then.
            uiIDA[0] = IDS_CONFIRM_CANCEL;
            uiIDA[1] = IDS_EXIT_GAME_SAVE;
            uiIDA[2] = IDS_EXIT_GAME_NO_SAVE;
            ...
            if (g_NetworkManager.GetPlayerCount() > 1)
            {
                ui.RequestMessageBox(IDS_EXIT_GAME,
                                     IDS_CONFIRM_EXIT_GAME_CONFIRM_DISCONNECT_SAVE,
                                     uiIDA, 3, ProfileManager.GetPrimaryPad(),
                                     &UIScene_PauseMenu::ExitGameSaveDialogReturned,
                                     pClass, app.GetStringTable(), 0, 0, false);
            }
            else
            {
                ui.RequestMessageBox(IDS_EXIT_GAME, IDS_CONFIRM_EXIT_GAME, uiIDA, 3,
                                     ProfileManager.GetPrimaryPad(),
                                     &UIScene_PauseMenu::ExitGameSaveDialogReturned,
                                     pClass, app.GetStringTable(), 0, 0, false);
            }
...
int UIScene_PauseMenu::SaveWorldThreadProc(LPVOID lpParameter)
{
    ...
    {
        // 4J Stu - This causes the fullscreenprogress scene to ignore the action it
        // was given
        hr = ERROR_CANCELLED;
    }
    return hr;
...
void UIScene_PauseMenu::_ExitWorld(LPVOID lpParameter)
{
    ...
    if (pMinecraft->isClientSide() || g_NetworkManager.IsInSession())
    {
        if (lpParameter != NULL)
        {
            ...
            // Incorrect/misleading messages after signing out a profile during online
            // game session. If the primary player is signed out, then that is most
            // likely the cause of the disconnection so don't display a message box.
            // This will allow the message box requested by the libraries to be
            // brought up
            if (ProfileManager.IsSignedIn(ProfileManager.GetPrimaryPad()))
            {
                ui.RequestMessageBox(exitReasonTitleId, exitReasonStringId, uiIDA, 1,
                                     ProfileManager.GetPrimaryPad(), NULL, NULL,
                                     app.GetStringTable());
            }
            exitReasonStringId = -1;

            // 4J - Force a disconnection, this handles the situation that the server
