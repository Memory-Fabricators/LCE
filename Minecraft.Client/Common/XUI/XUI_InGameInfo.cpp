                                     ...
                                     BOOL &rfHandled)
{
    ...
    // 4J-PB - ignore repeats to stop the scene displaying and quitting right away
    // if you hold the back button down
                                           ...
                                           BOOL &rfHandled)
{
    ...
    if (hObjPressed == playersList)
    {
     ...
     defined(_DEBUG_MENUS_ENABLED))
            ...
            {
                // Only ops will hit this, can kick anyone not local and not local to
                // the host
                ...
                *smallId = m_players[playersList.GetCurSel()];
                ...
                StorageManager.RequestMessageBox(
                    ...
                    m_iPad, &CScene_InGameInfo::KickPlayerReturned, smallId,
                    app.GetStringTable());
            }
        }
    }
    ...
    XUIMessageGetSourceText *pGetSourceTextData, BOOL &bHandled)
{
    if (pGetSourceTextData->bItemData)
    {
        if (pGetSourceTextData->iItem < m_playersCount)
        {
            ...
            XuiElementPlayTimeline(hVoiceIcon, playFrame, playFrame, playFrame, FALSE,
                                   FALSE);
...
HRESULT
CScene_InGameInfo::OnGetItemCountAll(XUIMessageGetItemCount *pGetItemCountData,
                                     BOOL &bHandled)
{
