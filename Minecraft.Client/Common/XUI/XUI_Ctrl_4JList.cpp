                             ...
                             int iSortListFromIndex, int iSortFunction)
{
    ...
    // need to remember the original index of this addition before it gets sorted
    // - this will get used to load the game
    ...
    if (iSortListFromIndex != -1)
    {
        ...
        {
        case eSortList_Date:
            // sort from the index passed (to leave create world and tutorial in the
            // saves list)
            sort(m_vListData.begin() + iSortListFromIndex, m_vListData.end(),
                 CXuiCtrl4JList::TimeSortFn);
            break;
        case eSortList_Alphabetical:
            // alphabetical sort
            sort(m_vListData.begin() + iSortListFromIndex, m_vListData.end(),
                 CXuiCtrl4JList::AlphabeticSortFn);
            break;
        case eSortList_Index:
            sort(m_vListData.begin() + iSortListFromIndex, m_vListData.end(),
                 CXuiCtrl4JList::IndexSortFn);
            break;
        }
    }
...
HRESULT
CXuiCtrl4JList::OnGetSourceDataText(XUIMessageGetSourceText *pGetSourceTextData,
                                    BOOL &bHandled)
{
    if ((0 == pGetSourceTextData->iData) && ((pGetSourceTextData->bItemData)))
    {
        EnterCriticalSection(&m_AccessListData);
        pGetSourceTextData->szText = GetData(pGetSourceTextData->iItem).pwszText;
        LeaveCriticalSection(&m_AccessListData);
        bHandled = TRUE;
    }
    return S_OK;
...
HRESULT
CXuiCtrl4JList::OnGetItemCountAll(XUIMessageGetItemCount *pGetItemCountData,
                                  BOOL &bHandled)
{
...
HRESULT
CXuiCtrl4JList::OnGetItemEnable(XUIMessageGetItemEnable *pGetItemEnableData,
                                BOOL &bHandled)
{
    if (m_vListData.size() != 0)
    {
        EnterCriticalSection(&m_AccessListData);
        pGetItemEnableData->bEnabled = GetData(pGetItemEnableData->iItem).fEnabled;
        LeaveCriticalSection(&m_AccessListData);
    }
    bHandled = TRUE;
    return S_OK;
...
HRESULT
CXuiCtrl4JList::OnNotifySelChanged(HXUIOBJ hObjSource,
