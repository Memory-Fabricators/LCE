                                        ...
                                        BOOL &bHandled)
{
    ...
    m_CustomSkinAnim.SetCheck(
        (app.GetGameSettings(m_iPad, eGameSetting_CustomSkinAnim) != 0) ? TRUE
                                                                        : FALSE);
    ...
    else
    {
        ...
        if (bIsPrimaryPad)
        {
            ...
            if (!g_NetworkManager.IsHost())
            {
                ...
                // m_SliderA[SLIDER_SETTINGS_GAMMA] m_SliderA[SLIDER_SETTINGS_GAMMA] ->
                // m_CustomSkinAnim m_CustomSkinAnim -> m_BedrockFog
...
HRESULT
CScene_SettingsGraphics::OnTransitionStart(XUIMessageTransition *pTransition,
                                           BOOL &bHandled)
{
    HRESULT hr;
    WCHAR TempString[256];
        ...
        pTransition->dwTransType == XUI_TRANSITION_BACKTO)
    {
        ...
        if (fMaxLen < fMaxTextLen)
        {
            ...
            {
                hr =
                    XuiElementGetChildById(m_SliderA[i].m_hObj, L"XuiSlider", &hSlider);
...
HRESULT
CScene_SettingsGraphics::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                           BOOL &bHandled)
{
    bHandled = true;
