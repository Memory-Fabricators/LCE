...
void CScene_TutorialPopup::UpdateInteractScenePosition(bool visible)
{
    ...
    BOOL isCreativeScene =
        XuiClassDerivesFrom(sceneClass, creativeInventoryClass);
                                              ...
                                              bool allowFade, bool isReminder)
{
    ...
    else
    {
        ...
        if (titleShowAtStart != titleShowAtEnd)
        {
            ...
            // 4J Stu - For some reason when we resize the scene it resets the size of
            // the HTML control We don't want that to happen, so get it's size before
                                             ...
                                             TutorialPopupInfo *info)
{
    ...
    if (XuiClassDerivesFrom(objClass, thisClass))
    {
        ...
        wstring parsed =
            pThis->_SetIcon(info->icon, info->iAuxVal, info->isFoil, info->desc);
...
wstring CScene_TutorialPopup::_SetIcon(int icon, int iAuxVal, bool isFoil,
                                       LPCWSTR desc)
{
    wstring temp(desc);
    ...
    if (icon != TUTORIAL_NO_ICON)
    {
        ...
        itemIsFoil = (shared_ptr<ItemInstance>(new ItemInstance(icon, 1, iAuxVal)))
                         ->isFoil();
        if (!itemIsFoil)
        {
            itemIsFoil = isFoil;
    ...
    else
    {
        ...
        if (iconTagStartPos > 0 && iconStartPos < (int)temp.length())
        {
            ...
            if (iconEndPos > iconStartPos && iconEndPos < (int)temp.length())
            {
                ...
                bool itemIsFoil = false;
                itemIsFoil =
                    (shared_ptr<ItemInstance>(new ItemInstance(iconId, 1, iAuxVal)))
                        ->isFoil();
                if (!itemIsFoil)
                {
                    itemIsFoil = isFoil;
                ...
                m_pCraftingPic->SetIcon(m_iPad, iconId, iAuxVal, 1, 10, 31, false,
                                        itemIsFoil);

                temp.replace(iconTagStartPos,
                             iconEndPos - iconTagStartPos + closeTag.length(), L"");
            }
        }
        ...
        {
            m_pCraftingPic->SetIcon(m_iPad, Item::hatchet_wood->id, 0, 1, 10, 31,
                                    false);
        }
        else if (temp.find(L"{*WoodenPickaxeIcon*}") != wstring::npos)
        {
            m_pCraftingPic->SetIcon(m_iPad, Item::pickAxe_wood->id, 0, 1, 10, 31,
                                    false);
        }
    ...
    if (imageTagStartPos > 0 && imageStartPos < (int)desc.length())
    {
        ...
        if (imageEndPos > imageStartPos && imageEndPos < (int)desc.length())
        {
            ...
            desc.replace(imageTagStartPos,
                         imageEndPos - imageTagStartPos + closeTag.length(), L"");
        }
    }
    else
    {
...
wstring CScene_TutorialPopup::ParseDescription(int iPad, wstring &text)
{
    ...
    if (app.GetLocalPlayerCount() > 1)
    {
        // TODO TU-1 - This should really be a string as well rather than hardcoded
        // here
        text = replaceAll(text, L"{*EXIT_PICTURE*}",
                          L"<div align=\"center\"><img height=\"90\" width=\"160\" "
                          L"src=\"TutorialExitScreenshot.png\"/></div>");

        // TODO TU-1 - This should also be separate strings, or move these things
        // out of a localisable file so we can add/change them at will
...
HRESULT
CScene_TutorialPopup::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                        BOOL &bHandled)
{
    bHandled = true;
