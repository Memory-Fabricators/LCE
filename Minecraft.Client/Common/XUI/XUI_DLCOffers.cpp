...
HRESULT CScene_DLCOffers::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    // If this is the avatar items, we need to init the avatar system to allow us
    // to find out what the player avatar gender is (since we should only display
    // tshirts etc with the right gender) The init reserves 3MB of memory, so we
    // shut down on leaving this.
    if (m_iType == e_DLC_AvatarItems)
    {
        ...
        hRes =
            XAvatarInitialize(XAVATAR_COORDINATE_SYSTEM_LEFT_HANDED, 0, 0, 0, NULL);
...
HRESULT CScene_DLCOffers::GetDLCInfo(int iOfferC, bool bUpdateOnly)
{
    ...
    if (bUpdateOnly) // Just update the info on the current list
    {
        for (int i = 0; i < iOfferC; i++)
        {
            ...
            // can't trust the offer type - partnernet is giving avatar items the
            // CONTENT type
                ...
                if (xOffer.fUserHasPurchased)
                {
                    ...
                    if (RenderManager.IsHiDef() || RenderManager.IsWidescreen())
                    {
                        swprintf(szResourceLocator, LOCATOR_SIZE, L"section://%X,%ls#%ls",
                                 c_ModuleHandle, L"media", L"media/Graphics/DLC_Tick.png");
                    ...
                    else
                    {
                        swprintf(szResourceLocator, LOCATOR_SIZE, L"section://%X,%ls#%ls",
                                 ...
                                 L"media/Graphics/DLC_TickSmall.png");
                        XuiCreateTextureBrush(szResourceLocator, &hBrush);
                    }
    ...
    else
    {
        ...
        for (int i = 0; i < iOfferC; i++)
        {
            ...
            if (pDLC->eDLCType == (eDLCContentType)m_iType)
            {
                ...
                // Bug 49249 - JPN: Code Defect: Missing Text: String 'Minecraft' is
                // missing in contents download screen. Looks like we shouldn't be
                // removing this text for Japanese, and probably Chinese & Korean
        ...
        // Check if there is nothing to display, and display the default "nothing
    ...
    if (iCount != 0)
    {
        // get the right index for the first list item - it will have been re-sorted
        ...
        if (dlc != NULL)
        {
            ...
            else
            {
                ...
                if (!bPresent)
                {
                    ...
                    // Set the item monitored in the timer, so we can set the image when
                    // it comes in
...
HRESULT
CScene_DLCOffers::OnNotifySelChanged(HXUIOBJ hObjSource,
        ...
        if (dlc != NULL)
        {
            ...
            else
            {
                ...
                if (!bPresent)
                {
                    ...
                    // We can't trust the dwContentCategory from partnernet - it has
                    // avatar items as content instead of avatars
