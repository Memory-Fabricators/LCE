...
class CScene_Main : public CXuiSceneImpl
{
  private:
    ...
    enum eSplashIndexes
    {
        ...
        // The start index in the splashes vector from which we can select a random
        // splash
        eSplashRandomStart,
    };
  ...
  protected:
    ...
    HRESULT OnNotifyPressEx(HXUIOBJ hObjPressed, XUINotifyPress *pNotifyPressData,
                            BOOL &rfHandled);
  ...
  public:
    ...
    static int MustSignInReturned(void *pParam, int iPad,
                                  C4JStorage::EMessageResult result);
#ifdef _XBOX
    static int TMSReadFileListReturned(void *pParam, int iPad,
