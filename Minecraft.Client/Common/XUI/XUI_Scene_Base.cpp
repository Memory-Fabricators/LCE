...
DWORD CXuiSceneBase::m_dwTrialTimerLimitSecs =
...
HRESULT CXuiSceneBase::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    m_iWrongTexturePackTickC = 20 * 5; // default 5 seconds before bringing up the
                                       // upsell for not having the texture pack
    MapChildControls();
...
void CXuiSceneBase::_TickAllBaseScenes()
{
    ...
    // 4J-PB - added to upsell the texture pack if you join from invite and don't
    // have it what texture pack are we using?
    ...
    if (bCheckTexturePack && app.DLCInstallProcessCompleted())
    {
        ...
        if (tPack->getId() != app.GetRequiredTexturePackID())
        {
            ...
            // Is it available?
            TexturePack *pRequiredTPack =
                pMinecraft->skins->getTexturePackById(app.GetRequiredTexturePackID());
                                       ...
                                       unsigned int uiTooltip, int iTextID)
{
    ...
    if (iTextID >= 0)
    {
        ...
        // Change the size of the whole button to be the width of the measured text,
        // plus the position the text element starts in the visual (which is the
        // offset by the size of the button graphic)
        XuiElementGetBounds(m_Buttons[iPad][uiTooltip].m_hObj, &fWidth, &fHeight);
        XuiElementSetBounds(m_Buttons[iPad][uiTooltip].m_hObj,
                            xuiRect.right + 1 + m_vPosTextInTooltip[uiTooltip].x,
                            fHeight);
        ...
        XuiElementSetBounds(m_ButtonsSmall[iPad][uiTooltip].m_hObj,
                            xuiRectSmall.right + 1 +
                                m_vPosTextInTooltipSmall[uiTooltip].x,
                            fHeight);

        // Change the width of the text element to be the width of the measured text
        XuiElementGetBounds(hTooltipTextSmall[iPad][uiTooltip], &fWidth, &fHeight);
        XuiElementSetBounds(hTooltipTextSmall[iPad][uiTooltip], xuiRectSmall.right,
                            fHeight);
                                    ...
                                    bool show)
{
    ...
    // else
    {
        ...
        if (ProfileManager.GetLockedProfile() != -1)
        {
            if (app.GetGameSettings(iPad, eGameSetting_DisplaySplitscreenGamertags) ==
                1)
            {
                unsigned char ucAlpha = app.GetGameSettings(
                    ProfileManager.GetPrimaryPad(), eGameSetting_InterfaceOpacity);
...
HRESULT CXuiSceneBase::_ShowOtherPlayersBaseScene(int iPad, bool show)
{
    for (int i = 0; i < XUSER_MAX_COUNT; ++i)
    {
        if (i == iPad)
        {
            ...
            // Fix for #61051 - TU7: Content: UI: Player specific unresponsive state
            // may be triggered during the Save Game process in the splitscreen mode
...
HRESULT CXuiSceneBase::_ShowPressStart(unsigned int iPad)
{
    ...
    // get the size of the button, and apply the change in size due to the text to
    // the whole button
...
HRESULT
CXuiSceneBase::_SetPlayerBaseScenePosition(unsigned int iPad,
                                           EBaseScenePosition position)
{
    ...
    // The move applies to the whole scene, so we'll need to move tooltips back in
    // some cases
    switch (position)
    {
    ...
    case e_BaseScene_Top: // Top & Bottom - indent by a quarter screen
        ...
        vBackPos.y = 0.0f;
        vGamertagPos.x = XUI_BASE_SCENE_WIDTH - XUI_BASE_SCENE_WIDTH_QUARTER -
                         fGamertagWidth - SAFEZONE_HALF_WIDTH;
        vGamertagPos.y = SAFEZONE_HALF_HEIGHT;
        vBossHealthPos.x = XUI_BASE_SCENE_WIDTH_QUARTER - (fBossHealthWidth / 2);
        vBossHealthPos.y = SAFEZONE_HALF_HEIGHT;
        break;
    case e_BaseScene_Bottom:
        ...
        tooltipsPos.x = SAFEZONE_HALF_WIDTH - XUI_BASE_SCENE_WIDTH_QUARTER;
        tooltipsPos.y =
            XUI_BASE_SCENE_HEIGHT_HALF - SAFEZONE_HALF_HEIGHT - fTooltipHeight;
        crouchIconPos.x = SAFEZONE_HALF_WIDTH - XUI_BASE_SCENE_WIDTH_QUARTER;
    ...
    case e_BaseScene_Bottom_Right:
        ...
        fBackHeight = XUI_BASE_SCENE_HEIGHT_HALF;
        vGamertagPos.x =
            XUI_BASE_SCENE_WIDTH_HALF - fGamertagWidth - SAFEZONE_HALF_WIDTH;
        vGamertagPos.y = 0.0f;
        vBossHealthPos.x = XUI_BASE_SCENE_WIDTH_QUARTER - (fBossHealthWidth / 2);
        vBossHealthPos.y = 0.0f;
        break;
    case e_BaseScene_Left:
        ...
        tooltipsPos.x = SAFEZONE_HALF_WIDTH;
        tooltipsPos.y = XUI_BASE_SCENE_HEIGHT_HALF + XUI_BASE_SCENE_HEIGHT_QUARTER -
    ...
    // their positions Fix for #10960 - All Lang: UI: Split-screen: Changing split
    // screen mode (vertical/horizontal) make window layout strange
    if (Minecraft::GetInstance() != NULL &&
        Minecraft::GetInstance()->localplayers[iPad] != NULL)
    {
        // 4J-PB - Can only do this once we know what the player's UI settings are,
        // so we need to have the player game settings read
...
void CXuiSceneBase::_UpdateSelectedItemPos(unsigned int iPad)
{
    ...
    // Only adjust if fullscreen for now, leaving code to move others if required,
    // but it's too far up the screen when on the bottom quadrants
        ...
        (RenderManager.IsHiDef() || RenderManager.IsWidescreen()))
    {
        ...
        XuiElementGetBounds(m_selectedItemA[iPad], &fSelectedItemWidth,
                            &fSelectedItemHeight);
        XuiElementGetBounds(m_selectedItemSmallA[iPad], &fSelectedItemSmallWidth,
                            &fSelectedItemSmallHeight);
        if (m_playerBaseScenePosition[iPad] != e_BaseScene_Fullscreen)
        {
            fTooltipHeight = fTooltipHeightSmall;
    ...
    else
    {
        ...
        case e_BaseScene_Left:
            selectedItemPos.x =
                XUI_BASE_SCENE_WIDTH_QUARTER - (fSelectedItemWidth / 2.0f);
            selectedItemPos.y =
                XUI_BASE_SCENE_HEIGHT_HALF; // + yOffset; - don't need the offset for
                                            // the boss health since we're displaying
                                            // the item at the bottom of the screen,
                                            // not the top
            break;
        case e_BaseScene_Right:
            selectedItemPos.x =
...
CXuiSceneBase::EBaseScenePosition
CXuiSceneBase::_GetPlayerBasePosition(int iPad)
{
    ...
    for (unsigned int i = 0; i < BUTTONS_TOOLTIP_MAX; ++i)
    {
        if (m_visible[iPad][i] == TRUE)
        {
            if (i > 0 && lastVisible != -1)
            {
                ...
                // 4J Stu - This is for horizontal layout, will need changed if we do
                // vertical layout
                lastPos.x += width + m_iTooltipSpacingGap;

                XuiElementGetBounds(m_ButtonsSmall[iPad][lastVisible].m_hObj, &width,
                                    &height);
                // 4J Stu - This is for horizontal layout, will need changed if we do
                // vertical layout
...
HRESULT CXuiSceneBase::SetTooltipsEnabled(unsigned int iPad, bool bA, bool bB,
                                          ...
                                          bool bLB, bool bRB, bool bLS)
{
    return CXuiSceneBase::Instance->_SetTooltipsEnabled(iPad, bA, bB, bX, bY, bLT,
                                                        bRT, bLB, bRB, bLS);
}
...
HRESULT CXuiSceneBase::UpdatePlayerBasePositions()
{
    ...
    // If the game is not started (or is being held paused for a bit) then display
    // all scenes fullscreen
    if (pMinecraft == NULL)
    {
...
CXuiSceneBase::EBaseScenePosition
CXuiSceneBase::GetPlayerBasePosition(int iPad)
{
