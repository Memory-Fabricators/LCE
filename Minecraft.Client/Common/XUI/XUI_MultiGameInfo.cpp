    ...
    for (unsigned int i = 0; i < MINECRAFT_NET_MAX_PLAYERS; ++i)
    {
        if (m_selectedSession->data.players[i] != NULL)
        {
...
#endif
            {
                playersList.SetText(
                    i,
                    convStringToWstring(m_selectedSession->data.szPlayers[i]).c_str());
            }
        }
        else
        {
...
void CScene_MultiGameInfo::JoinGame(CScene_MultiGameInfo *pClass)
{
    ...
    // Check if user-created content is allowed, as we cannot play multiplayer if
    // it's not
    bool noUGC = false;
    ...
    if (noUGC)
    {
        ...
        StorageManager.RequestMessageBox(IDS_CONNECTION_FAILED, messageText, uiIDA,
                                         1, ProfileManager.GetPrimaryPad(), NULL,
    ...
    else
    {
        CGameNetworkManager::eJoinGameResult result =
            g_NetworkManager.JoinGame(pClass->m_selectedSession, dwLocalUsersMask);
