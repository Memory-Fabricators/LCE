...
CXuiCtrlMinecraftSlot::CXuiCtrlMinecraftSlot()
    : // m_hBrush(NULL),
      m_bDirty(FALSE), m_iPassThroughDataAssociation(0),
      m_iPassThroughIdAssociation(0), m_fScale(1.0f), m_fAlpha(1.0f), m_iID(0),
      m_iCount(0), m_iAuxVal(0), m_bDecorations(false), m_isFoil(false),
      m_popTime(0)
{
                                                ...
                                                BOOL &rfHandled)
{
    ...
    if (HRESULT_SUCCEEDED(hr))
    {
        ...
        if (Message.bHandled)
        {
            ...
            {
                m_iID =
                    GET_SLOTDISPLAY_ID_FROM_ITEM_BITMASK(MsgGetSlotItem.iItemBitField);

                // if the id is greater than or equal to 32000, then it's an xzp icon,
                // not a game icon
                if (m_iID < 32000)
                {
                    // 4J Stu - Some parent controls may overide this, others will leave
                    // it as what we passed in
                ...
                if (m_item != NULL &&
                    (m_item->id != m_iID || m_item->getAuxValue() != m_iAuxVal ||
                                        ...
                                        BOOL &bHandled)
{
    ...
    HRESULT hr;
    XuiMessageGetSourceImage(&Message, &MsgGetImage, m_iPassThroughIdAssociation,
    ...
    if (m_item != NULL || (m_iID > 0 && m_iID < 32000))
    {
        ...
        if (m_item == NULL)
        {
            m_item = shared_ptr<ItemInstance>(
        ...
        // Make sure that pMinecraft->player is the correct player so that player
        // specific rendering
        ...
        m_pItemRenderer->renderAndDecorateItem(
            pMinecraft->font, pMinecraft->textures, m_item, x, y, scaleX, scaleY,
            m_fAlpha, m_isFoil, false);
        ...
        if (m_bDecorations)
        {
            if ((scaleX != 1.0f) || (scaleY != 1.0f))
            {
                ...
                m_pItemRenderer->renderGuiItemDecorations(
                    pMinecraft->font, pMinecraft->textures, m_item, iX, iY, m_fAlpha);
                glPopMatrix();
            }
            else
            {
                m_pItemRenderer->renderGuiItemDecorations(pMinecraft->font,
                                                          pMinecraft->textures, m_item,
                                                          (int)x, (int)y, m_fAlpha);
            }
        }
...
void CXuiCtrlMinecraftSlot::SetIcon(int iPad, shared_ptr<ItemInstance> item,
                                    int iScale, unsigned int uiAlpha,
                                    bool bDecorations, BOOL bShow)
{
    m_item = item;
