...
HRESULT CScene_Main::OnNotifySetFocus(HXUIOBJ hObjSource,
                                      ...
                                      BOOL &bHandled)
{
    // If we sign out when in the saves list, we get a notifysetfocus in the saves
    // list after the init of the main menu
    ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT);
...
HRESULT CScene_Main::OnNotifyPressEx(HXUIOBJ hObjPressed,
                                     ...
                                     BOOL &rfHandled)
{
    // should we ignore the button press? This will be set if we're waiting for a
    // callback from a function launched from a button press
    if (m_bIgnorePress)
    {
        return S_OK;
    }
    ...
    case BUTTON_PLAYGAME:
        ...
        else
        {
            ...
            StorageManager.RequestMessageBox(
                ...
                pNotifyPressData->UserIndex, &CScene_Main::MustSignInReturned, this,
                app.GetStringTable());
        }
                                       ...
                                       BOOL &bHandled)
{
    ...
    {
        // 4J-PB - remove the "hobo humping" message legal (Sony) say we can't have
        // - pretty sure Microsoft would say the same if they noticed it.
        int splashIndex =
        ...
        else if (LocalSysTime.wMonth == 6 && LocalSysTime.wDay == 1)
        {
            splashIndex = eSplashHappyBirthdayNotch;
                 ...
                 LocalSysTime.wDay == 24) // the Java game shows this on Christmas
                                          // Eve, so we will too
        {
        ...
        // 4J-PB - Going to resize buttons if the text is too big to fit on any of
        // them (Br-pt problem with the length of Unlock Full Game)
        ...
        if (fTextVisualLen < fMaxTextLen)
        {
            ...
            for (int i = 0; i < BUTTONS_MAX; i++)
            {
                ...
                m_Buttons[i].SetBounds(fMaxButton + fMaxTextLen - fTextVisualLen,
                                       fHeight);
            }
        }
#endif
...
HRESULT
CScene_Main::OnControlNavigate(XUIMessageControlNavigate *pControlNavigateData,
                               BOOL &bHandled)
{
#ifdef _MINECON
                                                   ...
                                                   int iPad)
{
    ...
    if (bContinue == true)
    {
        ...
        else
        {
            ...
            // Can't apply the player's settings here - they haven't come back from
            // the QuerySignInStatud call above yet. Need to let them action in the
            // main loop when they come in ensure we've applied this player's settings
    ...
    {
        // force a sign-in - they were offline, and they want to be online, so don't
        // let it display offline players set the bAddUser to false to allow offline
        // to go online by selecting the already signed in player again
    ...
    if (bContinue == true)
    {
        ...
        else
        {
                    ...
                    else
                    {
                            ...
                            app.GetBanListRead(iPad))
                        {
                            if (StorageManager.SetSaveDevice(
                                    &CScene_Main::DeviceSelectReturned, pClass) == true)
                            {
                                // save device already selected
            ...
            else
            {
                ...
                // Can't apply the player's settings here - they haven't come back from
                // the QuerySignInStatud call above yet. Need to let them action in the
                // main loop when they come in ensure we've applied this player's
                // settings
                // app.ApplyGameSettingsChanged(iPad);
                                    ...
                                    C4JStorage::EMessageResult result)
{
    ...
    {
        // we need to specify local game here to display local and LIVE profiles in
        // the list
        switch (pClass->m_eAction)
        {
        case eAction_RunGame:
            ProfileManager.RequestSignInUI(false, true, false, false, true,
                                           &CScene_Main::CreateLoad_SignInReturned,
                                           pClass, iPad);
            break;
        case eAction_RunLeaderboards:
            ProfileManager.RequestSignInUI(false, false, true, false, true,
                                           &CScene_Main::Leaderboards_SignInReturned,
                                           pClass, iPad);
            break;
        case eAction_RunAchievements:
            ProfileManager.RequestSignInUI(false, false, true, false, true,
                                           &CScene_Main::Achievements_SignInReturned,
                                           pClass, iPad);
            break;
                                               ...
                                               int iPad)
{
    ...
    if (bContinue == true)
    {
        ...
        else
        {
            ...
            // block all input
            pClass->m_bIgnorePress = true;
            // We want to hide everything in this scene and display a timer until we
            // get a completion for the TMS files
            for (int i = 0; i < BUTTONS_MAX; i++)
            {
                pClass->m_Buttons[i].SetShow(FALSE);
            }
...
void CScene_Main::RunPlayGame(int iPad)
{
    ...
    else
    {
        ...
        // If the player was signed in before selecting play, we'll not have read
        // the profile yet, so query the sign-in status to get this to happen
        ...
        if (ProfileManager.IsFullVersion())
        {
            ...
            if (!ProfileManager.IsSignedInLive(iPad))
            {
                ...
                ProfileManager.DisplayOfflineProfile(
                    &CScene_Main::CreateLoad_OfflineProfileReturned, this, iPad);
            }
            else
            {
                ...
                else
                {
                    ...
                    // flag the timer to start the TMS calls when there is nothing
                    // happening with TMS. fix for X360 - 162325 - TCR 001: BAS Game
                    // Stability: TU17: The game goes into an infinite loading upon
                    // entering the Play Game menu shortly after visiting the Minecraft
                    // Store
                    ...
                    // block all input
                    m_bIgnorePress = true;
                    // We want to hide everything in this scene and display a timer until
                    // we get a completion for the TMS files
                    for (int i = 0; i < BUTTONS_MAX; i++)
                    {
...
HRESULT CScene_Main::OnTMSBanFileRetrieved()
{
    ...
    if (StorageManager.SetSaveDevice(&CScene_Main::DeviceSelectReturned, this) ==
        true)
    {
        // change the minecraft player name
        pMinecraft->user->name = convStringToWstring(
...
void CScene_Main::RunUnlockOrDLC(int iPad)
{
    ...
    if (ProfileManager.IsFullVersion())
    {
        ...
        if (ProfileManager.IsSignedInLive(iPad))
        {
            ...
            else
            {
                ...
                // We want to navigate to the DLC scene, but block input until we get
                // the DLC file in from TMS Don't navigate - we might have an uplink
                // disconnect
                // app.NavigateToScene(ProfileManager.GetPrimaryPad(),eUIScene_DLCMainMenu);
            }
        }
    ...
    else
    {
        ...
        if (ProfileManager.IsGuest(iPad))
        {
            StorageManager.RequestMessageBox(IDS_UNLOCK_TITLE, IDS_UNLOCK_GUEST_TEXT,
                                             uiIDA, 1, iPad);
        }
        else if (!ProfileManager.IsSignedInLive(iPad))
        {
