  ...
  this debug option anymore.
    ...
    else if (hObjPressed == m_setCamera)
    {
        ...
        hr = XuiSceneCreate(szResourceLocator,
                            app.GetSceneName(eUIScene_DebugSetCamera, false, false),
                            NULL, &hScene);
        this->NavigateForward(hScene);
        // app.NavigateToScene(ProfileManager.GetPrimaryPad(),eUIScene_DebugCreateSchematic);
#endif
    ...
    BOOL &bHandled)
{
    if (hObjSource == m_setTime)
    {
        ...
        // Need to set the time on both levels to stop the flickering as the local
        // level tries to predict the time Only works if we are on the host machine,
        // but shouldn't break if not
...
void CScene_DebugOverlay::SaveLimitedFile(int chunkRadius)
{
    ...
    currentSave->readFile(origFileEntry,
    ...
    for (int xPos = playerChunkX - chunkRadius; xPos < playerChunkX + chunkRadius;
         ++xPos)
    {
             ...
             zPos < playerChunkZ + chunkRadius; ++zPos)
        {
            ...
            DataOutputStream *os =
                getChunkDataOutputStream(newFileCache, &newSave, L"", xPos, zPos);
            if (os != NULL)
            {
...
RegionFile *CScene_DebugOverlay::getRegionFile(
    unordered_map<File, RegionFile *, FileKeyHash, FileKeyEq> &newFileCache,
    ConsoleSaveFile *saveFile, const wstring &prefix, int chunkX,
...
DataOutputStream *CScene_DebugOverlay::getChunkDataOutputStream(
    unordered_map<File, RegionFile *, FileKeyHash, FileKeyEq> &newFileCache,
    ConsoleSaveFile *saveFile, const wstring &prefix, int chunkX, int chunkZ)
{
