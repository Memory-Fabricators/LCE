                                          ...
                                          BOOL &rfHandled)
{
    ...
    case VK_PAD_START:
    case VK_PAD_BACK:
        // We need to send a packet to the server to update it's representation of
        // this item
        if (m_slot != NULL && m_menu != NULL)
        {
            ...
            if (player != NULL && player->connection)
            {
                player->connection->send(
                    shared_ptr<ContainerSetSlotPacket>(new ContainerSetSlotPacket(
                        m_menu->containerId, m_slot->index, m_item)));
            }
        }
    ...
    BOOL &bHandled)
{
    if (m_item == NULL)
    {
        m_item = shared_ptr<ItemInstance>(new ItemInstance(0, 1, 0));
    }
    ...
    m_icon->SetIcon(m_iPad, m_item->id, m_item->getAuxValue(), m_item->count, 10,
                    31, false, m_item->isFoil());
