...
HRESULT CScene_Credits::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    if (!RenderManager.IsHiDef() && !RenderManager.IsWidescreen())
    {
        ...
        CREDITS_SCREEN_MAX_Y =
            630.0f / 1.5f; // Y pos at which credits appear at bottom of screen.
        CREDITS_FADE_HEIGHT =
...
HRESULT CScene_Credits::OnTimer(XUIMessageTimer *pTimer, BOOL &bHandled)
{
    ...
    if (pTimer->nId == CREDITS_TICK_TIMER_ID)
    {
        ...
        if ((m_iCurrDefIndex == -1) || (m_fMoveSinceLastDef >= m_fMoveToNextDef))
        {
            ...
            if (pTextType->m_iNumUsedElements < pTextType->m_iMaxElements)
            {
                ...
                else
                {
                    // Determine space to next item.
                    m_fMoveToNextDef =
                        gs_aLineSpace[app.GetDLCCredits(iNextDef - MAX_CREDIT_STRINGS)
                                          ->m_eType];
                }
        ...
        for (int i = 0; i < eNumTextTypes; ++i)
        {
            ...
            // Process in reverse order from latest one back (so that we can easily
            // remove oldest if it has scrolled off the top).
            ...
            for (int j = 0; j < pTextType->m_iNumUsedElements; ++j)
            {
                ...
                else
                {
                    ...
                    {
                        fOpacity =
                            (vTextPos.y - CREDITS_SCREEN_MIN_Y) / CREDITS_FADE_HEIGHT;
                    }
                    else if (vTextPos.y >
                             (CREDITS_SCREEN_MAX_Y - CREDITS_FADE_HEIGHT))
                    {
                        fOpacity =
                            (CREDITS_SCREEN_MAX_Y - vTextPos.y) / CREDITS_FADE_HEIGHT;
                    }
