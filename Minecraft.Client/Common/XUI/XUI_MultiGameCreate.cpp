    ...
    HXUIOBJ hObjPressed, XUINotifyPress *pNotifyPressData, BOOL &rfHandled)
{
    ...
    if (hObjPressed == m_NewWorld)
    {
                    ...
                    !m_pDLCPack->hasPurchasedFile(DLCManager::e_DLCType_Texture, L""))
                {
                    ...
                    else
                    {
                        ...
                        StorageManager.RequestMessageBox(
                            ...
                            &CScene_MultiGameCreate::WarningTrialTexturePackReturned, this,
                            app.GetStringTable());
                        return S_OK;
                    }
                }
            ...
            m_MoreOptionsParams.bHostPrivileges == TRUE)
        {
            ...
            {
                StorageManager.RequestMessageBox(
                    IDS_TITLE_START_GAME, IDS_CONFIRM_START_CREATIVE, uiIDA, 2, m_iPad,
            ...
            else
            {
                StorageManager.RequestMessageBox(
                    ...
                    m_iPad, &CScene_MultiGameCreate::ConfirmCreateReturned, this,
                    app.GetStringTable());
            }
        }
        else
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    else
    {
        ...
        {
            // This is called from a storage manager thread... need to set up thread
            // storage for IntCache as CreateGame requires this to search for a
            // suitable seed if we haven't set a seed.
    ...
    BOOL &rfHandled)
{
    ...
    else if (hObjSource == m_SliderDifficulty.GetSlider())
    {
        ...
        swprintf(
            ...
            app.GetString(IDS_SLIDER_DIFFICULTY),
            app.GetString(m_iDifficultyTitleSettingA[pValueChangedData->nValue]));
        m_SliderDifficulty.SetText(TempString);
    }
                                                     ...
                                                     bool bContinue, int iPad)
{
    ...
    if (bContinue == true)
    {
        ...
        if (ProfileManager.IsSignedIn(iPad))
        {
            ...
            {
                // This is NOT called from a storage manager thread, and is in fact
                                        ...
                                        DWORD dwLocalUsersMask)
{
    ...
    // not installed, since this will run all through the create game, and will
    ...
    if (wSeed.length() != 0)
    {
        ...
        // If the value is not 0 use it, otherwise use the algorithm from the java
        // String.hashCode() function to hash it
        if (value != 0)
        {
            seedValue = value;
        }
        ...
        seedValue = BiomeSource::findSeed(
            isFlat ? LevelType::lvl_flat
                   : LevelType::lvl_normal); // 4J - was (new Random())->nextLong()
                                             // - now trying to actually find a seed
...
HRESULT
CScene_MultiGameCreate::OnTransitionStart(XUIMessageTransition *pTransition,
                                          BOOL &bHandled)
{
...
HRESULT
CScene_MultiGameCreate::OnTransitionEnd(XUIMessageTransition *pTransition,
                                        BOOL &bHandled)
{
