...
{
    // Extract pad and required page from init data. We just put the data into the
    // pointer rather than using it as an address.
...
void CScene_HowToPlay::StartPage(EHowToPlayPage ePage)
{
    ...
    if (pDef->m_iTextControlIndex != eHowToPlay_TextNone)
    {
        ...
        XuiElementSetUserFocus(m_aTextControls[pDef->m_iTextControlIndex].m_hObj,
                               m_iPad);
    }
