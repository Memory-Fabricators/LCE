...
//////////////////////////////////////////////////////////////////////////
HRESULT
CXuiSceneCraftingPanel::OnTransitionEnd(XUIMessageTransition *pTransData,
                                        BOOL &bHandled)
{
        ...
        pTransData->dwTransType == XUI_TRANSITION_BACKTO)
    {
        ...
        if (m_iContainerType == RECIPE_TYPE_3x3)
        {
            ...
            {
                m_hGroupIconA[i].PlayVisualRange(m_GroupIconNameA[m_pGroupA[i]], NULL,
...
//////////////////////////////////////////////////////////////////////////
HRESULT
CXuiSceneCraftingPanel::OnGetSourceImage(XUIMessageGetSourceImage *pData,
                                         BOOL &rfHandled)
{
    HRESULT hr = S_OK;
    // int iId=pData->iItem;
...
HRESULT CXuiSceneCraftingPanel::OnDestroy()
{
    ...
    // closed, even if it is forced to close by some other reason (like the player
    // dying)
...
HRESULT
CXuiSceneCraftingPanel::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                          BOOL &bHandled)
{
    bHandled = true;
...
void CXuiSceneCraftingPanel::setCraftHSlotItem(int iPad, int iIndex,
...
void CXuiSceneCraftingPanel::setCraftVSlotItem(int iPad, int iIndex,
                                               shared_ptr<ItemInstance> item,
                                               unsigned int uiAlpha)
{
    ...
    int iPad, shared_ptr<ItemInstance> item)
{
    ...
    else
    {
        m_pCraftingOutput->SetIcon(iPad, item->id, item->getAuxValue(),
                                   item->GetCount(), 12, 31, true, item->isFoil());
    }
}
...
void CXuiSceneCraftingPanel::updateHighlightAndScrollPositions()
{
    ...
    vec.y = m_vSlot0HighlightPos.y + ((m_iCurrentSlotVIndex - 1) *
