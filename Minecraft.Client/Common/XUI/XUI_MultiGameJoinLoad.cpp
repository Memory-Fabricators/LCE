                                         ...
                                         BOOL &bHandled)
{
    ...
    {
        // if we're waiting for DLC to mount, don't fill the save list. The custom
        // message on end of dlc mounting will do that
        ...
        else if (StorageManager.GetSaveDisabled())
        {
            if (StorageManager.GetSaveDeviceSelected(m_iPad))
            {
                ...
                ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT,
                               IDS_TOOLTIPS_BACK, IDS_TOOLTIPS_CHANGEDEVICE, -1, -1, -1,
                               iLB, IDS_TOOLTIPS_DELETESAVE);
            ...
            {
                ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT,
                               IDS_TOOLTIPS_BACK, IDS_TOOLTIPS_SELECTDEVICE, -1, -1, -1,
                               iLB);
        ...
        else
        {
            ...
            ui.SetTooltips(
                ...
                IDS_TOOLTIPS_CHANGEDEVICE, -1, -1, -1, -1,
    ...
    // the player disconnects while starting a new world and then opts to play the
    // tutorial once they have been returned to the Main Menu.
    ...
    for (int i = 0; i < TMS_COUNT; i++)
    {
        if (app.TMSFileA[i].eTMSType == eTMSFileType_TexturePack)
        {
            ...
            app.AddMemoryTPDFile(app.TMSFileA[i].iConfig, app.TMSFileA[i].pbData,
                                 app.TMSFileA[i].uiSize);
        }
    }

    // 4J-PB - there may be texture packs we don't have, so use the info from TMS
    // for this
...
void CScene_MultiGameJoinLoad::AddDefaultButtons()
{
    ...
    for (AUTO_VAR(it, m_generators->begin()); it != m_generators->end(); ++it)
    {
        ...
        if (uiTexturePackID != 0)
        {
            ...
            m_iMashUpButtonsC++;
            TexturePack *tp = Minecraft::GetInstance()->skins->getTexturePackById(
            ...
            if (dwImageBytes > 0 && pbImageData)
            {
                ...
                // the index inside the list item for this will be i+1 because they
                // start at m_vListData.size(), so the first etry (tutorial) is 1
...
HRESULT CScene_MultiGameJoinLoad::GetSaveInfo()
{
    ...
    else
    {
        ...
        C4JStorage::ESGIStatus eSGIStatus = StorageManager.GetSavesInfo(
            ...
            &CScene_MultiGameJoinLoad::GetSavesInfoCallback, this, "savegame.dat");
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    if (result == C4JStorage::EMessage_ResultDecline)
    {
        ...
        // use the device select returned function to wipe the saves list and change
        // the tooltip
    ...
    HXUIOBJ hObjPressed, XUINotifyPress *pNotifyPressData, BOOL &rfHandled)
{
        ...
        if (m_pGamesList->GetItemCount() > 0 && nIndex < currentSessions.size())
        {
            ...
            if (m_initData->selectedSession->data.texturePackParentId != 0)
            {
                ...
                if (bHasTexturePackInstalled == false)
                {
                    ...
                    TelemetryManager->RecordUpsellPresented(pNotifyPressData->UserIndex,
                    ...
                    if (pDLCInfo->ullOfferID_Trial != 0LL)
                    {
                        ...
                        StorageManager.RequestMessageBox(
                            ...
                            ProfileManager.GetPrimaryPad(),
                            &CScene_MultiGameJoinLoad::TexturePackDialogReturned, this,
                            app.GetStringTable());
                    }
                    else
                    {
    ...
    else if (hObjPressed == m_SavesList)
    {
        ...
        if (iIndex == JOIN_LOAD_CREATE_BUTTON_INDEX)
        {
            ...
            params->iPad = m_iPad;
            app.NavigateToScene(pNotifyPressData->UserIndex, eUIScene_CreateWorldMenu,
                                (void *)params);
        }
        else if (info.iData >= 0)
        {
            ...
            else
            {
                ...
                params->iPad = m_iPad;
                // need to get the iIndex from the list item, since the position in the
                // list doesn't correspond to the GetSaveGameInfo list because of
                // sorting
                ...
                app.NavigateToScene(ProfileManager.GetPrimaryPad(), eUIScene_LoadMenu,
                                    params);
            }
        }
        else
        {
            ...
            else
            {
                ...
                else
                {
                    LoadMenuInitData *params = new LoadMenuInitData();
                    params->iPad = m_iPad;
                    // need to get the iIndex from the list item, since the position in
                    // the list doesn't correspond to the GetSaveGameInfo list because of
                    // sorting
                    params->iSaveGameInfoIndex =
                        m_pSavesList->GetData(iIndex).iIndex - m_iDefaultButtonsC;
                    // params->pbSaveRenamed=&m_bSaveRenamed;
                                            ...
                                            BOOL &rfHandled)
{
    ...
    case VK_PAD_X:
        ...
        // Change device
        // Fix for #12531 - TCR 001: BAS Game Stability: When a player selects to
        // change a storage device, and repeatedly backs out of the SD screen,
        // disconnects from LIVE, and then selects a SD, the title crashes.
        m_bIgnoreInput = true;
        StorageManager.SetSaveDevice(
    ...
    case VK_PAD_Y:
        ...
        {
            // save transfer - make sure they want to overwrite a save that is up
            // there
            if (ProfileManager.IsSignedInLive(m_iPad))
            {
                // 4J-PB - required for a delete of the save if it's found to be a
                // corrupted save
                ...
                ui.RequestMessageBox(
                    IDS_SAVE_TRANSFER_TITLE, IDS_SAVE_TRANSFER_TEXT, uiIDA, 2,
                    pInputData->UserIndex,
                    &CScene_MultiGameJoinLoad::SaveTransferDialogReturned, this,
                    app.GetStringTable());
            }
        }
        break;
        ...
        if (DoesSavesListHaveFocus())
        {
            ...
            else
            {
                if (StorageManager.EnoughSpaceForAMinSaveGame())
                {
                    ...
                    StorageManager.RequestMessageBox(
                        IDS_TOOLTIPS_SAVEOPTIONS, IDS_TEXT_SAVEOPTIONS, uiIDA, 3,
                        pInputData->UserIndex,
                        &CScene_MultiGameJoinLoad::SaveOptionsDialogReturned, this,
                        app.GetStringTable());
                }
                else
                {
                                              ...
                                              BOOL &rfHandled)
{
    ...
    if (m_bMultiplayerAllowed)
    {
        ...
        // If we are navigating back from a full screen progress scene, then that
        // means a connection attempt failed
    ...
    {
        // clear out the saves list, since the disable save may have happened in the
        // load screen because of a device removal
...
HRESULT
CScene_MultiGameJoinLoad::OnTransitionStart(XUIMessageTransition *pTransition,
                                            BOOL &bHandled)
{
    ...
    {
        // 4J Stu - We may have had to unload our font renderer in this scene if one
        // of the save files uses characters not in our font (eg asian chars) so
        // restore our font renderer This will not do anything if our font renderer
        // is already loaded
             ...
             pTransition->dwTransType == XUI_TRANSITION_BACKTO)
    {
        ...
        // 4J-PB - Need to check for installed DLC, which might have happened while
        // you were on the info scene
        if (pTransition->dwTransType == XUI_TRANSITION_BACKTO)
        {
            // Can't call this here because if you back out of the load info screen
            // and then go back in and load a game, it will attempt to use the dlc as
            // it's running a mount of the dlc
...
void CScene_MultiGameJoinLoad::UpdateGamesList()
{
    ...
    currentSessions = *g_NetworkManager.GetSessionList(m_iPad, m_localPlayers,
                                                       m_bShowingPartyGamesOnly);
             ...
             ++it)
        {
            ...
            if (sessionInfo->data.texturePackParentId != 0)
            {
                ...
                if (tp == NULL)
                {
                    ...
                    if (dwImageBytes > 0 && pbImageData)
                    {
                        hr = XuiCreateTextureBrushFromMemory(pbImageData, dwImageBytes,
                                                             &hXuiBrush);
            ...
            else
            {
                ...
                XuiCreateTextureBrushFromMemory(m_DefaultMinecraftIconData,
                                                m_DefaultMinecraftIconSize, &hXuiBrush);
                m_pGamesList->UpdateGraphic(sessionIndex, hXuiBrush);
            }
                                          ...
                                          BOOL &bHandled)
{
    ...
    case JOIN_LOAD_ONLINE_TIMER_ID:
        {
            ...
            bool bMultiplayerAllowed = ProfileManager.IsSignedInLive(m_iPad) &&
                                       ProfileManager.AllowedToPlayMultiplayer(m_iPad);
    ...
    case JOIN_LOAD_SCROLL_GAME_NAMES_TIMER_ID:
        {
            // This is called by the gameslist callback function, so isn't needed on a
            // timer
...
show the sign-in UI again DWORD connectedControllers = 0; for(unsigned int i =
0; i < XUSER_MAX_COUNT; ++i)
                {
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    if (result == C4JStorage::EMessage_ResultDecline)
    {
        ...
        else
        {
            ...
            StorageManager.GetSaveCacheFileInfo(pClass->m_iChangingSaveGameInfoIndex -
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    if (result == C4JStorage::EMessage_ResultAccept)
    {
        ...
        // 4J-PB - ensure we've switched to the right title group id for uploading
        // to
        ...
        C4JStorage::ELoadGameStatus eLoadStatus = StorageManager.LoadSaveData(
            &ContentData, CScene_MultiGameJoinLoad::LoadSaveDataReturned, pClass);
    ...
    LPVOID lpParameter)
{
    ...
    // Upload the metadata and thumbnail
    {
...

        {
            ...
            {
                DWORD dwResult =
                    XContentGetThumbnail(ProfileManager.GetPrimaryPad(), &XContentData,
                                         NULL, &dwImageBytes, NULL);
                if (dwResult == ERROR_SUCCESS)
                {
                    pClass->m_pbSaveTransferData = new BYTE[dwImageBytes];
                    pbImageData =
                        pClass
                            ->m_pbSaveTransferData; // Copy pointer so that we can use the
                                                    // same name as the library owned one,
                                                    // but m_pbSaveTransferData will get
                                                    // deleted when done
                    XContentGetThumbnail(ProfileManager.GetPrimaryPad(), &XContentData,
                                         pbImageData, &dwImageBytes, NULL);
                }
            }
                                          ...
                                          char *filename)
{
    ...
    C4JStorage::ETMSStatus result = StorageManager.TMSPP_DeleteFile(
        ProfileManager.GetPrimaryPad(), filename, C4JStorage::TMS_FILETYPE_BINARY,
                                          ...
                                          DWORD size)
{
    ...
    if (app.GetWriteSavesToFolderEnabled())
    {
        ...
        string path = string(wstringtofilename(targetFileDir.getPath()))
                          .append("\\")
                          .append(filename);
        HANDLE hSaveFile = CreateFile(path.c_str(), GENERIC_WRITE, 0, NULL,
...
int CScene_MultiGameJoinLoad::KeyboardReturned(void *pParam, bool bSet)
{
    ...
    if ((pClass->m_wchNewName[0] != 0) && bSet)
    {
        ...
        C4JStorage::ELoadGameStatus eLoadStatus = StorageManager.LoadSaveData(
            &XContentData, CScene_MultiGameJoinLoad::LoadSaveDataForRenameReturned,
            pClass);
        ...
        if (eLoadStatus == C4JStorage::ELoadGame_DeviceRemoved)
        {
            ...
            uiIDA[0] = IDS_OK;
            StorageManager.RequestMessageBox(
                IDS_STORAGEDEVICEPROBLEM_TITLE, IDS_FAILED_TO_LOADSAVE_TEXT, uiIDA, 1,
                ProfileManager.GetPrimaryPad(),
                                                            ...
                                                            bool bContinue)
{
    ...
    if (bContinue == true)
    {
        ...
        // if there is no thumbnail, retrieve the default one from the file.
        // Don't delete the image data after creating the xuibrush, since we'll use
        // it in the rename of the save
        if (pbImageData == NULL)
        {
            DWORD dwResult =
                XContentGetThumbnail(ProfileManager.GetPrimaryPad(), &XContentData,
...
HRESULT CScene_MultiGameJoinLoad::OnCustomMessage_DLCMountingComplete()
{
    ...
    else if (StorageManager.GetSaveDisabled())
    {
        if (StorageManager.GetSaveDeviceSelected(m_iPad))
        {
            // saving is disabled, but we should still be able to load from a selected
            // save device
    ...
    {
        // 4J-PB - we need to check that there is enough space left to create a copy
        // of the save (for a rename)
        bool bCanRename = StorageManager.EnoughSpaceForAMinSaveGame();
        ui.SetTooltips(DEFAULT_XUI_MENU_USER, IDS_TOOLTIPS_SELECT,
                       IDS_TOOLTIPS_BACK, IDS_TOOLTIPS_CHANGEDEVICE, iY, -1, -1, -1,
    ...
    for (int i = 0; i < iTotalSaveInfoC; i++)
    {
        ...
        if (!(app.DebugSettingsOn() && app.GetLoadSavesFromFolderEnabled()))
        {
            ...
            if (InfoA[i].dwImageBytes != 0)
            {
                ...
                hr = XuiCreateTextureBrushFromMemory(pbCurrentImagePtr,
                                                     InfoA[i].dwImageBytes, &hXuiBrush);
                pClass->m_pSavesList->UpdateGraphic(i + pClass->m_iDefaultButtonsC,
                                                    hXuiBrush);
            }
            else
            {
