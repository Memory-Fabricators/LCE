...
void CXuiSceneAbstractContainer::PlatformInitialize(int iPad, int startIndex)
{
    ...
    // Determine min and max extents for pointer, it needs to be able to move off
    // the container to drop items.
    ...
    for (int iSection = m_eFirstSection; iSection < m_eMaxSection; ++iSection)
    {
        ...
        for (int iItem = 0; iItem < iNumItems; ++iItem)
        {
            ...
            GetSectionSlotList(eSection)->GetCXuiCtrlSlotItem(iItem,
                                                              &(pCXuiCtrlSlotItem));
...
HRESULT
CXuiSceneAbstractContainer::OnTransitionStart(XUIMessageTransition *pTransition,
                                              BOOL &bHandled)
{
    ...
    {
        // 4J Stu - Added to support auto-save. Need to re-associate on a navigate
        // back
        ...
        hr =
            XuiElementGetChildById(hObj, L"text_measurer", &m_hPointerTextMeasurer);
                                              ...
                                              BOOL &bHandled)
{
    ...
    bHandled =
        handleKeyDown(pInputData->UserIndex, mapVKToAction(pInputData->dwKeyCode),
...
CXuiSceneAbstractContainer::getSlotItem(ESceneSection eSection, int iSlot)
{
    CXuiCtrlSlotItemListItem *pCXuiCtrlSlotItem;
                                            ...
                                            BOOL &bHandled)
{
    ...
    {
        // Some scenes may have their own timers for other events, so handle them
        // here
...
HRESULT
CXuiSceneAbstractContainer::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                              BOOL &bHandled)
{
    bHandled = true;
...
void CXuiSceneAbstractContainer::SetPointerText(
    const wstring &description, vector<wstring> &unformattedStrings,
    bool newSlot)
{
    ...
    wstring desc = L"<font size=\"" + _toString<int>(smallPointer ? 12 : 14) +
                   L"\">" + description + L"</font>";

    XUIRect tempXuiRect, xuiRect;
    HRESULT hr;
    xuiRect.right = 0;

    for (AUTO_VAR(it, unformattedStrings.begin()); it != unformattedStrings.end();
         ++it)
    {
        XuiTextPresenterMeasureText(m_hPointerTextMeasurer,
                                    parseXMLSpecials((*it)).c_str(), &tempXuiRect);
    ...
    // Set the new height
    XuiElementSetBounds(m_hPointerTextBkg, xuiRect.right + 4.0f + 4.0f + 4.0f,
                        xuiRect.bottom + 4.0f +
                            4.0f); // edge graphics are 8 pixels, with 4 for the
                                   // border, extra 4 for the background is fudge
    XuiElementSetBounds(m_hPointerText, xuiRect.right + 4.0f + 4.0f,
                        xuiRect.bottom + 4.0f +
...
void CXuiSceneAbstractContainer::adjustPointerForSafeZone()
{
                                            ...
                                            baseHeight))
    {
        ...
        if (((pointerBackgroundMatrix._41 / pointerBackgroundMatrix._11) + width) >
            (baseSceneOrigin.x + baseWidth))
        {
            // app.DebugPrintf("Pointer is outside the safe zone for this base
            // scene\n");

            // get the size of the button, and apply the change in size due to the
            // text to the whole button
            float fImgWidth, fImgHeight;
            XuiElementGetBounds(m_hPointerImg, &fImgWidth, &fImgHeight);
            // 4J-PB - changing to calculate values
            D3DXVECTOR3 vPosText, vPosTextBkg, vPosImg;
