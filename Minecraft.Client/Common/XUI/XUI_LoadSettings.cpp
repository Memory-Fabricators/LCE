                                        ...
                                        BOOL &bHandled)
{
    ...
    // 4J-PB - read the settings for the online flag. We'll only save this setting
    ...
    if (m_bMultiplayerAllowed)
    {
        ...
        if (bGameSetting_Online)
        {
            m_MoreOptionsParams.bInviteOnly =
                (app.GetGameSettings(m_iPad, eGameSetting_InviteOnly) != 0) ? TRUE
                                                                            : FALSE;
    ...
    else
    {
        ...
        if (bGameSetting_Online)
        {
            // The profile settings say Online, but either the player is offline, or
            // they are not allowed to play online
    ...
    else
    {
        ...
        {
            DWORD dwResult =
                XContentGetThumbnail(ProfileManager.GetPrimaryPad(), &m_XContentData,
        ...
        else
        {
            ...
            app.GetImageTextData(pbImageData, dwImageBytes,
                                 ...
                                 bHostOptionsRead, m_MoreOptionsParams.dwTexturePack);
            XuiCreateTextureBrushFromMemory(pbImageData, dwImageBytes, &m_hXuiBrush);

            // #ifdef _DEBUG
            // 			// dump out the thumbnail
            // 			HANDLE hThumbnail =
            // CreateFile("GAME:\\thumbnail.png", GENERIC_WRITE, 0, NULL, OPEN_ALWAYS,
            // FILE_FLAG_RANDOM_ACCESS, NULL); 			DWORD dwBytes;
            ...
            if (m_szSeed[0] != 0)
            {
                swprintf((WCHAR *)TempString, 256, L"%ls: %hs", app.GetString(IDS_SEED),
                         m_szSeed);
        ...
        // Setup all the text and checkboxes to match what the game was saved with
        // on
        if (bHostOptionsRead)
        {
            m_MoreOptionsParams.bPVP =
                app.GetGameHostOption(uiHostOptions, eGameHostOption_PvP) > 0 ? TRUE
                                                                              : FALSE;
            m_MoreOptionsParams.bTrust =
                app.GetGameHostOption(uiHostOptions, eGameHostOption_TrustPlayers) > 0
                    ? TRUE
                    : FALSE;
            m_MoreOptionsParams.bFireSpreads =
                app.GetGameHostOption(uiHostOptions, eGameHostOption_FireSpreads) > 0
                    ? TRUE
                    : FALSE;
            m_MoreOptionsParams.bTNT =
                app.GetGameHostOption(uiHostOptions, eGameHostOption_TNT) > 0 ? TRUE
                                                                              : FALSE;
            m_MoreOptionsParams.bHostPrivileges =
                app.GetGameHostOption(uiHostOptions, eGameHostOption_CheatsEnabled) >
                        0
                    ? TRUE
                    : FALSE;
    ...
    XUIMessageControlNavigate *pControlNavigateData, BOOL &bHandled)
{
    pControlNavigateData->hObjDest = XuiControlGetNavigation(
        pControlNavigateData->hObjSource, pControlNavigateData->nControlNavigate,
        TRUE, TRUE);
...
{
    // stop the timer running that causes a check for new texture packs in TMS but
    // not installed, since this will run all through the load game, and will
    // crash if it tries to create an hbrush
        ...
        m_MoreOptionsParams.bHostPrivileges == TRUE)
    {
        ...
        if (m_bGameModeSurvival != true || m_bHasBeenInCreative)
        {
            // 4J-PB - Need different text for Survival mode with a level that has
            // been saved in Creative
            if (m_bGameModeSurvival)
            {
                StorageManager.RequestMessageBox(
                    IDS_TITLE_START_GAME, IDS_CONFIRM_START_SAVEDINCREATIVE, uiIDA, 2,
                    m_iPad, &CScene_LoadGameSettings::ConfirmLoadReturned, this,
                    app.GetStringTable());
            }
            else // it's creative mode
            {
                ...
                {
                    // 4J-PB - We don't really need to tell the user this will have
                    // achievements disabled, since they already saved it in creative and
                    // they got the warning then inform them that leaderboard writes and
                    ...
                    else
                    {
                        ...
                        if (eLoadStatus == C4JStorage::ELoadGame_DeviceRemoved)
                        {
                            ...
                            StorageManager.RequestMessageBox(
                                ...
                                uiIDA, 1, m_iPad,
                                &CScene_LoadGameSettings::DeviceRemovedDialogReturned, this);
                        }
                    }
                }
                else
        ...
        {
            StorageManager.RequestMessageBox(
                IDS_TITLE_START_GAME, IDS_CONFIRM_START_HOST_PRIVILEGES, uiIDA, 2,
                m_iPad, &CScene_LoadGameSettings::ConfirmLoadReturned, this,
                app.GetStringTable());
        }
    }
    else
    {
        ...
        else
        {
            C4JStorage::ELoadGameStatus eLoadStatus = StorageManager.LoadSaveData(
                &m_XContentData, CScene_LoadGameSettings::LoadSaveDataReturned, this);
            ...
            if (eLoadStatus == C4JStorage::ELoadGame_DeviceRemoved)
            {
                ...
                uiIDA[0] = IDS_OK;
                StorageManager.RequestMessageBox(
                    IDS_STORAGEDEVICEPROBLEM_TITLE, IDS_FAILED_TO_LOADSAVE_TEXT, uiIDA,
                    1, m_iPad, &CScene_LoadGameSettings::DeviceRemovedDialogReturned,
                    this);
            }
        }
    }
    ...
    HXUIOBJ hObjPressed, XUINotifyPress *pNotifyPressData, BOOL &rfHandled)
{
    ...
    if (hObjPressed == m_ButtonLoad)
    {
        ...
        if (m_MoreOptionsParams.dwTexturePack != 0)
        {
            ...
            if (pTexturePack == NULL)
            {
                ...
                // 4J-PB - if the full offer id is 0, then the texture pack dlc load
                // failed
            ...
            else
            {
                    ...
                    !m_pDLCPack->hasPurchasedFile(DLCManager::e_DLCType_Texture, L""))
                {
                    ...
                    else
                    {
                        ...
                        DLC_INFO *pDLCInfo =
                            app.GetDLCInfoForTrialOfferID(m_pDLCPack->getPurchaseOfferId());
                        ULONGLONG ullOfferID_Full;
                        ...
                        StorageManager.RequestMessageBox(
                            ...
                            &CScene_LoadGameSettings::UnlockTexturePackReturned, this,
                            app.GetStringTable());
                        return S_OK;
                    }
                }
    ...
    else if (hObjPressed == m_MoreOptions)
    {
        app.NavigateToScene(pNotifyPressData->UserIndex,
                            eUIScene_LaunchMoreOptionsMenu, &m_MoreOptionsParams);
    }
        ...
        else
        {
            ...
            if (eLoadStatus == C4JStorage::ELoadGame_DeviceRemoved)
            {
                ...
                StorageManager.RequestMessageBox(
                    IDS_STORAGEDEVICEPROBLEM_TITLE, IDS_FAILED_TO_LOADSAVE_TEXT, uiIDA,
                    1, pClass->m_iPad,
                    &CScene_LoadGameSettings::DeviceRemovedDialogReturned, pClass);
            }
        }
    }
    else
    ...
    case CHECKFORAVAILABLETEXTUREPACKS_TIMER_ID:
        {
            ...
            CXuiCtrl4JList::LIST_ITEM_INFO ListInfo;
            // for each iConfig, check if the data is available, and add it to the List,
            // then remove it from the viConfig
                                                  ...
                                                  bool bContinue)
{
    ...
    if (bContinue == true)
    {
            ...
            !RenderManager.IsHiDef())
        {
            ...
            {
                DWORD dwLocalUsersMask = CGameNetworkManager::GetLocalPlayerMask(
                    ProfileManager.GetPrimaryPad());
    ...
    BOOL &bHandled)
{
    ...
    if (hObjSource == m_SliderDifficulty.GetSlider())
    {
        ...
        swprintf(
            (WCHAR *)TempString, 256, L"%ls: %ls",
            app.GetString(IDS_SLIDER_DIFFICULTY),
            app.GetString(m_iDifficultyTitleSettingA[pNotifyValueChanged->nValue]));
        m_SliderDifficulty.SetText(TempString);
    }
    return S_OK;
}
...
HRESULT
CScene_LoadGameSettings::OnTransitionStart(XUIMessageTransition *pTransition,
                                           BOOL &bHandled)
{
             ...
             pTransition->dwTransType == XUI_TRANSITION_BACKTO)
    {
        ...
        // you were on the More Options scene
...
HRESULT
CScene_LoadGameSettings::OnTransitionEnd(XUIMessageTransition *pTransition,
                                         BOOL &bHandled)
{
    // if(pTransition->dwTransAction==XUI_TRANSITION_ACTION_DESTROY ) return S_OK;
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
    ...
    if (result == C4JStorage::EMessage_ResultAccept)
    {
        if (ProfileManager.IsSignedIn(iPad))
        {
            ...
            // the license change coming in when the offer has been installed will
            // cause this scene to refresh
        }
    }
    else
    {
...
void CScene_LoadGameSettings::UpdateTexturePackDescription(int index)
{
    ...
    if (tp == NULL)
    {
        ...
        app.GetFileFromTPD(eTPDFileType_Comparison, pbData, dwBytes, &pbFileData,
                           &dwFileBytes);
        if (dwFileBytes >= 0 && pbFileData)
        {
            XuiCreateTextureBrushFromMemory(pbFileData, dwFileBytes,
...
void CScene_LoadGameSettings::UpdateCurrentTexturePack()
{
    ...
    if (tp == NULL)
    {
        ...
        // Need to check if the texture pack has both Full and Trial versions - we
        // may do some as free ones, so only Full
        DLC_INFO *pDLCInfo = app.GetDLCInfoForFullOfferID(ullOfferID_Full);
        ...
        // do set the texture pack id, and on the user pressing create world, check
        // they have it
    ...
    void *pParam, int iPad, C4JStorage::EMessageResult result)
{
        ...
        result == C4JStorage::EMessage_ResultAccept)
    {
        ...
        // get the current index of the list, and then get the data
        ListItem =
            pClass->m_pTexturePacksList->GetData(pClass->m_currentTexturePackIndex);
        app.GetDLCFullOfferIDForPackID(ListItem.iData, &ullOfferID_Full);
