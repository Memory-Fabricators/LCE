...
HRESULT
CScene_TextEntry::OnNotifyValueChanged(HXUIOBJ hObjSource,
...
#ifdef __PS3__
                // 4J Stu - The Xbox version uses swscanf_s which isn't available in GCC.
                swscanf(wsText.c_str(), L"%40s%c%d%c%d", wchCommand, wchSep, &x, wchSep,
                        &z);
#else
                swscanf_s(wsText.c_str(), L"%s%c%d%c%d", wchCommand, 40, wchSep, 2, &x,
                          wchSep, 2, &z);
#endif
        ...
        case eCommand_Give:
            {
...
#ifdef __PS3__
                // 4J Stu - The Xbox version uses swscanf_s which isn't available in GCC.
                swscanf(wsText.c_str(), L"%40s%c%d%c%d", wchCommand, wchSep, &iItem,
                        wchSep, &iCount);
#else
                swscanf_s(wsText.c_str(), L"%s%c%d%c%d", wchCommand, 40, wchSep, 2,
                          &iItem, wchSep, 2, &iCount);
#endif
                ...
                for (int i = 0; i < iCount; i++)
                {
                    pMinecraft->localplayers[m_iPad]
                        ->drop(); // shared_ptr<ItemInstance>(new ItemInstance( iItem, 1, 0
                                  // )) );
                }
            }
