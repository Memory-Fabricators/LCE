...
HRESULT CXuiSceneTrading::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    if (pMinecraft->localgameModes[m_iPad] != NULL)
    {
        ...
        gameMode->getTutorial()->changeTutorialState(e_Tutorial_State_Trading_Menu,
                                                     this);
    }

    m_menu =
        new MerchantMenu(initData->inventory, initData->trader, initData->level);
...
HRESULT CXuiSceneTrading::OnDestroy()
{
    ...
    // 4J Stu - Fix for #11302 - TCR 001: Network Connectivity: Host crashed after
    // being killed by the client while accessing a chest during burst packet
    // loss. We need to make sure that we call closeContainer() anytime this menu
    // is closed, even if it is forced to close by some other reason (like the
    // player dying)
...
void CXuiSceneTrading::setOfferDescription(
    const wstring &name, vector<wstring> &unformattedStrings)
{
    if (name.empty())
    {
    ...
    XuiElementSetBounds(m_hOfferInfoTextBkg, backgroundWidth,
                        xuiRect.bottom + 4.0f +
                            4.0f); // edge graphics are 8 pixels, with 4 for the
                                   // border, extra 4 for the background is fudge
    XuiElementSetBounds(m_hOfferInfoText, xuiRect.right + 4.0f + 4.0f,
                        xuiRect.bottom + 4.0f +
    ...
    else
    {
        // Display on the left
        offerInfoPos.x =
            Math::round(highlightPos.x - backgroundWidth - highlightWidth * 0.1);
    }
