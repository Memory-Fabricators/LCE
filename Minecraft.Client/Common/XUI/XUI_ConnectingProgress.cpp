    ...
    HXUIOBJ hObjPressed, XUINotifyPress *pNotifyPressData, BOOL &rfHandled)
{
    ...
    if (hObjPressed == m_buttonConfirm)
    {
        ...
        {
            // The connection failed if we see the button, so the temp player should
            // be removed and the viewports updated again
...
HRESULT
CScene_ConnectingProgress::OnTransitionStart(XUIMessageTransition *pTransition,
                                             BOOL &bHandled)
{
...
HRESULT
CScene_ConnectingProgress::OnTransitionEnd(XUIMessageTransition *pTransition,
                                           BOOL &bHandled)
{
