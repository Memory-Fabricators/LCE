...
void CXuiCtrlMinecraftSkinPreview::SetTexture(const wstring &url,
                                              TEXTURE_NAME backupTexture)
{
    ...
    if (app.GetGameSettings(eGameSetting_CustomSkinAnim) == 0)
    {
        ...
        if ((uiAnimOverrideBitmask &
             HumanoidModel::m_staticBitmaskIgnorePlayerCustomAnimSetting) != 0)
        {
                                               ...
                                               BOOL &bHandled)
{
    if (m_bAnimatingToFacing)
    {
        ...
        m_yRot =
            m_fOriginalRotation +
            m_framesAnimatingRotation *
                ((m_fTargetRotation - m_fOriginalRotation) / CHANGING_SKIN_FRAMES);
    ...
    float xo = ((matrix._41 + ((bwidth * matrix._11) / 2)) / m_fScreenWidth) *
               m_fRawWidth;
    float yo =
        ((matrix._42 + (bheight * matrix._22)) / m_fScreenHeight) * m_fRawHeight;
    ...
    // EntityRenderDispatcher::instance->render(pMinecraft->localplayers[0], 0, 0,
    // 0, 0, 1);
                                          ...
                                          float a)
{
    ...
    if (bindTexture(m_capeTextureUrl, L""))
    {
        ...
        flap += 1; // sin((mob->walkDistO + (mob->walkDist - mob->walkDistO) * a) *
                   // 6) * 32 * pow;
        if (model->sneaking)
        {
            flap += 25;
  ...
  false);
    ...
    */
    ...
    // Reset the model values to stop the changes we made here affecting anything
    // in game (like the player hand render)
    model->attackTime = 0;
    model->sneaking = false;
    model->holdingRightHand = false;
    model->holdingLeftHand = false;
...
bool CXuiCtrlMinecraftSkinPreview::bindTexture(const wstring &urlTexture,
                                               int backupTexture)
{
