...
HRESULT CXuiSceneAnvil::OnInit(XUIMessageInit *pInitData, BOOL &bHandled)
{
    ...
    if (pMinecraft->localgameModes[m_iPad] != NULL)
    {
        ...
        gameMode->getTutorial()->changeTutorialState(e_Tutorial_State_Anvil_Menu,
                                                     this);
    }
    ...
    CXuiSceneAbstractContainer::Initialize(m_iPad, m_repairMenu, true,
...
HRESULT
CXuiSceneAnvil::OnNotifyValueChanged(HXUIOBJ hObjSource,
                                     ...
                                     BOOL &rfHandled)
{
    if (hObjSource == m_editName)
    {
        ...
        else
        {
            if (end == wstring::npos)
            {
                end = (int)newValue.size() - 1;
            }
            b = newValue.substr(start, (end - start) + 1);
            newValue = b;
        }

        // 4J-PB - This was stopping the player deleting the last character in the
        // name using the chatpad
                                          ...
                                          int startIndex /*= 0*/)
{
    ...
    CXuiSceneAbstractContainer::InitDataAssociations(iPad, menu,
                                                     RepairMenu::INV_SLOT_START);
}
