...
class CXuiCtrlSlotItemCtrlBase
{
  ...
  public:
    ...
    // call the protected version by passing in the HXUIOBJ of the control as the
    // first parameter. We do not have access to that normally due to the
    ...
    virtual HRESULT
    OnCustomMessage_GetSlotItem(CustomMessage_GetSlotItem_Struct *pData,
                                BOOL &bHandled) = 0;

    // 4J WESTY : Pointer Prototype : Added to support prototype only.
    virtual HRESULT
    OnControlNavigate(XUIMessageControlNavigate *pControlNavigateData,
                      BOOL &bHandled) = 0;
