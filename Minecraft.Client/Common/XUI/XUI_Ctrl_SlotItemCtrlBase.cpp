    ...
    HXUIOBJ hObj, CustomMessage_GetSlotItem_Struct *pData, BOOL &bHandled)
{
    ...
    {
        shared_ptr<Player> player = dynamic_pointer_cast<Player>(
    ...
    if (item != NULL)
    {
        ...
        pData->iDataBitField = MAKE_SLOTDISPLAY_DATA_BITMASK(
            pUserDataContainer->m_iPad, (int)(31 * pUserDataContainer->m_fAlpha),
            true, item->GetCount(), 7, item->popTime);
    }
    else
    {
...
bool CXuiCtrlSlotItemCtrlBase::isEmpty(HXUIOBJ hObj)
{
             ...
             pUserDataContainer->m_iPad < XUSER_MAX_COUNT)
    {
        ...
        if (player != NULL)
        {
            return player->inventory->getCarried() == NULL;
        }
    }
    return true;
}

wstring CXuiCtrlSlotItemCtrlBase::GetItemDescription(
    HXUIOBJ hObj, vector<wstring> &unformattedStrings)
{
    void *pvUserData;
    XuiElementGetUserData(hObj, &pvUserData);
    SlotControlUserDataContainer *pUserDataContainer =
    ...
    if (pUserDataContainer->slot != NULL)
    {
        ...
        for (AUTO_VAR(it, strings->begin()); it != strings->end(); ++it)
        {
            ...
            else
            {
                ...
                swprintf(formatted, 256, L"<font color=\"#%08x\">%s</font>", colour,
                         thisString.c_str());
...
CXuiCtrlSlotItemCtrlBase::getItemInstance(HXUIOBJ hObj)
{
    void *pvUserData;
    XuiElementGetUserData(hObj, &pvUserData);
    SlotControlUserDataContainer *pUserDataContainer =
                                            ...
                                            HXUIOBJ hOtherObj)
{
    ...
    {
        shared_ptr<Player> player = dynamic_pointer_cast<Player>(
            Minecraft::GetInstance()->localplayers[pThisUserDataContainer->m_iPad]);
        if (player != NULL && player->inventory->getCarried() != NULL)
        {
    ...
    if (pUserDataContainer->slot != NULL)
    {
        if (pUserDataContainer->slot->hasItem())
        {
            ...
            if (bStackable)
            {
                iCount = pUserDataContainer->slot->getItem()->GetCount();
                iMaxStackSize =
                    min(pUserDataContainer->slot->getItem()->getMaxStackSize(),
