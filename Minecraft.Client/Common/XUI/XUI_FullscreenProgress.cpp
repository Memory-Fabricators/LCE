                                             ...
                                             BOOL &rfHandled)
{
    ...
    case VK_PAD_B:
    case VK_ESCAPE:
        // 4J-JEV: Fix for Xbox360 #162749 - TU17: Save Upload: Content: UI: Player
        // is presented with non-functional Tooltips after the Upload Save For Xbox
        // One is completed.
...
HRESULT
CScene_FullscreenProgress::OnTransitionStart(XUIMessageTransition *pTransition,
                                             BOOL &bHandled)
{
    if (!threadStarted)
    {
    ...
    if (exitcode != STILL_ACTIVE)
    {
        ...
        if (exitcode != S_OK)
        {
            if (exitcode == ERROR_CANCELLED)
            {
                ...
                // CConsoleMinecraftApp::RemoteSaveThreadProc thread Assume to just
                // ignore this thread as something else is now running that will cause
                // another action
            }
            else
            {
        ...
        else
        {
            ...
            else
            {
                ...
                case e_ProgressCompletion_AutosaveNavigateBack:
                    ...
                    {
                        // store these - they get wiped by the destroy caused by
                        // navigateback
                        ...
                        // 4J Stu - Fix for #65437 - Customer Encountered: Code: Settings:
                        // Autosave option doesn't work when the Host goes into idle state
                        // during gameplay. Autosave obviously cannot occur if an ignore
                        // autosave menu is displayed, so even if we navigate back to a
                        // scene and not empty then we still want to reset this flag which
                        // was set true by the navigate to the fullscreen progress
                        app.SetIgnoreAutosaveMenuDisplayed(iPad, false);

                        // the navigate back leaves SetMenuDisplayed as true, but there may
                        // not have been a menu up when autosave was kicked off
