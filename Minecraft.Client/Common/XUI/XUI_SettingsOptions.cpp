                                       ...
                                       BOOL &bHandled)
{
    ...
    m_ViewBob.SetCheck(
        (app.GetGameSettings(m_iPad, eGameSetting_ViewBob) != 0) ? TRUE : FALSE);
    ...
    m_Tooltips.SetCheck(
        (app.GetGameSettings(m_iPad, eGameSetting_Tooltips) != 0) ? TRUE : FALSE);
    ...
    if (bRemoveDifficulty)
    {
        ...
        hr = XuiElementGetChildById(m_SliderA[SLIDER_SETTINGS_DIFFICULTY].m_hObj,
                                    L"XuiSlider", &hSlider);
    ...
    if (hObjSource == m_SliderA[SLIDER_SETTINGS_AUTOSAVE].GetSlider())
    {
        ...
        else
        {
            ...
            swprintf((WCHAR *)TempString, 256, L"%ls: %d %ls",
                     ...
                     pNotifyValueChanged->nValue * 15, app.GetString(IDS_MINUTES));
        }
        m_SliderA[SLIDER_SETTINGS_AUTOSAVE].SetText(TempString);
    }
...
HRESULT
CScene_SettingsOptions::OnTransitionStart(XUIMessageTransition *pTransition,
                                          BOOL &bHandled)
{
    HRESULT hr;
    WCHAR TempString[256];
        ...
        pTransition->dwTransType == XUI_TRANSITION_BACKTO)
    {
        ...
        // have...
        hr = XuiTextPresenterMeasureText(hCheckboxText, m_InGameGamertags.GetText(),
                                         &xuiRect);
        m_InGameGamertags.GetBounds(&fWidth, &fHeight);
        ...
        hr =
            XuiTextPresenterMeasureText(hCheckboxText, m_Hints.GetText(), &xuiRect);
        ...
        if (fMaxLen < fMaxTextLen)
        {
            ...
            {
                float fDiffTextW;
                // reposition the difficulty text - this is positioned from the scene,
            ...
            // the checkboxes are left aligned - put them at the same position as the
            // sliders
...
HRESULT
CScene_SettingsOptions::OnCustomMessage_Splitscreenplayer(bool bJoining,
                                                          BOOL &bHandled)
{
    bHandled = true;
