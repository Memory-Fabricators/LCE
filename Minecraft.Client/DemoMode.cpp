...
void DemoMode::tick()
{
    ...
    /* 4J - TODO - seems unlikely we need this demo mode anyway
        std::int64_t time = minecraft->level->getTime();
        std::int64_t day = (time / Level::TICKS_PER_DAY) + 1;
        ...
        if ((time % Level::TICKS_PER_DAY) == 500)
        {
            if (day <= (DEMO_DAYS + 1))
            {
                minecraft->gui->displayClientMessage(L"demo.day." + _toString<std::int64_t>(day));
            }
        }
        else if (day == 1)
        {
