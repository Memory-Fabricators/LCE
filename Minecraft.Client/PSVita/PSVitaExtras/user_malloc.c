...
typedef struct
{
    Block **Malloc_MemoryPool; // this is an array of available memory allocations
                               // up to 1036 bytes in size
    Block *Malloc_Blocks;      // this is a stack of available block nodes used to
                               // store memory chunks
    Block *Malloc_BlocksMemory
        [Malloc_BlocksMemorySize]; // this is a pool of block nodes allocated in
                                   // large chunks (256 blocks at a time)
    int Malloc_BlocksAlloced;      // this shows how many block node chunks have been
                                   // allocated in Malloc_BlocksMemory
} SThreadStorage;
...
void *user_malloc(size_t size)
{
    ...
    if (psStorage)
    {
        ...
        {
            // create an array of pointers to Block nodes, one pointer for each memory
            // bytes size up to 1036
        ...
        // are we interested in retaining this size of memory (less 4 bytes to store
        ...
        if (size < (MaxRetainedBytes - 4))
        {
            ...
            // round to the nearest malloc boundary. This is what happens internally
            // in the malloc library
            if (size <= 12)
            {
                size = 12;
            }
...
void user_free(void *ptr)
{
    ...
    if (psStorage)
    {
        ...
        if (RoundedSize < (MaxRetainedBytes + 1))
        {
            ...
            {
                // don't worry about retaining memory allocated from a different thread.
                // too much mucking about
                mspace_free(s_mspace, ptr);
                return;
            }

            // we need a block node to retain the memory on our stack. do we have any
            // block nodes available
            if (psStorage->Malloc_Blocks == NULL)
            {
                if (psStorage->Malloc_BlocksAlloced == Malloc_BlocksMemorySize)
                {
                    // Max blocks allocated
                    // The worst case for this running out is when the player has explored
                    // a large amount of the map in a single session and then quits. All
                    // the allocations are suddenly freed up so we need lots of space to
                    // store them.
                    printf(
                        "oh nos, max blocks allocated. increase Malloc_BlocksMemorySize");
                    assert(0);
                }

                // allocate some more block space in a large chunk
                int chunkSize = 256;
                psStorage->Malloc_BlocksMemory[psStorage->Malloc_BlocksAlloced] =
                ...
                for (int i = 0; i < chunkSize; i += 1)
                {
                    Block *NewBlock =
                        &psStorage
                             ->Malloc_BlocksMemory[psStorage->Malloc_BlocksAlloced][i];
            ...
            // set up the block data and retain it to the correct slot. We can now
            // reuse this memory on the next malloc
            NewBlock->Memory = ptr;
