...
BOOL CConsoleMinecraftApp::ReadProductCodes()
{
    ...
    // HANDLE file = CreateFile("orbis/DLCImages/TP01_360x360.png", GENERIC_READ,
    // 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    HANDLE file = CreateFile("orbis/PS4ProductCodes.bin", GENERIC_READ, 0, NULL,
    ...
    if (dwFileSize != 0)
    {
        ...
        for (unsigned int i = 0; i < uiDLC; i++)
        {
            ...
            WRAPPED_READFILE(file, &uiVal, sizeof(int), &bytesRead, NULL);
            WRAPPED_READFILE(file, pDLCInfo->chDLCPicname, sizeof(char) * uiVal,
                             &bytesRead, NULL);
...
int CConsoleMinecraftApp::LoadLocalDLCImage(SONYDLC *pDLCInfo)
{
    ...
    sprintf(pchFilename, "orbis/DLCImages/%s_360x360.png",
            pDLCInfo->chDLCPicname);
    // 4J-PB - Read the file containing the product codes. This will be different
    // for the SCEE/SCEA/SCEJ builds
    HANDLE hFile = CreateFile(pchFilename, GENERIC_READ, 0, NULL, OPEN_EXISTING,
    ...
    if (pDLCInfo->dwImageBytes != 0)
    {
        ...
        if (ReadFile(hFile, pDLCInfo->pbImageData, pDLCInfo->dwImageBytes,
...
// 		{
// 			std::vector<SonyCommerce::ProductInfo>*
// pProductList=&m_ProductListA[i]; 			AUTO_VAR(itEnd,
// pProductList->end());
//
// 			for (AUTO_VAR(it, pProductList->begin()); it != itEnd;
// it++)
// 			{
// 				SonyCommerce::ProductInfo Info=*it;
        ...
        for (int i = 0; i < status.eventNum; i++)
        {
            ...
            if (ret == SCE_OK)
            {
                ...
                case SCE_SYSTEM_SERVICE_EVENT_GAME_CUSTOM_DATA:
                    {
                        OrbisNPToolkit::getMessageData(
                            (SceNpGameCustomDataEventParam *)event.data.param);
                        // Processing after invitation
                        // SceNpSessionInvitationEventParam* pInvite =
                ...
                case SCE_SYSTEM_SERVICE_EVENT_ON_RESUME:
                    // Resume means that the user signed out (but came back), sensible
                    // thing to do is exit to main menu
...
void CConsoleMinecraftApp::SaveDataDialogTick()
{
    if (m_bOptionsSaveDataDialogRunning)
    {
        ...
        if (status == SCE_COMMON_DIALOG_STATUS_FINISHED)
        {
            ...
            // R4099 doesn't say we need give the user a delete dialog to create
            // space, so we won't
        }
    }
    else if (m_bSaveDataDialogRunning)
    {
            ...
            status == SCE_COMMON_DIALOG_STATUS_NONE)
        {
            ...
            if (ui.RequestMessageBox(
                    IDS_SAVE_INCOMPLETE_TITLE, message, uiIDA, 3, XUSER_INDEX_ANY,
                    Callback_SaveGameIncompleteMessageBoxReturned, this,
                    app.GetStringTable()) == C4JStorage::EMessage_Busy)
            {
                // If this failed to display, continue as if we cancelled. This isn't
                // ideal, but the user should already have had some system notification
                // of being out of memory, and if we instantly retry then they may not
                // be able to navigate whatever other error is blocking this from
                // appearing
                Callback_SaveGameIncompleteMessageBoxReturned(
                    this, 0, C4JStorage::EMessage_Cancelled);
            }
        }
    }
}
    ...
    if (m_bPatchAvailableDialogRunning)
    {
        ...
        if (stat == SCE_ERROR_DIALOG_STATUS_FINISHED)
        {
            ...
            ui.RequestMessageBox(IDS_ONLINE_SERVICE_TITLE,
                                 IDS_CONTENT_RESTRICTION_PATCH_AVAILABLE, uiIDA, 1,
