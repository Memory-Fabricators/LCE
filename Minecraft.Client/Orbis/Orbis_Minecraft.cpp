...
#define FIFTY_ONE_MB \
    (1000000 * 51) // Maximum TCR space required for a save is 52MB (checking for
                   // this on a selected device)
...
{
    // The app needs to define the actions required, and the possible mappings for
    // these
    ...
    InputManager.SetGameJoypadMaps(MAP_STYLE_1, ACTION_MENU_Y,
                                   _PS4_JOY_BUTTON_TRIANGLE);
    InputManager.SetGameJoypadMaps(MAP_STYLE_1, ACTION_MENU_UP,
                                   _PS4_JOY_BUTTON_DPAD_UP |
                                       _PS4_JOY_BUTTON_LSTICK_UP);
    InputManager.SetGameJoypadMaps(MAP_STYLE_1, ACTION_MENU_DOWN,
                                   _PS4_JOY_BUTTON_DPAD_DOWN |
                                       _PS4_JOY_BUTTON_LSTICK_DOWN);
    InputManager.SetGameJoypadMaps(MAP_STYLE_1, ACTION_MENU_LEFT,
                                   _PS4_JOY_BUTTON_DPAD_LEFT |
                                       _PS4_JOY_BUTTON_LSTICK_LEFT);
    InputManager.SetGameJoypadMaps(MAP_STYLE_1, ACTION_MENU_RIGHT,
                                   _PS4_JOY_BUTTON_DPAD_RIGHT |
                                       _PS4_JOY_BUTTON_LSTICK_RIGHT);
    ...
    InputManager.SetGameJoypadMaps(MAP_STYLE_2, ACTION_MENU_Y,
                                   _PS4_JOY_BUTTON_TRIANGLE);
    InputManager.SetGameJoypadMaps(MAP_STYLE_2, ACTION_MENU_UP,
                                   _PS4_JOY_BUTTON_DPAD_UP |
                                       _PS4_JOY_BUTTON_LSTICK_UP);
    InputManager.SetGameJoypadMaps(MAP_STYLE_2, ACTION_MENU_DOWN,
                                   _PS4_JOY_BUTTON_DPAD_DOWN |
                                       _PS4_JOY_BUTTON_LSTICK_DOWN);
    InputManager.SetGameJoypadMaps(MAP_STYLE_2, ACTION_MENU_LEFT,
                                   _PS4_JOY_BUTTON_DPAD_LEFT |
                                       _PS4_JOY_BUTTON_LSTICK_LEFT);
    InputManager.SetGameJoypadMaps(MAP_STYLE_2, ACTION_MENU_RIGHT,
                                   _PS4_JOY_BUTTON_DPAD_RIGHT |
                                       _PS4_JOY_BUTTON_LSTICK_RIGHT);
    InputManager.SetGameJoypadMaps(MAP_STYLE_2, ACTION_MENU_PAGEUP,
    ...
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, ACTION_MENU_Y,
                                   _PS4_JOY_BUTTON_TRIANGLE);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, ACTION_MENU_UP,
                                   _PS4_JOY_BUTTON_DPAD_UP |
                                       _PS4_JOY_BUTTON_LSTICK_UP);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, ACTION_MENU_DOWN,
                                   _PS4_JOY_BUTTON_DPAD_DOWN |
                                       _PS4_JOY_BUTTON_LSTICK_DOWN);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, ACTION_MENU_LEFT,
                                   _PS4_JOY_BUTTON_DPAD_LEFT |
                                       _PS4_JOY_BUTTON_LSTICK_LEFT);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, ACTION_MENU_RIGHT,
                                   _PS4_JOY_BUTTON_DPAD_RIGHT |
                                       _PS4_JOY_BUTTON_LSTICK_RIGHT);
    ...
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, MINECRAFT_ACTION_ACTION,
                                   _PS4_JOY_BUTTON_R1);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, MINECRAFT_ACTION_RIGHT_SCROLL,
                                   _PS4_JOY_BUTTON_R2 |
                                       _PS4_JOY_BUTTON_DPAD_RIGHT);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, MINECRAFT_ACTION_LEFT_SCROLL,
                                   _PS4_JOY_BUTTON_L2 |
                                       _PS4_JOY_BUTTON_DPAD_LEFT);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3,
                                   MINECRAFT_ACTION_RENDER_THIRD_PERSON,
                                   _PS4_JOY_BUTTON_L3 | _PS4_JOY_BUTTON_DPAD_UP);
    InputManager.SetGameJoypadMaps(MAP_STYLE_3, MINECRAFT_ACTION_SNEAK_TOGGLE,
                                   _PS4_JOY_BUTTON_R3 |
                                       _PS4_JOY_BUTTON_DPAD_DOWN);
...
HRESULT InitDevice()
{
         ...
         driverTypeIndex++)
    {
        ...
        hr = D3D11CreateDeviceAndSwapChain(
            ...
            numFeatureLevels, D3D11_SDK_VERSION, &sd, &g_pSwapChain, &g_pd3dDevice,
    ...
    hr = g_pd3dDevice->CreateDepthStencilView(g_pDepthStencilBuffer, &descDSView,
                                              &g_pDepthStencilView);
...
void RegisterAwardsWithProfileManager()
{
    ...
    ProfileManager.RichPresenceRegisterContextString(
        CONTEXT_GAME_STATE, CONTEXT_GAME_STATE_RIDING_PIG, pchRichPresenceString);
    pchRichPresenceString =
    ...
    ProfileManager.RichPresenceRegisterContextString(
        CONTEXT_GAME_STATE, CONTEXT_GAME_STATE_ENCHANTING, pchRichPresenceString);
    pchRichPresenceString =
...
int main(int argc, const char *argv[])
{
    ...
    StorageManager.Init(0, app.GetString(IDS_DEFAULT_SAVENAME), "savegame.dat",
                        FIFTY_ONE_MB, &CConsoleMinecraftApp::DisplaySavingMessage,
                        (LPVOID)&app, "");
    ...
    // Set the number of possible joypad layouts that the user can switch between,
    ...
    // Initialise the profile manager with the game Title ID, Offer ID, a profile
    // version number, and the number of profile values and settings
    ...
    ProfileManager.Initialise(
        ...
        dwProfileSettingsA, app.GAME_DEFINED_PROFILE_DATA_BYTES * XUSER_MAX_COUNT,
        &app.uiGameDefinedDataChangedBitmask);

    // register the awards
    RegisterAwardsWithProfileManager();

    // register the get string function with the profile lib, so it can be called
    // within the lib
    ...
    StorageManager.SetDefaultImages(
        (PBYTE)baOptionsIcon.data, baOptionsIcon.length, (PBYTE)baSaveImage.data,
        baSaveImage.length, (PBYTE)baSaveThumbnail.data, baSaveThumbnail.length);
    ...
    // Temporary - set a single user for quadrant 0 for the storage manager. Will
    // need to do more to link this aspect of the storage manager is once we have
...
#endif
    // set a function to be called when there's a sign in change, so we can exit a
    // level if the primary player signs out
    ProfileManager.SetSignInChangeCallback(
        &CConsoleMinecraftApp::SignInChangeCallback, (LPVOID)&app);

    // Set a callback for the default player options to be set - when there is no
    // profile data for the player
    ...
    // QNet needs to be setup after profile manager, as we do not want its Notify
    // listener to handle XN_SYS_SIGNINCHANGED notifications. This does mean that
    // we need to have a callback in the ProfileManager for
    ...
    C4JThread *minecraftThread =
        new C4JThread(&StartMinecraftThreadProc, NULL, "Running minecraft start");
    minecraftThread->Run();
    do
    {
...
#pragma message(__LOC__ "Need to define the _FINAL_BUILD before submission")
    ...
    while (TRUE)
    {
        ...
        else
        {
            ...
            {
                pMinecraft->tickAllConnections(); // Added to stop timing out when we
                                                  // are waiting after converting to an
                                                  // offline game
            }
        }
        ...
        if (app.uiGameDefinedDataChangedBitmask != 0)
        {
            ...
            for (int i = 0; i < XUSER_MAX_COUNT; i++)
            {
                ...
                {
                    // It has - game needs to update its values with the data from the
                    // profile
...
#endif
                    // clear the stats first - there could have beena signout and sign
                    // back in in the menus need to clear the player stats - can't assume
                    // it'll be done in setlevel - we may not be in the game
        ...
        {
            // need to turn off the trial timer if it was on , and we've unlocked the
            // full version
