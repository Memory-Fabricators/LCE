...
void ConsoleUIController::init(S32 w, S32 h)
{
    ...
    of a resource pool.  */
    gdraw_orbis_SetResourceMemory(
        GDRAW_ORBIS_RESOURCE_rendertarget, // Pool type
        0,                                 // Number of handles (unused for render targets)
        RenderManager.MemoryAllocateGPUMem(GDRAW_ORBIS_TEXTURE_ALIGNMENT,
                                           rt_mem_size), // Pointer
        rt_mem_size);                                    // Size
    gdraw_orbis_SetResourceMemory(
        GDRAW_ORBIS_RESOURCE_texture, tex_handles,
        RenderManager.MemoryAllocateGPUMem(GDRAW_ORBIS_TEXTURE_ALIGNMENT,
                                           tex_mem_size),
        tex_mem_size);
    gdraw_orbis_SetResourceMemory(
        GDRAW_ORBIS_RESOURCE_vertexbuffer, vert_handles,
        RenderManager.MemoryAllocateGPUMem(GDRAW_ORBIS_VERTEXBUFFER_ALIGNMENT,
                                           vert_mem_size),
        vert_mem_size);
    ...
    double-buffer commands, you need to use the same scheme for the staging
    buffer, and so forth. Since this example uses only one GfxContext, we also use
    a single staging buffer.

    It's not immediately obvious how big the staging buffer should be; this
    example sizes it so it's big enough to replace all resource pools every frame.
    This keeps things simple but is excessive in practice. $gdraw_orbis_End can
    report back how much of the staging buffer was used every frame, so you can
    use this as a guideline to size it appropriately. If the staging buffer is too
    small, Iggy will issue a warning, and some textures or shapes might pop in a
    frame late (because they couldn't be uploaded in time).
    */
    staging_buf_size =
        12 * 1024 * 1024; // This value determined by experimentation - was (with
                          // original comment):  tex_mem_size + vert_mem_size; //
                          // This is very generous!
    staging_buf[0] = RenderManager.MemoryAllocateCPUMem(
        Gnm::kAlignmentOfBufferInBytes, staging_buf_size);
    staging_buf[1] = RenderManager.MemoryAllocateCPUMem(
        Gnm::kAlignmentOfBufferInBytes, staging_buf_size);
    currentStagingBuf = 0;
    ...
    memory for GDraw to store its shaders etc. in. Unlike the rest of the memory
    configuration, the work area is reserved to GDraw and can't be freed or
    relocated afterwards without shutting GDraw and all attached Iggys down
    completely. But it's fairly small (64k as of this writing) so this shouldn't
    be much of a problem. It should be allocated in Garlic memory. */
    void *gdraw_context_mem =
        RenderManager.MemoryAllocateGPUMem(1, GDRAW_ORBIS_CONTEXT_MEM_SIZE);

    /* Finally, we can actually create the GDraw GNM driver and pass it to Iggy.
     */
    gdraw_funcs = gdraw_orbis_CreateContext(w, h, gdraw_context_mem);
...
void ConsoleUIController::render()
{
    ...
    gdraw_orbis_Begin(RenderManager.GetCurrentBackBufferContext(),
                      staging_buf[currentStagingBuf], staging_buf_size);
    currentStagingBuf ^= 1;
    ...
    gdraw_orbis_SetTileOrigin(RenderManager.GetCurrentBackBufferTarget(),
                              RenderManager.GetCurrentBackBufferDepthTarget(), 0,
                              0);
    ...
    IggyCustomDrawCallbackRegion *region, CustomDrawData *customDrawRegion)
{
    ...
    // get the correct object-to-world matrix from GDraw, and set the render state
    // to a normal state
...
CustomDrawData *
ConsoleUIController::setupCustomDraw(UIScene *scene,
...
{
    gdraw_orbis_SetTileOrigin(RenderManager.GetCurrentBackBufferTarget(),
                              RenderManager.GetCurrentBackBufferDepthTarget(),
                              xPos, yPos);
}
...
GDrawTexture *ConsoleUIController::getSubstitutionTexture(int textureId)
{
    ...
    you create and manage on your own. For example, you might render to this
    texture, or stream video into it. Wrapped textures take up a handle. They will
    never be freed or otherwise modified by GDraw; nor will GDraw change any
    reference counts. All this is up to the application. */
