...
class C_4JProfile
{
  public:
    ...
    void InitialiseTrophies(); // const SceNpCommunicationId* commsId, const
    ...
    void RegisterAward(int iAwardNumber, int iGamerconfigID, eAwardType eType,
                       ...
                       char *pszThemeName = NULL, unsigned int uiThemeSize = 0L);
    ...
    void RefreshChatAndContentRestrictions(void (*Func)(void *pParam),
                                           void *pParam);
    ...
    getUserID(int iQuadrant); //  grab the PS4 userID for this quadrant
                              //  (SCE_USER_SERVICE_USER_ID_INVALID if it's not
                              //  signed in)
    int getQuadrant(SceUserServiceUserId userId);
    void SignedInPSNStateCallback(SceUserServiceUserId userId, SceNpState state,
                                  const SceNpId *npId);
