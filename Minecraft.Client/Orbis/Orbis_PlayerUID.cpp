...
{
    //  now only hashing against the local ID, as this is now unique across the
    //  network too.
    std::size_t seed = 0;
...
PlayerUID::PlayerUID(SceUserServiceUserId userID, SceNpOnlineId &onlineID,
                     bool bSignedInPSN, int quadrant)
{
    m_userID = userID;
    m_quadrant = quadrant;
...
std::wstring PlayerUID::macAddressStr() const
{
    ...
    swprintf(macAddr, 16, L"%02x%02x%02x%02x%02x%02x", m[0], m[1], m[2], m[3],
             m[4], m[5]);
...
std::wstring PlayerUID::toString() const
{
    ...
    const uint8_t *m = m_macAddress;
    sprintf(macAddr, "%02x%02x%02x%02x%02x%02x", m[0], m[1], m[2], m[3], m[4],
            m[5]);
    sprintf(finalString, "%s_%s_%08d_%s", isPrimaryUser() ? "P" : "N", macAddr,
            m_userID, isSignedIntoPSN() ? m_onlineID : "");
    mbstowcs(finalStringW, finalString, 64);
...
bool PlayerUID::isPrimaryUser() const /* only true if we're on the local machine
                                         and signed into the first quadrant */
{
    if (m_quadrant != 0)
    {
        return false;
    ...
    return macIsLocal; // only true if we're on the local machine and signed into
                       // the first quadrant
}
