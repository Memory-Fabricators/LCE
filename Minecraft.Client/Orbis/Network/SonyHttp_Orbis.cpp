...
void SonyHttp_Orbis::printSslError(SceInt32 sslErr, SceUInt32 sslErrDetail)
{
    ...
    case (SCE_HTTPS_ERROR_CERT): /* Verify error */
        ...
        {
            app.DebugPrintf(
                "ssl verify error: server cert or CA cert isn't validated yet.\n");
        }
        /* Unknown CA error */
...
bool SonyHttp_Orbis::getDataFromURL(const char *szURL, void **ppOutData,
                                    int *pDataSize)
{
    if (!bInitialised)
    {
...
SceInt32 SonyHttp_Orbis::sslCallback(int libsslCtxId, unsigned int verifyErr,
                                     SceSslCert *const sslCert[], int certNum,
                                     void *userArg)
{
    SceInt32 i;
    (void)userArg;
...
bool SonyHttp_Orbis::http_get_close(bool bOK, SceInt32 tmplId, SceInt32 connId,
                                    SceInt32 reqId)
{
    SceInt32 ret;
    if (reqId > 0)
...
bool SonyHttp_Orbis::http_get(const char *targetUrl, void **ppOutData,
                              int *pDataSize)
{
    ...
    ret = sceHttpCreateTemplate(libhttpCtxId, TEST_USER_AGENT,
    ...
    if (statusCode == 200)
    {
        ...
        ret = sceHttpGetResponseContentLength(reqId, &contentLengthType,
                                              &contentLength);
        if (ret < 0)
        {
