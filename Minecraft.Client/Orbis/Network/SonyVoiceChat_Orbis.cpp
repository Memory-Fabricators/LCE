...
SQRLocalVoiceDevice
    SonyVoiceChat_Orbis::m_localVoiceDevices[MAX_LOCAL_PLAYER_COUNT];
uint32_t SonyVoiceChat_Orbis::m_voiceOutPort;
bool SonyVoiceChat_Orbis::m_forceSendPacket =
...
VoicePacket::Flags
    SonyVoiceChat_Orbis::m_localPlayerFlags[MAX_LOCAL_PLAYER_COUNT];
...
void LoadPCMVoiceData()
{
    for (int i = 0; i < 4; i++)
    {
        ...
        if (g_loadedPCMVoiceDataSizes[i] != 0)
        {
            ...
            DWORD bytesRead;
            BOOL bSuccess = ReadFile(file, g_loadedPCMVoiceData[i],
...
void SonyVoiceChat_Orbis::shutdown()
{
    ...
    {
        app.DebugPrintf(
            "Error: sceSysmoduleUnloadModule(SCE_SYSMODULE_VOICE), ret 0x%08x\n",
            returnCode);
        assert(0);
    }
}
...
void SQRVoiceConnection::SendMoreInternal()
{
    ...
    do
    {
        ...
        if (m_sendQueue.size() > 0)
        {
            ...
            if (ret == dataSize)
            {
                // Fully sent, remove from queue - will loop in the while loop to see if
                // there's anything else in the queue we could send
...
SQRVoiceConnection::SQRVoiceConnection(
    ...
    : m_rudpCtx(rudpCtx), m_remoteRoomMemberId(remoteRoomMemberId),
      m_bConnected(false), m_headsetConnectionMask(0),
        ...
        for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++)
        {
            if (m_localVoiceDevices[i].isValid())
            {
                result = sceVoiceWriteToIPort(
                    ...
                    &g_loadedPCMVoiceData[0][g_loadedPCMVoiceDataPos[0]], &readSize, 0);
            }
        }
    }
...
void SonyVoiceChat_Orbis::sendAllVoiceData()
{
    ...
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++)
    {
        if (m_localVoiceDevices[i].isValid())
        {
            ...
            ProfileManager.GetChatAndContentRestrictions(i, true, &bChatRestricted,
                                                         NULL, NULL);
            ...
            else
            {
                /* Obtain port state */
                int32_t state;
                int err = sceVoiceGetPortAttr(m_localVoiceDevices[i].m_microphonePort,
    ...
    if (outputPortBytes || flagsChanged || m_forceSendPacket)
    {
        ...
        if (outputPortBytes)
        {
            ...
            result =
                sceVoiceReadFromOPort(m_voiceOutPort, packetToSend.m_data, &readSize);
            if (result != SCE_OK)
            {
...
void SonyVoiceChat_Orbis::playAllReceivedData()
{
    ...
    for (int rIdx = 0; rIdx < m_remoteConnections.size(); rIdx++)
    {
        ...
        while (pVoice->getNextPacket(
            packet)) // MGH -  changed to a while loop, so all the packets are sent
                     // to the voice port, and it can handle delayed packets due to
                     // the size of it's internal buffer
        {
            int frameGap;
...
void SonyVoiceChat_Orbis::tick()
{
    if (m_bInitialised)
    {
        ...
        // MGH -  added to hopefully fix a lockup with shutdowns happening on
        // different threads, when more than 1 player leaves the game
        for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++)
        {
...
bool SonyVoiceChat_Orbis::hasMicConnected(SQRNetworkPlayer *pNetPlayer)
{
    ...
    else
    {
        ...
        for (int i = 0; i < m_remoteConnections.size(); i++)
        {
            ...
            {
                bool bMicConnected =
                    pVoice->m_remotePlayerFlags[pNetPlayer->GetLocalPlayerIndex()]
                        .m_bHasMicConnected;
                LeaveCriticalSection(&m_csRemoteConnections);
...
void SQRLocalVoiceDevice::init(SceUserServiceUserId localUserID,
...
SQRVoiceConnection *SonyVoiceChat_Orbis::addRemoteConnection(
...
void SonyVoiceChat_Orbis::connectPorts(uint32_t inPort, uint32_t outPort)
{
    ...
    {
        app.DebugPrintf("sceVoiceConnectIPortToOPort failed (0x%08x), inPort "
                        "0x%08x, outPort 0x%08x\n",
...
void SonyVoiceChat_Orbis::disconnectPorts(uint32_t inPort, uint32_t outPort)
{
    ...
    if (returnCode != SCE_OK)
    {
        app.DebugPrintf("sceVoiceDisconnectIPortFromOPort failed (0x%08x), inPort "
                        "0x%08x, outPort 0x%08x\n",
                        returnCode, inPort, outPort);
        assert(0);
    }
...
void SonyVoiceChat_Orbis::initLocalPlayer(int playerIndex)
{
    if (m_localVoiceDevices[playerIndex].isValid() == false)
    {
        ...
        m_localVoiceDevices[playerIndex].init(ProfileManager.getUserID(playerIndex),
                                              chatRestricted);
        ...
        m_forceSendPacket =
            true; // new local device, so we'll force a packet through for the flags
    }
    makeLocalConnections();
}

void SonyVoiceChat_Orbis::connectPlayer(SQRVoiceConnection *pConnection,
                                        int playerIndex)
{
    if ((pConnection->m_headsetConnectionMask & (1 << playerIndex)) == 0)
    {
        initLocalPlayer(
            playerIndex); // added this as we can get a client->client connection
                          // coming in first, and the network player hasn't been
                          // created yet (so this hasn't been initialised)
        if (m_localVoiceDevices[playerIndex].m_bChatRestricted == false)
        {
...
SQRVoiceConnection *
SonyVoiceChat_Orbis::GetVoiceConnectionFromRudpCtx(int RudpCtx)
{
    for (int i = 0; i < m_remoteConnections.size(); i++)
    {
...
SQRVoiceConnection *SonyVoiceChat_Orbis::getVoiceConnectionFromRoomMemberID(
...
void SonyVoiceChat_Orbis::disconnectRemoteConnection(
