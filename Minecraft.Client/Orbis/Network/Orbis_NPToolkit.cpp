...
sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::NpSessionInformation>
    OrbisNPToolkit::sm_createJoinFuture;
...
void OrbisNPToolkit::coreCallback(const sce::Toolkit::NP::Event &event)
{
    ...
    case sce::Toolkit::NP::Event::loggedIn:
        ...
        ProfileManager.SignedInPSNStateCallback(event.userInformation.userId,
                                                event.userInformation.state,
                                                &event.userInformation.npId);
        break;
...
void OrbisNPToolkit::sceNpToolkitCallback(
...
void OrbisNPToolkit::sessionsCallback(const sce::Toolkit::NP::Event &event)
{
    ...
    case sce::Toolkit::NP::Event::
        npSessionCreateResult: ///< An event generated when the %Np session
        ...
        break;
    case sce::Toolkit::NP::Event::
        npSessionJoinResult: ///< An event generated when the join %Np session
        ...
        break;
    case sce::Toolkit::NP::Event::
        npSessionError: ///< An event generated when there was error performing
                        ///< the current %Np session process.
        app.DebugPrintf("npSessionError");
        break;
    case sce::Toolkit::NP::Event::
        npSessionLeaveResult: ///< An event generated when the user has left the
                              ///< current %Np session.
        app.DebugPrintf("npSessionLeaveResult");
        break;
    case sce::Toolkit::NP::Event::npSessionModified: ///< An event generated when
                                                     ///< the %Np session has been
                                                     ///< modified.
        app.DebugPrintf("npSessionModified");
        break;
    case sce::Toolkit::NP::Event::npSessionUpdateResult: ///< An event generated
                                                         ///< when the %Np session
                                                         ///< has been updated.
        app.DebugPrintf("npSessionUpdateResult");
        break;
    case sce::Toolkit::NP::Event::
        npSessionGetInfoResult: ///< An event generated when the %Np session info
                                ///< has been retrieved.
        app.DebugPrintf("npSessionGetInfoResult");
        break;
    case sce::Toolkit::NP::Event::
        npSessionGetInfoListResult: ///< An event generated when the %Np session
                                    ///< info has been retrieved.
        app.DebugPrintf("npSessionGetInfoListResult");
        break;
    case sce::Toolkit::NP::Event::
        npSessionGetSessionDataResult: ///< An event generated when the %Np
                                       ///< session data has been retrieved.
        app.DebugPrintf("npSessionGetSessionDataResult");
        break;
    case sce::Toolkit::NP::Event::
        npSessionSearchResult: ///< An event generated when the %Np session search
                               ///< request has been completed.
        app.DebugPrintf("npSessionSearchResult");
        break;
    case sce::Toolkit::NP::Event::
        npSessionInviteNotification: ///< An event generated when the %Np session
                                     ///< push notification is received.
        app.DebugPrintf("npSessionInviteNotification");
        break;
    case sce::Toolkit::NP::Event::
        npSessionInviteGetInfoResult: ///< An event generated when the %Np session
                                      ///< info has been retrieved.
        app.DebugPrintf("npSessionInviteGetInfoResult");
        break;
    case sce::Toolkit::NP::Event::
        npSessionInviteGetInfoListResult: ///< An event generated when the %Np
                                          ///< session info has been retrieved.
        app.DebugPrintf("npSessionInviteGetInfoListResult");
        break;
    case sce::Toolkit::NP::Event::
        npSessionInviteGetDataResult: ///< An event generated when the %Np session
...
void OrbisNPToolkit::gameCustomDataCallback(
    const sce::Toolkit::NP::Event &event)
{
    switch (event.event)
    {
...
void OrbisNPToolkit::messagingCallback(const sce::Toolkit::NP::Event &event)
{
    switch (event.event)
    {
    case sce::Toolkit::NP::Event::messageSent: ///< An event generated when a
                                               ///< message has been sent.
        app.DebugPrintf("sce::Toolkit::NP::Event::messageSent\n");
        SQRNetworkManager_Orbis::s_bInviteDialogRunning = false;
        break;
    case sce::Toolkit::NP::Event::messageError: ///< An event generated when a
        ...
        break;
    case sce::Toolkit::NP::Event::
        messageDialogTerminated: ///< An event generated when a message dialog box
                                 ///< is terminated.
        app.DebugPrintf("sce::Toolkit::NP::Event::messageDialogTerminated\n");
        SQRNetworkManager_Orbis::s_bInviteDialogRunning = false;
        break;
        // case sce::Toolkit::NP::Event::messageRetrieved:
        // ///< An event generated when a message attachment has been retrieved.
        // case sce::Toolkit::NP::Event::messageInGameDataReceived:
        // ///< An event generated when in-game data is received. case
        // sce::Toolkit::NP::Event::messageInGameDataRetrievalDone:		///< An
        // event generated when in-game data retrieval is complete.

        // case sce::Toolkit::NP::Event::messageAttachmentReceived:
        // ///< An event generated when a message with a data attachment has been
        // received. case sce::Toolkit::NP::Event::messageAttachmentOpened:
        // ///< An event generated when a message with a data attachment has been
        // opened (and the sysutil GUI is closed). case
        // sce::Toolkit::NP::Event::messageInviteReceived:
        // ///< An event generated when a message with an invite has been received.
        // case sce::Toolkit::NP::Event::messageInviteAccepted:
        // ///< An event generated when a message with an invite has been accepted
        // via ToolkitNp (and the sysutil GUI is closed).
    }
}
...
void OrbisNPToolkit::init()
{
    ...
    nptTitleId.setTitleSecret(*SQRNetworkManager_Orbis::GetSceNpTitleId(),
                              *SQRNetworkManager_Orbis::GetSceNpTitleSecret());
    ...
    sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::NetStateBasic>
        netStateFuture = sce::Toolkit::NP::Utilities::Future<
            sce::Toolkit::NP::NetStateBasic>();
    ...
    if (netStateFuture.hasResult())
    {
        ...
        ProfileManager.SetNetworkStatus(netState->connectionStatus ==
...
#define SESSION_STATUS \
    "Minecraft online game (this text needs defined and localised)"
#define SESSION_NAME "Minecraft(this text needs defined and localised)"
    ...
    createSessionRequest.sessionTypeFlag =
        SCE_TOOLKIT_NP_CREATE_SESSION_TYPE_PUBLIC;
    createSessionRequest.maxSlots = maxSlots;
    strncpy(createSessionRequest.sessionImgPath, SESSION_IMAGE_PATH,
            strlen(SESSION_IMAGE_PATH));
    strncpy(createSessionRequest.sessionStatus, SESSION_STATUS,
            strlen(SESSION_STATUS));
    createSessionRequest.userInfo.userId = userId;
    char test[3] = {'R', 'K', 'B'};
    createSessionRequest.sessionData = test;
    createSessionRequest.sessionDataSize = 3;
    ret = sce::Toolkit::NP::Sessions::Interface::create(&createSessionRequest,
                                                        &sm_createJoinFuture);
}
...
void OrbisNPToolkit::getMessageData(SceNpGameCustomDataEventParam *paramData)
{
    ...
    // 	sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::MessageAttachment>
    // actualMessage; 	ret =
    // sce::Toolkit::NP::GameCustomData::Interface::getGameData(&req,&actualMessage,false);
    // 	if( ret < 0 ) {
    // 		//Error handling
    // 	} else {
    // 		//Error handling
