...
void SonyRemoteStorage_Orbis::staticInternalCallback(
...
void SonyRemoteStorage_Orbis::internalCallback(
    const SceRemoteStorageEvent event, int32_t retCode)
{
    ...
    case ERROR_OCCURRED:
        app.DebugPrintf(
            "SonyRemoteStorage_Orbis: An error occurred with retCode: 0x%x \n",
            retCode);
        m_status = e_error;
    ...
    case GET_DATA_RESULT:
        ...
        {
            app.DebugPrintf("SonyRemoteStorage_Orbis: An error occurred while Get "
                            "Data was being processed. retCode: 0x%x \n",
                            retCode);
            m_status = e_error;
        }
    ...
    case GET_STATUS_RESULT:
        if (retCode >= 0)
        {
            ...
            app.DebugPrintf(
                "SonyRemoteStorage_Orbis: Remaining Syncs for this user: %llu\n",
                outputGetStatus->remainingSyncs);
            app.DebugPrintf(
                "SonyRemoteStorage_Orbis: Number of files on the cloud: %d\n",
        ...
        {
            app.DebugPrintf("SonyRemoteStorage_Orbis: An error occurred while Get "
                            "Status was being processed. retCode: 0x%x \n",
                            retCode);
            m_status = e_error;
        }
        runCallback();
    ...
    case PSN_SIGN_IN_REQUIRED:
        app.DebugPrintf("SonyRemoteStorage_Orbis: User's PSN sign-in through web "
                        "browser is required \n");
    ...
    case SET_DATA_RESULT:
        ...
        {
            app.DebugPrintf("SonyRemoteStorage_Orbis: An error occurred while Set "
                            "Data was being processed. retCode: 0x%x \n",
                            retCode);
            m_status = e_error;
        }
    ...
    case USER_ACCOUNT_LINKED:
        app.DebugPrintf(
            "SonyRemoteStorage_Orbis: User's account has been linked with PSN \n");
        m_bInitialised = true;
    ...
    case WEB_BROWSER_RESULT:
        app.DebugPrintf("SonyRemoteStorage_Orbis: This function is not used on PS "
                        "Vita, as the account will be linked, it is not needed to "
                        "open a browser to link it \n");
        assert(0);
        break;
...
bool SonyRemoteStorage_Orbis::init(CallbackFunc cb, LPVOID lpParam)
{
    ...
    // NULL); 	if (ret < 0) { 		app.DebugPrintf("Failed to get auth code
    // 0x%x\n", ret);
    // 	}
...
bool SonyRemoteStorage_Orbis::getRemoteFileInfo(SceRemoteStorageStatus *pInfo,
                                                CallbackFunc cb,
                                                LPVOID lpParam)
{
    m_callbackFunc = cb;
...
bool SonyRemoteStorage_Orbis::setData(PSAVE_INFO info, CallbackFunc cb,
                                      LPVOID lpParam)
{
    assert(0);
    return false;
    ...
    // 	memcpy(params.secureFileId, m_secureFileId,
...
bool SonyRemoteStorage_Orbis::getData(const char *remotePath,
