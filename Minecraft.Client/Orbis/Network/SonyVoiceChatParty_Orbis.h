...
class SonyVoiceChatParty_Orbis
{
  ...
  private:
    ...
    static void partyRoomEventHandler(SceNpPartyRoomEventType eventType,
                                      const void *data, void *userdata);
    static void
    partyVoiceEventHandler(const SceNpPartyMemberVoiceInfo *memberVoiceInfo,
                           void *userdata);
    static void partyBinaryMessageEventHandler(SceNpPartyBinaryMessageEvent event,
                                               const void *data, void *userdata);
    ...
    class PartyInfo
    {
      public:
        ...
        MemberInfo *getMember(const PlayerUID &memberUID)
        {
            for (int i = 0; i < m_numMembers; i++)
            {
                if (strcmp(m_members[i].m_NpId.handle.data, memberUID.getOnlineID()) ==
                    0)
                {
                    return &m_members[i];
                }
        ...
        void addMember(SceNpPartyRoomMemberId roomID, SceNpId npID, bool local,
                       bool leader)
        {
