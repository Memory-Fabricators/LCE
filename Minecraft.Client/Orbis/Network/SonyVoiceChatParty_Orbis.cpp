...
void SonyVoiceChatParty_Orbis::init()
{
    ...
    stPartyEventHandler.binaryMessageEventHandler =
...
void SonyVoiceChatParty_Orbis::mutePlayer(
...
void SonyVoiceChatParty_Orbis::muteLocalPlayer(
...
bool SonyVoiceChatParty_Orbis::isMutedPlayer(const PlayerUID &memberUID)
{
    ...
    // 	{
    // 		uint8_t bMute;
    // 		int err = cellSysutilAvc2GetPlayerVoiceMuting(member_id,
    // &bMute); 		assert(err == CELL_OK); 		return bMute;
    // 	}
    // 	return false;
}
...
void SonyVoiceChatParty_Orbis::partyRoomEventHandler(
    SceNpPartyRoomEventType eventType, const void *data, void *userdata)
{
    ...
    case SCE_NP_PARTY_ROOM_EVENT_JOINED:
        {
            ...
            {
                m_partyInfo.addMember(pPartyJoinedInfo->memberIds[i],
                                      pPartyJoinedInfo->members[i], false, false);
                app.DebugPrintf("Member room ID : %d - Member PSN ID : %s\n",
                                pPartyJoinedInfo->memberIds[i],
                                pPartyJoinedInfo->members[i].handle.data);
            }
        }
        break;
    ...
    case SCE_NP_PARTY_ROOM_EVENT_MEMBER_JOINED:
        {
            ...
            m_partyInfo.addMember(
                pNewPartyMemberInfo->memberId, pNewPartyMemberInfo->npId,
                pNewPartyMemberInfo->memberFlags & SCE_NP_PARTY_MEMBER_FLAG_IS_LOCAL,
                pNewPartyMemberInfo->memberFlags &
                    SCE_NP_PARTY_MEMBER_FLAG_IS_PARTY_LEADER);
        }
        break;
    ...
    case SCE_NP_PARTY_ROOM_EVENT_MEMBER_LEFT:
        {
            ...
            SceNpPartyMemberInfo *pLeavingPartyMemberInfo =
                (SceNpPartyMemberInfo *)data;
    ...
    case SCE_NP_PARTY_ROOM_EVENT_CREATE_RESPONSE:
        {
            ...
            SceNpPartyCreateResponseInfo *pCreateResponseInfo =
                (SceNpPartyCreateResponseInfo *)data;

            switch (pCreateResponseInfo->status)
            {
            case SCE_NP_PARTY_ROOM_CREATE_RESPONSE_STATUS_OK:
                app.DebugPrintf(
                    "Local party member userId[0x%x] response for creating a party. "
                    "Status = SCE_NP_PARTY_ROOM_CREATE_RESPONSE_STATUS_OK\n",
                    pCreateResponseInfo->userId);
                break;
            case SCE_NP_PARTY_ROOM_CREATE_RESPONSE_STATUS_CANCELLED:
                app.DebugPrintf(
                    "Local party member userId[0x%x] response for creating a party. "
                    "Status = SCE_NP_PARTY_ROOM_CREATE_RESPONSE_STATUS_CANCELLED\n",
                    pCreateResponseInfo->userId);
                break;
            default:
                app.DebugPrintf(
                    "Warning: Unknown SceNpPartyCreateResponseStatus [%d]. Ignore.\n",
    ...
    case SCE_NP_PARTY_ROOM_EVENT_SHOW_INVITATION_RESPONSE:
        {
            ...
            SceNpPartyShowInvitationResponseInfo *pShowInvitationResponseInfo =
                (SceNpPartyShowInvitationResponseInfo *)data;

            switch (pShowInvitationResponseInfo->status)
            {
            case SCE_NP_PARTY_ROOM_SHOW_INVITATION_RESPONSE_STATUS_OK:
                app.DebugPrintf("Local party member userId[0x%x] response for sending an "
                                "invitation/suggestion. Status = "
                                "SCE_NP_PARTY_ROOM_SHOW_INVITATION_RESPONSE_STATUS_OK\n",
                                pShowInvitationResponseInfo->userId);
                break;
            case SCE_NP_PARTY_ROOM_SHOW_INVITATION_RESPONSE_STATUS_CANCELLED:
                app.DebugPrintf(
                    "Local party member userId[0x%x] response for sending an "
                    "invitation/suggestion. Status = "
                    "SCE_NP_PARTY_ROOM_SHOW_INVITATION_RESPONSE_STATUS_CANCELLED\n",
                    pShowInvitationResponseInfo->userId);
                break;
            default:
                app.DebugPrintf("Warning: Unknown SceNpPartyShowInvitationResponseStatus "
                                "[%d]. Ignore.\n",
...
void SonyVoiceChatParty_Orbis::partyVoiceEventHandler(
    const SceNpPartyMemberVoiceInfo *memberVoiceInfo, void *userdata)
{
...
void SonyVoiceChatParty_Orbis::partyBinaryMessageEventHandler(
    SceNpPartyBinaryMessageEvent event, const void *data, void *userdata)
{
    switch (event)
    {
