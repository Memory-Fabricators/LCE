...
int (*SQRNetworkManager_Orbis::s_SignInCompleteCallbackFn)(void *pParam,
                                                           bool bContinue,
                                                           int pad) = NULL;
...
SQRNetworkManager_Orbis::PresenceSyncInfo
    SQRNetworkManager_Orbis::s_lastPresenceSyncInfo = {0};
SQRNetworkManager_Orbis::PresenceSyncInfo
    SQRNetworkManager_Orbis::c_presenceSyncInfoNULL = {0};
// SceNpBasicAttachmentDataId SQRNetworkManager_Orbis::s_lastInviteIdToRetry =
...
SQRNetworkManager_Orbis::PresenceSyncInfo
    *SQRNetworkManager_Orbis::m_gameBootInvite;
SQRNetworkManager_Orbis::PresenceSyncInfo
    SQRNetworkManager_Orbis::m_gameBootInvite_data;
...
const SQRNetworkManager_Orbis::eSQRNetworkManagerState SQRNetworkManager_Orbis::
    m_INTtoEXTStateMappings[SQRNetworkManager_Orbis::SNM_INT_STATE_COUNT] = {
...
SQRNetworkManager_Orbis::SQRNetworkManager_Orbis(
    ISQRNetworkManagerListener *listener)
{
    ...
    int ret =
        sceKernelCreateEqueue(&m_basicEventQueue, "SQRNetworkManager_Orbis EQ");
    assert(ret == SCE_OK);
    ret = sceKernelAddUserEvent(m_basicEventQueue, sc_UserEventHandle);
    assert(ret == SCE_OK);

    m_basicEventThread =
        new C4JThread(&BasicEventThreadProc, this, "Basic Event Handler");
...
// ContextCallback. The SQRNetworkManager_Orbis is then finally moved into
...
void SQRNetworkManager_Orbis::InitialiseAfterOnline()
{
    ...
    // stage (SQRNetworkManager_Orbis::Initialise) or we've had a network
    ...
    // callback or SQRNetworkManager_Orbis::Initialise) by which this could be
    // called
    if ((m_state != SNM_INT_STATE_SIGNING_IN) &&
        !((m_state == SNM_INT_STATE_IDLE) && m_offlineSQR))
    {
        // If we aren't going to continue on with this sign-in but are expecting a
        // callback, then let the game know that we have completed the bit we are
        // expecting to do. This will happen whilst in game, when we want to be able
        // to sign into PSN, but don't expect the full matching stuff to get set up.
        if (s_SignInCompleteCallbackFn)
        {
            s_SignInCompleteCallbackFn(s_SignInCompleteParam, true,
    ...
    initParam.threadPriority =
        SCE_KERNEL_PRIO_FIFO_DEFAULT; // This seems to be completely ignored and
                                      // always set to default anyway
    ...
    {
        app.DebugPrintf("SQRNetworkManager_Orbis::InitialiseAfterOnline - "
                        "sceNpMatching2Initialize failed with error 0x%08x\n",
                        ret);
        SetState(SNM_INT_STATE_INITIALISE_FAILED);
        return;
    }
    app.DebugPrintf("SQRNetworkManager::InitialiseAfterOnline - matching context "
                    "is now valid\n");
    ...
    {
        app.DebugPrintf("SQRNetworkManager_Orbis::InitialiseAfterOnline - Primary "
                        "pad not signed in to live");
        SetState(SNM_INT_STATE_INITIALISE_FAILED);
        return;
    }
    ...
    {
        app.DebugPrintf("SQRNetworkManager_Orbis::InitialiseAfterOnline - "
                        "sceNpMatching2CreateContext failed with error 0x%08x\n",
                        ret);
        SetState(SNM_INT_STATE_INITIALISE_FAILED);
        return;
    }
    ...
    {
        app.DebugPrintf("SQRNetworkManager_Orbis::InitialiseAfterOnline - "
                        "RegisterCallbacks failed with error 0x%08x\n",
                        ret);
        SetState(SNM_INT_STATE_INITIALISE_FAILED);
        return;
    }
    ...
    {
        app.DebugPrintf("SQRNetworkManager_Orbis::InitialiseAfterOnline - "
                        "sceNpMatching2ContextStart failed with error 0x%08x\n",
                        ret);
        SetState(SNM_INT_STATE_INITIALISE_FAILED);
    }
}

void SQRNetworkManager_Orbis::
    RefreshChatAndContentRestrictionsReturned_HandleInvite(void *pParam)
{
    SQRNetworkManager_Orbis *netMan = (SQRNetworkManager_Orbis *)pParam;

    netMan->m_listener->HandleInviteReceived(ProfileManager.GetPrimaryPad(),
                                             netMan->m_gameBootInvite);
...
void SQRNetworkManager_Orbis::Tick()
{
    ...
    TickNotify();
    tickErrorDialog();
    SignallingEventsTick(); // process the signalling events here now instead of
                            // in the matching2 callback
        ...
        !m_bRefreshingRestrictionsForInvite)
    {
        ...
        ProfileManager.RefreshChatAndContentRestrictions(
            RefreshChatAndContentRestrictionsReturned_HandleInvite, this);
        // m_listener->HandleInviteReceived( ProfileManager.GetPrimaryPad(),
        // m_gameBootInvite ); m_gameBootInvite = NULL;
    }
    ...
    // Client only - do the final transition to a starting & playing state once we
    // have fully joined the room, And told the game about all the local players
    // so they are also all valid
    if (m_state == SNM_INT_STATE_JOINING_WAITING_FOR_LOCAL_PLAYERS)
    {
        if (m_localPlayerJoined == m_localPlayerCount)
        {
            // Since we're now fully joined, we can update our presence info so that
            // our friends could find us in this game. This data was set up at the
            // point that we joined the game (either from search info, or an
            // invitation).
    ...
    // 4J-PB - SQRNetworkManager_PS3::AttemptPSNSignIn was causing crashes in Iggy
    // by calling LoadMovie from a callback, so call it frmo the tick instead
    if (m_bCallPSNSignInCallback)
    {
        // TODO: This may be a sign in attempt for a different pad, need to fix this
        app.DebugPrintf(
            "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ "
            "s_SignInCompleteCallbackFn false "
            "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
...
void SQRNetworkManager_Orbis::tickErrorDialog()
{
    ...
    // Long story short we need to wait for a bit for the system UI to appear, If
    // it doesn't appear or it disappears we wait for a bit then check the sign in
    // results. Not perfect but it is safe!
    ...
    if (s_SignInCompleteCallbackPending)
    {
        ...
        if (sinceErrorClosed > SYSTEM_UI_WAIT_TIME)
        {
            ...
            bool systemUiDisplayed =
                status.isInBackgroundExecution || status.isSystemUiOverlaid;
            ...
            else
            {
                ...
                {
                    int sinceSystemClosed =
                        System::currentTimeMillis() - s_systemDialogClosed;

                    // If the system dialog has been closed a while, we're ready to
                    // callback
                    if (sinceSystemClosed > SYSTEM_UI_WAIT_TIME)
                    {
                        // 4J-PB - don't call this here, let the
                        // SQRNetworkManager_Orbis::Tick call it
    ...
    case SNM_INT_STATE_INITIALISE_FAILED:
        ...
        if (m_isInSession &&
            m_offlineGame) // m_offlineSQR )  // MGH - changed this to
                           // m_offlineGame, as m_offlineSQR can be true when
                           // running an online game but the init has failed because
                           // the servers are down
        {
            // This is a fix for an issue where a player attempts (and fails) to sign
            // in, whilst in an offline game. This was setting the state to idle,
            // which in turn sets the game to Not be in a session anymore (but the
            // game wasn't generally aware of, and so keeps playing). Howoever, the
            // game's connections use their tick to determine whether to empty their
            // queues or not and so no communications (even though they don't actually
            // use this network manager for local connections) were happening.
            SetState(SNM_INT_STATE_PLAYING);
        }
        else
        {
...
void SQRNetworkManager_Orbis::CreateAndJoinRoom(int hostIndex,
                                                int localPlayerMask,
...
void SQRNetworkManager_Orbis::UpdateExternalRoomData()
{
    ...
    if (m_isHosting)
    {
        ...
        int ret = sceNpMatching2SetRoomDataExternal(m_matchingContext, &reqParam,
                                                    NULL, &m_setRoomDataRequestId);
        app.DebugPrintf(CMinecraftApp::USER_RR,
                        "sceNpMatching2SetRoomDataExternal returns 0x%x, number of "
                        "players %d\n",
        ...
        if ((ret < 0) || ForceErrorPoint(SNM_FORCE_ERROR_SET_EXTERNAL_ROOM_DATA))
        {
            // If we ever fail to send the external room data, we start a countdown so
            // that we attempt to resend. Not sure how likely it is that updating this
            // will fail without the whole network being broken, but if in particular
            // we don't update the flag to say that the session is joinable, then
            // nobody is ever going to see this session.
...
bool SQRNetworkManager_Orbis::FriendRoomManagerSearch()
{
    ...
    // Get friend list - doing this in another thread as it can lock up for a few
    // seconds
...
bool SQRNetworkManager_Orbis::FriendRoomManagerSearch2()
{
    ...
    if (m_aFriendSearchResults.size() > 0)
    {
        ...
        // have any duplicate rooms here if more than one friend is playing in the
        // same room.
...
int SQRNetworkManager_Orbis::BasicEventThreadProc(void *lpParameter)
{
    ...
    do
    {
        ret = sceKernelWaitEqueue(manager->m_basicEventQueue, &event, 1, &outEv,
                                  NULL);

        // If the sys_event_t we've sent here from the handler has a non-zero data1
        // element, this is to signify that we should terminate the thread
        if (event.udata == 0)
        {
            // 			int iEvent;
            // 			SceNpUserInfo from;
            ...
            // 			size_t bufferSize =
            // SCE_NP_BASIC_MAX_MESSAGE_SIZE; 			int ret =
            ...
            // 				{
            // 					// 4J Stu - Don't do this here
            // as it can be very disruptive to gameplay. Players can bring this up
            // from LoadOrJoinMenu, PauseMenu and InGameInfoMenu
            ...
            // *result = (SceNpBasicExtendedAttachmentData  *)buffer;
            ...
            // 					}
            // 				}
            // 				app.DebugPrintf("Incoming basic event of
            // type %d\n",iEvent);
            // 			}
        }
...
int SQRNetworkManager_Orbis::GetFriendsThreadProc(void *lpParameter)
{
    ...
    bool async = false;
    ret = sce::Toolkit::NP::Friends::Interface::getFriendslist(
        &friendList, &requestParam, async);
    if (ret != SCE_OK)
    {
    ...
    if (friendList.hasResult())
    {
        ...
        int i = 1;
        bool noFriends = true;
        for (iter = friendList.get()->begin(); iter != friendList.get()->end();
             ++iter, i++)
        {
            ...
            sce::Toolkit::NP::Utilities::Future<sce::Toolkit::NP::PresenceInfo>
                presenceInfo;
            memset(&presenceRequest, 0, sizeof(presenceRequest));
            strncpy(presenceRequest.onlineId.data, iter->npid.handle.data,
                    strlen(iter->npid.handle.data));
            presenceRequest.presenceType =
                SCE_TOOLKIT_NP_PRESENCE_TYPE_INCONTEXT_INFO;
            presenceRequest.userInfo.userId =
                ProfileManager.getUserID(ProfileManager.GetPrimaryPad());
            ;

            ret = sce::Toolkit::NP::Presence::Interface::getPresence(
                &presenceRequest, &presenceInfo, false);
            if (ret < 0)
            {
            ...
            else
            {
                ...
                app.DebugPrintf("Online Status: %s\n",
                                (presenceInfo.get()->onlineStatus ==
                                 SCE_NP_GAME_PRESENCE_STATUS_OFFLINE)
                                    ? "OFFLINE"
                                    : "ONLINE");
                app.DebugPrintf("Presence Type (Refer to "
                                "SCE_TOOLKIT_NP_PRESENCE_TYPE_* flags): %d\n",
                ...
                // Only include the friend's game if its the same network id ( this also
                // filters out generally Zeroed PresenceSyncInfo, which we do when we
                // aren't in an active game session)
                // 				if( presenceDetails.size ==
                // sizeof(PresenceSyncInfo) )
                {
                    PresenceSyncInfo *pso =
                        (PresenceSyncInfo *)presenceInfo.get()->gameInfo.gameData;
    ...
    {
        app.DebugPrintf(
            "Error occurred while retrieving FriendsList, ret =  0x%x\n",
            friendList.getError());
...
// returns false
void SQRNetworkManager_Orbis::FriendRoomManagerGetRoomInfo(
    int idx, SQRNetworkManager_Orbis::SessionSearchResult *searchResult)
{
...
SQRNetworkManager_Orbis::eSQRNetworkManagerState
SQRNetworkManager_Orbis::GetState()
{
...
SQRNetworkPlayer *
SQRNetworkManager_Orbis::GetPlayerIfReady(SQRNetworkPlayer *player)
{
    if (player == NULL)
    {
        return NULL;
...
void SQRNetworkManager_Orbis::SetState(
    SQRNetworkManager_Orbis::eSQRNetworkManagerInternalState state)
{
...
// FriendRoomManagerGetCount(). Only valid when FriendRoomManagerIsBusy()
// returns false
bool SQRNetworkManager_Orbis::JoinRoom(
    SQRNetworkManager_Orbis::SessionSearchResult *searchResult,
    int localPlayerMask)
{
    ...
    CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
        ...
        searchResult->m_sessionId.m_RoomId, searchResult->m_sessionId.m_ServerId);
    return JoinRoom(searchResult->m_sessionId.m_RoomId,
                    searchResult->m_sessionId.m_ServerId, localPlayerMask, NULL);
}
...
bool SQRNetworkManager_Orbis::JoinRoom(
    ...
    int localPlayerMask,
    const SQRNetworkManager_Orbis::PresenceSyncInfo *presence)
{
    // The presence info will be directly passed in if we are joining from an
    // invite, otherwise it has already been set up. This is synchronised out when
    // we have fully joined the game.
    if (presence)
    {
...
void SQRNetworkManager_Orbis::LeaveRoom(bool bActuallyLeaveRoom)
{
    ...
    // Attempt to leave the room if we are in any of the states we could be in if
    // we have successfully created it
    if (bActuallyLeaveRoom)
    {
            ...
            (m_state == SNM_INT_STATE_PLAYING))
        {
            ...
            SetState(SNM_INT_STATE_LEAVING);
            int ret = sceNpMatching2LeaveRoom(m_matchingContext, &reqParam, NULL,
        ...
        {
            // Haven't created the room yet, but will have created the server context
            // so need to recover from that
            DeleteServerContext();
        }
        else
        {
    ...
    else
    {
        ...
        // destroyed, so no need to leave it again since it doesn't exist anymore.
        // Still need to destroy server context which may be valid
        DeleteServerContext();
    }
}
...
bool SQRNetworkManager_Orbis::AddLocalPlayerByUserIndex(int idx)
{
    if (m_isHosting)
    {
        ...
        // Host's players are always at the start of the sync data, so we just need
        // to find the first entry that isn't us to determine what we want to insert
        // before
        ...
        // And do any adjusting necessary to the mappings from this room data, to
        // the SQRNetworkPlayers. This will also create the required new
    ...
    {
        // Don't attempt to join if our client's view of the players indicates that
        ...
        // Add the requested player to the mask of local players currently in the
        // game, and update this data - this will also then resync with the server
        ...
        // member id.
        bool rudpOk =
            CreateRudpConnections(m_room, m_roomSyncData.players[0].m_roomMemberId,
                                  mask, m_localMemberId);
...
bool SQRNetworkManager_Orbis::RemoveLocalPlayerByUserIndex(int idx)
{
    if (m_isHosting)
    {
        ...
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++)
        {
                ...
                (m_roomSyncData.players[i].m_localIdx == idx))
            {
                ...
                // to the SQRNetworkPlayers. This will also delete the SQRNetworkPlayer
                // and do all the callbacks that requires etc.
    ...
    {
        // Remove the requested player from the mask of local players currently in
...
void SQRNetworkManager_Orbis::SendInviteGUI()
{
    ...
    {
        app.DebugPrintf("SendInviteGUI - Invite dialog is already running so "
                        "ignoring request\n");
        return;
    }
    ...
    // (whether we came in from an invite, or joining a game we discovered). If we
    // were hosting, then we'll need to set this up now from the external dasta.
    if (m_isHosting)
    {
        CPlatformNetworkManagerSony::SetSQRPresenceInfoFromExtData(
...
#define SESSION_IMAGE_PATH "/app0/Orbis/session_image.jpg"
    ...
    // Attachment
    sce::Toolkit::NP::MessageAttachment attachment =
        sce::Toolkit::NP::MessageAttachment();
    attachment.setAttachmentData(
        (SceChar8 *)&s_lastPresenceSyncInfo,
        sizeof(PresenceSyncInfo)); // This step is important, allocates space on
                                   // heap for the data
    ...
    // 	int ret = sce::Toolkit::NP::Messaging::Interface::sendMessage(&messData,
    // SCE_TOOLKIT_NP_MESSAGE_TYPE_INVITE);
...
// persists forever.
void SQRNetworkManager_Orbis::GetInviteDataAndProcess(
    sce::Toolkit::NP::MessageAttachment *pInvite)
{
    ...
    // 	sce::Toolkit::NP::Utilities::Future< NpSessionInvitationInfo >
    // inviteInfo;
    //
    // 	requestInfo.invitationId = pInvite->invitationId;
    // 	requestInfo.userInfo.npId = pInvite->onlineId;
    // 	int err =
    // sce::Toolkit::NP::Sessions::Interface::getInvitationInfo(&requestInfo,
    // &inviteInfo, false); 	if(err == SCE_OK)
    // 	{
    ...
    // 		{
    // 			app.DebugPrintf("inviteInfo.getError() 0x%08x",
    // inviteInfo.getError());
    // 		}
    // 	}
    // 	else
    ...
    //
    // 	// If we fail ( which we might do if we aren't online at this point)
    // then zero the invite information and we'll try and get it later after
    // (possibly) signing in 	if( ret != 0 )
    // 	{
...
// bool
// SQRNetworkManager_Orbis::UpdateInviteData(SQRNetworkManager_Orbis::PresenceSyncInfo
// *invite)
// {
...
void SQRNetworkManager_Orbis::FindOrCreateNonNetworkPlayer(
    ...
    int localPlayerIdx, int smallId)
{
    ...
    // Create the player - non-network players can be considered complete as soon
    // as we create them as we aren't waiting on their network connections
    ...
    // For offline games, set name directly from gamertag as the PlayerUID will be
    // full of zeroes.
    if (m_offlineGame)
    {
...
void SQRNetworkManager_Orbis::LocalDataSend(SQRNetworkPlayer *playerFrom,
                                            ...
                                            unsigned int dataSize)
{
    ...
    if (m_listener)
    {
        m_listener->HandleDataReceived(playerFrom, playerTo, (unsigned char *)data,
                                       dataSize);
    }
}
...
void SQRNetworkManager_Orbis::MapRoomSlotPlayers(
    int roomSlotPlayerCount /*=-1*/)
{
    EnterCriticalSection(&m_csRoomSyncData);

    // If we pass an explicit roomSlotPlayerCount, it is because we are removing a
    // player, and this is the count of slots that there were *before* the
    // removal.
    ...
    if (m_isHosting)
    {
        for (int i = 0; i < roomSlotPlayerCount; i++)
        {
            if (m_aRoomSlotPlayers[i])
            {
                ...
                // connections being established and removed, so don't go deleting them
                // here. Other types are managed by this mapping. Note that
                // m_vecTempPlayers is used as a pool of players to consider by
        ...
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++)
        {
            if (i == 0)
            {
                ...
                FindOrCreateNonNetworkPlayer(i, SQRNetworkPlayer::SNP_TYPE_HOST,
                ...
                m_roomSyncData.players[i].m_UID =
                    m_aRoomSlotPlayers[i]->GetUID(); // On host, UIDs flow from player
                                                     // data -> m_roomSyncData
            }
            else
            {
                if (m_roomSyncData.players[i].m_roomMemberId == m_localMemberId)
                {
                    FindOrCreateNonNetworkPlayer(i, SQRNetworkPlayer::SNP_TYPE_LOCAL,
                                                 m_roomSyncData.players[i].m_roomMemberId,
                                                 m_roomSyncData.players[i].m_localIdx,
                                                 m_roomSyncData.players[i].m_smallId);
                    m_roomSyncData.players[i].m_UID =
                ...
                else
                {
                    m_aRoomSlotPlayers[i] = GetPlayerFromRoomMemberAndLocalIdx(
                        m_roomSyncData.players[i].m_roomMemberId,
                        m_roomSyncData.players[i].m_localIdx);
                    // If we're the host, then we allocated the small id so can flag now
                    // if we've got a player to flag...
                    if (m_aRoomSlotPlayers[i])
                    {
                        NetworkPlayerSmallIdAllocated(m_aRoomSlotPlayers[i],
                                                      m_roomSyncData.players[i].m_smallId);
                    }
                }
            }
    ...
    else
    {
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++)
        {
            if (m_aRoomSlotPlayers[i])
            {
                // On clients, local players are created and destroyed by the Rudp
                // connections being established and removed, so don't go deleting them
                // here. Other types are managed by this mapping. Note that
                // m_vecTempPlayers is used as a pool of players to consider by
                // FindOrCreateNonNetworkPlayer
        ...
        for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++)
        {
            if (i == 0)
            {
                ...
                m_aRoomSlotPlayers[i]->SetUID(
                    m_roomSyncData.players[i].m_UID); // On client, UIDs flow from
            ...
            else
            {
                if (m_roomSyncData.players[i].m_roomMemberId == m_localMemberId)
                {
                    // This player is local to this machine - don't bother setting UID
                    // from sync data, as it will already have been set accurately when we
                    // (locally) made this player
                    m_aRoomSlotPlayers[i] = GetPlayerFromRoomMemberAndLocalIdx(
                        m_roomSyncData.players[i].m_roomMemberId,
                        m_roomSyncData.players[i].m_localIdx);
                    // If we've got the room sync data back from the server, then we've
                    // got our smallId. Set flag for this.
                    if (m_aRoomSlotPlayers[i])
                    {
                        NetworkPlayerSmallIdAllocated(m_aRoomSlotPlayers[i],
                                                      m_roomSyncData.players[i].m_smallId);
                ...
                {
                    FindOrCreateNonNetworkPlayer(i, SQRNetworkPlayer::SNP_TYPE_REMOTE,
                                                 m_roomSyncData.players[i].m_roomMemberId,
                                                 m_roomSyncData.players[i].m_localIdx,
                                                 m_roomSyncData.players[i].m_smallId);
                    m_aRoomSlotPlayers[i]->SetUID(
...
void SQRNetworkManager_Orbis::UpdatePlayersFromRoomSyncUIDs()
{
    ...
    for (int i = 0; i < m_roomSyncData.getPlayerCount(); i++)
    {
        if (m_aRoomSlotPlayers[i])
        {
            ...
            {
                // Don't sync local players as we already set those up with their UID in
                // the first place...
                if (m_roomSyncData.players[i].m_roomMemberId != m_localMemberId)
                {
                    m_aRoomSlotPlayers[i]->SetUID(m_roomSyncData.players[i].m_UID);
...
bool SQRNetworkManager_Orbis::AddRemotePlayersAndSync(
    ...
    bool *isFull /*==NULL*/)
{
    ...
    // Add all remote players determined from the player mask to our own slots of
    // active players
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++)
    {
        if (playerMask & (1 << i))
...
void SQRNetworkManager_Orbis::RemoveRemotePlayersAndSync(
    ...
    for (int i = 0; i < m_roomSyncData.getPlayerCount();)
    {
            ...
            ((1 << m_roomSyncData.players[i].m_localIdx) & mask))
        {
            ...
            if (player)
            {
                ...
                // Delete the player itself and the mapping from context to player map
                // as this context is no longer valid
                delete player;
...
void SQRNetworkManager_Orbis::RemoveNetworkPlayers(int mask)
{
         ...
         it != m_RudpCtxToPlayerMap.end();)
    {
        ...
        {
            // Get Rudp context for this player, close that context down ( which will
            // in turn close the socket if required)
            int ctx = it->first;
            int err = sceRudpTerminate(ctx);
            g_numRUDPContextsBound--;
            assert(err == SCE_OK);
            ...
            // Delete the player itself and the mapping from context to player map as
            // this context is no longer valid
            delete player;
        }
        else
        {
...
bool SQRNetworkManager_Orbis::GetMatchingContext(
    eSQRNetworkManagerInternalState asyncState)
{
    ...
    if ((ret < 0) && (ret != SCE_NP_MATCHING2_ERROR_ALREADY_INITIALIZED))
    {
        app.DebugPrintf("SQRNetworkManager::GetMatchingContext - "
                        "sceNpMatching2Init2 failed with code 0x%08x\n",
                        ret);
        return false;
    }
    m_matching2initialised = true;
    ...
    {
        app.DebugPrintf(
            "SQRNetworkManager::GetMatchingContext - RegisterCallbacks failed\n");
        return false;
    }
    ...
    app.DebugPrintf("SQRNetworkManager::GetMatchingContext - matching context is "
                    "now valid\n");
    m_matchingContextValid = true;
    return true;
}
...
bool SQRNetworkManager_Orbis::GetServerContext()
{
    ...
    // If this caused an async thing to be started up, then we've done as much as
    // we can here - the rest of the code will happen when the async matching 2
    // context starting completes ( event SCE_NP_MATCHING2_CONTEXT_EVENT_Start is
    // received )
    ...
    SceNpMatching2ServerId serverId)
{
    ...
    {
        app.DebugPrintf("SQRNetworkManager::GetServerContext - Failed due to no "
                        "matching context\n");
        return false;
    }
    ...
    // application. We don't actually need to do this, but it is as good a way as
    // any to try a matching2 service and check that
    ...
    // If one of the previous GetMatchingContext calls caused an async thing to be
    // started up, then we've done as much as we can here - the rest of the code
    // will happen when the async matching 2 context starting completes ( event
...
void SQRNetworkManager_Orbis::ServerContextTick()
{
    ...
    case SNM_INT_STATE_JOINING_SERVER_SEARCH_SERVER_ERROR:
        ...
        SetState((m_state == SNM_INT_STATE_HOSTING_SERVER_SEARCH_SERVER_ERROR)
    ...
    case SNM_INT_STATE_HOSTING_CREATE_ROOM_WORLD_FOUND:
        {
            ...
            int ret = sceNpMatching2CreateJoinRoom(m_matchingContext, &reqParam, NULL,
...
// registered callback.
void SQRNetworkManager_Orbis::NetworkPlayerConnectionComplete(
    SQRNetworkPlayer *player)
{
    ...
    if (!m_isHosting)
    {
        ...
        // small id) then confirm to the host that this is the case, which makes us
        // now fully ready at this end
...
void SQRNetworkManager_Orbis::NetworkPlayerSmallIdAllocated(
...
void SQRNetworkManager_Orbis::NetworkPlayerInitialDataReceived(
...
void SQRNetworkManager_Orbis::NonNetworkPlayerComplete(SQRNetworkPlayer *player,
...
void SQRNetworkManager_Orbis::HandlePlayerJoined(SQRNetworkPlayer *player)
{
    ...
    // about. We can only transition to telling the game that we are playing once
    // the room is set up And all the local players are valid to use.
    if (!m_isHosting)
    {
        if (player->IsLocal())
        {
...
bool SQRNetworkManager_Orbis::SelectRandomServer()
{
    ...
    // not really selecting a random server, as we've already been allocated one,
    // but calling this to match PS3
    int serverIdx;
    serverIdx = 0;
    m_serverCount--;
...
bool SQRNetworkManager_Orbis::CreateVoiceRudpConnections(
    ...
    int playerMask)
{
    ...
    if (pConnection == NULL)
    {
        ...
        pConnection =
            SonyVoiceChat_Orbis::addRemoteConnection(rudpCtx, peerMemberId);
    }
...
bool SQRNetworkManager_Orbis::CreateRudpConnections(
    SceNpMatching2RoomId roomId, SceNpMatching2RoomMemberId peerMemberId,
    int playerMask, SceNpMatching2RoomMemberId playersMemberId)
{
    ...
    // Create an Rudp context for each local player that is required. These can be
    // used as individual virtual connections between room members (ie consoles),
    // which are multiplexed over the socket we have just made
    for (int i = 0; i < MAX_LOCAL_PLAYER_COUNT; i++)
    {
        if ((playerMask & (1 << i)) == 0)
        {
        ...
        if (m_isHosting)
        {
            m_RudpCtxToPlayerMap[rudpCtx] =
                new SQRNetworkPlayer(this, SQRNetworkPlayer::SNP_TYPE_REMOTE, true,
                                     playersMemberId, i, rudpCtx, NULL);
        }
        else
        {
            // Local players can establish their UID at this point
            ...
            m_RudpCtxToPlayerMap[rudpCtx] =
                new SQRNetworkPlayer(this, SQRNetworkPlayer::SNP_TYPE_LOCAL, false,
                                     m_localMemberId, i, rudpCtx, &localUID);
        }
...
SQRNetworkPlayer *
SQRNetworkManager_Orbis::GetPlayerFromRoomMemberAndLocalIdx(int roomMember,
                                                            int localIdx)
{
    for (AUTO_VAR(it, m_RudpCtxToPlayerMap.begin());
...
bool SQRNetworkManager_Orbis::RegisterCallbacks()
{
    ...
    if (ret < 0)
    {
        app.DebugPrintf("SQRNetworkManager::RegisterCallbacks - "
                        "cellRudpSetEventHandler failed with code 0x%08x\n",
                        ret);
        return false;
    }
...
void SQRNetworkManager_Orbis::ContextCallback(
    SceNpMatching2ContextId id, SceNpMatching2Event event,
    SceNpMatching2EventCause eventCause, int errorCode, void *arg)
{
    app.DebugPrintf("SQRNetworkManager_Orbis::ContextCallback id:%d, event:%d, "
                    "eventCause:%d, errorCode:0x%08x\n",
    ...
    case SCE_NP_MATCHING2_CONTEXT_EVENT_STARTED:
        if (errorCode < 0)
        {
            ...
            {
                // matching context failed to start (this can happen when you block the
                // IP addresses of the matching servers on your router
        ...
        // Some special cases to detect when this event is coming in, in case we had
        // to start the matching context because there wasn't a valid context when
        // we went to get a server context. These two responses here complete what
        // should then happen to get the server context in each case (for hosting or
        // joining a game)
        if (manager->m_state == SNM_INT_STATE_IDLE_RECREATING_MATCHING_CONTEXT)
        {
        ...
        {
            app.DebugPrintf(
                "SQRNetworkManager_Orbis::ContextCallback - "
                "SCE_NP_MATCHING2_CONTEXT_EVENT_STARTED failed with error 0x%08x\n",
                errorCode);
            manager->SetState(SNM_INT_STATE_INITIALISE_FAILED);
        }
        else
        {
            ...
            // Iggy by calling LoadMovie from a callback, so call it from the tick
            // instead
            m_bCallPSNSignInCallback = true;
            app.DebugPrintf(
                "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ "
            ...
            // Check to see if we were booted from an invite. Only do this once, the
            // first time we have all our networking stuff set up on boot-up
    ...
    case SCE_NP_MATCHING2_CONTEXT_EVENT_STOPPED:
        if (manager->m_state ==
            SNM_INT_STATE_HOSTING_CREATE_ROOM_RESTART_MATCHING_CONTEXT)
        {
            sceNpMatching2ContextStart(manager->m_matchingContext,
                                       (10 * 1000 * 1000));
            break;
        }
        assert(false);
    ...
    case SCE_NP_MATCHING2_CONTEXT_EVENT_START_OVER:
        ...
        if (manager->m_state == SNM_INT_STATE_STARTING_CONTEXT)
        {
            ...
            app.DebugPrintf("SQRNetworkManager_Orbis::ContextCallbackSCE_NP_"
                            "MATCHING2_CONTEXT_EVENT_START_OVER failed\n");
            manager->SetState(SNM_INT_STATE_INITIALISE_FAILED);
        }
        switch (errorCode)
        {
        ...
        case SCE_NP_ERROR_LATEST_PATCH_PKG_DOWNLOADED:
            app.DebugPrintf("SQRNetworkManager_Orbis::ContextCallbackSCE_NP_ERROR_"
                            "LATEST_PATCH_PKG_EXIST failed\n");
            // 4J-PB - need to fail the init, so the external state will be set to
            // idle, and we can continue offline
            manager->SetState(SNM_INT_STATE_INITIALISE_FAILED);
            break;
        }
...
void SQRNetworkManager_Orbis::DefaultRequestCallback(
    ...
    SceNpMatching2Event event, int errorCode, const void *data, void *arg)
{
    ...
    // MGH - added this here for the NAT failure, should only happen for the join
    // request, but without being able to test we can't be sure
    ...
    switch (event)
    {
    // This is the response to sceNpMatching2GetWorldInfoList, which is called as
    // part of the process to create a room (which needs a world to be created
    // in). We aren't anticipating using worlds in a meaningful way so just
    // getting the first world we find on the server here, and then advancing the
    // state so that the tick can get on with the rest of the process.
    case SCE_NP_MATCHING2_REQUEST_EVENT_GET_WORLD_INFO_LIST:
        if (errorCode == SCE_NP_MATCHING2_ERROR_NP_SIGNED_OUT)
        {
            // If we've already signed out, then we should have detected this already
            // elsewhere and so can silently ignore any errors coming in for pending
            // requests here
            break;
        }
        ...
        if (errorCode == 0)
        {
            if (data != 0)
            {
                // Currently just using first world - this may well be all that we need
                // anyway
        ...
        // We get this error when starting a new game after a disconnect occurred in
        // a previous game with at least one remote player. Fix by stopping/starting
        // the matching context. We stop the context here, which is picked up in the
        // callback, and started again. Then the start event is picked up and
        // reattempts the sceNpMatching2GetWorldInfoList.
        ...
        break;
    // This is the response to sceNpMatching2CreateJoinRoom, which if successful
    // means that we are just about ready to move to an online state as host of a
    // game. The final transition actually occurs in the create room tick, on
    // detecting that the state has transitioned to
    // SNM_INT_STATE_HOSTING_CREATE_ROOM_SUCCESS here.
    case SCE_NP_MATCHING2_REQUEST_EVENT_CREATE_JOIN_ROOM:
        ...
        manager->SetState(SNM_INT_STATE_HOSTING_CREATE_ROOM_FAILED);
        break;
    // This is the response to sceNpMatching2JoinRoom, which is called as the
    // final stage of the process started when calling the JoinRoom method. If
    // this is successful, then the state can change to
    // SNM_INT_STATE_JOINING_WAITING_FOR_LOCAL_PLAYERS. We can transition out of
    // that state once we have told the application that all the local players
    // have joined.
    case SCE_NP_MATCHING2_REQUEST_EVENT_JOIN_ROOM:
        ...
        if (errorCode == 0)
        {
            if (data != 0)
            {
                ...
                for (int i = 0; i < roomData->roomDataInternal->roomBinAttrInternalNum;
                     i++)
                {
                    if (roomData->roomDataInternal->roomBinAttrInternal[i].data.id ==
                        SCE_NP_MATCHING2_ROOM_BIN_ATTR_INTERNAL_1_ID)
                    {
                        assert(
                            roomData->roomDataInternal->roomBinAttrInternal[i].data.size ==
                            sizeof(manager->m_roomSyncData));
                        memcpy(
                            &manager->m_roomSyncData,
                            roomData->roomDataInternal[i].roomBinAttrInternal[0].data.ptr,
                            sizeof(manager->m_roomSyncData));

                        //							manager->UpdatePlayersFromRoomSyncUIDs();
                        // Update mapping from the room slot players to SQRNetworkPlayer
                        // instances
        ...
        if (errorCode ==
            SCE_NP_MATCHING2_SERVER_ERROR_ROOM_FULL) // MGH - added to fix "host has
                                                     // exited" error when 2 players
                                                     // go after the final slot
        {
        ...
        break;
    // This is the response to sceNpMatching2GetRoomMemberDataInternal.This only
    // happens on the host, as a response to an incoming connection being
    // established, when we kick off the request for room member internal data so
    // that we can determine what local players that remote machine is intending
    // to bring into the game. At this point we can activate the host end of each
    // of the Rupd connection that this machine requires. We can also update our
    // player slot data (which gets syncronised back out to other room members) at
    // this point.
    case SCE_NP_MATCHING2_REQUEST_EVENT_GET_ROOM_MEMBER_DATA_INTERNAL:
        if (errorCode == 0)
        {
            ...
            if (data != 0)
            {
                SceNpMatching2GetRoomMemberDataInternalResponse *pRoomMemberData =
                    (SceNpMatching2GetRoomMemberDataInternalResponse *)data;
                ...
                if (manager->m_isHosting)
                {
                    ...
                    bool success1 = manager->AddRemotePlayersAndSync(
                        pRoomMemberData->roomMemberDataInternal->memberId, playerMask,
                        &isFull);
                    bool success2;
                    if (success1)
                    {
                        ...
                        if (success2)
                        {
                            bool ret = manager->CreateVoiceRudpConnections(
                                manager->m_room,
                                pRoomMemberData->roomMemberDataInternal->memberId, 0);
                            assert(ret == true);
                            break;
                        }
                    }
                    // Something has gone wrong adding these players to the room - kick
                    // out the player
                    ...
                    // Set flag to indicate whether we were kicked for being out of room
                    // or not
                    reqParam.optData.data[0] = isFull ? 1 : 0;
                    reqParam.optData.len = 1;
                    int ret = sceNpMatching2KickoutRoomMember(manager->m_matchingContext,
                                                              &reqParam, NULL,
                                                              &manager->m_kickRequestId);
                    app.DebugPrintf(
                        CMinecraftApp::USER_RR,
                        "sceNpMatching2KickoutRoomMember returns error 0x%x\n", ret);
                }
                else
                {
                    if (pRoomMemberData->roomMemberDataInternal->roomMemberBinAttrInternal
                            ->data.ptr == NULL)
                    {
                        // the host doesn't send out data, so this must be the host we're
                        // connecting to

                        // If we are the client, then we locally know what Rupd connections
                        // we need (from m_localPlayerJoinMask) and can kick this off.
                        manager->m_hostMemberId =
                            pRoomMemberData->roomMemberDataInternal->memberId;
                        bool ret = manager->CreateRudpConnections(
                            manager->m_room,
                            pRoomMemberData->roomMemberDataInternal->memberId,
                            manager->m_localPlayerJoinMask, manager->m_localMemberId);
                        if (ret == false)
                        {
    ...
    case SCE_NP_MATCHING2_REQUEST_EVENT_LEAVE_ROOM:
        // This is the response to sceNpMatching2LeaveRoom - from the Sony docs,
        // this doesn't ever fail so no need to do error checking here
        // 			SonyVoiceChat::signalDisconnected();
        assert(manager->m_state == SNM_INT_STATE_LEAVING);
        manager->DeleteServerContext();
        break;
    // This is the response to SceNpMatching2GetRoomDataExternalListRequest, which
    // happens when we request the full details of a room we are interested in
    // joining
    case SCE_NP_MATCHING2_REQUEST_EVENT_GET_ROOM_DATA_EXTERNAL_LIST:
        if (errorCode == 0)
        {
            if (data != 0)
            {
                SceNpMatching2GetRoomDataExternalListResponse *pExternalData =
                    (SceNpMatching2GetRoomDataExternalListResponse *)data;
        ...
        {
            manager->m_FriendSessionUpdatedFn(false,
                                              manager->m_pParamFriendSessionUpdated);
        }
        break;
    case SCE_NP_MATCHING2_REQUEST_EVENT_SET_ROOM_DATA_EXTERNAL:
        if ((errorCode != 0) ||
            manager->ForceErrorPoint(SNM_FORCE_ERROR_SET_ROOM_DATA_CALLBACK))
        {
            app.DebugPrintf(CMinecraftApp::USER_RR,
...
void SQRNetworkManager_Orbis::RoomEventCallback(SceNpMatching2ContextId id,
                                                ...
                                                const void *data, void *arg)
{
    ...
    case SCE_NP_MATCHING2_ROOM_EVENT_KICKEDOUT:
        {
            ...
            // We've been kicked out. This server has rejected our attempt to join, most
            // likely because there wasn't enough space in the server to have us.
            // There's a flag set so we can determine which thing has happened
            // 				assert ( dataSize <=
    ...
    case SCE_NP_MATCHING2_ROOM_EVENT_ROOM_DESTROYED:
...

        {
            ...
            // If we're hosting, then handle this a bit like a disconnect, in that we
            // will shift the game into an offline game - but don't need to actually
            // leave the room since that has been destroyed and so isn't there to be
            // left anymore. Don't do this if we are disconnected though, as we've
            // already handled this.
            if ((manager->m_isHosting) && !manager->m_bLinkDisconnected)
            {
                // MGH - we're not receiving an SCE_NP_MATCHING2_SIGNALING_EVENT_DEAD
                // after this so we have to remove all the remote players
                while (manager->m_RudpCtxToPlayerMap.size())
                {
                    SQRNetworkPlayer *pRemotePlayer =
                        manager->m_RudpCtxToPlayerMap.begin()->second;
                    manager->RemoveRemotePlayersAndSync(pRemotePlayer->m_roomMemberId,
                                                        15);
                }

                if (pUpdateInfo && (pUpdateInfo->eventCause ==
    ...
    case SCE_NP_MATCHING2_ROOM_EVENT_UPDATED_ROOM_DATA_INTERNAL:
        // We are using the room internal data to synchronise the player data stored
        // in m_roomSyncData from the host to clients. The host is the thing
        // creating the internal room data, so it doesn't need to update itself.
        if (!manager->m_isHosting)
        {
            // 				assert ( dataSize <=
                ...
                !manager->ForceErrorPoint(SNM_FORCE_ERROR_UPDATED_ROOM_DATA))
            {
                ...
                for (int i = 0; i < pRoomData->newRoomBinAttrInternalNum; i++)
                {
                    if (pRoomData->newRoomBinAttrInternal[i]->data.id ==
                        SCE_NP_MATCHING2_ROOM_BIN_ATTR_INTERNAL_1_ID)
                    {
                        assert(pRoomData->newRoomBinAttrInternal[i]->data.size ==
                               sizeof(manager->m_roomSyncData));
    ...
    case SCE_NP_MATCHING2_ROOM_EVENT_UPDATED_ROOM_MEMBER_DATA_INTERNAL:
        ...
        {
            // We'll get this sync'd round all the connected clients, but we only care
            // about it on the host where we can use it to work out if any RUDP
            // connections need to be made or released
            if (manager->m_isHosting)
            {
                // 					assert( dataSize <=
                ...
                // (void *)(manager->cRoomMemberDataInternalUpdate),
                // SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_RoomMemberDataInternalUpdateInfo);
                if ((data) &&
                ...
                {
                    gotEventData = true;
                    SceNpMatching2RoomMemberDataInternalUpdateInfo *pRoomMemberData =
                    ...
                    if (addedMask != 0)
                    {
                        ...
                        {
                            // Failed for some reason - signal back to the client that this is
                            // the case, by updating its internal data back again, rather than
                            // have it wait for a timeout on its rudp connection
                            // initialisation.
                            SceNpMatching2SetRoomMemberDataInternalRequest reqParam;
                            SceNpMatching2BinAttr binAttr;
                        ...
                        else
                        {
                            success = manager->CreateVoiceRudpConnections(
                                manager->m_room,
                                pRoomMemberData->newRoomMemberDataInternal->memberId, 0);
                            assert(success);
                        }
                    }
            ...
            {
                // If, as a client, we receive an updated room member data this could be
                // for two reason. (1) Another client in the game has updated their own
                // data as someone has joined/left the session (2) The server has set
                // someone's data back, due to a failed attempt to join a game We're
                // only interested in scenario (2), when the data that has been updated
                // is our own, in which case we know to abandon creating rudp
                // connections etc. for a new player
                // 					assert( dataSize <=
                // SCE_NP_MATCHING2_EVENT_DATA_MAX_SIZE_RoomMemberDataInternalUpdateInfo
                // ); 					int ret =
                // sceNpMatching2GetEventData(manager->m_matchingContext, eventKey,
...
void SQRNetworkManager_Orbis::SignallingCallback(
...
{
    // MGH - changed this to queue up the signalling events from the callback and
    // process them later on the server thread
...
void SQRNetworkManager_Orbis::ProcessSignallingEvent(
    SceNpMatching2ContextId ctxId, SceNpMatching2RoomId roomId,
    SceNpMatching2RoomMemberId peerMemberId, SceNpMatching2Event event,
    int error_code)
{
    ...
    case SCE_NP_MATCHING2_SIGNALING_EVENT_DEAD:
        {
            ...
            {
                // Host has left the game... so its all over for this client too. Finish
                // everything up now, including deleting the server context which belongs
                // to this gaming session This also might be a response to a request to
                // leave the game from our end too so don't need to do anything in that
                // case
            ...
            {
                // we've lost connection to another client (voice only) so kill the voice
                // connection no players left on the remote machine once we remove this
                // one
    ...
    case SCE_NP_MATCHING2_SIGNALING_EVENT_ESTABLISHED:
        {
            // MGH - changed this to always get the data now, as we need to know if the
            // connecting peer is the host or not If we're the host, then we need to get
            // the data associated with the connecting peer to know what connections we
            // should be trying to match. So the actual creation of connections happens
            // when the response for this request is processed.
            ...
            sceNpMatching2GetRoomMemberDataInternal(m_matchingContext, &reqParam, NULL,
                                                    &m_roomMemberDataRequestId);
        }
        break;
    }
}
...
void SQRNetworkManager_Orbis::SignallingEventsTick()
{
    ...
    for (int i = 0; i < m_signallingEventList.size(); i++)
    {
        ...
        ProcessSignallingEvent(ev.ctxId, ev.roomId, ev.peerMemberId, ev.event,
                               ev.error_code);
    }
...
int SQRNetworkManager_Orbis::BasicEventCallback(int event, int retCode,
...
void SQRNetworkManager_Orbis::OnlineCheck()
{
    ...
    {
        // If it's an online game, and the primary profile is no longer signed into
        // LIVE then we act as if disconnected
        if ((bSignedIn == false) && m_isInSession &&
...
void SQRNetworkManager_Orbis::SysUtilCallback(uint64_t status, uint64_t param,
                                              void *userdata)
{
...
void SQRNetworkManager_Orbis::RudpContextCallback(int ctx_id, int event_id,
                                                  int error_code, void *arg)
{
    SQRNetworkManager_Orbis *manager = (SQRNetworkManager_Orbis *)arg;
    switch (event_id)
    ...
    case SCE_RUDP_CONTEXT_EVENT_CLOSED:
        {
            app.DebugPrintf(sc_verbose,
                            ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
                            ">>    SCE_RUDP_CONTEXT_EVENT_CLOSED\n");
            SQRVoiceConnection *pVoice =
                SonyVoiceChat_Orbis::GetVoiceConnectionFromRudpCtx(ctx_id);
            if (pVoice)
            {
    ...
    case SCE_RUDP_CONTEXT_EVENT_ESTABLISHED:
        {
            app.DebugPrintf(sc_verbose,
                            ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
                            ">>    SCE_RUDP_CONTEXT_EVENT_ESTABLISHED\n");
    ...
    case SCE_RUDP_CONTEXT_EVENT_ERROR:
        app.DebugPrintf(sc_verbose,
                        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
                        ">>    SCE_RUDP_CONTEXT_EVENT_ERROR\n");
        break;
    case SCE_RUDP_CONTEXT_EVENT_WRITABLE:
        {
            SQRNetworkPlayer *player = manager->GetPlayerFromRudpCtx(ctx_id);
            // This event signifies that room has opened up in the write buffer, so
    ...
    case SCE_RUDP_CONTEXT_EVENT_READABLE:
        if (manager->m_listener)
        {
            ...
            else
            {
                ...
                // If we're the host, and this player hasn't yet had its small id
                // confirmed, then the first byte sent to us should be this id
                if (manager->m_isHosting)
                {
                    ...
                    if (playerFrom && !playerFrom->HasSmallIdConfirmed())
                    {
                        if (dataSize >= sizeof(SQRNetworkPlayer::InitSendData))
                        {
                            ...
                            int bytesRead =
                                sceRudpRead(ctx_id, &ISD,
                                            sizeof(SQRNetworkPlayer::InitSendData), 0, NULL);
                ...
                if (dataSize > 0)
                {
                    ...
                    if (bytesRead > 0)
                    {
                        ...
                        {
                            manager->m_listener->HandleDataReceived(playerFrom, playerTo,
                                                                    data, bytesRead);
                        }
                    }
                    delete[] data;
    ...
    case SCE_RUDP_CONTEXT_EVENT_FLUSHED:
        app.DebugPrintf(sc_verbose,
                        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
                        ">>    SCE_RUDP_CONTEXT_EVENT_FLUSHED\n");
        break;
    }
}

// Implementation of CellRudpEventHandler
int SQRNetworkManager_Orbis::RudpEventCallback(
    int event_id, int soc, uint8_t const *data, size_t datalen,
...
void SQRNetworkManager_Orbis::ServerContextValid_JoinRoom()
{
    ...
    // Join the room, passing the local player mask as initial binary data so that
    // the host knows what local players are here
...
void SQRNetworkManager_Orbis::GetExtDataForRoom(
    ...
    void (*FriendSessionUpdatedFn)(bool success, void *pParam), void *pParam)
{
    ...
    {
        // No matching context, and failed to try and make one. We're really broken
        // here.
    ...
    {
        app.DebugPrintf("Having to recreate matching context, setting state to "
    ...
    if (ret ==
        SCE_NP_MATCHING2_ERROR_CONTEXT_NOT_STARTED) // Also checking for this as a
                                                    // means of simulating the
                                                    // previous error
    {
        sceNpMatching2DestroyContext(m_matchingContext);
        m_matchingContextValid = false;
...
int ErrorPSNDisconnectedDialogReturned(
    void *pParam, int iPad, const C4JStorage::EMessageResult iResult)
{
...
void SQRNetworkManager_Orbis::AttemptPSNSignIn(
    int (*SignInCompleteCallbackFn)(void *pParam, bool bContinue, int pad),
    void *pParam, bool callIfFailed /*=false*/, int iPad /*=-1*/)
{
    ...
    if (ProfileManager.isSignedInPSN(iPad) == false)
    {
        ...
        if (NPError == SCE_NP_ERROR_SIGNED_OUT ||
            NPError == SCE_NP_ERROR_NOT_SIGNED_UP)
        {
            ...
            if (ret == SCE_OK)
            {
                ...
                dialogParameter.errorCode =
                    SCE_NP_ERROR_SIGNED_OUT; // for force display.
    ...
    {
        // we're signed into PSN, but we don't have a net connection, throw up a lan
        // error
        UINT uiIDA[1];
        uiIDA[0] = IDS_CONFIRM_OK;
        ui.RequestMessageBox(IDS_ERROR_NETWORK_TITLE, IDS_ERROR_NETWORK, uiIDA, 1,
                             iPad, ErrorPSNDisconnectedDialogReturned, pParam,
                             app.GetStringTable());
    }
}
...
void SQRNetworkManager_Orbis::UpdateRichPresenceCustomData(
...
void SQRNetworkManager_Orbis::SendLastPresenceInfo()
{
    ...
    // If we are trying to send at a rate faster than we should be, then use the
    // resend mechanism to send at an appropriate time
    if ((System::currentTimeMillis() - s_lastPresenceTime) <
        MIN_PRESENCE_RESEND_TIME)
    {
    ...
    {
        // Prioritise data over status as it is critical to discovering the network
        // game
    ...
    if (ProfileManager.IsSignedInLive(
            ProfileManager.getQuadrant(s_lastPresenceInfo.userInfo.userId)))
    {
        err =
            sce::Toolkit::NP::Presence::Interface::setPresence(&s_lastPresenceInfo);
    ...
    {
        s_resendPresenceTime =
            System::currentTimeMillis() + MIN_PRESENCE_RESEND_TIME;
    }
}
...
void SQRNetworkManager_Orbis::SetPresenceDataStartHostingGame()
{
    if (m_offlineGame)
    {
        SQRNetworkManager_Orbis::UpdateRichPresenceCustomData(
            &c_presenceSyncInfoNULL,
            sizeof(SQRNetworkManager_Orbis::PresenceSyncInfo));
    }
    else
    {
        ...
        SQRNetworkManager_Orbis::UpdateRichPresenceCustomData(
            &presenceInfo, sizeof(SQRNetworkManager_Orbis::PresenceSyncInfo));
...
void SQRNetworkManager_Orbis::removePlayerFromVoiceChat(
    SQRNetworkPlayer *pPlayer)
{
    ...
    else
    {
        ...
        if (numRemotePlayersLeft == 0)
        {
            ...
            SQRVoiceConnection *pVoice =
                SonyVoiceChat_Orbis::getVoiceConnectionFromRoomMemberID(
...
void SQRNetworkManager_Orbis::TickNotify()
{
    if (g_NetworkManager.IsInSession() && !g_NetworkManager.IsLocalGame())
    {
        ...
        // Note: interval at which to notify Sony of realtime play, according to
        // docs an interval greater than 1 sec is bad but in testing NP debug was
        // happy with 5 seconds, to be on the safe side call it 0.75 seconds
...
void SQRNetworkManager_Orbis::NotifyRealtimePlusFeature(int iQuadrant)
{
    ...
    if (err != SCE_OK)
    {
        app.DebugPrintf("SQRNetworkManager_Orbis::NotifyRealtimePlusFeature: "
                        "sceNpNotifyPlusFeature failed (0x%x)\n",
                        err);
        assert(0);
    }
}
...
// 	if( s_SignInCompleteCallbackFn)
// 	{
// 		app.DebugPrintf("============ Calling CallSignInCompleteCallback
// and s_SignInCompleteCallbackFn is OK\n"); 		bool isSignedIn =
// ProfileManager.IsSignedInLive(s_SignInCompleteCallbackPad);
//
// 		s_SignInCompleteCallbackFn(s_SignInCompleteParam, isSignedIn,
// s_SignInCompleteCallbackPad); 		s_SignInCompleteCallbackFn  =
// NULL; 		s_SignInCompleteCallbackPad = -1;
// 	}
// 	else
// 	{
// 		app.DebugPrintf("============ Calling CallSignInCompleteCallback
// but s_SignInCompleteCallbackFn is NULL\n");
// 	}
//}
