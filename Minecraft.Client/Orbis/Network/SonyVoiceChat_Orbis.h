...
class SQRLocalVoiceDevice
{
  ...
  public:
    ...
    {
        m_bFlaggedForShutdown = true;
    }
    void shutdownWhenFlagged();
    bool isValid()
    {
        return m_localUserID != SCE_USER_SERVICE_USER_ID_INVALID;
    }
    // 	void setBitRate()
    // 	{
...
class SQRVoiceConnection
{
    ...
    static const int SNP_MAX_PAYLOAD =
        1346; // This is the default RUDP payload size - if we want to change this
              // we'll need to use cellRudpSetOption to set something else &
              // adjust segment size
  ...
  public:
    int m_rudpCtx;
    bool m_bConnected;
    uint32_t m_voiceInPort;      // 1 input port per connection, incoming UDP packets
                                 // are written to each of these, and then they're
                                 // connected out to all headsets
    ...
    SceNpMatching2RoomMemberId
        m_remoteRoomMemberId; // Assigned by Matching2 lib, we can use to indicate
                              // which machine this player belongs to (note - 16
                              // bits)
...
class SonyVoiceChat_Orbis
{
  public:
    ...
    static SQRVoiceConnection *
    addRemoteConnection(int RudpCxt, SceNpMatching2RoomMemberId peerMemberId);
  ...
  private:
    ...
    static uint32_t
        m_voiceOutPort; // single output port that all local devices are mixed to,
                        // and then sent out to all other remote machines
    ...
    static bool m_forceSendPacket; // force a packet across the network, even if
