...
// SonyCommerce_Orbis::m_sessionInfo;
...
std::vector<SonyCommerce_Orbis::ProductInfo>
    *SonyCommerce_Orbis::m_pProductInfoList = NULL;
SonyCommerce_Orbis::ProductInfoDetailed
    *SonyCommerce_Orbis::m_pProductInfoDetailed = NULL;
...
SonyCommerce_Orbis::CheckoutInputParams
    SonyCommerce_Orbis::m_checkoutInputParams;
SonyCommerce_Orbis::DownloadListInputParams
    SonyCommerce_Orbis::m_downloadInputParams;

SonyCommerce_Orbis::CallbackFunc SonyCommerce_Orbis::m_callbackFunc = NULL;
// sys_memory_container_t
// SonyCommerce_Orbis::m_memContainer = SYS_MEMORY_CONTAINER_ID_INVALID;
...
bool SonyCommerce_Orbis::m_contextCreated =
    false; ///< npcommerce2 context ID created?
SonyCommerce_Orbis::Phase SonyCommerce_Orbis::m_currentPhase =
    e_phase_stopped; ///< Current commerce2 util
// char
// SonyCommerce_Orbis::m_commercebuffer[SCE_NP_COMMERCE2_RECV_BUF_SIZE];

C4JThread *SonyCommerce_Orbis::m_tickThread = NULL;
bool SonyCommerce_Orbis::m_bLicenseChecked =
...
void SonyCommerce_Orbis::CheckForTrialUpgradeKey_Callback(LPVOID param,
                                                          bool bFullVersion)
{
    ...
    if (ProfileManager.IsFullVersion())
    {
        ...
        //  SQRNetworkManager_Orbis::GetSceNpCommsId(),
...
void SonyCommerce_Orbis::copyProductList(
    ...
    std::vector<sce::Toolkit::NP::ProductInfo> *pNPProductList)
{
    ...
    pNPProductList->clear(); // clear the vector now we're done, this doesn't
                             // happen automatically for the next query

    // Set our result
    *pProductList = tempProductVec;
}

int SonyCommerce_Orbis::getProductList(std::vector<ProductInfo> *productList,
                                       char *categoryId)
{
    int ret;
    ...
    {
        // result has returned immediately (don't think this should happen, but was
        // handled in the samples
...
void SonyCommerce_Orbis::copyCategoryInfo(
...
int SonyCommerce_Orbis::getCategoryInfo(CategoryInfo *pInfo, char *categoryId)
{
    ...
    ret = sce::Toolkit::NP::Commerce::Interface::getCategoryInfo(&g_categoryInfo,
                                                                 params, true);
    if (ret < 0)
    {
        // error
...
void SonyCommerce_Orbis::copyDetailedProductInfo(
...
int SonyCommerce_Orbis::getDetailedProductInfo(ProductInfoDetailed *pInfo,
...
void SonyCommerce_Orbis::copyAddDetailedProductInfo(
    ProductInfo *pInfo, sce::Toolkit::NP::ProductInfoDetailed *pNPInfo)
{
    ...
    // npInfo.releaseDate; 	strncpy(pInfo->ratingSystemId,
    // npInfo.ratingSystemId, SCE_NP_COMMERCE2_RATING_SYSTEM_ID_LEN);
...
int SonyCommerce_Orbis::addDetailedProductInfo(ProductInfo *pInfo,
                                               const char *productId,
                                               char *categoryId)
{
    int ret;
...
int SonyCommerce_Orbis::checkout(CheckoutInputParams &params)
{
    ...
    while (iter != iterEnd)
    {
        npParams.skuIds.push_back(
            (char *)*iter); // have to remove the const here, not sure why the libs
                            // pointers aren't const
        iter++;
    }
...
int SonyCommerce_Orbis::downloadList_game(DownloadListInputParams &params)
{
    ...
    app.DebugPrintf("Starting Store Download List...\n");
    // 	ret =
    // sce::Toolkit::NP::Commerce::Interface::displayDownloadList(npParams, true);
    // 	if (ret < 0)
    // 	{
...
void SonyCommerce_Orbis::UpgradeTrialCallback1(LPVOID lpParam, int err)
{
    ...
    if (err == SCE_OK)
    {
        ...
        else
        {
            ...
            pCommerce->DownloadAlreadyPurchased_Game(UpgradeTrialCallback2, pCommerce,
                                                     skuID);
        }
    }
    else
...
int SonyCommerce_Orbis::createSession()
{
    ...
    //
    // 	This function is provided to maintain compatibility with the
    // PlayStation®Vita and PlayStation®3 platforms. Because commerce on the
    // PlayStation®4 is not session based, SCE_TOOLKIT_NP_SUCCESS is always
    // returned.
...
void SonyCommerce_Orbis::commerce2Handler(
    const sce::Toolkit::NP::Event &event)
{
    ...
    {
        // 4J-PB - this happens when we've signed out of the PSn, and we were in the
        // process of retrieving DLC product info Ignore this event
        app.DebugPrintf(
            "@@@@@@@@ IGNORING COMMERCE EVENT AFTER COMMERCE SHUTDOWN @@@@@@@@@\n");
        return;
    }
...
void SonyCommerce_Orbis::processMessage()
{
    ...
    case e_message_commerceAddDetailedProductInfo:
        ...
        // 			} else {
        //
        // 				ret = consumeEntitlement(*(EntitlementToConsume
        // *)msg.inputArgs); 				if (ret < 0) {
        // m_event = e_event_commerceError; 					m_errorCode = ret; 				} else { 					m_event =
...
void SonyCommerce_Orbis::processEvent()
{
    ...
    case e_event_commerceProductBrowseFinished:
        app.DebugPrintf(4,
                        "e_event_commerceProductBrowseFinished succeeded: 0x%x\n",
                        m_errorCode);
    ...
    case e_event_commerceCheckoutFinished:
        ...
        // 4J-PB - if there's been an error - like dlc already purchased, the
        // runcallback has already happened, and will crash this time
        if (m_callbackFunc != NULL)
        {
            runCallback();
        }
...
int SonyCommerce_Orbis::commerceEnd()
{
    ...
    // &m_sessionInfo); 	else if (m_currentPhase == e_phase_checkoutPhase)
    // ret = sceNpCommerce2DoCheckoutFinishAsync(m_contextId); 	else if
    // (m_currentPhase
...
void SonyCommerce_Orbis::CreateSession(CallbackFunc cb, LPVOID lpParam)
{
    ...
    // the m_errorCode was picked up in the message queue after the commerce init
    // call
    if (m_errorCode != 0)
    {
    ...
    //	This function is provided to maintain compatibility with the
    // PlayStation®Vita and PlayStation®3 	platforms. Because commerce on
    // the PlayStation®4 is not session based, SCE_TOOLKIT_NP_SUCCESS is always
    // returned.
...
void SonyCommerce_Orbis::CloseSession()
{
    ...
    // 4J-PB - don't call shutdown here - the SonyCommerce_Orbis::TickLoop thread
    // is still running and could crash accessing the critical section that
    // Shutdown destroys
    // Shutdown();
}

void SonyCommerce_Orbis::GetProductList(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce_Orbis::GetDetailedProductInfo(
    CallbackFunc cb, LPVOID lpParam, ProductInfoDetailed *productInfo,
    const char *productId, const char *categoryId)
{
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Orbis::AddDetailedProductInfo(CallbackFunc cb, LPVOID lpParam,
...
void SonyCommerce_Orbis::GetCategoryInfo(CallbackFunc cb, LPVOID lpParam,
                                         CategoryInfo *info,
                                         const char *categoryId)
{
    EnterCriticalSection(&m_queueLock);
...
void SonyCommerce_Orbis::Checkout(CallbackFunc cb, LPVOID lpParam,
                                  const char *skuID)
{
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Orbis::DownloadAlreadyPurchased(CallbackFunc cb,
                                                  LPVOID lpParam,
                                                  const char *skuID)
{
    EnterCriticalSection(&m_queueLock);
...
void SonyCommerce_Orbis::Checkout_Game(CallbackFunc cb, LPVOID lpParam,
                                       const char *skuID)
{
    EnterCriticalSection(&m_queueLock);
    setCallback(cb, lpParam);
...
void SonyCommerce_Orbis::DownloadAlreadyPurchased_Game(CallbackFunc cb,
                                                       LPVOID lpParam,
                                                       const char *skuID)
{
    EnterCriticalSection(&m_queueLock);
