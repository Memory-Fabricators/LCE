...
LeaderboardManager *LeaderboardManager::m_instance =
    new OrbisLeaderboardManager(); // Singleton instance of the
                                   // LeaderboardManager
...
int OrbisLeaderboardManager::scoreboardThreadEntry(LPVOID lpParam)
{
    ...
    OrbisLeaderboardManager *self =
        reinterpret_cast<OrbisLeaderboardManager *>(lpParam);
...
void OrbisLeaderboardManager::scoreboardThreadInternal()
{
    ...
    case eStatsState_Getting:
        ...
        // AsyncMultiplay Note 1: The following NP call should be reserved for
        // asynchronous multiplayer modes that require PS Plus to be accessed.
        //
        //	Note 2:
...
bool OrbisLeaderboardManager::getScoreByIds()
{
    ...
    {
        app.DebugPrintf("[LeaderboardManager] OpenSession() fail: User isn't "
                        "signed in to PSN\n");
        return false;
    }
    ...
    if (m_eFilterMode == eFM_Friends)
    {
        ...
        ret = sce::Toolkit::NP::Friends::Interface::getFriendslist(&s_friendList,
                                                                   &request, false);
        ...
        else if (s_friendList.hasResult())
        {
            ...
            for (itr = s_friendList.get()->begin(); itr != s_friendList.get()->end();
                 itr++)
            {
                npIds[i] = itr->npid;
                i++;
    ...
    for (int batch = 0; batch < num; batch += 100)
    {
        ...
        if (m_eStatsState == eStatsState_Canceled)
        {
            ...
            app.DebugPrintf("[LeaderboardManager]\tgetScoreByIds() - m_eStatsState "
        ...
        else if (ret < 0)
        {
            ...
            app.DebugPrintf("[LeaderboardManager]\tgetScoreByIds() - "
                            "createTransaction failed, ret=0x%X\n",
                            ret);
    ...
    app.DebugPrintf("[LeaderboardManager] getScoreByIds(), Success!\n"
                    "\t Board %i\n"
...
bool OrbisLeaderboardManager::getScoreByRange()
{
    ...
    {
        // Cancel operation has been called, abort.
        app.DebugPrintf("[LeaderboardManager]\tgetScoreByRange() - m_eStatsState "
                        "== eStatsState_Canceled.\n");
        sceNpScoreDestroyTransactionCtx(ret);
        return false;
    }
    else if (ret < 0)
    {
        // Error occurred creating a transacion, abort.
        m_eStatsState = eStatsState_Failed;
        app.DebugPrintf("[LeaderboardManager]\tgetScoreByRange() - "
                        "createTransaction failed, ret=0x%X\n",
                        ret);
        return false;
    }
    ...
    else if (ret == SCE_NP_COMMUNITY_SERVER_ERROR_GAME_RANKING_NOT_FOUND)
    {
        ret = sceNpScoreDestroyTransactionCtx(m_requestId);
        app.DebugPrintf(
            "[LeaderboardManager] getScoreByRange(): Game ranking not found.");
    ...
    {
        app.DebugPrintf(
            "[LeaderboardManager] getScoreByRange(), success, 1stScore=%i.\n",
            ptr->scoreValue);
    }
...
bool OrbisLeaderboardManager::setScore()
{
    ...
    {
        app.DebugPrintf("[LeaderboardManager] setScore(): sceNpScoreCreateTitleCtx "
                        "FAILED, ret == %X.\n",
                        ret);
        return false;
    }
    ...
    {
        app.DebugPrintf(
            "[LeaderboardManager] setScore(), doesn't beat current score, %i.\n",
            tmp);
    }
    ...
    {
        app.DebugPrintf(
            "[LeaderboardManager] setScore(), success. boardId=%i, score=%i\n",
...
bool OrbisLeaderboardManager::OpenSession()
{
    if (m_openSessions == 0)
    {
        ...
        app.DebugPrintf(
            "[LeaderboardManager] OpenSession(): Starting sceNpScore utility.\n");
    }
    else
    {
...
void OrbisLeaderboardManager::CloseSession()
{
    ...
    {
        app.DebugPrintf(
            "[LeaderboardManager] CloseSession(): Quitting sceNpScore utility.\n");
    }
    else
    {
        app.DebugPrintf(
...
bool OrbisLeaderboardManager::WriteStats(unsigned int viewCount, ViewIn views)
{
    ...
    {
        app.DebugPrintf("[LeaderboardManager] WriteStats(), starting. "
                        "difficulty=%i, statsType=%i, score=%i\n",
...
bool OrbisLeaderboardManager::ReadStats_Friends(
...
// myUID ignored on PS3.
bool OrbisLeaderboardManager::ReadStats_MyScore(
    LeaderboardReadListener *listener, int difficulty, EStatsType type,
...
// myUID ignored on PS3.
bool OrbisLeaderboardManager::ReadStats_TopRank(
    LeaderboardReadListener *listener, int difficulty, EStatsType type,
...
void OrbisLeaderboardManager::CancelOperation()
{
    ...
    if (m_requestId != 0)
    {
        ...
        {
            app.DebugPrintf("[LeaderboardManager] CancelOperation(): Operation "
    ...
    {
        app.DebugPrintf(
            "[LeaderboardManager] CancelOperation(): No current operation.\n");
    }
}
...
/*
SceNpScoreRankData *OrbisLeaderboardManager::filterJustScorers(unsigned int
&num, SceNpScorePlayerRankData *friendsData)
{
int num2 = 0;
...
SceNpScorePlayerRankData *
OrbisLeaderboardManager::addPadding(unsigned int num,
...
void OrbisLeaderboardManager::convertToOutput(
    unsigned int &num, ReadScore *out, SceNpScorePlayerRankData *rankData,
...
void OrbisLeaderboardManager::initReadScoreStruct(
    ReadScore &out, SceNpScoreRankData &rankData)
{
    ...
    wstring wstrName =
        convStringToWstring(string(rankData.npId.handle.data)).c_str();
    // memcpy(&out.m_name, wstrName.c_str(), XUSER_NAME_SIZE);
    out.m_name = wstrName;
}

void OrbisLeaderboardManager::fillReadScoreStruct(ReadScore &out,
                                                  SceNpScoreComment &comment)
{
    StatsData statsData;
...
bool OrbisLeaderboardManager::SortByRank(const ReadScore &lhs,
                                         const ReadScore &rhs)
{
...
SceNpNotifyPlusFeatureParameter(); param.userId =
ProfileManager.getUserID(ProfileManager.GetPrimaryPad()); param.size =
sizeof(SceNpNotifyPlusFeatureParameter); param.features =
SCE_NP_PLUS_FEATURE_ASYNC_MULTIPLAY; ZeroMemory(param.padding, sizeof(char) *
4); ZeroMemory(param.reserved, sizeof(uint8_t) * 32);

        int err = sceNpNotifyPlusFeature(&param);
        if (err != SCE_OK)
        {
                app.DebugPrintf("OrbisLeaderboardManager::NotifyAsyncPlusFeature:
