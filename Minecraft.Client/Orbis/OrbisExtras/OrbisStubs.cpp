...
// These are required so that the heap will automatically expand - default is
// limited to 256KB
...
VOID OrbisInit()
{
    ...
    sceAudioOutInit();
    hBGMAudio = sceAudioOutOpen(SCE_USER_SERVICE_USER_ID_SYSTEM,
                                SCE_AUDIO_OUT_PORT_TYPE_BGM, 0, 256, 48000, 2);

    err = sceCommonDialogInitialize();
    // err = sceNpCommerceDialogInitialize();
    assert(err == SCE_OK);

    // 4J-PB - Can't keep this initialised as we monitor and handle the finished
    // status in the main menu for patches
    // 	err = sceErrorDialogInitialize();
    // 	assert(err == SCE_OK);

    // 4J-PB - can't init this here - it conflicts with the commerce dialog for
    // dlc checkouts/PSPlus upsells. We need to init it when we use it, and then
    // terminate it.
...
// we have to manage our own virtual allocs here, so this class stores all the
...
class OrbisVAlloc
{
  public:
    ...
    {
        uint64_t sizeToAdd =
            size - m_allocatedSize; // the extra memory size that we have to add on
        assert(sizeToAdd >= 0);
        ...
        int err = sceKernelAllocateDirectMemory(0, SCE_KERNEL_MAIN_DMEM_SIZE,
                                                sizeToAdd, 16 * 1024,
                                                SCE_KERNEL_WB_ONION, &physAddr);
        if (err != SCE_OK)
        {
            assert(0);
            return NULL;
        }
        // work out where the next page should be in virtual addr space, and pass
        // that to the mapping function
        void *pageVirtualAddr = ((char *)m_virtualAddr) + m_allocatedSize;

        void *inAddr = pageVirtualAddr;
        err = sceKernelMapDirectMemory(&inAddr, sizeToAdd,
                                       SCE_KERNEL_PROT_CPU_READ |
                                           SCE_KERNEL_PROT_CPU_WRITE,
                                       SCE_KERNEL_MAP_FIXED, physAddr, 16 * 1024);

        if (inAddr != pageVirtualAddr) // make sure we actually get the virtual
                                       // address that we requested
        {
            assert(0);
            return NULL;
        }
    ...
    void Decommit()
    {
        ...
        {
            int err = sceKernelMunmap(m_pagesAllocated[i].m_virtualAddr,
                                      m_pagesAllocated[i].m_size);
            assert(err == SCE_OK);
            err = sceKernelReleaseDirectMemory(m_pagesAllocated[i].m_physAddr,
                                               m_pagesAllocated[i].m_size);
            assert(err == SCE_OK);
        }
...
BOOL VirtualFree(LPVOID lpAddress, SIZE_T dwSize, DWORD dwFreeType)
{
    ...
    assert(idx >= 0);
    assert(dwSize ==
           s_orbisVAllocs[idx]->m_virtualSize); // only supporting decommitting
               ...
               LPDWORD lpNumberOfBytesWritten, LPOVERLAPPED lpOverlapped)
{
    ...
    SceFiosSize bytesRead = sceFiosFHWriteSync(
        NULL, fh, lpBuffer, (SceFiosSize)nNumberOfBytesToWrite);
    if (bytesRead < 0)
    {
        // error
        return FALSE;
              ...
              LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped)
{
    ...
    // sceFiosFHReadSync - Non-negative values are the number of bytes read, 0 <=
    // result <= length. Negative values are error codes.
    SceFiosSize bytesRead =
        sceFiosFHReadSync(NULL, fh, lpBuffer, (SceFiosSize)nNumberOfBytesToRead);
...
DWORD GetFileAttributesA(LPCSTR lpFileName)
{
    ...
    {
        sprintf(filePath, "%s/%s", getUsrDirPath(),
                lpFileName); // set to load from host
    }
...
DWORD GetTickCount()
{
    ...
    // The system time is represented the time elapsed since the system starts up
    // in microseconds.
