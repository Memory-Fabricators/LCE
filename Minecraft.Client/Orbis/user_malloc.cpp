...
#include <stdlib.h>
    ...
    int user_malloc_init(void)
    {
        ...
        s_heapLength =
            ((size_t)4608) * 1024 * 1024;          // Initial allocation for the application
        s_heapLength -= ((size_t)4) * 1024 * 1024; // Allocated for TLS
        s_heapLength -= ((size_t)2) * 1024 *
                        1024; // 64K (sometimes?) allocated for razor - rounding up to
                              // 2MB here to match our alignment
        s_heapLength -=
            ((size_t)24) * 1024 *
            1024; // 24MB for save file, which is handled by virtual allocs
        s_heapLength -=
            ((size_t)512) * 1024 * 1024; // 512MB allocated for GPU by renderer
        s_heapLength -=
            ((size_t)112) * 1024 * 1024; // 112MB allocated for CPU by renderer
        s_memLength = s_heapLength;

        // E Allocate direct memory
        res = sceKernelAllocateDirectMemory(0, SCE_KERNEL_MAIN_DMEM_SIZE, s_memLength,
                                            s_memAlign, SCE_KERNEL_WB_ONION,
                                            &s_memStart);
        if (res < 0)
        {
            // E Error handling
        ...
        res = sceKernelMapDirectMemory(
            &addr, s_heapLength, SCE_KERNEL_PROT_CPU_READ | SCE_KERNEL_PROT_CPU_WRITE,
            0, s_memStart, s_memAlign);
        if (res < 0)
        {
            // E Error handling
            return 1;
