...
LeaderboardManager *LeaderboardManager::m_instance =
    new XboxLeaderboardManager(); // Singleton instance of the
                                  // LeaderboardManager
#endif

const XboxLeaderboardManager::LeaderboardDescriptor XboxLeaderboardManager::
    LEADERBOARD_DESCRIPTORS[XboxLeaderboardManager::NUM_LEADERBOARDS][4] = {
        {
            XboxLeaderboardManager::LeaderboardDescriptor(
...
void XboxLeaderboardManager::Tick()
{
    if (m_eStatsState == eStatsState_Getting)
    {
        ...
        if (IsStatsReadComplete())
        {
            ...
            {
                // 4J Stu - If the state is other than ready, then we don't have any
                // stats to sort
                if (m_eFilterMode == LeaderboardManager::eFM_Friends &&
                    m_eStatsState == eStatsState_Ready)
                {
                    SortFriendStats();
                }

                bool ret = m_readListener->OnStatsReadComplete(
                    m_eStatsState == eStatsState_Ready, 0, m_stats); // TODO
                if (ret)
                {
                    m_eStatsState = eStatsState_Idle;
                }
...
bool XboxLeaderboardManager::OpenSession()
{
    ...
    XUserSetContext(lockedProfile, X_CONTEXT_GAME_TYPE,
                    X_CONTEXT_GAME_TYPE_STANDARD);

    // We already have a QNet session (ie we are joining/in/leaving a multiplayer
    // game)
    ...
    DWORD ret = XSessionCreate(XSESSION_CREATE_USES_STATS | XSESSION_CREATE_HOST,
                               lockedProfile, 8, 8, &sessionNonce, &sessionInfo,
                               NULL, &m_hSession);
...
bool XboxLeaderboardManager::WriteStats(unsigned int viewCount, ViewIn views)
{
    ...
    if (m_isQNetSession == true)
    {
        ...
        if (player != NULL)
        {
            ret = ((NetworkPlayerXbox *)player)
                      ->GetQNetPlayer()
                      ->WriteStats(viewCount, views);
            // printf("Wrote stats to QNet player\n");
        }
        else
        {
            ret = ERROR_NO_SUCH_USER;
            assert(false && "Failed to write stats to a QNET session as no player "
                            "exists with that XUID");
            app.DebugPrintf("Failed to write stats to a QNET session as no player "
                            "exists with that XUID\n");
        }
    }
    else
...
bool XboxLeaderboardManager::ReadStats_MyScore(
    LeaderboardReadListener *callback, int difficulty, EStatsType type,
    PlayerUID myUID, unsigned int readCount)
{
    ...
    if (!LeaderboardManager::ReadStats_MyScore(callback, difficulty, type, myUID,
                                               readCount))
    {
        return false;
    }
    ...
    m_numStats = 0;
    ret = XUserCreateStatsEnumeratorByXuid(0, m_myXUID, readCount,
                                           1, // specCount,
                                           m_spec, &m_numStats, &hEnumerator);
    ...
    ret = XEnumerate(hEnumerator,    // Enumeration handle
                     m_stats,        // Buffer
...
bool XboxLeaderboardManager::ReadStats_Friends(
    LeaderboardReadListener *callback, int difficulty, EStatsType type,
    PlayerUID myUID, unsigned int startIndex, unsigned int readCount)
{
    ...
    if (!LeaderboardManager::ReadStats_Friends(callback, difficulty, type, myUID,
    ...
    {
        app.DebugPrintf("XboxLeaderboardManager::ReadStats_Friends - No friends "
                        "found. Possibly you are offline?\n");
        return false;
    }
    assert(friendCount > 0 && friends != NULL);

    m_numStats = 0;
    ret = XUserReadStats(0, friendCount, friends,
                         1, // specCount,
                         m_spec, &m_numStats, NULL, NULL);

    // Annoyingly, this returns ERROR_INSUFFICIENT_BUFFER when it is being asked
    // to calculate the size of the buffer by passing zero resultsSize
    ...
    ret = XUserReadStats(0, friendCount, friends, 1, m_spec, &m_numStats, m_stats,
                         &m_overlapped);
...
bool XboxLeaderboardManager::ReadStats_TopRank(
    LeaderboardReadListener *callback, int difficulty, EStatsType type,
    unsigned int startIndex, unsigned int readCount)
{
    ...
    if (!LeaderboardManager::ReadStats_TopRank(callback, difficulty, type,
                                               startIndex, readCount))
    {
        return false;
    }
...
bool XboxLeaderboardManager::readStats(int difficulty, EStatsType type)
{
    ...
    m_spec[0].dwNumColumnIds =
        LEADERBOARD_DESCRIPTORS[(int)type][difficulty].m_columnCount;
    for (unsigned int i = 0; i < m_spec[0].dwNumColumnIds; ++i)
    {
        m_spec[0].rgwColumnIds[i] =
            LEADERBOARD_DESCRIPTORS[(int)type][difficulty].m_columnIds[i];
    }
...
{
    if (m_hSession == NULL || m_isQNetSession)
    {
        return;
    }
...
bool XboxLeaderboardManager::IsStatsReadComplete()
{
    ...
    if (XHasOverlappedIoCompleted(&m_overlapped))
    {
        ...
        else
        {
            ...
            if (!m_stats || m_stats->dwNumViews == 0 ||
                m_stats->pViews[0].dwNumRows == 0)
            {
...
{
    return ((int)((XUSER_STATS_ROW *)a)->dwRank) -
           ((int)((XUSER_STATS_ROW *)b)->dwRank);
}

void XboxLeaderboardManager::SortFriendStats()
{
    for (unsigned int leaderboardIndex = 0;
         leaderboardIndex < m_stats->dwNumViews; ++leaderboardIndex)
    {
        // First filter out any friends who aren't in the leaderboard
        XUSER_STATS_ROW *pRow = m_stats->pViews[leaderboardIndex].pRows;
        // View rows is returned not including XUIDs that aren't in this
        // leaderboard, but we want to recalculate that sort of thing
        m_stats->pViews[leaderboardIndex].dwTotalViewRows =
            m_stats->pViews[leaderboardIndex].dwNumRows;
        for (unsigned int rowIndex = 0;
             rowIndex < m_stats->pViews[leaderboardIndex].dwTotalViewRows;)
        {
            if (pRow->dwRank == 0)
            {
                memmove(
                    pRow, pRow + 1,
                    ((m_stats->pViews[leaderboardIndex].dwTotalViewRows - rowIndex) -
                     1) *
                        sizeof(XUSER_STATS_ROW));
                m_stats->pViews[leaderboardIndex].dwTotalViewRows--;
            }
            else
            {
        ...
        // Then sort by rank
        qsort(m_stats->pViews[leaderboardIndex].pRows,
              m_stats->pViews[leaderboardIndex].dwTotalViewRows,
              sizeof(XUSER_STATS_ROW), FriendSortFunction);
        m_stats->pViews[leaderboardIndex].dwNumRows =
            m_stats->pViews[leaderboardIndex].dwTotalViewRows;
    }
}
...
bool XboxLeaderboardManager::getFriends(unsigned int &friendsCount,
                                        PlayerUID **friends)
{
    DWORD resultsSize;
    HANDLE hEnumerator;
    ...
    // First, get a list of (up to 100) friends (this is the maximum that the
    // enumerator currently supports)
    ret = XFriendsCreateEnumerator(ProfileManager.GetLockedProfile(), 0, 100,
                                   &resultsSize, &hEnumerator);
    if (ret != ERROR_SUCCESS)
    {
        return false;
    }
    ...
    ret = XEnumerate(hEnumerator, xonlineFriends, resultsSize, &numFriends, NULL);
    ...
    {
        if ((xonlineFriends[friendIndex].dwFriendState &
    ...
    filteredFriends[friendsCount++] =
        m_myXUID; // m_filteredFriends[m_numFilteredFriends++] =
                  // LeaderboardManager::Instance()->GetMyXUID();
