...
bool ConsoleUIController::NavigateToScene(int iPad, EUIScene scene,
                                          void *initData, EUILayer layer,
                                          EUIGroup group)
{
...
bool ConsoleUIController::NavigateBack(int iPad, bool forceUsePad,
                                       EUIScene eScene, EUILayer eLayer)
{
...
void ConsoleUIController::SetIgnoreAutosaveMenuDisplayed(int iPad,
                                                         bool displayed)
{
...
void ConsoleUIController::SetTooltipText(unsigned int iPad,
                                         unsigned int tooltip, int iTextID)
{
...
void ConsoleUIController::ShowTooltip(unsigned int iPad, unsigned int tooltip,
                                      bool show)
{
    CXuiSceneBase::ShowTooltip(iPad, tooltip, show);
}

void ConsoleUIController::SetTooltips(unsigned int iPad, int iA, int iB, int iX,
                                      int iY, int iLT, int iRT, int iLB,
                                      int iRB, int iLS, bool forceUpdate)
{
    CXuiSceneBase::SetTooltips(iPad, iA, iB, iX, iY, iLT, iRT, iLB, iRB, iLS,
                               forceUpdate);
}

void ConsoleUIController::EnableTooltip(unsigned int iPad, unsigned int tooltip,
                                        bool enable)
{
...
void ConsoleUIController::SetSelectedItem(unsigned int iPad,
                                          const wstring &name)
{
...
void ConsoleUIController::HandleDLCInstalled(int iPad)
{
    ...
    // The DLC message should only happen in the main menus, so it should go to
    // the default xui scenes
    bool bNotInGame = (Minecraft::GetInstance()->level == NULL);

    if (bNotInGame)
    {
        XuiBroadcastMessage(
            CXuiSceneBase::GetPlayerBaseScene(DEFAULT_XUI_MENU_USER), &xuiMsg);
    }
    else
    {
...
void ConsoleUIController::ShowOtherPlayersBaseScene(unsigned int iPad,
                                                    bool show)
{
...
void ConsoleUIController::ShowSavingMessage(unsigned int iPad,
...
C4JStorage::EMessageResult ConsoleUIController::RequestMessageBox(
    ...
    CXuiStringTable *pStringTable, WCHAR *pwchFormatString, DWORD dwFocusButton,
    bool bIsError)
{
    return StorageManager.RequestMessageBox(uiTitle, uiText, uiOptionA, uiOptionC,
                                            dwPad, Func, lpParam, pStringTable,
                                            pwchFormatString, dwFocusButton);
}

C4JStorage::EMessageResult ConsoleUIController::RequestUGCMessageBox(
