...
///					identify a given instance of UGC.  This
/// ID 					will is referenced by every other UGC
/// function.
///
/// @param[in]		userIndex
...
///
/// @param[out]		outResult
///					The unique ID that has been generated
/// and provisioned 					for an instance of UGC.
///
/// @param[in]      userCallback
///                 If this call returns a success code,
///					the userCallback will be called at the
/// end of the 					asynchronous process.
///
...
HRESULT SenUGCCreatePublishingUGCID(int userIndex, SenUGCID *outResult,
...
///
/// @details Contains progress or retry information in addition to a
/// cancellation token
///
struct SenUGCProgressInfo
{
    SenHandle out_taskHandle; /// token for canceling the upload process
    INT8 percentageComplete;  /// 1-100, how much percent is complete of upload
                              /// process for blob
    size_t bytesCompleted;    /// how many bytes have been successfully transmitted
                              /// for the task
    HRESULT lastStepResult;   /// sentient client SDK HRESULT value
    int numRetries;           /// does not reset between internal steps for a long-running
                              /// task
};
...
//   rest of the UGC. This could save bandwidth and make the game more
//   responsive.
...
// NOTE: for large items, use the SenUGCUploadMainData method with the
// SenUGCProgressInfo
//   signature so you can get the running progress and a cancellation token
//   to abort the upload (allowing UI for the user, etc)
//************************************
HRESULT SenUGCUpload(int userIndex, SenUGCID ugcID,
                     const SenUGCMetaData *metaData, int nrMainDataBlobs,
...
// NOTE: data blob at index 0 should be the main level data blob, for the
// automatic download counter to work. The metadata will also have a data blob
// associated, but this should be kept to a minimum, as UGC download menus will
// probably want to download metadata for a lot of UGCs at once.
// NOTE: If a creator uploads metadata again, it will overwrite the previous
// stored blob with the new one.
//************************************
HRESULT SenUGCUploadMetadata(int userIndex, SenUGCID ugcID,
...
//   stored blob with the new one.
//************************************
HRESULT SenUGCUploadMainDataBlob(int userIndex, SenUGCID ugcID,
...
//************************************
HRESULT SenUGCDelete(int userIndex, SenUGCID ugcID,
                     SenSysCompletedCallback userCallback,
                     void *userCallbackData);
...
// outBuffer should be an preallocated array of
// [sizeof(SenUGCSearchResult)*maxNrResults] bytes.
//************************************
HRESULT
SenUGCEnumerate(int userIndex, SenSysTitleID titleID, SenUGCSortBy sortBy,
                SenUGCType type, SenUGCAuthorType authorType, int nrAuthors,
                const PlayerUID *authorList,
                SenUGCMetaDataFlags metaDataFlagFilter, SYSTEMTIME *newerThan,
                SenUGCDescriptor descriptor, int maxNrResults,
                SenUGCSearchResult *outBuffer, UINT *outNrResults,
...
/// @param[in]		userCallbackData
///
/// @return			Search the database for all the UGCs that match
/// various search criteria.
///					outBuffer should be an preallocated
/// array of [sizeof(SenUGCSearchResult)*maxNrResults] bytes.
///
/// @details		Enumerate by name will perform a look based on a various
/// search criteria.  The Collection
///					of descriptors will perform an equality
/// lookup on Descriptor
///
...
SenUGCEnumerate(int userIndex, SenSysTitleID titleID, SenUGCSortBy sortBy,
                ...
                const PlayerUID *authorList,
                SenUGCMetaDataFlags metaDataFlagFilter, SYSTEMTIME *newerThan,
                int nrDescriptors, INT64 *descriptors, int maxNrResults,
                SenUGCSearchResult *outBuffer, UINT *outNrResults,
                SenSysCompletedCallback userCallback, void *userCallbackData);
...
/// array of [sizeof(SenUGCSearchResult)*maxNrResults] bytes.
///
/// @details		Enumerate by Descriptor using a Logical And against all
/// submitted Descriptor values.
///					The API filters the results on a
/// specific UGC Type as well as an author
...
HRESULT SenUGCEnumerateByDescriptorWithLogicalAnd(
    int userIndex, SenSysTitleID titleID, SenUGCSortBy sortBy, SenUGCType type,
    SenUGCAuthorType authorType, int nrAuthors, const PlayerUID *authorList,
    SenUGCMetaDataFlags metaDataFlagFilter, SYSTEMTIME *newerThan,
    int nrDescriptors, INT64 *descriptors, int maxNrResults,
    SenUGCSearchResult *outBuffer, UINT *outNrResults,
    SenSysCompletedCallback userCallback, void *userCallbackData);
...
/// array of [sizeof(SenUGCSearchResult)*maxNrResults] bytes.
///
/// @details		Enumerate by name will perform a wild card lookup on UGC
/// name.  The lookup will return anything
///					in the range of "%<NAME>%".  The API
/// filters the results on a specific UGC Type as well as an author
/// list type. Author List type of Everyone is NOT supported.
///
/// @related		SenUGCEnumerate
__declspec(deprecated("Use SenUGCEnumerateByName() instead")) HRESULT
SenUGCEnumerate(int userIndex, SenSysTitleID titleID, SenUGCType type,
                SenUGCAuthorType authorType, int nrAuthors,
                const PlayerUID *authorList, const wchar_t *name,
                int maxNrResults, SenUGCSearchResult *outBuffer,
                UINT *outNrResults, SenSysCompletedCallback userCallback,
                void *userCallbackData);
...
/// array of [sizeof(SenUGCSearchResult)*maxNrResults] bytes.
///
/// @details		Enumerate by name will perform an exact or wild card
/// string lookup on UGC name.  The API filters the results
///                 on a specific UGC Type as well as an author	list type.
///                 Author List type of Everyone is NOT supported.
///
/// @related		SenUGCEnumerate
HRESULT SenUGCEnumerateByName(int userIndex, SenSysTitleID titleID,
                              SenUGCType type, SenUGCAuthorType authorType,
                              int nrAuthors, const PlayerUID *authorList,
                              const wchar_t *name, bool performWildCardLookup,
                              int maxNrResults, SenUGCSearchResult *outBuffer,
                              UINT *outNrResults,
                              SenSysCompletedCallback userCallback,
                              void *userCallbackData);
...
// outBuffer should be an preallocated array of
// [(sizeof(SenUGCDownloadedMetaData)+SenUGCMetaData::BlobSizeLimit)*nrUGCs]
// bytes. This new signature is compatible with resubmission feature and 64-bit
// UGC Ids.
//************************************
HRESULT SenUGCDownloadMetaData(int userIndex, SenSysTitleID titleID, int nrUGCs,
...
__declspec(deprecated("Use signature with SenUGCDownloadedMetaData2.")) HRESULT
SenUGCDownloadMetaData(int userIndex, SenSysTitleID titleID, int nrUGCs,
                       const SenUGCID *ugcIDList,
...
//************************************
HRESULT SenUGCDownloadMainDataBlob(int userIndex, SenSysTitleID titleID,
                                   SenUGCID ugcID, UINT mainDataBlobIndex,
                                   size_t bufferSize, UINT blobVersion,
                                   void *outBuffer, size_t *outSize,
                                   SenSysCompletedCallback userCallback,
                                   void *userCallbackData);

__declspec(deprecated("Use signature with blobVersion.")) HRESULT
SenUGCDownloadMainDataBlob(int userIndex, SenSysTitleID titleID, SenUGCID ugcID,
                           int mainDataBlobIndex, size_t bufferSize,
                           void *outBuffer, size_t *outBytesReceived,
                           SenSysCompletedCallback userCallback,
                           void *userCallbackData);
...
//************************************
HRESULT SenUGCSetReviewScore(int userIndex, SenUGCID ugcId,
                             SenUGCReviewScoreType type, unsigned int score,
                             bool isFavorite, SenSysCompletedCallback callback,
                             void *callbackData);
...
// Parameter: void * callbackData
//************************************
HRESULT SenUGCGetReviewScore(int userIndex, SenUGCID ugcId,
...
// the search results
//************************************
HRESULT SenUGCSetFavoriteFlag(int userIndex, SenSysTitleID titleID,
                              SenUGCID ugcID, BOOL isFavorite,
                              SenSysCompletedCallback userCallback,
                              void *userCallbackData);
...
// the search results
//************************************
HRESULT SenUGCGetFavoriteFlag(int userIndex, SenSysTitleID titleID,
                              SenUGCID ugcID, BOOL *outResult,
                              SenSysCompletedCallback userCallback,
                              void *userCallbackData);
...
//************************************
HRESULT SenUGCGetFriendFavoriteCount(int userIndex, SenSysTitleID titleID,
                                     SenUGCID ugcID, int *outResult,
                                     SenSysCompletedCallback userCallback,
                                     void *userCallbackData);
...
///
/// @param[in]		reason
///					Reason for marking a given piece of UGC
/// as offensive.
///
/// @param[in]      userCallback
///                 If this call returns a success code,
...
///
/// @details		Users can flag a level that they think is offensive. The
/// goal is that
///					the Sentient system will automatically
/// be able to take down 					levels after
/// enough people have flagged a UGC as bad.
/// The number of votes to take
/// something down will depend on the
...
///
/// @related		<Related API>
HRESULT SenUGCSetOffensivenessFlag(int userIndex, SenUGCID ugcID,
                                   SenUGCOffensivenessFlag offensivenessFlag,
                                   const wchar_t *reason,
                                   SenSysCompletedCallback userCallback,
                                   void *userCallbackData);

/// @brief			This function will return whether or not a
/// particular
...
///
/// @param[in]		True if content is banned (and should not be viewed by
/// user).
///
/// @param[in]      userCallback
///                 If this call returns a success code,
...
///					moderation engine.  This API allows
/// clients to verify 					if a given piece of UGC
/// has been banned.
///
/// @related		SenUGCCreatePublishingUGCID()
/// @related		SenUGCSetOffensivenessFlag()
/// @related		SenUGCPublish()
HRESULT SenUGCIsBanned(int userIndex, SenUGCID ugcID, BOOL *out_result,
                       SenSysCompletedCallback userCallback,
                       void *userCallbackData);
...
///
/// @details        When enumerating feeds, these are the available feeds that
/// can be retrieved.
///
...
///
/// @param[in]		feedType
///					Feed Type identifier for for the given
/// feed being retrieved.
///
/// @param[in]		maxNumberOfFeedItems
///					Used to indicate the number of items to
/// be returned by @a out_feedInfo. 					If the
/// actual number of items exceeds this, you will receive an error.
///
/// @param[out]		out_buffer
///					Pointer to the collection of structures
/// to fill with SenUGCFeedItem data.
///
/// @param[out]		out_buffersize
///					The number of entries actually
/// enumerated by the call.
///
/// @param[in]      userCallback
///                 If this call returns a success code,
///					the userCallback will be called at the
...
/// to determine success.
///					Specific values include:
///					E_POINTER: out_buffer or out_buffersize
/// are null.
///
/// @details		<Insert detailed method documentation>
///
/// @related		SenUGCEnumerateFeeds()
HRESULT SenUGCGetFeed(int userIndex, SenSysTitleID titleID,
                      SenUGCFeedType feedType, size_t maxNumberOfFeedItems,
                      SenUGCFeedItem *out_buffer, UINT *out_buffersize,
                      SenSysCompletedCallback userCallback,
                      void *userCallbackData);

/// @brief			Retrieves a collection of feeds that are
/// viewable by the
///					current user.
///
/// @param[in]		userIndex
...
///
/// @param[in]      culture
///                 This is the result of a call to SenCultureFind() or
///                 SenCultureGet*(). You may also pass NULL to use the culture
///                 set with SenCultureSetCurrent().
...
///					Pointer to a collection of structures to
/// fill with SenUGCFeedInfo data.
///
...
/// to determine success.
///					Specific values include:
///					E_POINTER: out_feedInfo or
/// out_resultCount are null.
///
/// @details		<Insert detailed method documentation>
///
/// @related		SenUGCGetFeed()
HRESULT SenUGCEnumerateFeeds(int userIndex, size_t maxResults,
                             SenUGCFeedInfo *out_feedInfo,
                             size_t *out_resultCount,
                             SenSysCompletedCallback userCallback,
                             void *userCallbackData);

/// @brief			API that publishes UGC and makes the content
/// accessible
...
///					Optional parameter.  Definition for a
/// Leaderboard that 					will be created and
/// associated to newly published UGC.
///
/// @param[out]		out_leaderboardId
///					Created Leaderboard Id.  Only returned
/// to the client if 					a Leaderboards
/// Definition is passed to the server.
///
/// @param[in]      userCallback
///                 If this call returns a success code,
///					the userCallback will be called at the
...
///					Specific values include:
///
/// @details		UGC is only accessible to the  author until it has been
/// published.
///					The user making the call must be the
/// author of the UGC item. 					The call will
/// fail if this UGC item has previously been published.
/// By supplying an optional Leaderboard definition a Leaderboard is
/// automatically
//					allocated and associated with the UGC
// item.
///					This is the preferred way of creating
/// UGC Leaderboards.
///
/// @related		SenCreateLeaderboard()
HRESULT SenUGCPublish(int userIndex, SenUGCID ugcID,
                      const SenLeaderboardDefinition *leaderboardDefinition,
                      SenLeaderboardId *out_leaderboardId,
                      SenSysCompletedCallback userCallback,
                      void *userCallbackData);

/// @brief			API that publishes a new version of a UGC item
/// and makes the revised content accessible
///					to everyone on the Sentient service.
///
/// @param[in]		userIndex
///                 The index of the initiating user on the console.
...
///					Specific values include:
///
/// @details		New versions of UGC are only accessible to the author
/// until it has been republished.
///					The user making the call must be the
/// author of the UGC item.
///
/// @related		SenUGCPublish()
HRESULT SenUGCRepublish(int userIndex, SenUGCID ugcID,
                        SenSysCompletedCallback userCallback,
                        void *userCallbackData);
