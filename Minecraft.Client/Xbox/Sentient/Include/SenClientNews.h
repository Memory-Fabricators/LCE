...
//  News cache expire time controls the duration between service
//  synchronization. This duration is measured in seconds.  Default value is 6
//  minutes
...
class NewsItemCountClass
{
  public:
    NewsItemCountClass()
    {
        newsItemCount = MAX_NEWS_ITEM_COUNT;
    };
};
...
//  Used for displaying default news messages through the SetDefaultNewsStrings
//  API
...
// Ticker messages need to be pulled whenever the ticker is displayed. Call this
// function to get the next message to display
...
///
/// @related		<Related API>
HRESULT SenGetTickerMessage(int userIndex, SenTickerData *out_tickerData);
