...
HRESULT CSentientManager::Tick()
{
    ...
    case S_OK:
        {
            ...
            if (g_NetworkManager.IsInSession())
            {
                ...
                if (m_bFirstFlush)
                {
                    ...
                    {
                        if (Minecraft::GetInstance()->localplayers[i] != NULL &&
                            m_fLevelStartTime[i] - currentTime > 60)
                        {
                            Flush();
                        }
                    }
    ...
    case Sentient::SENTIENT_S_NOT_SIGNED_IN_TO_LIVE:
        {
            // Login required
            // DebugPrintf("\nSentient: WARNING: an Xbox LIVE-enabled user needs to be
            // logged in.\n");
...
{
    return SenStatPlayerSessionStart(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetTitleBuildId(), 0, 0, 0, (INT)app.getDeploymentType());
}
                                        ...
                                        DWORD numberOfOnlinePlayers)
{
    ...
    return SenStatLevelStart(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID(),
        GetSingleOrMultiplayer(), friendsOrMatch, competeOrCoop,
        GetDifficultyLevel(difficulty), numberOfLocalPlayers,
        numberOfOnlinePlayers, GetLicense(), GetDefaultGameControls(),
        GetAudioSettings(dwUserId), 0, 0);
}
                                       ...
                                       ESen_LevelExitStatus levelExitStatus)
{
    ...
    return SenStatLevelExit(dwUserId, GetSecondsSinceInitialize(),
                            GetMode(dwUserId), GetSubMode(dwUserId),
                            GetLevelId(dwUserId), GetSubLevelId(dwUserId),
                            GetLevelInstanceID(), GetMultiplayerInstanceID(),
                            levelExitStatus, GetLevelExitProgressStat1(),
                            GetLevelExitProgressStat2(), (INT)levelDuration);
}
...
{
    float levelDuration = app.getAppTime() - m_fLevelStartTime[dwUserId];
    return SenStatLevelSaveOrCheckpoint(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID(),
        GetLevelExitProgressStat1(), GetLevelExitProgressStat2(),
        (INT)levelDuration, saveOrCheckPointID, saveSizeInBytes);
}
                                         ...
                                         INT saveOrCheckPointID)
{
    return SenStatLevelResume(
        ...
        GetDifficultyLevel(difficulty), numberOfLocalPlayers,
        numberOfOnlinePlayers, GetLicense(), GetDefaultGameControls(),
        saveOrCheckPointID, GetAudioSettings(dwUserId), 0, 0);
}

BOOL CSentientManager::RecordPauseOrInactive(DWORD dwUserId)
{
    return SenStatPauseOrInactive(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID());
}
...
{
    return SenStatMenuShown(dwUserId, GetSecondsSinceInitialize(),
                            GetMode(dwUserId), GetSubMode(dwUserId),
                            GetLevelId(dwUserId), GetSubLevelId(dwUserId), menuID,
                            optionalMenuSubID, GetLevelInstanceID(),
...
{
    return SenStatAchievementUnlocked(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID(), achievementID,
...
{
    return SenStatMediaShareUpload(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID(), mediaDestination,
        mediaType);
}
...
{
    return SenStatUpsellPresented(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID(), upsellId,
        marketplaceOfferID);
}
    ...
    INT playerWeaponID, INT enemyWeaponID, ETelemetryChallenges enemyTypeID)
{
    ...
    return SenStatPlayerDiedOrFailed(
        dwUserId, GetMode(dwUserId), GetSubMode(dwUserId), GetLevelId(dwUserId),
        GetSubLevelId(dwUserId), GetLevelInstanceID(), GetMultiplayerInstanceID(),
        lowResMapX, lowResMapY, lowResMapZ, mapID, playerWeaponID, enemyWeaponID,
        enemyTypeID, secs, secs);
}
...
{
    INT secs = GetSecondsSinceInitialize();
    return SenStatEnemyKilledOrOvercome(
        dwUserId, GetMode(dwUserId), GetSubMode(dwUserId), GetLevelId(dwUserId),
        GetSubLevelId(dwUserId), GetLevelInstanceID(), GetMultiplayerInstanceID(),
        lowResMapX, lowResMapY, lowResMapZ, mapID, playerWeaponID, enemyWeaponID,
        enemyTypeID, secs, secs);
...
{
    return SenStatSkinChanged(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID(), dwSkinId);
}

BOOL CSentientManager::RecordBanLevel(DWORD dwUserId)
{
    return SenStatBanLevel(dwUserId, GetSecondsSinceInitialize(),
                           GetMode(dwUserId), GetSubMode(dwUserId),
                           GetLevelId(dwUserId), GetSubLevelId(dwUserId),
                           GetLevelInstanceID(), GetMultiplayerInstanceID());
}
...
{
    return SenStatUnBanLevel(dwUserId, GetSecondsSinceInitialize(),
                             GetMode(dwUserId), GetSubMode(dwUserId),
                             GetLevelId(dwUserId), GetSubLevelId(dwUserId),
                             GetLevelInstanceID(), GetMultiplayerInstanceID());
}

BOOL CSentientManager::RecordTexturePackLoaded(DWORD dwUserId,
                                               INT texturePackId,
                                               INT purchased)
{
    return SenStatTexturePackChanged(
        dwUserId, GetSecondsSinceInitialize(), GetMode(dwUserId),
        GetSubMode(dwUserId), GetLevelId(dwUserId), GetSubLevelId(dwUserId),
        GetLevelInstanceID(), GetMultiplayerInstanceID(), texturePackId,
        purchased);
}
