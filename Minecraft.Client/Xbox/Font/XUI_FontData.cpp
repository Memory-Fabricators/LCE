...
HRESULT XUI_FontData::Create(SFontData &sfontdata)
{
    ...
    m_iFontTexture = Minecraft::GetInstance()->textures->getTexture(
        img, C4JRender::TEXTURE_FORMAT_RxGyBzAw, false);
...
void XUI_FontData::GetCharMetrics( const WCHAR* strChar, XUICharMetrics
*xuiMetrics)
{
        unsigned int uiChar = (unsigned int) *strChar;
        unsigned short usGlyph = m_fontData->getGlyphId(uiChar);

        xuiMetrics->fAdvance = m_fontData->getWidth(usGlyph);
//.getAdvance(usGlyph) * (float) m_fontData.getFontData()->m_uiGlyphHeight;
        xuiMetrics->fMaxX = (float) m_fontData->getFontData()->m_uiGlyphWidth;
        xuiMetrics->fMinX = 0.0f;
        xuiMetrics->fMaxY = 0;// m_fontData.getFontData()->m_fAscent * (float)
m_fontData.getFontData()->m_uiGlyphHeight; xuiMetrics->fMinY =
0;//m_fontData.getFontData()->m_fDescent * (float)
m_fontData.getFontData()->m_uiGlyphHeight;
}
