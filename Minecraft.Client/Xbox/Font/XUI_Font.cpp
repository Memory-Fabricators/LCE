                             ...
                             FLOAT *pHeight, BOOL bFirstLineOnly) const
{
    ...
    FLOAT fWidth =
        static_cast<FLOAT>(iWidth); // Delay the use if fWidth to reduce LHS pain
...
VOID XUI_Font::Begin()
{
    ...
    if (0 == m_dwNestedBeginCount)
    {
        ...
        // m_pFontTexture->GetLevelDesc( 0, &TextureDesc );		// Get the
        // description

        // Set render state
        assert(m_fontData->m_pFontTexture != NULL ||
               m_fontData->m_iFontTexture > 0);
        ...
        //// Read the TextureDesc here to ensure no load/hit/store from
        /// GetLevelDesc()
        // FLOAT vTexScale[4];
        // vTexScale[0] = 1.0f / TextureDesc.Width;		// LHS due to int->float
        // conversion vTexScale[1] = 1.0f / TextureDesc.Height; vTexScale[2] = 0.0f;
        // vTexScale[3] = 0.0f;
        //
...
// Desc: Draws text as textured polygons
//--------------------------------------------------------------------------------------
VOID XUI_Font::DrawShadowText(FLOAT fOriginX, FLOAT fOriginY, DWORD dwColor,
                              DWORD dwShadowColor, const WCHAR *strText,
                              DWORD dwFlags, FLOAT fMaxPixelWidth)
{
    float fXShadow = 1.0f, fYShadow = 1.0f;
    // 4J Stu - Don't move the drop shadow as much
    // DrawText( fOriginX + (1*m_fXScaleFactor), fOriginY + (1*m_fYScaleFactor),
    // dwColor, strText, dwFlags, fMaxPixelWidth, true );

    // 4J-PB - if we're in 480 widescreen, we need to draw the drop shadow at +2
    // pixels, so that when the scene is halved, it's at +1
    ...
    DrawText(fOriginX + fXShadow, fOriginY + fYShadow, dwColor, strText, dwFlags,
             fMaxPixelWidth, true);
    DrawText(fOriginX, fOriginY, dwColor, strText, dwFlags, fMaxPixelWidth);

    // DrawText( fOriginX + 1, fOriginY + 1, dwShadowColor, strText, dwFlags,
    // fMaxPixelWidth);
    // DrawText( fOriginX, fOriginY, dwColor, strText, dwFlags, fMaxPixelWidth );
}
...
// Name: DrawText()
// Desc: Draws text as textured polygons
//       TODO: This function should use the Begin/SetVertexData/End() API when
//       it becomes available.
//--------------------------------------------------------------------------------------
VOID XUI_Font::DrawText(FLOAT fOriginX, FLOAT fOriginY, DWORD dwColor,
                        const WCHAR *strText, DWORD dwFlags,
                        FLOAT fMaxPixelWidth, bool darken /*= false*/)
{
    if (NULL == strText)
    {
        return;
    ...
    // Create a PIX user-defined event that encapsulates all of the text draw
    // calls. This makes DrawText calls easier to recognize in PIX captures, and
    // it makes them take up fewer entries in the event list.
    ...
    // 4J-PB - not sure what this code was intending to do, but it removed a line
    // of text that is slightly off the top of the control, rather than having it
    // partially render
    //     if( fOriginY < 0.0f )
    //     {
    ...
    FLOAT fEllipsesPixelWidth =
        m_fXScaleFactor * 3.0f * (sChar.getOffset() + sChar.getWAdvance());
    ...
    while (*strText)
    {
        ...
        FLOAT fWidth = m_fXScaleFactor *
                       (sChar.tu2() - sChar.tu1()); //( FLOAT )pGlyph->wWidth;
        ...
        if (0 == dwNumEllipsesToDraw)
        {
            if (dwFlags & ATGFONT_TRUNCATED)
            {
                ...
                if (m_fCursorX + fOffset + fWidth + fEllipsesPixelWidth +
                        m_fSlantFactor >
