...
extern void GetRenderAndSamplerStates(IDirect3DDevice9 *pDevice,
                                      DWORD *RenderStateA,
                                      DWORD *SamplerStateA);
extern void SetRenderAndSamplerStates(IDirect3DDevice9 *pDevice,
                                      DWORD *RenderStateA,
                                      DWORD *SamplerStateA);
...
XUI_FontRenderer::XUI_FontRenderer()
{
    ...
    // pMinecraft->height_phys); m_fScreenWidth=(float)pMinecraft->width_phys;
...
HRESULT
XUI_FontRenderer::CreateFont(const TypefaceDescriptor *pTypefaceDescriptor,
                             ...
                             HFONTOBJ *phFont)
{
    ...
    // We don't want to make the text larger as then it may not fit in the box
    // specified so we decrease the size until we find one that will look ok
...
VOID XUI_FontRenderer::ReleaseFont(HFONTOBJ hFont)
{
    ...
    if (xuiFont != NULL)
    {
        ...
        {
            AUTO_VAR(it, m_loadedFonts[xuiFont->m_iFontData].find(
                             xuiFont->m_fScaleFactor));
...
HRESULT XUI_FontRenderer::GetFontMetrics(HFONTOBJ hFont,
                                         XUIFontMetrics *pFontMetrics)
{
    ...
    pFontMetrics->fLineHeight =
        (font->m_fontData->getFontYAdvance() + 1) * font->m_fYScaleFactor;
    pFontMetrics->fMaxAscent =
        font->m_fontData->getMaxAscent() * font->m_fYScaleFactor;
    pFontMetrics->fMaxDescent =
        font->m_fontData->getMaxDescent() * font->m_fYScaleFactor;
    pFontMetrics->fMaxHeight =
        font->m_fontData->getFontHeight() * font->m_fYScaleFactor;
    pFontMetrics->fMaxWidth =
        font->m_fontData->getFontMaxWidth() * font->m_fXScaleFactor;
    pFontMetrics->fMaxAdvance =
        font->m_fontData->getFontMaxWidth() * font->m_fXScaleFactor;
...
HRESULT XUI_FontRenderer::GetCharMetrics(HFONTOBJ hFont, WCHAR wch,
...
HRESULT XUI_FontRenderer::DrawCharToTexture(HFONTOBJ hFont, WCHAR wch,
                                            HXUIDC hDC, IXuiTexture *pTexture,
                                            UINT x, UINT y, UINT width,
                                            UINT height, UINT insetX,
                                            UINT insetY)
{
    if (hFont == 0 || pTexture == NULL)
    {
        return E_INVALIDARG;
    }
    return (S_OK);
...
HRESULT XUI_FontRenderer::DrawCharsToDevice(HFONTOBJ hFont, CharData *pCharData,
                                            DWORD dwCount, RECT *pClipRect,
                                            HXUIDC hDC,
                                            D3DXMATRIX *pWorldViewProj)
{
    ...
    // 4J-PB - if we're in 480 Widescreen mode, we need to ensure that the font
    // characters are aligned on an even boundary if they are a 2x multiple
    if (!RenderManager.IsHiDef())
    {
        ...
        else
        {
            ...
            // 480 SD mode - y needs to be on a pixel boundary when multiplied by 1.5,
            // so if it's an odd number, subtract 1/3 from it
            int iWorldY = pWorldViewProj->_42;
            if (iWorldY % 2 == 1)
            {
    ...
    while (i < dwCount)
    {
        ...
        // int yPos = (int)pCharData[i].y + (int)(font->m_fontMetrics.fLineHeight -
        // font->m_fontMetrics.fMaxAscent)/2; if( (pCharData[i].dwStyle &
        // XUI_FONT_STYLE_VERTICAL_CENTER) == XUI_FONT_STYLE_VERTICAL_CENTER)
        //{
        ...
        if (dropShadow)
        {
            ...
            font->DrawShadowText(lineXPos, lineYPos, colour, shadowColour,
                                 string.c_str());
