...
// 4J This class is partially based of the ATG font implementation, modified to
// suit our uses for XUI

//--------------------------------------------------------------------------------------
// Flags for the Font::DrawText() function (Or them together to combine
// features)
//--------------------------------------------------------------------------------------
...
class XUI_Font
{
  ...
  public:
    ...
    D3DRECT
    m_rcWindow;       // Bounds rect if the text window, modify via accessors only!
  ...
  public:
    ...
    void GetScaleFactors(FLOAT *pfXScaleFactor, FLOAT *pfYScaleFactor)
    {
        *pfXScaleFactor = m_fScaleFactor;
        *pfYScaleFactor = m_fScaleFactor;
    }
    // Accessor functions
    ...
    void SetFallbackFont(const wstring &fallbackFont)
    {
  ...
  public:
    ...
    VOID GetTextExtent(const WCHAR *strText, FLOAT *pWidth, FLOAT *pHeight,
                       BOOL bFirstLineOnly = FALSE) const;
    ...
    // performance, they should batch multiple calls together, bracketed by calls
    // to Begin() and End().
    VOID Begin();
    ...
    VOID DrawText(FLOAT sx, FLOAT sy, DWORD dwColor, const WCHAR *strText,
                  ...
                  bool darken = false);
    VOID DrawShadowText(FLOAT sx, FLOAT sy, DWORD dwColor, DWORD dwShadowColor,
