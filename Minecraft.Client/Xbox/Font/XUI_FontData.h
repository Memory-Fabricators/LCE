...
#define ATGCALCFONTFILEHEADERSIZE(x) \
    (sizeof(DWORD) + (sizeof(FLOAT) * 4) + sizeof(WORD) + (sizeof(WCHAR) * (x)))
#define ATGFONTFILEVERSION 5

typedef struct FontFileHeaderImage_t
{
    DWORD
    m_dwFileVersion;            // Version of the font file (Must match FONTFILEVERSION)
    FLOAT m_fFontHeight;        // Height of the font strike in pixels
    FLOAT m_fFontTopPadding;    // Padding above the strike zone
    FLOAT m_fFontBottomPadding; // Padding below the strike zone
    FLOAT m_fFontYAdvance;      // Number of pixels to move the cursor for a line feed
    WORD m_cMaxGlyph;           // Number of font characters (Should be an odd number to
                                // maintain DWORD Alignment)
    WCHAR m_TranslatorTable[1]; // ASCII to Glyph lookup table, NOTE: It's
                                // m_cMaxGlyph+1 in size. Entry 0 maps to the
                                // "Unknown" glyph.
...
{
    DWORD m_dwNumGlyphs;    // Size of font strike array (First entry is the unknown
                            // glyph)
    GLYPH_ATTR m_Glyphs[1]; // Array of font strike uv's etc... NOTE: It's
                            // m_dwNumGlyphs in size
...
class XUI_FontData
{
  ...
  public:
    ...
    // void	GetCharMetrics( const WCHAR* strChar, XUICharMetrics
    // *xuiMetrics); unsigned short getGlyphId(wchar_t character);
};
