...
HRESULT InitD3D(IDirect3DDevice9 **ppDevice, D3DPRESENT_PARAMETERS *pd3dPP)
{
    ...
    // Create the device.
    return pD3D->CreateDevice(0, D3DDEVTYPE_HAL, NULL,
...
int __cdecl main()
{
    ...
    // XUI render can change the following renderstates, so store then before and
    // re-set them after the xui render
    ...
    // 4J Stu - XACT was creating these automatically, but we need them for QNet.
    // The setup params are just copied from a sample app and may need changed for
    // our purposes Start XAudio2
    ...
    hr = g_pXAudio2->CreateMasteringVoice(&g_pXAudio2MasteringVoice,
    ...
    ProfileManager.RegisterAward(eAward_mine100Blocks, GAMER_PICTURE_GAMERPIC1,
                                 eAwardType_GamerPic, false, app.GetStringTable(),
                                 IDS_AWARD_TITLE, IDS_AWARD_GAMERPIC1,
                                 IDS_CONFIRM_OK);
    ProfileManager.RegisterAward(eAward_kill10Creepers, GAMER_PICTURE_GAMERPIC2,
                                 eAwardType_GamerPic, false, app.GetStringTable(),
                                 IDS_AWARD_TITLE, IDS_AWARD_GAMERPIC2,
                                 IDS_CONFIRM_OK);
    ...
    ProfileManager.RegisterAward(
        eAward_play100Days, AVATARASSETAWARD_WATCH, eAwardType_AvatarItem, false,
        app.GetStringTable(), IDS_AWARD_TITLE, IDS_AWARD_AVATAR2, IDS_CONFIRM_OK);
    ...
    // initialise the storage manager with a default save display name, a Minimum
    // save size, and a callback for displaying the saving message
    StorageManager.Init(app.GetString(IDS_DEFAULT_SAVENAME), "savegame.dat",
