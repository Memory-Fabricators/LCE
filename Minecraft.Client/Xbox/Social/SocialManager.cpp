...
void CSocialManager::Tick()
{
    ...
    case eStateGetPostingCapability:
        {
            // Poll for result.
            DWORD dwResult = XGetOverlappedResult(&(m_Overlapped),
                                                  &(m_dwOverlappedResultCode), false);
            ...
            default:
                ...
                app.DebugPrintf("XSocialGetCapabilities - FAILED\n",
                                m_dwSocialPostingCapability);
                // WESTY: TO DO: Get error code, and possibly extened error code to figure
                // out what went wrong.
                // assert( false );
                m_eCurrState = eStateReady;
                break;
    ...
    case eStatePostingImage:
        {
            ...
            {
            case ERROR_SUCCESS:
                TelemetryManager->RecordMediaShareUpload(m_dwCurrRequestUser,
                                                         ESen_MediaDestination_Facebook,
                                                         eSen_MediaType_Picture);
            ...
            default:
                ...
                // SDK docs give no indication why this may be, but could be user
...
bool CSocialManager::RefreshPostingCapability()
{
    ...
    DWORD dwResult =
        XSocialGetCapabilities(&(m_dwSocialPostingCapability), &(m_Overlapped));
    ...
    case ERROR_FUNCTION_FAILED:
        {
            ...
            app.DebugPrintf(
                "XSocialGetCapabilities ASYNC : FAILED : Extended Error %d\n",
                dwExtendedError);
            // assert( false );
        }
        break;
...
bool CSocialManager::AreAllUsersAllowedToPostImages()
{
    ...
    for (int i = 0; i < XUSER_MAX_COUNT; i++)
    {
        ...
        {
            XUserCheckPrivilege(i, XPRIVILEGE_SOCIAL_NETWORK_SHARING,
                                &bUserAllowedToPost);
...
void CSocialManager::SetSocialPostText(LPCWSTR pwchTitle, LPCWSTR pwchCaption,
                                       LPCWSTR pwchDesc)
{
    wstring wTemp;
                                             ...
                                             bool bUsingKinect)
{
    ...
    // Ensure title is actually allowed to post links ... should always be the
    // case, but we have to check for TCR.
    ...
    // 4J Stu - MessageText is not a member of the struct in the header of the
    // latest XDK 20871.6 The documentation hasn't changed though...
    ...
    m_PostLinkParams.Flags =
        (XSOCIAL_POST_GAMECONTENT |
         XSOCIAL_POST_USERGENERATEDCONTENT); // WESTY: Need to set correct flags
                                             // for content type.
    ...
    if (bUsingKinect)
    {
        dwResult = XShowNuiSocialNetworkLinkPostUI(
            dwUserTrackingIndex, dwUserIndex, &(m_PostLinkParams), &(m_Overlapped));
    }
    else // using normal controller UI
    {
        dwResult = XShowSocialNetworkLinkPostUI(dwUserIndex, &(m_PostLinkParams),
                                                &(m_Overlapped));
    }
    ...
    case ERROR_FUNCTION_FAILED:
        // Hmm ... why would it fail, and what are we meant to do about it? Get
        // extended error code!?
        DestroyMainPostImage();
        DestroyPreviewPostImage();
        // assert( false );
                                              ...
                                              bool bUsingKinect)
{
    ...
    // Ensure title is actually allowed to post images ... should always be the
    // case, but we have to check for TCR.
    ...
    {
        dwResult =
            XShowNuiSocialNetworkImagePostUI(dwUserTrackingIndex, dwUserIndex,
                                             &(m_PostImageParams), &(m_Overlapped));
    }
    else // using normal controller UI
    {
        dwResult = XShowSocialNetworkImagePostUI(dwUserIndex, &(m_PostImageParams),
                                                 &(m_Overlapped));
    }
...
// So far we have only created a syncronous version of this function, and the
xbox functionality does not seem to work yet! bool
CSocialManager::GetSocialNetworkAccessKey( ESocialNetwork eSocialNetwork, DWORD
...
bShowNetworkSignin )
{
        ...
        LPCWSTR	pRequiredPermissions = kaSocialNetworkRequiredPermissions[
eSocialNetwork ];
        ...
        if ( bShowNetworkSignin )
        {
                if ( bUsingKinect )
                {
                        dwResult = XShowNuiSocialGetUserToken(
dwUserTrackingIndex, dwUserIndex, dwSocialNetworkID, pRequiredPermissions,
m_pAccessKeyText, &( m_dwAccessKeyTextSize ), NULL );

                        // If buffer for key text was not large enough,
reallocate buffer that is large enough and try again. if ( dwResult ==
ERROR_INSUFFICIENT_BUFFER )
                        {
                                delete [] m_pAccessKeyText;
                                m_pAccessKeyText = new wchar_t[
m_dwAccessKeyTextSize ]; dwResult = XShowNuiSocialGetUserToken(
dwUserTrackingIndex, dwUserIndex, dwSocialNetworkID, pRequiredPermissions,
m_pAccessKeyText, &( m_dwAccessKeyTextSize ), NULL );
                        }
                }
                ...
                {
                        dwResult = XShowSocialGetUserToken( dwUserIndex,
dwSocialNetworkID, pRequiredPermissions, m_pAccessKeyText, &(
m_dwAccessKeyTextSize ), NULL );
...
ERROR_INSUFFICIENT_BUFFER )
                        {
                                delete [] m_pAccessKeyText;
                                m_pAccessKeyText = new wchar_t[
m_dwAccessKeyTextSize ]; dwResult = XShowSocialGetUserToken( dwUserIndex,
dwSocialNetworkID, pRequiredPermissions, m_pAccessKeyText, &(
m_dwAccessKeyTextSize ), NULL );
                        }
                }
        ...
        {
                dwResult = XSocialGetUserToken( dwUserIndex, dwSocialNetworkID,
pRequiredPermissions, m_pAccessKeyText, &( m_dwAccessKeyTextSize ), NULL );

                // If buffer for key text was not large enough, reallocate
buffer that is large enough and try again. if ( dwResult ==
ERROR_INSUFFICIENT_BUFFER )
                {
                        delete [] m_pAccessKeyText;
                        m_pAccessKeyText = new wchar_t[ m_dwAccessKeyTextSize ];
                        dwResult = XSocialGetUserToken( dwUserIndex,
dwSocialNetworkID, pRequiredPermissions, m_pAccessKeyText, &(
m_dwAccessKeyTextSize ), NULL );
                }
        }
        ...
        if ( dwResult != ERROR_SUCCESS )
        {
                switch( dwResult )
                {
                        // ERROR_OUT_MEMORY does not seem to exist, despite
being mentioned in the XDK docs.
                        ...
                        case ERROR_INVALID_PARAMETER:
                                // dwUserIndex or dwSocialNetworkID are
incorrect! assert( false ); break;

                        case ERROR_ACCESS_DENIED:
                                // Either no user is signed in, or title not
allowed to use social posting (the latter should never occur!). break;

                        case ERROR_FUNCTION_FAILED:
                                // dwTrackingID is not valid. Kinect only. Not
sure what we would do about this one? assert( false ); break;

                        case ERROR_INSUFFICIENT_BUFFER:
                                // This should not be possible, we already
resized the buffer and retried if it was too small. assert( false ); break;

                        case ERROR_GRACEFUL_DISCONNECT:
                                // User disconnected before process was
complete. break;
                        ...
                        case ERROR_BAD_NET_RESP:
                                // Error in access, or some kind of network
related error.
...
// So far we have only created a syncronous version of this function, and the
xbox functionality does not seem to work yet! bool
CSocialManager::ObtainSocialNetworkAccessKey( ESocialNetwork eSocialNetwork,
DWORD dwUserIndex, bool bUsingKinect )
{
        ...
        bool bSocialNetworkAccessKeyAvailable = GetSocialNetworkAccessKey(
eSocialNetwork, dwUserIndex, bUsingKinect, dwUserTrackingIndex, false );

        if ( bSocialNetworkAccessKeyAvailable == false )
        {
                // No chached user key, so we will try the UI version to signin
to social network? bSocialNetworkAccessKeyAvailable = GetSocialNetworkAccessKey(
eSocialNetwork, dwUserIndex, bUsingKinect, dwUserTrackingIndex, true );
        }
