...
class CConsoleMinecraftApp : public CMinecraftApp
{
  ...
  public:
    ...
    XTITLE_DEPLOYMENT_TYPE getDeploymentType()
    {
        return m_titleDeploymentType;
    }
    ...
    void TMSPP_RetrieveFileList(int iPad,
                                C4JStorage::eGlobalStorage eStorageFacility,
                                CHAR *szPath, eTMSAction NextAction);
    ...
    static int
    Callback_TMSPPReadFileList(void *pParam, int iPad, int iUserData,
                               C4JStorage::PTMSPP_FILE_LIST pTmsFileList);
    static int Callback_TMSPPReadXuidsFile(void *pParam, int iPad, int iUserData,
                                           C4JStorage::PTMSPP_FILEDATA pFileData,
                                           LPCSTR szFilename);
    static int Callback_TMSPPReadConfigFile(void *pParam, int iPad, int iUserData,
                                            C4JStorage::PTMSPP_FILEDATA pFileData,
                                            LPCSTR szFilename);
    static int Callback_TMSPPReadDLCFile(void *pParam, int iPad, int iUserData,
                                         C4JStorage::PTMSPP_FILEDATA pFileData,
                                         LPCSTR szFilename);
    static int Callback_TMSPPReadBannedList(void *pParam, int iPad, int iUserData,
                                            C4JStorage::PTMSPP_FILEDATA pFileData,
                                            LPCSTR szFilename);
    ...
    {
        return m_bRead_TMS_DLCINFO_XML;
    }
    virtual bool GetTMSXUIDsFileRead()
    {
        return m_bRead_TMS_XUIDS_XML;
    }
    ...
    virtual int
    GetLocalTMSFileIndex(WCHAR *wchTMSFile, bool bFilenameIncludesExtension,
    ...
    virtual void ToggleFontRenderer()
    {
  ...
  private:
    ...
    bool m_bTMSPP_GlobalFileListRead; // track whether we have already read the
                                      // file list from TMSPP
    bool m_bRead_TMS_XUIDS_XML;       // track whether we have already read the TMS
                                      // xuids.xml file
    bool m_bRead_TMS_Config_XML;      // track whether we have already read the config
                                      // file
    bool m_bRead_TMS_DLCINFO_XML;     // track whether we have already read the TMS
                                      // DLC.xml file
  ...
  public:
    ...
    // 	void ReadXuidsFileFromTMS(int iPad,eTMSAction NextAction,bool
    // bCallback); 	void ReadDLCFileFromTMS(int iPad,eTMSAction NextAction,
    // bool bCallback);
    ...
    // bResult, int iAction);
    static int CallbackBannedListFileFromTMS(LPVOID lpParam, WCHAR *wchFilename,
                                             int iPad, bool bResult, int iAction);

    HRESULT RegisterFont(eFont eFontLanguage, eFont eFontFallback,
                         bool bSetAsDefault = false);

    WCHAR *GetSceneName(EUIScene eScene, bool bAppendToName,
                        bool bSplitscreenScene);

    virtual HRESULT NavigateToScene(int iPad, EUIScene eScene,
                                    void *initData = NULL,
    ...
    virtual HRESULT CloseXuiScenesAndNavigateToScene(int iPad, EUIScene eScene,
                                                     void *initData = NULL,
                                                     bool forceUsePad = false);
    ...
    virtual void ReloadChatScene(int iPad, bool bJoining = false,
                                 bool bForce = false);
    virtual void ReloadHudScene(int iPad, bool bJoining = false,
                                bool bForce = false);
    ...
    {
        return m_bPauseMenuDisplayed[iPad];
    }
    bool IsContainerMenuDisplayed(int iPad)
    {
        return m_bContainerMenuDisplayed[iPad];
    }
    bool IsIgnoreAutosaveMenuDisplayed(int iPad)
    {
        return m_bIgnoreAutosaveMenuDisplayed[iPad];
    }
    void SetIgnoreAutosaveMenuDisplayed(int iPad, bool displayed)
    {
        m_bIgnoreAutosaveMenuDisplayed[iPad] = displayed;
    }
    bool IsIgnorePlayerJoinMenuDisplayed(int iPad)
    {
        return m_bIgnorePlayerJoinMenuDisplayed[iPad];
    }
    void SetIgnorePlayerJoinMenuDisplayed(int iPad, bool displayed)
    {
        m_bIgnorePlayerJoinMenuDisplayed[iPad] = displayed;
    }
    ...
    HXUIOBJ GetCurrentScene(int iPad)
    {
        return m_hCurrentScene[iPad];
    }

    // Move splitscreen scenes based on which screen segment they are in
    void AdjustSplitscreenScene(HXUIOBJ hScene, D3DXVECTOR3 *pvOriginalPosition,
                                int iPad, bool bAdjustXForSafeArea = true);
    void AdjustSplitscreenScene(HXUIOBJ hScene, D3DXVECTOR3 *pvOriginalPosition,
                                int iPad, float fXAdjust);
    HRESULT AdjustSplitscreenScene_PlayerChanged(HXUIOBJ hScene,
                                                 D3DXVECTOR3 *pvOriginalPosition,
                                                 int iPad, bool bJoining,
                                                 bool bAdjustXForSafeArea = true);
    HRESULT AdjustSplitscreenScene_PlayerChanged(HXUIOBJ hScene,
                                                 D3DXVECTOR3 *pvOriginalPosition,
                                                 int iPad, bool bJoining,
                                                 float fXAdjust);
