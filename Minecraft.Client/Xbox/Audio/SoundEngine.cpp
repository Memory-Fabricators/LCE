...
IXAudio2 *g_pXAudio2 =
    NULL; // pointer to XAudio2 instance used by QNet and XACT
IXAudio2MasteringVoice *g_pXAudio2MasteringVoice =
    NULL; // pointer to XAudio2 mastering voice
...
void SoundEngine::init(Options *pOptions)
{
    ...
    // 4J-PB - move this to the title update, since we've corrected it to allow
    // sounds to be pitch varied when they weren't before
    HANDLE file;
#ifdef _TU_BUILD
    file = CreateFile("UPDATE:\\res\\audio\\Minecraft.xgs", GENERIC_READ, 0, NULL,
                      OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
    file =
        CreateFile("GAME:\\res\\TitleUpdate\\audio\\Minecraft.xgs", GENERIC_READ,
    ...
    DWORD bytesRead = 0;
    DWORD memFlags = MAKE_XALLOC_ATTRIBUTES(
        0, FALSE, TRUE, FALSE, 0, XALLOC_PHYSICAL_ALIGNMENT_DEFAULT,
        XALLOC_MEMPROTECT_READWRITE, FALSE, XALLOC_MEMTYPE_PHYSICAL);
    ...
    // Set up common structures that can be re-used between sounds & just have
    // required bits updated
    ...
    m_DSPSettings.pMatrixCoefficients =
        new FLOAT32[m_DSPSettings.SrcChannelCount *
    ...
    // Create resident wave bank - leave memory for this managed by xact so it can
    // free it

    file = CreateFile("GAME:\\res\\audio\\resident.xwb", GENERIC_READ, 0, NULL,
                      OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (file == INVALID_HANDLE_VALUE)
    {
        app.FatalLoadError();
    ...
    if (FAILED(hr = m_pXACT3Engine->CreateInMemoryWaveBank(
                   pvWaveBank, dwFileSize, XACT_FLAG_ENGINE_CREATE_MANAGEDATA,
                   memFlags, &m_pWaveBank)))
    {
        app.FatalLoadError();
        assert(false);
        return;
...
#ifdef _TU_BUILD
    file = CreateFile("UPDATE:\\res\\audio\\additional.xwb", GENERIC_READ, 0,
                      NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
    file =
        CreateFile("GAME:\\res\\TitleUpdate\\audio\\additional.xwb", GENERIC_READ,
                   0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
    if (file == INVALID_HANDLE_VALUE)
    {
    ...
    if (FAILED(hr = m_pXACT3Engine->CreateInMemoryWaveBank(
                   pvWaveBank2, dwFileSize, XACT_FLAG_ENGINE_CREATE_MANAGEDATA,
                   memFlags, &m_pWaveBank2)))
    {
        app.FatalLoadError();
        assert(false);
        return;
    ...
    file = CreateFile("GAME:\\res\\audio\\streamed.xwb", GENERIC_READ, 0, NULL,
                      OPEN_ALWAYS, FILE_FLAG_OVERLAPPED | FILE_FLAG_NO_BUFFERING,
                      NULL);
    ...
    streamParams.packetSize = 16; // Not sure what to pick for this - suggests a
                                  // "multiple of 16" for DVD playback

    if (FAILED(hr = m_pXACT3Engine->CreateStreamingWaveBank(
                   &streamParams, &m_pStreamedWaveBank)))
    {
        app.FatalLoadError();
        assert(false);
        return;
    ...
    // file = CreateFile("GAME:\\res\\audio\\AdditionalMusic.xwb", GENERIC_READ,
    // 0, NULL, OPEN_ALWAYS, FILE_FLAG_OVERLAPPED | FILE_FLAG_NO_BUFFERING, NULL);
#ifdef _TU_BUILD
    file = CreateFile("UPDATE:\\res\\audio\\AdditionalMusic.xwb", GENERIC_READ, 0,
                      NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
    file = CreateFile("GAME:\\res\\TitleUpdate\\audio\\AdditionalMusic.xwb",
    ...
    if (FAILED(hr = m_pXACT3Engine->CreateStreamingWaveBank(
                   &streamParams, &m_pStreamedWaveBankAdditional)))
    {
        app.FatalLoadError();
        assert(false);
        return;
    ...
    // file = CreateFile("GAME:\\res\\audio\\minecraft.xsb", GENERIC_READ, 0,
    // NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#ifdef _TU_BUILD
    file = CreateFile("UPDATE:\\res\\audio\\minecraft.xsb", GENERIC_READ, 0, NULL,
                      OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
    file =
        CreateFile("GAME:\\res\\TitleUpdate\\audio\\minecraft.xsb", GENERIC_READ,
                   0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
    if (file == INVALID_HANDLE_VALUE)
    {
    ...
    if (FAILED(hr = m_pXACT3Engine->CreateSoundBank(
                   pvSoundBank, dwFileSize, XACT_FLAG_ENGINE_CREATE_MANAGEDATA,
                   memFlags, &m_pSoundBank)))
    {
        app.FatalLoadError();
        assert(false);
        return;
    ...
    // Create sound bank2 - leave memory for this managed by xact so it can free
    // it

#ifdef _TU_BUILD
    file = CreateFile("UPDATE:\\res\\audio\\additional.xsb", GENERIC_READ, 0,
                      NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#else
    file =
        CreateFile("GAME:\\res\\TitleUpdate\\audio\\additional.xsb", GENERIC_READ,
                   0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
    if (file == INVALID_HANDLE_VALUE)
    {
    ...
    if (FAILED(hr = m_pXACT3Engine->CreateSoundBank(
                   pvSoundBank2, dwFileSize, XACT_FLAG_ENGINE_CREATE_MANAGEDATA,
                   memFlags, &m_pSoundBank2)))
    {
        app.FatalLoadError();
        assert(false);
        return;
...
void SoundEngine::CreateStreamingWavebank(const char *pchName,
                                          IXACT3WaveBank **ppStreamedWaveBank)
{
    // Create streamed sound bank
    HRESULT hr;

    HANDLE file = CreateFile(pchName, GENERIC_READ, 0, NULL, OPEN_ALWAYS,
...
void SoundEngine::CreateSoundbank(const char *pchName,
...
void SoundEngine::XACTNotificationCallback(
    const XACT_NOTIFICATION *pNotification)
{
    if (pNotification->pvContext != NULL)
    {
        if (pNotification->type == XACTNOTIFICATIONTYPE_WAVEBANKPREPARED)
        {
            ...
            if (pNotification->waveBank.pWaveBank ==
                pSoundEngine->m_pStreamedWaveBank)
            {
                pSoundEngine->m_bStreamingWaveBank1Ready = true;
            }
            if (pNotification->waveBank.pWaveBank ==
                pSoundEngine->m_pStreamedWaveBankAdditional)
            {
                pSoundEngine->m_bStreamingWaveBank2Ready = true;
            }

            if (pSoundEngine->m_bStreamingWaveBank1Ready &&
                pSoundEngine->m_bStreamingWaveBank2Ready)
            {
                       ...
                       float pitch)
{
    ...
    // 4J-PB - check how many of this cue are already playing and ignore if there
    // are loads
    int iSameSoundC = 0;
    ...
    // Register to receive callbacks for cues stopping so we can keep a track of
    // active sounds
...
void SoundEngine::playStreaming(const wstring &name, float x, float y, float z,
                                float vol, float pitch, bool bMusicDelay)
{
    ...
    if (name.empty())
    {
        ...
        SetIsPlayingStreamingGameMusic(
            false); // will be set to true when the sound is started in the tick
        if (bMusicDelay)
        {
    ...
    {
        // don't play the game music - it will start playing in the tick when
        // noMusicDelay is 0
    ...
    if (GetIsPlayingStreamingGameMusic())
    {
        if (m_MusicInfo.pCue != NULL)
        {
            ...
            if ((playerInEnd && !GetIsPlayingEndMusic()) ||
                (!playerInEnd && GetIsPlayingEndMusic()))
            {
                // remove the sound and reset the music
                playStreaming(L"", 0, 0, 0, 0, 0);
            }
            else if ((playerInNether && !GetIsPlayingNetherMusic()) ||
                     (!playerInNether && GetIsPlayingNetherMusic()))
            {
                // remove the sound and reset the music
                playStreaming(L"", 0, 0, 0, 0, 0);
            }
...
void SoundEngine::update3DPosition(SoundEngine::soundInfo *pInfo,
                                   bool bPlaceEmitterAtListener,
                                   bool bIsCDMusic)
{
    X3DAUDIO_LISTENER *listener =
        &m_listeners[0]; // Default case for single listener

    if ((m_validListenerCount > 1) && !bPlaceEmitterAtListener)
    {
        // More than one listener. Find out which one is closest
        float nearDistSq =
            (listener->Position.x - pInfo->x) * (listener->Position.x - pInfo->x) +
        ...
        {
            float distSq = (m_listeners[i].Position.x - pInfo->x) *
                               (m_listeners[i].Position.x - pInfo->x) +
                           (m_listeners[i].Position.y - pInfo->y) *
                               (m_listeners[i].Position.y - pInfo->y) +
                           (m_listeners[i].Position.z + pInfo->z) *
                               (m_listeners[i].Position.z + pInfo->z);
        ...
        // More than one listener, don't do directional sounds - point our listener
        // towards the sound
        float xzDist = sqrtf(
            (listener->Position.x - pInfo->x) * (listener->Position.x - pInfo->x) +
            (listener->Position.z + pInfo->z) * (listener->Position.z + pInfo->z));
    ...
    {
        // Update the position of the emitter - we aren't dynamically changing
        // anything else
        m_emitter.Position.x = pInfo->x;
        m_emitter.Position.y = pInfo->y;
        m_emitter.Position.z = -pInfo->z; // Flipped sign of z as x3daudio is
    ...
    // 10000.0f is passed as the volume for thunder... treat this as a special
    // case, and use a volume curve that doesn't decay with distance rather than
    // just trying to guess at making something really really loud...
    ...
    // Put volume curve back to default in case something else is depending on
    // this
    m_emitter.pVolumeCurve = &m_VolumeCurve;
    // m_emitter.pLFECurve = &m_VolumeCurve;
    m_emitter.CurveDistanceScaler = 16.0f;
    // Apply our general volume too by scaling the calculated coefficients - so
    // long as this isn't our special case of 10000.0f (see comment above)
    if (pInfo->volume != 10000.0f)
    {
...
void SoundEngine::tick(shared_ptr<Mob> *players, float a)
{
    ...
    if (players)
    {
        for (int i = 0; i < 4; i++)
        {
            if (players[i] != NULL)
            {
                ...
                m_listeners[listenerCount].Position.x =
                    (float)(players[i]->xo + (players[i]->x - players[i]->xo) * a);
                m_listeners[listenerCount].Position.y =
                    (float)(players[i]->yo + (players[i]->y - players[i]->yo) * a);
                m_listeners[listenerCount].Position.z =
                    -(float)(players[i]->zo +
                             (players[i]->z - players[i]->zo) *
                                 a); // Flipped sign of z as x3daudio is expecting left
                                     // handed coord system
                ...
                m_listeners[listenerCount].OrientFront.z =
                    yCos; // Flipped sign of z as x3daudio is expecting left handed
                          // coord system
    ...
    {
        doPosUpdate =
            false; // Don't bother updating positions of sounds already placed
