...
#ifdef _XBOX
extern IXAudio2
    *g_pXAudio2; // pointer to XAudio2 instance used by QNet and XACT
extern IXAudio2MasteringVoice
    *g_pXAudio2MasteringVoice; // pointer to XAudio2 mastering voice
#endif
...
class SoundEngine : public ConsoleSoundEngine
{
    ...
    struct soundInfo
    {
        // 4J-PB - adding the cue index so we can limit the number of the same
        // sounds playing (rain)
        XACTINDEX idx;
        int iSoundBank;
    ...
    void update3DPosition(soundInfo *pInfo, bool bPlaceEmitterAtListener = false,
                          bool bIsCDMusic = false);
  ...
  public:
    ...
    virtual void tick(shared_ptr<Mob> *players,
    ...
    void CreateStreamingWavebank(const char *pchName,
                                 IXACT3WaveBank **ppStreamedWaveBank);
    void CreateSoundbank(const char *pchName, IXACT3SoundBank **ppSoundBank);

#endif // __PS3__
    virtual char *ConvertSoundPathToName(const wstring &name,
