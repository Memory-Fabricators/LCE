...
VOID CPlatformNetworkManagerXbox::NotifyStateChanged(__in QNET_STATE OldState,
                                                     ...
                                                     __in HRESULT hrInfo)
{
    ...
    app.DebugPrintf("State: %s ==> %s, result 0x%08x.\n",
                    c_apszStateNames[OldState], c_apszStateNames[NewState],
                    hrInfo);
    ...
    else if (NewState == QNET_STATE_SESSION_JOINING)
    {
        ...
        g_NetworkManager.StateChange_AnyToJoining();
             ...
             OldState == QNET_STATE_SESSION_JOINING)
    {
        ...
        g_NetworkManager.StateChange_JoiningToIdle(reason);
             ...
             OldState == QNET_STATE_SESSION_HOSTING)
    {
        m_bLeavingGame = true;
    }
    ...
    // multiplayer client if host of the game will exit during the clients loading
    // to created world. 4J Stu - If the client joins just as the host is exiting,
    // then they can skip to leaving without passing through ending
    else if (NewState == QNET_STATE_SESSION_ENDING ||
             (NewState == QNET_STATE_SESSION_LEAVING &&
              OldState == QNET_STATE_GAME_PLAY))
    {
    ...
    __in IQNetPlayer *pQNetPlayer)
{
    ...
    // queue of game data. Outbound is all handled by QNet so we don't need that.
    // Therefore each client player has one, and the host has one for each client
    // player.
    ...
    if (pQNetPlayer->IsLocal())
    {
        ...
        if (pQNetPlayer->IsHost())
        {
            ...
            // 4J Stu - No socket for the localhost as it uses a special loopback
            // queue
        ...
        else
        {
            ...
            // We need an inbound queue on all local players to receive data from the
            // host
    ...
    for (int idx = 0; idx < XUSER_MAX_COUNT; ++idx)
    {
        ...
        {
            playerChangedCallback[idx](playerChangedCallbackParam[idx], networkPlayer,
                                       false);
        }
    }
    ...
    __in IQNetPlayer *pQNetPlayer)
{
    ...
    if (socket != NULL)
    {
        ...
        // in which case we they will already have left the game server, or from a
        // disconnection where we then have to remove them from the game server
        if (m_pIQNet->IsHost() && !m_bHostChanged)
        {
        ...
        // TODO 4J Stu - We may still be using this at the point that the player
        // leaves the session. We need this as long as the game server still needs
    ...
    if (m_pIQNet->IsHost() && !m_bHostChanged)
    {
        if (isSystemPrimaryPlayer(pQNetPlayer))
        {
            ...
            for (unsigned int i = 0; i < m_pIQNet->GetPlayerCount(); ++i)
            {
                ...
                if (pQNetPlayer2 != pQNetPlayer &&
                    pQNetPlayer2->IsSameSystem(pQNetPlayer))
                {
                    pNewQNetPrimaryPlayer = pQNetPlayer2;
                    break;
                }
            }
            AUTO_VAR(it, find(m_machineQNetPrimaryPlayers.begin(),
                              m_machineQNetPrimaryPlayers.end(), pQNetPlayer));
...
VOID CPlatformNetworkManagerXbox::NotifyNewHost(__in IQNetPlayer *pQNetPlayer)
{
    app.DebugPrintf("Player 0x%p \"%ls\" (local %i) is new host.\n", pQNetPlayer,
                    pQNetPlayer->GetGamertag(), (int)pQNetPlayer->IsLocal());
...
VOID CPlatformNetworkManagerXbox::NotifyDataReceived(
    __in IQNetPlayer *pQNetPlayerFrom, __in DWORD dwNumPlayersTo,
    __in_ecount(dwNumPlayersTo) IQNetPlayer **apQNetPlayersTo,
    __in_bcount(dwDataSize) const BYTE *pbData, __in DWORD dwDataSize)
{
  ...
  for( dwPlayer = 0; dwPlayer < dwNumPlayersTo; dwPlayer++ )
  {
    ...
    */
    ...
    for (dwPlayer = 0; dwPlayer < dwNumPlayersTo; dwPlayer++)
    {
        ...
        {
            // If we are not the host the message must have come from the host, so we
            // care more about who it is addressed to
...
VOID CPlatformNetworkManagerXbox::NotifyReadinessChanged(
    __in IQNetPlayer *pQNetPlayer, __in BOOL bReady)
{
...
VOID CPlatformNetworkManagerXbox::NotifyGameSearchComplete(
    __in IQNetGameSearch *pGameSearch, __in HRESULT hrComplete,
...
VOID CPlatformNetworkManagerXbox::NotifyGameInvite(
    __in DWORD dwUserIndex, __in const INVITE_INFO *pInviteInfo)
{
...
bool CPlatformNetworkManagerXbox::Initialise(
    CGameNetworkManager *pGameNetworkManager, int flagIndexSize)
{
    ...
    hr = QNetCreateUsingXAudio2(QNET_SESSIONTYPE_LIVE_STANDARD, this, NULL,
                                g_pXAudio2, &m_pIQNet);
    if (FAILED(hr))
    {
    ...
    // We DO NOT want QNet to handle XN_SYS_SIGNINCHANGED but so far everything
    // else should be fine We DO WANT QNet to handle XN_LIVE_INVITE_ACCEPTED at a
    // minimum Receive all types that QNet needs, and filter out the specific ones
    // we don't want later
    m_notificationListener =
        XNotifyCreateListener(XNOTIFY_SYSTEM | XNOTIFY_FRIENDS | XNOTIFY_LIVE);
...
bool CPlatformNetworkManagerXbox::isSystemPrimaryPlayer(
...
void CPlatformNetworkManagerXbox::DoWork()
{
                          ...
                          &dwNotifyId, &ulpNotifyParam))
    {
        ...
        case XN_LIVE_INVITE_ACCEPTED:
            // ignore these - we're catching them from the game listener, so we can
            // get the one from the dashboard
            break;
        default:
...
bool CPlatformNetworkManagerXbox::RemoveLocalPlayerByUserIndex(int userIndex)
{
    ...
    if (pNetworkPlayer != NULL)
    {
        ...
        if (socket != NULL)
        {
            // We can't remove the player from qnet until we have stopped using it to
            // communicate
            C4JThread *thread = new C4JThread(
                &CPlatformNetworkManagerXbox::RemovePlayerOnSocketClosedThreadProc,
...
bool CPlatformNetworkManagerXbox::IsInStatsEnabledSession()
{
    ...
    // Use QNET_LIVE_STATS_MODE_AUTO if there is another way to check if stats are
    // enabled or not
    bool statsEnabled = statsMode == QNET_LIVE_STATS_MODE_ENABLED;
    return m_pIQNet->GetState() != QNET_STATE_IDLE && statsEnabled;
}

bool CPlatformNetworkManagerXbox::SessionHasSpace(
    unsigned int spaceRequired /*= 1*/)
{
    // This function is used while a session is running, so all players trying to
    // join should use public slots,
    DWORD publicSlots = 0;
    ...
    m_pIQNet->GetOpt(QNET_OPTION_FILLED_PUBLIC_SLOTS, &filledPublicSlots,
                     &dataSize);
    m_pIQNet->GetOpt(QNET_OPTION_TOTAL_PRIVATE_SLOTS, &privateSlots, &dataSize);
    m_pIQNet->GetOpt(QNET_OPTION_FILLED_PRIVATE_SLOTS, &filledPrivateSlots,
                     &dataSize);

    DWORD spaceLeft =
        (publicSlots - filledPublicSlots) + (privateSlots - filledPrivateSlots);
...
bool CPlatformNetworkManagerXbox::LeaveGame(bool bMigrateHost)
{
    ...
    // online works
    IQNetPlayer *pQNetPlayer =
        m_pIQNet->GetLocalPlayerByUserIndex(g_NetworkManager.GetPrimaryPad());
    INetworkPlayer *pNetworkPlayer = getNetworkPlayer(pQNetPlayer);
    ...
    if (pNetworkPlayer != NULL)
    {
        ...
        if (socket != NULL)
        {
            ...
            if (IsInSession())
            {
                ...
                pQNetPlayer = m_pIQNet->GetLocalPlayerByUserIndex(
...
bool CPlatformNetworkManagerXbox::_LeaveGame(bool bMigrateHost,
                                             bool bLeaveRoom)
{
    // 4J Stu - Fix for #10490 - TCR 001 BAS Game Stability: When a party of four
    // players leave a world to join another world without saving the title will
    // crash. Changed this to make it threadsafe
...
void CPlatformNetworkManagerXbox::HostGame(
...
void CPlatformNetworkManagerXbox::_HostGame(
    ...
    unsigned char privateSlots /*= 0*/)
{
    ...
    hr = m_pIQNet->HostGame(g_NetworkManager.GetLockedProfile(), // dwUserIndex
    ...
    hr = m_pIQNet->SetOpt(QNET_OPTION_QOS_DATA_BUFFER, &m_hostGameSessionData,
...
bool CPlatformNetworkManagerXbox::_StartGame()
{
    ...
    BOOL enableJip = TRUE; // Must always be true othewise nobody can join the
                           // game while in the PLAY state
    m_pIQNet->SetOpt(QNET_OPTION_JOIN_IN_PROGRESS_ALLOWED, &enableJip,
                     sizeof BOOL);
...
int CPlatformNetworkManagerXbox::JoinGame(FriendSessionInfo *searchResult,
                                          int localUsersMask,
                                          int primaryUserIndex)
{
    // Being a bit over-cautious here, but the xbox code pre-refactoring took a
    // copy of XSESSION_SEARCHRESULT (although not in static memory) so seems
    // safest to replicate this kind of thing here rather than risk data being
    // pointed to by the searchResult being altered whilst
    ...
    HRESULT hr =
        m_pIQNet->JoinGameFromSearchResult(primaryUserIndex,   // dwUserIndex
...
bool CPlatformNetworkManagerXbox::SetLocalGame(bool isLocal)
{
    if (m_pIQNet->GetState() == QNET_STATE_IDLE)
    {
        QNET_SESSIONTYPE sessionType =
            isLocal ? QNET_SESSIONTYPE_LOCAL : QNET_SESSIONTYPE_LIVE_STANDARD;
        m_pIQNet->SetOpt(QNET_OPTION_TYPE_SESSIONTYPE, &sessionType,
                         sizeof QNET_SESSIONTYPE);

        // The default value for this is QNET_LIVE_STATS_MODE_AUTO, but that decides
        // based on the players in when the game starts. As we may want a non-live
        // player to join the game we cannot have stats enabled when we create the
        // sessions. As a result of this, the NotifyWriteStats callback will not be
        // called for LIVE players that are connected to LIVE so we write their
        // stats data on a state change.
        QNET_LIVE_STATS_MODE statsMode =
            isLocal ? QNET_LIVE_STATS_MODE_DISABLED : QNET_LIVE_STATS_MODE_ENABLED;
        m_pIQNet->SetOpt(QNET_OPTION_LIVE_STATS_MODE, &statsMode,
                         sizeof QNET_LIVE_STATS_MODE);

        // Also has a default of QNET_LIVE_PRESENCE_MODE_AUTO as above, although the
        // effects are less of an issue
        QNET_LIVE_PRESENCE_MODE presenceMode =
            isLocal ? QNET_LIVE_PRESENCE_MODE_NOT_ADVERTISED
                    : QNET_LIVE_PRESENCE_MODE_ADVERTISED;
        m_pIQNet->SetOpt(QNET_OPTION_LIVE_PRESENCE_MODE, &presenceMode,
                         sizeof QNET_LIVE_PRESENCE_MODE);
    ...
    {
        app.DebugPrintf(
            "Tried to change QNet Session type while not in idle state\n");
    }
    return true;
}
...
void CPlatformNetworkManagerXbox::RegisterPlayerChangedCallback(
    int iPad,
...
void CPlatformNetworkManagerXbox::UnRegisterPlayerChangedCallback(
    int iPad,
    void (*callback)(void *callbackParam, INetworkPlayer *pPlayer,
                     bool leaving),
    void *callbackParam)
...
{
    // We delay actually starting the session so that we know the game server is
    // running by the time the clients try to join This does result in a host
    // advantage
...
void CPlatformNetworkManagerXbox::UpdateAndSetGameSessionData(
    INetworkPlayer *pNetworkPlayerLeaving /*= NULL*/)
{
    ...
    for (unsigned int i = 0; i < MINECRAFT_NET_MAX_PLAYERS; ++i)
    {
        if (i < playerCount)
        {
            ...
            // We can call this from NotifyPlayerLeaving but at that point the player
            // is still considered in the session
    ...
    m_hostGameSessionData.hostPlayerUID =
        ((NetworkPlayerXbox *)GetHostPlayer())->GetQNetPlayer()->GetXuid();
...
int CPlatformNetworkManagerXbox::RemovePlayerOnSocketClosedThreadProc(
    void *lpParam)
{
...
bool CPlatformNetworkManagerXbox::RemoveLocalPlayer(
    INetworkPlayer *pNetworkPlayer)
{
    if (pNetworkPlayer->IsLocal())
    {
        return (m_pIQNet->RemoveLocalPlayerByUserIndex(
    ...
    INetworkPlayer *pNetworkPlayer, unsigned int count)
{
    ...
    // calculated from a load of separate values, and makes tweaking world/render
    // sizes a pain if we hit an assert here
    count = (count + 8 - 1) & ~(8 - 1);
    // assert( ( count % 8 ) == 0 );
...
void CPlatformNetworkManagerXbox::SystemFlagAddPlayer(
    INetworkPlayer *pNetworkPlayer)
{
    PlayerFlags *newPlayerFlags =
        new PlayerFlags(pNetworkPlayer, m_flagIndexSize);
    // If any of our existing players are on the same system, then copy over flags
    // from that one
...
void CPlatformNetworkManagerXbox::SystemFlagRemovePlayer(
    INetworkPlayer *pNetworkPlayer)
{
    for (unsigned int i = 0; i < m_playerFlags.size(); i++)
    {
...
// shares that system
void CPlatformNetworkManagerXbox::SystemFlagSet(INetworkPlayer *pNetworkPlayer,
                                                int index)
{
...
// here
bool CPlatformNetworkManagerXbox::SystemFlagGet(INetworkPlayer *pNetworkPlayer,
                                                int index)
{
    if ((index < 0) || (index >= m_flagIndexSize))
    {
...
{
    return L"Queue messages: " +
           _toString(
               ((NetworkPlayerXbox *)GetHostPlayer())
                   ->GetQNetPlayer()
                   ->GetSendQueueSize(NULL, QNET_GETSENDQUEUESIZE_MESSAGES)) +
           L" Queue bytes: " +
           _toString(((NetworkPlayerXbox *)GetHostPlayer())
                         ->GetQNetPlayer()
                         ->GetSendQueueSize(NULL, QNET_GETSENDQUEUESIZE_BYTES));
}
...
void CPlatformNetworkManagerXbox::TickSearch()
{
    if (m_bSearchPending)
    {
        ...
        {
            m_currentSearchResultsCount[m_lastSearchPad] =
                m_searchResultsCount[m_lastSearchPad];
            ...
            m_pCurrentSearchResults[m_lastSearchPad] =
    ...
    else
    {
        ...
        if (m_SessionsUpdatedCallback != NULL &&
            (m_lastSearchStartTime[g_NetworkManager.GetPrimaryPad()] +
             MINECRAFT_XSESSION_SEARCH_DELAY_MILLISECONDS) < GetTickCount())
        {
            SearchForGames();
        }
    }
...
void CPlatformNetworkManagerXbox::SearchForGames()
{
    ...
    for (AUTO_VAR(it, friendsSessions[m_lastSearchPad].begin());
         it < friendsSessions[m_lastSearchPad].end(); ++it)
    {
        delete (*it);
    }
    ...
    bool bMultiplayerAllowed =
        g_NetworkManager.IsSignedInLive(g_NetworkManager.GetPrimaryPad()) &&
        g_NetworkManager.AllowedToPlayMultiplayer(
            g_NetworkManager.GetPrimaryPad());
    ...
    if (bMultiplayerAllowed)
    {
        ...
        if ((partyResult != XPARTY_E_NOT_IN_PARTY) &&
            (partyUserList.dwUserCount > 1))
        {
            for (unsigned int i = 0; i < partyUserList.dwUserCount; i++)
            {
                if (((partyUserList.Users[i].dwFlags & XPARTY_USER_ISLOCAL) !=
                     XPARTY_USER_ISLOCAL) &&
                    ((partyUserList.Users[i].dwFlags & XPARTY_USER_ISINGAMESESSION) ==
                     XPARTY_USER_ISINGAMESESSION) &&
                    partyUserList.Users[i].dwTitleId == TITLEID_MINECRAFT)
                {
                         ...
                         it < friendsSessions[m_lastSearchPad].end(); ++it)
                    {
                        ...
                        if (memcmp(&partyUserList.Users[i].SessionInfo.sessionID,
                                   &current->sessionId, sizeof(SessionID)) == 0)
                        {
        ...
        DWORD hr =
            XFriendsCreateEnumerator(g_NetworkManager.GetPrimaryPad(), 0,
                                     MAX_FRIENDS, &bufferSize, &hFriendsEnumerator);

        char *buffer = new char[bufferSize];
        DWORD itemsReturned;
        DWORD result = XEnumerate(hFriendsEnumerator, buffer, bufferSize,
                                  &itemsReturned, NULL);

        DWORD flagPlayingOnline =
            XONLINE_FRIENDSTATE_FLAG_ONLINE; // | XONLINE_FRIENDSTATE_FLAG_PLAYING;
        ...
        for (unsigned int i = 0; i < itemsReturned; i++)
        {
            ...
            if ((friends[i].dwFriendState & flagPlayingOnline) == flagPlayingOnline &&
                ((friends[i].dwFriendState & XONLINE_FRIENDSTATE_FLAG_JOINABLE) ==
                     XONLINE_FRIENDSTATE_FLAG_JOINABLE ||
                 (friends[i].dwFriendState &
                  XONLINE_FRIENDSTATE_FLAG_JOINABLE_FRIENDS_ONLY) ==
                     XONLINE_FRIENDSTATE_FLAG_JOINABLE_FRIENDS_ONLY) &&
                (friends[i].dwFriendState &
                ...
                friends[i].dwTitleID == TITLEID_MINECRAFT)
            {
                ...
                {
                    FriendSessionInfo *current = *it;
                    if (memcmp(&friends[i].sessionID, &current->sessionId,
                               sizeof(SessionID)) == 0)
                    {
                        // printf("We already have this session from another player.\n");
                        sessionAlreadyAdded = true;
                ...
                if (!sessionAlreadyAdded)
                {
                    ...
                    friendsSessions[m_lastSearchPad].push_back(sessionInfo);
                    // g_NetworkManager.SearchForGameById(friends[i].sessionID,&SearchForGameCallback,
                    // m_hObj);
                    //++m_searches;
                }
            }
    ...
    DWORD sessionIDCount =
        min(XSESSION_SEARCH_MAX_IDS, friendsSessions[m_lastSearchPad].size());
    ...
    dwStatus = XSessionSearchByIds(
        sessionIDCount, sessionIDList, g_NetworkManager.GetPrimaryPad(),
    ...
    // changed to reflect the size buffer that will be necessary for this call.
    // Use the new size to allocate a buffer of the appropriate size.
    if (ERROR_INSUFFICIENT_BUFFER == dwStatus && cbResults > 0)
    {
        // Allocate this on the main thread rather in the search thread which might
        // run out of memory
        m_pSearchResults[m_lastSearchPad] =
            (XSESSION_SEARCHRESULT_HEADER *)new BYTE[cbResults];
        ...
        dwStatus = XSessionSearchByIds(
            sessionIDCount, sessionIDList, g_NetworkManager.GetPrimaryPad(),
            &cbResults, // Pass in the address of the size variable
            m_pSearchResults[m_lastSearchPad], pOverlapped);
    }
    ...
    // If the function does not succeed after allocating a buffer of the
    // appropriate size succeed, something else is wrong.
    ...
    else if (cbResults > 0)
    {
        ...
        m_SearchingThread =
            new C4JThread(&CPlatformNetworkManagerXbox::SearchForGamesThreadProc,
                          threadData, "SearchForGames");
...
int CPlatformNetworkManagerXbox::SearchForGamesThreadProc(void *lpParameter)
{
    ...
    XSESSION_SEARCHRESULT_HEADER *pSearchResults =
        (XSESSION_SEARCHRESULT_HEADER *)threadData->searchBuffer;
    ...
    const XNADDR
        *QoSxnaddr[XSESSION_SEARCH_MAX_IDS]; // = new XNADDR*[sessionIDCount];
    const SessionID
        *QoSxnkid[XSESSION_SEARCH_MAX_IDS]; // = new XNKID*[sessionIDCount];
                                            // // Note SessionID is just typedef'd
                                            // to be a XNKID on xbox
    const XNKEY
        *QoSxnkey[XSESSION_SEARCH_MAX_IDS]; // = new XNKEY*[sessionIDCount];
    ...
    // Create an event object that is autoreset with an initial state of "not
    // signaled". Pass this event handle to the QoSLookup to receive notification
    // of each QoS lookup.
    ...
    *threadData->ppQos = new XNQOS();
    ...
    INT iRet = XNetQosLookup(
        pSearchResults
            ->dwSearchResults, // Number of remote Xbox 360 consoles to probe
        QoSxnaddr,             // Array of pointers to XNADDR structures
        QoSxnkid,              // Array of pointers to XNKID structures that contain session
                               // IDs for the remote Xbox 360 consoles
        QoSxnkey,              // Array of pointers to XNKEY structures that contain
                               // key-exchange keys for the remote Xbox 360 consoles
        0,                     // Number of security gateways to probe
        NULL,                  // Pointer to an array of IN_ADDR structures that contain the IP
                               // addresses of the security gateways
        NULL,                  // Pointer to an array of service IDs for the security gateway
        8,                     // Number of desired probe replies to receive
        0,                     // Maximum upstream bandwidth that the outgoing QoS probe packets can
                               // consume
        0,                     // Flags
        QoSLookupHandle,       // Event handle
        threadData->ppQos);    // Pointer to a pointer to an XNQOS structure that
                               // receives the results from the QoS probes
    ...
    else
    {
        ...
        // Wait for results to all complete.  cxnqosPending will eventually hit
        // zero. Pause thread waiting for QosLookup events to be triggered.
        while ((*threadData->ppQos)->cxnqosPending != 0)
        {
            // 4J Stu - We could wait for INFINITE if we weren't watching for the kill
            // flag
        ...
        g_pPlatformNetworkManager->SetSearchResultsReady(
            pSearchResults->dwSearchResults);
    }
...
vector<FriendSessionInfo *> *
CPlatformNetworkManagerXbox::GetSessionList(int iPad, int localPlayers,
                                            bool partyOnly)
{
    ...
    if (m_currentSearchResultsCount[iPad] > 0)
    {
        ...
        for (DWORD dwResult = 0; dwResult < m_currentSearchResultsCount[iPad];
             dwResult++)
        {
            ...
            if (pSearchResult == NULL ||
                pSearchResult->dwOpenPublicSlots < localPlayers)
            {
                continue;
            }
            ...
            for (itFriendSession = friendsSessions[iPad].begin();
                 itFriendSession < friendsSessions[iPad].end(); ++itFriendSession)
            {
                sessionInfo = *itFriendSession;
                if (memcmp(&pSearchResult->info.sessionID, &sessionInfo->sessionId,
                           sizeof(SessionID)) == 0 &&
                    (!partyOnly || (partyOnly && sessionInfo->hasPartyMember)))
                {
            ...
            // We received a search result for a session no longer in our list of
            // friends sessions
            if (!foundSession)
            {
                continue;
            ...
            if (pxnqi->bFlags & XNET_XNQOSINFO_TARGET_CONTACTED)
            {
                ...
                // pxnqi->wRttMinInMsecs, pxnqi->wRttMedInMsecs ); app.DebugPrintf( "
                // bps up = %u, down = %u\n", pxnqi->dwUpBitsPerSec,
                // pxnqi->dwDnBitsPerSec );
                ...
                if (pxnqi->cbData > 0)
                {
                         ...
                         (1L << eDebugSetting_DebugLeaderboards)))
                    {
                        swprintf(sessionInfo->displayLabel,
                                 app.GetString(IDS_GAME_HOST_NAME), L"WWWWWWWWWWWWWWWW");
                    }
                    else
#endif
                    {
                        swprintf(sessionInfo->displayLabel,
                                 app.GetString(IDS_GAME_HOST_NAME), gamerName.c_str());
                    }
                }
                else
                {
                    ...
                    sessionInfo->data.isJoinable)
                {
                    ...
                    memcpy(newInfo->displayLabel, sessionInfo->displayLabel,
                           100 * sizeof(wchar_t));
                ...
                if (sessionInfo->data.netVersion != MINECRAFT_NET_VERSION)
                {
                    wprintf(L"%ls version of %d does not match our version of %d\n",
                            sessionInfo->displayLabel, sessionInfo->data.netVersion,
...
// This runs through the search results for a session matching sessionId, then
// returns the full details in foundSessionInfo
bool CPlatformNetworkManagerXbox::GetGameSessionInfo(
             ...
             dwResult++)
        {
            ...
            if (pxnqi->bFlags & XNET_XNQOSINFO_TARGET_CONTACTED)
            {
                ...
                if (pxnqi->cbData > 0)
                {
                    ...
                    wstring gamerName = convStringToWstring(sessionInfo->data.hostName);
                    swprintf(sessionInfo->displayLabel, app.GetString(IDS_GAME_HOST_NAME),
                             L"MWWWWWWWWWWWWWWM"); // gamerName.c_str() );
                }
                else
                {
                    swprintf(sessionInfo->displayLabel,
                    ...
                    sessionInfo->data.isJoinable)
                {
                    ...
                    memcpy(foundSessionInfo->displayLabel, sessionInfo->displayLabel,
                           100 * sizeof(wchar_t));
...
void CPlatformNetworkManagerXbox::SetSessionsUpdatedCallback(
...
void CPlatformNetworkManagerXbox::GetFullFriendSessionInfo(
...
INetworkPlayer *
CPlatformNetworkManagerXbox::addNetworkPlayer(IQNetPlayer *pQNetPlayer)
{
...
void CPlatformNetworkManagerXbox::removeNetworkPlayer(
    IQNetPlayer *pQNetPlayer)
{
    INetworkPlayer *pNetworkPlayer = getNetworkPlayer(pQNetPlayer);
...
INetworkPlayer *
CPlatformNetworkManagerXbox::getNetworkPlayer(IQNetPlayer *pQNetPlayer)
{
...
INetworkPlayer *
CPlatformNetworkManagerXbox::GetLocalPlayerByUserIndex(int userIndex)
{
...
INetworkPlayer *
CPlatformNetworkManagerXbox::GetPlayerBySmallId(unsigned char smallId)
{
...
bool CPlatformNetworkManagerXbox::JoinGameFromInviteInfo(
    int userIndex, int userMask, const INVITE_INFO *pInviteInfo)
{
    return (m_pIQNet->JoinGameFromInviteInfo(userIndex, userMask, pInviteInfo) ==
            S_OK);
}
