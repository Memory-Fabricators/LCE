...
{
    return L"Queue messages: " + _toString(((NetworkPlayerXbox *)GetHostPlayer())->GetQNetPlayer()->GetSendQueueSize(NULL, QNET_GETSENDQUEUESIZE_MESSAGES)) + L" Queue bytes: " + _toString(((NetworkPlayerXbox *)GetHostPlayer())->GetQNetPlayer()->GetSendQueueSize(NULL, QNET_GETSENDQUEUESIZE_BYTES));
}
...
vector<FriendSessionInfo *> *CPlatformNetworkManagerXbox::GetSessionList(int iPad, int localPlayers, bool partyOnly)
{
    ...
    if (m_currentSearchResultsCount[iPad] > 0)
    {
        ...
        for (DWORD dwResult = 0; dwResult < m_currentSearchResultsCount[iPad]; dwResult++)
        {
            ...
            if (pSearchResult == NULL || pSearchResult->dwOpenPublicSlots < localPlayers)
            {
                continue;
            }
