...
// This is the xbox 360 specific implementation of CPlatformNetworkManager. It
// is implemented using QNET. There shouldn't be any general game code in here,
// this class is for providing a bridge between the common game-side network
// implementation, and the lowest level platform specific libraries (QNET in
// this case).

class CPlatformNetworkManagerXbox : public CPlatformNetworkManager,
                                    public IQNetCallbacks
{
  ...
  private:
    ...
    virtual void _HostGame(int dwUsersMask,
                           unsigned char publicSlots = MINECRAFT_NET_MAX_PLAYERS,
                           unsigned char privateSlots = 0);
  ...
  private:
    ...
    // The results from the previous search, which are currently displayed in the
    // game
  ...
  public:
    virtual vector<FriendSessionInfo *> *
    ...
    virtual void
    SetSessionsUpdatedCallback(void (*SessionsUpdatedCallback)(LPVOID pParam),
                               LPVOID pSearchParam);
    ...
    VOID NotifyStateChanged(__in QNET_STATE OldState, __in QNET_STATE NewState,
                            __in HRESULT hrInfo);
    ...
    VOID NotifyDataReceived(__in IQNetPlayer *pQNetPlayerFrom,
                            __in DWORD dwNumPlayersTo,
                            __in_ecount(dwNumPlayersTo)
                                IQNetPlayer **apQNetPlayersTo,
    ...
    VOID NotifyGameSearchComplete(__in IQNetGameSearch *pGameSearch,
                                  __in HRESULT hrComplete,
                                  __in DWORD dwNumResults);
    VOID NotifyGameInvite(__in DWORD dwUserIndex,
