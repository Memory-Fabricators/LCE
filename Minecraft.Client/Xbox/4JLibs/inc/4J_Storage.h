...
typedef VOID(CALLBACK *TMSCLIENT_PROG_CALLBACK)(
    __in float progress, __in_opt void *userCallbackData);
...
class C4JStorage
{
  ...
  public:
    ...
    C4JStorage::EMessageResult RequestMessageBox(
        ...
        LPVOID lpParam = NULL, CXuiStringTable *pStringTable = NULL,
    ...
    // savegame
    void Init(LPCWSTR pwchDefaultSaveName, char *pszSavePackName,
    ...
    void SaveSaveData(unsigned int uiBytes, PBYTE pbThumbnail = NULL,
                      DWORD cbThumbnail = 0, PBYTE pbTextData = NULL,
                      DWORD dwTextLen = 0);
    ...
    GetSavesInfo(int iPad,
                 bool (*Func)(LPVOID, int, CACHEINFOSTRUCT *, int, HRESULT),
    ...
    LoadSaveData(XCONTENT_DATA *pContentData,
                 int (*Func)(LPVOID lpParam, const bool), LPVOID lpParam);
    C4JStorage::EDeleteGameStatus
    DeleteSaveData(XCONTENT_DATA *pContentData,
                   int (*Func)(LPVOID lpParam, const bool), LPVOID lpParam);
    ...
    GetDLCOffers(int iPad, int (*Func)(LPVOID, int, DWORD, int), LPVOID lpParam,
    ...
    int GetOfferCount();
    DWORD InstallOffer(int iOfferIDC, unsigned __int64 *ullOfferIDA,
    ...
    ReadTMSFile(int iQuadrant, eGlobalStorage eStorageFacility,
                ...
                LPVOID lpParam = NULL, int iAction = 0);
    ...
    // TMS++
    C4JStorage::ETMSStatus
    TMSPP_WriteFile(int iPad, C4JStorage::eGlobalStorage eStorageFacility,
                    C4JStorage::eTMS_FILETYPEVAL eFileTypeVal,
                    C4JStorage::eTMS_UGCTYPE eUGCType, CHAR *pchFilePath,
                    CHAR *pchBuffer, DWORD dwBufferSize,
                    int (*Func)(LPVOID, int, int) = NULL, LPVOID lpParam = NULL,
                    int iUserData = 0);
    C4JStorage::ETMSStatus TMSPP_GetUserQuotaInfo(int iPad,
                                                  TMSCLIENT_CALLBACK Func,
                                                  LPVOID lpParam,
                                                  int iUserData = 0);
    C4JStorage::ETMSStatus
    TMSPP_ReadFile(int iPad, C4JStorage::eGlobalStorage eStorageFacility,
    ...
    TMSPP_ReadFileList(int iPad, C4JStorage::eGlobalStorage eStorageFacility,
                       CHAR *pchFilePath,
                       int (*Func)(LPVOID, int, int, PTMSPP_FILE_LIST) = NULL,
                       LPVOID lpParam = NULL, int iUserData = 0);
    C4JStorage::ETMSStatus TMSPP_DeleteFile(
        int iPad, LPCSTR szFilePath, C4JStorage::eTMS_FILETYPEVAL eFileTypeVal,
        int (*Func)(LPVOID, int, int), LPVOID lpParam = NULL, int iUserData = 0);
    bool TMSPP_InFileList(eGlobalStorage eStorageFacility, int iPad,
                          const wstring &Filename);
    unsigned int CRC(unsigned char *buf, int len);

    C4JStorage::ETMSStatus TMSPP_WriteFileWithProgress(
        int iPad, C4JStorage::eGlobalStorage eStorageFacility,
        C4JStorage::eTMS_FILETYPEVAL eFileTypeVal,
        C4JStorage::eTMS_UGCTYPE eUGCType, CHAR *pchFilePath, CHAR *pchBuffer,
        DWORD dwBufferSize, int (*Func)(LPVOID, int, int) = NULL,
        LPVOID lpParam = NULL, int iUserData = 0,
        int (*CompletionFunc)(LPVOID, float fComplete) = NULL,
        LPVOID lpCompletionParam = NULL);
