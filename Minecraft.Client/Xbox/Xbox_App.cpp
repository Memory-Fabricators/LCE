...
void CConsoleMinecraftApp::GetPreviewImage(int iPad,
                                           XSOCIAL_PREVIEWIMAGE *preview)
{
    ...
    unsigned int sizeBytes =
        XGAddress2DTiledExtent(m_PreviewBuffer[iPad].Width,
                               m_PreviewBuffer[iPad].Height, 4) *
        4;
    preview->pBytes = (BYTE *)XPhysicalAlloc(sizeBytes, MAXULONG_PTR, 0,
                                             PAGE_READWRITE | PAGE_WRITECOMBINE);
    memcpy((void *)preview->pBytes, (void *)m_PreviewBuffer[iPad].pBytes,
           sizeBytes);
...
{
    RenderManager.CaptureScreen(&m_ScreenshotBuffer[iPad],
                                &m_PreviewBuffer[iPad]);
}
...
HRESULT CConsoleMinecraftApp::LoadXuiResources()
{
    ...
    // 4J-PB - we need to load all fonts in an order, and set them as fallbacks of
    // each other - if you save a game in an Asian language, you must be able to
    // switch to English, and still see the Asian characters in the save name
    ...
    default:
        ...
        if (bOverrideLanguage == false)
        {
            ...
            case XC_LANGUAGE_DUTCH:
                // 4J-PB - the loc people gave us nl-BE but no nl-NL. They claim they
                // are identical, so use nl-BE.
            ...
            case XC_LANGUAGE_SCHINESE:
                // fall back to English until we have a font, and Microsoft are happy
                // with the Chinese Simplified text
    ...
    // 	wsprintfW(szResourceLocator,L"section://%X,%s#%s",c_ModuleHandle,L"media",
    // L"media/font/Mojangles.ttf");
    ...
    {
        hr = LoadFirstScene(szResourceLocator, L"xuiscene_base.xur", NULL,
                            &mainBaseScene);
    }
    else
    {
        ...
        {
            hr = LoadFirstScene(szResourceLocator, L"xuiscene_base_480.xur", NULL,
                                &mainBaseScene);
        }
    }
    if (FAILED(hr))
    ...
    {
        XuiElementGetChildById(m_PlayerBaseScene[idx], L"XuiSceneContainer",
                               &m_hCurrentScene[idx]);
        m_hFirstScene[idx] = m_hCurrentScene[idx];

        XuiElementGetChildById(m_PlayerBaseScene[idx], L"XuiSceneTutorialContainer",
                               &m_hCurrentTutorialScene[idx]);
        m_hFirstTutorialScene[idx] = m_hCurrentTutorialScene[idx];

        XuiElementGetChildById(m_PlayerBaseScene[idx], L"XuiSceneChatRoot",
                               &m_hCurrentChatScene[idx]);
        ...
        HRESULT hr = XuiSceneCreate(
            szResourceLocator, app.GetSceneName(eUIComponent_Chat, true, false),
            &idx, &hScene);
        if (FAILED(hr))
        {
            app.FatalLoadError();
        ...
        XuiElementGetChildById(m_PlayerBaseScene[idx], L"XuiSceneHudRoot",
                               &m_hCurrentHudScene[idx]);
    ...
    XuiElementGetChildById(mainBaseScene, L"XuiPressStartMessage",
                           &hPressStartMessage);
    // set the text
    XuiControlSetText(hPressStartMessage, GetString(IDS_PRESS_START_TO_JOIN));

    HXUIOBJ debugContainerObj;
    XuiElementGetChildById(mainBaseScene, L"XuiSceneDebugContainer",
                           &debugContainerObj);
...
HRESULT CConsoleMinecraftApp::RegisterFont(eFont eFontLanguage,
                                           eFont eFontFallback,
                                           bool bSetAsDefault)
{
    HRESULT hr = S_OK;
    ...
    if (bSetAsDefault)
    {
        ...
        {
            hr = RegisterDefaultTypeface(wchTypefaceA[eFontLanguage],
                                         szResourceLocator, NULL, 0.0f,
                                         wchTypefaceA[eFontFallback]);
        }
        else
        {
            hr = RegisterDefaultTypeface(wchTypefaceA[eFontLanguage],
...
void CConsoleMinecraftApp::TMSPP_RetrieveFileList(
    int iPad, C4JStorage::eGlobalStorage eStorageFacility, CHAR *szPath,
    eTMSAction NextAction)
{
    ...
    if (eStorageFacility == C4JStorage::eGlobalStorage_Title)
    {
        if (m_bTMSPP_GlobalFileListRead == false)
        {
            ...
            StorageManager.TMSPP_ReadFileList(
                iPad, eStorageFacility, szPath,
                &CConsoleMinecraftApp::Callback_TMSPPReadFileList, this, NextAction);
        }
        else
        {
...
int CConsoleMinecraftApp::Callback_TMSPPReadFileList(
    void *pParam, int iPad, int iUserData,
    C4JStorage::PTMSPP_FILE_LIST pTmsFileList)
{
...
void CConsoleMinecraftApp::TMSPP_ReadXuidsFile(int iPad,
                                               eTMSAction NextAction)
{
    app.DebugPrintf("CConsoleMinecraftApp::TMSPP_ReadXuidsFile\n");
    C4JStorage::ETMSStatus eResult;
    // try reading the xuids.xml file (from TMS global) - only allowed to do this
    // once an hour at the most, but we'll just read once each time the game
    // launches
    ...
    if (m_bRead_TMS_XUIDS_XML == false)
    {
        ...
        eResult = StorageManager.TMSPP_ReadFile(
            iPad, C4JStorage::eGlobalStorage_Title, C4JStorage::TMS_FILETYPE_BINARY,
            "xuids.xml", &CConsoleMinecraftApp::Callback_TMSPPReadXuidsFile, this,
            NextAction);
...
int CConsoleMinecraftApp::Callback_TMSPPReadXuidsFile(
    void *pParam, int iPad, int iUserData,
...
void CConsoleMinecraftApp::TMSPP_ReadConfigFile(int iPad,
                                                eTMSAction NextAction)
{
    ...
    if (m_bRead_TMS_Config_XML == false)
    {
        ...
        eResult = StorageManager.TMSPP_ReadFile(
            iPad, C4JStorage::eGlobalStorage_Title, C4JStorage::TMS_FILETYPE_BINARY,
            "Config.xml", &CConsoleMinecraftApp::Callback_TMSPPReadConfigFile, this,
            NextAction);
...
int CConsoleMinecraftApp::Callback_TMSPPReadConfigFile(
    void *pParam, int iPad, int iUserData,
    C4JStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename)
{
...
bool CConsoleMinecraftApp::TMSPP_ReadBannedList(int iPad,
                                                eTMSAction NextAction)
{
    ...
    if (GetBanListRead(iPad) == false)
    {
                                            ...
                                            iPad, L"BannedList"))
        {
            ...
            eResult = StorageManager.TMSPP_ReadFile(
                ...
                C4JStorage::TMS_FILETYPE_BINARY, "BannedList",
                &CConsoleMinecraftApp::Callback_TMSPPReadBannedList, this,
                NextAction);
            if (eResult != C4JStorage::ETMSStatus_ReadInProgress)
            {
                // something went wrong
...
int CConsoleMinecraftApp::Callback_TMSPPReadBannedList(
    void *pParam, int iPad, int iUserData,
    C4JStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename)
{
    ...
    if (pFileData)
    {
        ...
        {
            pClass->AddLevelToBannedLevelList(iPad, pData[i].xuid,
                                              pData[i].pszLevelName, false);
        }
        // mark the level as not checked against banned levels - it'll be checked
...
void CConsoleMinecraftApp::TMSPP_ReadDLCFile(int iPad, eTMSAction NextAction)
{
    ...
    // try reading the DLC.xml file (from TMS global) - only allowed to do this
    // once an hour at the most, but we'll just read once each time the game
    // launches
    C4JStorage::ETMSStatus eResult;
    if (m_bRead_TMS_DLCINFO_XML == false)
    {
        ...
        eResult = StorageManager.TMSPP_ReadFile(
            iPad, C4JStorage::eGlobalStorage_Title, C4JStorage::TMS_FILETYPE_BINARY,
            "DLC.cmp", &CConsoleMinecraftApp::Callback_TMSPPReadDLCFile, this,
            NextAction);
        if (eResult != C4JStorage::ETMSStatus_ReadInProgress)
        {
            // something went wrong
...
int CConsoleMinecraftApp::Callback_TMSPPReadDLCFile(
    void *pParam, int iPad, int iUserData,
    C4JStorage::PTMSPP_FILEDATA pFileData, LPCSTR szFilename)
{
    ...
    if (pFileData && pFileData->dwSize > 0)
    {
        ...
        Compression::getCompression()->Decompress(
            pDecompressedData, &uiDecompSize,
            &((unsigned int *)pFileData->pbData)[2], uiCompSize);
        delete[] pFileData->pbData;
        delete[] pFileData;
...
void CConsoleMinecraftApp::ReadBannedList(int iPad, eTMSAction NextAction,
                                          bool bCallback)
{
    // try reading the banned level list file (from TMS User) - only allowed to do
    // this once an hour at the most, but we'll just read once each time the user
    // signs in
    ...
    if (GetBanListCheck(iPad) == false)
    {
        ...
        if (bCallback)
        {
            eTMSStatus = StorageManager.ReadTMSFile(
                iPad, C4JStorage::eGlobalStorage_TitleUser,
                C4JStorage::eTMS_FileType_Normal, L"BannedList",
                &BannedListA[iPad].pBannedList, &BannedListA[iPad].dwBytes,
                &CConsoleMinecraftApp::CallbackBannedListFileFromTMS, this,
                NextAction);
        }
        else
        {
            ...
            if (eTMSStatus == C4JStorage::ETMSStatus_Idle)
            {
                ...
                for (int i = 0; i < iEntries; i++)
                {
                    AddLevelToBannedLevelList(iPad, pData[i].xuid, pData[i].pszLevelName,
                                              false);
                }
    ...
    {
        app.SetTMSAction(iPad, (eTMSAction)NextAction);
        // Fix for #70412 - TCR 1.00: BAS Game Stability: The title enters an
        // unresponsive state, on the following screen, when the client attempts to
        // start a new game after leaving a hosted game.

        // if bCallback is set and we've already got the ban list (from a join in
        // session previously), we need to send a message out to let the waiting
        // process proceed
...
int CConsoleMinecraftApp::CallbackBannedListFileFromTMS(LPVOID lpParam,
                                                        WCHAR *wchFilename,
                                                        int iPad, bool bResult,
                                                        int iAction)
{
    ...
    if (pClass->BannedListA[iPad].dwBytes > 0)
    {
        ...
        PBANNEDLISTDATA pData =
            (PBANNEDLISTDATA)pClass->BannedListA[iPad].pBannedList;
...
#ifdef _XBOX
    // We don't want to even attempt using our font renderer in the asian
    // languages
    switch (XGetLanguage())
    {
    case XC_LANGUAGE_JAPANESE:
...
void CConsoleMinecraftApp::EnableDebugOverlay(bool enable, int iPad)
{
    ...
    if (enable && !debugOverlayCreated)
    {
        ...
        wsprintfW(szResourceLocator, L"section://%X,%s#%s", c_ModuleHandle,
                  L"media", L"media/");
        hr = XuiSceneCreate(szResourceLocator, L"xuiscene_debugoverlay.xur", NULL,
                            &m_hDebugOverlay);
    ...
    if (bVal)
    {
        if (iPad == XUSER_INDEX_ANY)
        {
            for (int i = 0; i < XUSER_MAX_COUNT; i++)
            {
                ...
                // unplugged during active gameplay and plugged back in at the resulting
                // pause menu, it will demonstrate dual-functionality.
        ...
        else
        {
            ...
            // 4J Stu - Fix for #11018 - Functional: When the controller is unplugged
            // during active gameplay and plugged back in at the resulting pause menu,
            // it will demonstrate dual-functionality.
            m_bMenuToBeClosed[iPad] = false;
...
bool CConsoleMinecraftApp::IsSceneInStack(int iPad, EUIScene eScene)
{
    ...
    if (m_bGameStarted)
    {
        // If the game isn't running treat as user 0, otherwise map index directly
        // from pad
        if ((iPad != 255) && (iPad >= 0))
        {
            idx = iPad;
    ...
    // for(auto it = m_sceneStack[iPad].begin(), end = m_sceneStack[iPad].end();
    // it != end; ++it)
    {
        if (it->first == eScene)
        {
...
WCHAR *CConsoleMinecraftApp::GetSceneName(EUIScene eScene, bool bAppendToName,
...
HRESULT CConsoleMinecraftApp::NavigateToScene(
    int iPad, EUIScene eScene, void *initData /* = NULL */,
    bool forceUsePad /*= false*/, BOOL bStayVisible /* = FALSE */,
    HXUIOBJ *phResultingScene /*= NULL*/)
{
    ...
    // times
    if ((eScene == eUIScene_LoadOrJoinMenu) &&
    ...
    // you were pressing a few buttons at the same time) then ignore the navigate
    if (GetMenuDisplayed(iPad))
    {
        ...
        case eUIScene_TradingMenu:
            DebugPrintf(
                "IGNORING NAVIGATE - we're trying to navigate to a user selected "
                "scene when there's already a scene up: pad:%d, scene:%d\n",
                iPad, eScene);
            return S_FALSE;
            break;
        }
    ...
    else
    {
        ...
        CXuiSceneBase::SetPlayerBaseScenePosition(
            idx, CXuiSceneBase::e_BaseScene_Fullscreen);
    }

    if (eScene != eUIComponent_TutorialPopup && eScene != eUIComponent_Chat &&
        eScene != eUIScene_HUD)
    {
    ...
    BOOL bSceneIsFullscreen = CXuiSceneBase::GetPlayerBasePosition(idx) ==
    ...
    case eUIScene_LaunchMoreOptionsMenu:
        // if we're in low res widescreen, we just want to use the normal scenes for
        // these
        if (!bIsHiDef && RenderManager.IsWidescreen())
        {
            hr = XuiSceneCreate(szResourceLocator,
                                app.GetSceneName(eScene, false, false), initData,
                                &hScene);
        }
        else
        {
            hr = XuiSceneCreate(szResourceLocator,
                                app.GetSceneName(eScene, true, false), initData,
                                &hScene);
        }
        break;
    default:
        // some scenes are forced into fullscreen even though we are insplitscreen
        // mode (saving)
        if (eScene == eUIComponent_TutorialPopup || eScene == eUIComponent_Chat ||
            eScene == eUIScene_HUD)
        {
            hr = XuiSceneCreate(szResourceLocator,
                                app.GetSceneName(eScene, true, bIsSplitscreen),
                                initData, &hScene);
        }
        else
        {
            hr = XuiSceneCreate(
                szResourceLocator,
                app.GetSceneName(eScene, true, bIsSplitscreen && !bSceneIsFullscreen),
                initData, &hScene);
        }
        break;
    }
    ...
    {
        hr = XuiSceneNavigateForward(m_hCurrentTutorialScene[iPad], FALSE, hScene,
                                     XUSER_INDEX_NONE);
        if (FAILED(hr))
        {
    ...
    {
        hr = XuiSceneNavigateForward(m_hCurrentChatScene[iPad], FALSE, hScene,
                                     XUSER_INDEX_NONE);
        if (FAILED(hr))
        {
            app.DebugPrintf("scene navigate fail");
    ...
    {
        hr = XuiSceneNavigateForward(m_hCurrentHudScene[iPad], FALSE, hScene,
                                     XUSER_INDEX_NONE);
        if (FAILED(hr))
        {
            app.DebugPrintf("scene navigate fail");
    ...
    {
        MemSect(5);
        hr = XuiSceneNavigateForward(m_hCurrentScene[idx], bStayVisible, hScene,
                                     iPad);
        MemSect(0);
        if (FAILED(hr))
        {
...
HRESULT
CConsoleMinecraftApp::NavigateBack(int iPad, bool forceUsePad /*= false*/,
                                   EUIScene eScene /*= eUIScene_COUNT*/)
{
    ...
    {
        for (auto it = m_sceneStack[idx].begin(), end = m_sceneStack[idx].end();
             it != end; ++it)
        {
            if (it->first == eScene)
            {
        ...
        while (!m_sceneStack[idx].empty() &&
               m_sceneStack[idx].back().second != hBack)
        {
            m_sceneStack[idx].pop_back();
        }
    }
...
HRESULT
CConsoleMinecraftApp::TutorialSceneNavigateBack(int iPad,
...
HRESULT CConsoleMinecraftApp::CloseXuiScenes(int iPad,
                                             bool forceUsePad /*= false*/)
{
    HRESULT hr;
            ...
            if (pMinecraft->localplayers[i])
            {
                if (g_NetworkManager.IsLocalGame())
                {
                    ProfileManager.SetCurrentGameActivity(
                        i, CONTEXT_PRESENCE_MULTIPLAYEROFFLINE, false);
                }
                else
                {
                    ProfileManager.SetCurrentGameActivity(i, CONTEXT_PRESENCE_MULTIPLAYER,
                                                          false);
                }
            }
        }
    ...
    // because a player leaving can cause other players close scenes to be called,
    // and they may have been crafting, I'll run the presence update for all
    // players
...
HRESULT CConsoleMinecraftApp::CloseAllPlayersXuiScenes()
{
    ...
    // 4J Stu - Fix for #11211 - TCR 015 BAS Sign In Changes: One time-occurrence
    // where users were unable to access any in-game menus after several sign-in
    // changes We only hide them during a save, but this could be interrupted
...
HRESULT CConsoleMinecraftApp::CloseXuiScenesAndNavigateToScene(
    int iPad, EUIScene eScene, void *initData, bool forceUsePad /*= false*/)
{
    HRESULT hr;
...
HRESULT CConsoleMinecraftApp::NavigateToHomeMenu()
{
    ...
    for (unsigned int idx = 0; idx < XUSER_MAX_COUNT; ++idx)
    {
        ...
        {
            XuiSceneNavigateBack(m_hCurrentTutorialScene[idx],
                                 m_hFirstTutorialScene[idx], XUSER_INDEX_ANY);
    ...
    if (pTexPack->hasAudio())
    {
        // need to stop the streaming audio - by playing streaming audio from the
        // default texture pack now
...
void CConsoleMinecraftApp::ReloadChatScene(int iPad, bool bJoining /*= false*/,
                                           bool bForce /*= false*/)
{
    ...
    // Re-create the chat scene so it is the correct size. It starts without any
    // visible lines.
    ...
    if (bForce || (bJoining && iLocalPlayerC == 2) ||
        (!bJoining && iLocalPlayerC == 1) ||
        m_hFirstChatScene[iPad] == m_hCurrentChatScene[iPad])
    {
        if (m_hFirstChatScene[iPad] != m_hCurrentChatScene[iPad])
        {
            XuiSceneNavigateBack(m_hCurrentChatScene[iPad], m_hFirstChatScene[iPad],
                                 iPad);
        }
...
void CConsoleMinecraftApp::ReloadHudScene(int iPad, bool bJoining /*= false*/,
                                          bool bForce /*= false*/)
{
    ...
    // Re-create the hud scene so it is the correct size. It starts without any
    // visible lines.
        ...
        m_hFirstHudScene[iPad] == m_hCurrentHudScene[iPad])
    {
        if (m_hFirstHudScene[iPad] != m_hCurrentHudScene[iPad])
        {
            XuiSceneNavigateBack(m_hCurrentHudScene[iPad], m_hFirstHudScene[iPad],
                                 iPad);
        }
...
void CConsoleMinecraftApp::AdjustSplitscreenScene(
    HXUIOBJ hScene, D3DXVECTOR3 *pvOriginalPosition, int iPad,
...
void CConsoleMinecraftApp::AdjustSplitscreenScene(
    HXUIOBJ hScene, D3DXVECTOR3 *pvOriginalPosition, int iPad, float fXAdjust)
{
    D3DXVECTOR3 vec;
...
HRESULT CConsoleMinecraftApp::AdjustSplitscreenScene_PlayerChanged(
    HXUIOBJ hScene, D3DXVECTOR3 *pvOriginalPosition, int iPad, bool bJoining,
    bool bAdjustXForSafeArea)
{
    int iLocalPlayerC = GetLocalPlayerCount();
    Minecraft *pMinecraft = Minecraft::GetInstance();
    ...
    if ((bJoining && iLocalPlayerC == 2) || (!bJoining && iLocalPlayerC == 1))
    {
        // Fix for #101078 - TCR #15: BAS Sign-in Changes: TU12: Signing out the
        // secondary player's profile during the End Poem in local splitscreen game
        // causes Host invincibility and corruption of End Portal.
...
void CConsoleMinecraftApp::FatalLoadError(void)
{
    ...
    // HANDLE messageBoxThread = CreateThread(NULL, 0,
    // &CMinecraftApp::ShowFatalLoadMessageBoxThreadProc, &MessageBoxOverlap, 0,
    // NULL);
    //
    ...
    case XC_LANGUAGE_GERMAN:
        ...
        aStrings[1] = L"Minecraft: Xbox 360 Edition konnte nicht geladen werden "
                      L"und kann daher nicht fortgesetzt werden.";
        aStrings[2] = L"Spiel verlassen";
        break;
    case XC_LANGUAGE_SPANISH:
        aStrings[0] = L"Error al cargar";
        aStrings[1] = L"Minecraft: Xbox 360 Edition ha experimentado un error al "
                      L"cargar y no puede continuar.";
        aStrings[2] = L"Salir del juego";
        break;
    case XC_LANGUAGE_FRENCH:
        aStrings[0] = L"Échec du chargement";
        aStrings[1] = L"Le chargement de Minecraft: Xbox 360 Edition a échoué : "
                      L"impossible de continuer.";
        aStrings[2] = L"Quitter le jeu";
        break;
    case XC_LANGUAGE_ITALIAN:
        aStrings[0] = L"Errore caricamento";
        aStrings[1] = L"Caricamento \"Minecraft: Xbox 360 Edition\" non riuscito, "
                      L"impossibile continuare.";
        aStrings[2] = L"Esci dal gioco";
        break;
    case XC_LANGUAGE_JAPANESE:
        aStrings[0] = L"ロード エラー";
        aStrings[1] =
            L"Minecraft Xbox 360 版のロードに失敗しました。続行できません";
        aStrings[2] = L"ゲームを終了";
        break;
    case XC_LANGUAGE_KOREAN:
        aStrings[0] = L"불러오기 오류";
        aStrings[1] = L"Minecraft: Xbox 360 Edition을 불러오는 중에 오류가 "
                      L"발생하여 계속할 수 없습니다.";
        aStrings[2] = L"게임 나가기";
        break;
    case XC_LANGUAGE_PORTUGUESE:
        if (XGetLocale() == XC_LOCALE_BRAZIL)
        {
            ...
            aStrings[1] = L"\"Minecraft: Xbox 360 Edition\" falhou ao carregar e não "
                          L"é possível continuar.";
            aStrings[2] = L"Sair do Jogo";
        }
        else
        {
            aStrings[0] = L"Erro de Carregamento";
            aStrings[1] = L"Não foi possível carregar \"Minecraft: Edição Xbox 360\" "
                          L"e não é possível continuar.";
            aStrings[2] = L"Sair do Jogo";
        }
        break;
    case XC_LANGUAGE_TCHINESE:
    ...
    default:
        ...
        aStrings[1] = L"\"Minecraft: Xbox 360 Edition\" has failed to load, and "
                      L"cannot continue.";
    ...
    while (XShowMessageBoxUI(XUSER_INDEX_ANY, aStrings[0], aStrings[1], 1,
                             wszButtons, 0, XMB_ALERTICON, &MessageResult,
                             &MessageBoxOverlap) == ERROR_ACCESS_DENIED)
    {
        Sleep(250);
    }
                                               ...
                                               eFileExtensionType eEXT)
{
    ...
    {
        swprintf(wchTemp, 256, L"%ls.%ls", TMSFileA[i].wchFilename,
                 wchExt[TMSFileA[i].eEXT]);
...
int CConsoleMinecraftApp::LoadLocalTMSFile(WCHAR *wchTMSFile)
{
    ...
    // split the file name into file and extension and find it in out tms file
    // list
    ...
    if (iTMSFileIndex != -1)
    {
        ...
        if (TMSFileA[iTMSFileIndex].pbData ==
            NULL) // if we haven't already loaded it
        {
            swprintf(szResourceLocator, LOCATOR_SIZE, L"%ls#TMSFiles/%ls",
                     m_wchTMSXZP, wchTMSFile);

            hr = XuiResourceLoadAllNoLoc(szResourceLocator,
                                         &TMSFileA[iTMSFileIndex].pbData,
...
TMS_FILE CConsoleMinecraftApp::TMSFileA[TMS_COUNT] = {
    ...
    // avatar items
    {L"AH_0001", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0002", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0003", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0004", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0005", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0006", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0007", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0008", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0009", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0010", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0011", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0012", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AH_0013", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},

    {L"AT_0001", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0002", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0003", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0004", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0005", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0006", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0007", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0008", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0009", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0010", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0011", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0012", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0013", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0014", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0015", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0016", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0017", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0018", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0019", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0020", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0021", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0022", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0023", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0024", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0025", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AT_0026", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},

    {L"AP_0001", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0002", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0003", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0004", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0005", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0006", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0007", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0009", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0010", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0011", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0012", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0013", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0014", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0015", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0016", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0017", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0018", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},

    {L"AP_0019", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0020", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0021", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0022", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0023", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0024", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0025", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0026", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0027", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0028", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0029", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0030", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0031", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0032", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
    {L"AP_0033", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},

    {L"AA_0001", eFileExtensionType_PNG, eTMSFileType_MinecraftStore, NULL, 0,
     0},
...
#else
WCHAR CConsoleMinecraftApp::m_wchTMSXZP[] =
    L"file://UPDATE:/res/TMS/TMSFiles.xzp";
#endif

void CConsoleMinecraftApp::GetFileFromTPD(eTPDFileType eType, PBYTE pbData,
                                          DWORD dwBytes, PBYTE *ppbData,
                                          DWORD *pdwBytes)
{
    PBYTE pbPos = pbData;
    ...
    if (pbData && dwBytes > 0)
    {
        ...
        if (eType == eTPDFileType_Loc)
        {
            ...
            *ppbData = new BYTE[uiDecompSize];
            ...
            Compression::getCompression()->Decompress(
                *ppbData, (UINT *)pdwBytes, &((unsigned int *)pbPos)[2], uiCompSize);
            return;
        }
        else
        {
