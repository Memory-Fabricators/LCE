...
//	printf("Client: handlePreLogin\n");
    ...
    if (!canPlay || !canPlayLocal || !isAtLeastOneFriend || cantPlayContentRestricted)
    {
        ...
        else
        {
            ...
            if (!isFriendsWithHost)
            {
                ui.RequestMessageBox(IDS_CANTJOIN_TITLE, IDS_NOTALLOWED_FRIENDSOFFRIENDS, uiIDA, 1, m_userIndex, NULL, NULL, app.GetStringTable());
            }
            else
            {
                ui.RequestMessageBox(IDS_CANTJOIN_TITLE, IDS_NO_USER_CREATED_CONTENT_PRIVILEGE_SINGLE_LOCAL, uiIDA, 1, m_userIndex, NULL, NULL, app.GetStringTable());
            }
...
{
    shared_ptr<Entity> e = getEntity(packet->entityId);
    if (e == NULL || dynamic_pointer_cast<Mob>(e) == NULL)
    {
        return;
    }
...
#elif defined(_SDL3)
            // TODO: UIScene_TradingMenu (Iggy UI) isn't available on SDL3 yet -
            // trader stays null until the UI layer is ported.
#else
