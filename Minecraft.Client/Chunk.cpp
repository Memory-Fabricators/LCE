...
{
    RenderManager.MatrixTranslate((float)xRenderOffs, (float)yRenderOffs, (float)zRenderOffs);
}
...
void Chunk::rebuild()
{
    ...
    static thread_local unsigned char tileIds[16 * 16 * Level::maxBuildHeight];
    ...
    for (int yy = y0; yy < y1; yy++)
    {
        for (int zz = 0; zz < 16; zz++)
        {
            for (int xx = 0; xx < 16; xx++)
            {
                ...
                if (!((tileId == Tile::rock_Id) || (tileId == Tile::dirt_Id) || (tileId == Tile::unbreakable_Id) || (tileId == 255)))
                {
                    continue;
                }
                ...
                for (int y = y0; y < y1; y++)
                {
                    ...
                    if (tileId > 0)
                    {
                        if (!started)
                        {
                            ...
                            MemSect(31);
                            RenderManager.CBuffStart(lists + currentLayer);
                            MemSect(0);
                            RenderManager.MatrixPush();
                            RenderManager.StateSetDepthMask(true); // 4J added
...
void Chunk::renderBB()
{
    //	RenderManager.CBuffCall(lists + 2);	// 4J - removed - TODO put back in
}
