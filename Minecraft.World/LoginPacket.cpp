...
// Server -> Client
LoginPacket::LoginPacket(const wstring &userName, int clientVersion, LevelType *pLevelType, std::int64_t seed, int gameType, char dimension, BYTE mapHeight, BYTE maxPlayers, char difficulty, INT multiplayerInstanceId, BYTE playerIndex, bool newSeaLevel, unsigned int uiGamePrivileges, int xzSize, int hellScale)
{
    this->userName = userName;
    this->clientVersion = clientVersion;
    this->seed = seed;
...
int LoginPacket::getEstimatedSize()
{
    ...
    return (int)(sizeof(int) + userName.length() + 4 + 6 + sizeof(std::int64_t) + sizeof(char) + sizeof(int) + (2 * sizeof(PlayerUID)) + 1 + sizeof(char) + sizeof(BYTE) + sizeof(bool) + sizeof(bool) + length + sizeof(unsigned int));
}
