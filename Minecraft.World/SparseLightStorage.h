...
// (1) The storage details for the class are now packed into a single std::int64_t, which contains both a pointer to the data that is required and a count of how many planes worth
...
// (2) The data pointer referenced in this std::int64_t points to an area of memory which is 128 + 128 * plane_count bytes long, where the first 128 bytes stoere the plane indices, and
...
class SparseLightStorage
{
  ...
  private:
    ...
    std::atomic<std::int64_t> dataAndCount; // Contains packed-together data pointer (lower 48-bits), and count of lines used (upper 16-bits)
