...
#else
            sprintf(buf, "Average load time: %f (%d)", 0.000001 * (double)timeSpentLoading / (double)loadCount, loadCount);
#endif
            app.DebugPrintf(buf);
#endif
        }
...
#else
            sprintf(buf, "Average save time: %f (%d)", 0.000001 * (double)timeSpentSaving / (double)loadCount, loadCount);
#endif
            app.DebugPrintf(buf);
#endif
        }
