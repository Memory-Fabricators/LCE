...
{
    if (level->getTile(x, y + 1, z) != Tile::hellRock_Id)
    {
        return false;
    }
    if (level->getTile(x, y - 1, z) != Tile::hellRock_Id)
    ...
    if (level->getTile(x, y, z) != 0 && level->getTile(x, y, z) != Tile::hellRock_Id)
    {
        return false;
    }

    int rockCount = 0;
    if (level->getTile(x - 1, y, z) == Tile::hellRock_Id)
    {
        rockCount++;
    }
    if (level->getTile(x + 1, y, z) == Tile::hellRock_Id)
    ...
    if (level->getTile(x, y, z - 1) == Tile::hellRock_Id)
    {
        rockCount++;
    }
    if (level->getTile(x, y, z + 1) == Tile::hellRock_Id)
