...
bool MeleeAttackGoal::canContinueToUse()
{
    ...
    if (!mob->isWithinRestriction(Mth::floor(target.lock()->x), Mth::floor(target.lock()->y), Mth::floor(target.lock()->z)))
    {
        return false;
    }
    return true;
...
void MeleeAttackGoal::tick()
{
    ...
    if (mob->distanceToSqr(target.lock()->x, target.lock()->bb->y0, target.lock()->z) > meleeRadiusSqr)
    {
        return;
    }
    if (attackTime > 0)
