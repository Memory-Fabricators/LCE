...
shared_ptr<Container> ChestTile::getContainer(Level *level, int x, int y, int z)
{
    ...
    if (level->getTile(x - 1, y, z) == id)
    {
        container = shared_ptr<Container>(new CompoundContainer(IDS_CHEST_LARGE, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x - 1, y, z)), container));
    }
    if (level->getTile(x + 1, y, z) == id)
    {
        container = shared_ptr<Container>(new CompoundContainer(IDS_CHEST_LARGE, container, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x + 1, y, z))));
    }
    if (level->getTile(x, y, z - 1) == id)
    {
        container = shared_ptr<Container>(new CompoundContainer(IDS_CHEST_LARGE, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x, y, z - 1)), container));
    }
    if (level->getTile(x, y, z + 1) == id)
    {
        container = shared_ptr<Container>(new CompoundContainer(IDS_CHEST_LARGE, container, dynamic_pointer_cast<ChestTileEntity>(level->getTileEntity(x, y, z + 1))));
    }
