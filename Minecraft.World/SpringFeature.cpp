...
bool SpringFeature::place(Level *level, Random *random, int x, int y, int z)
{
    ...
    if (level->getTile(x, y + 1, z) != Tile::rock_Id)
    {
        return false;
    }
    if (level->getTile(x, y - 1, z) != Tile::rock_Id)
    ...
    if (level->getTile(x, y, z) != 0 && level->getTile(x, y, z) != Tile::rock_Id)
    {
        return false;
    }

    int rockCount = 0;
    if (level->getTile(x - 1, y, z) == Tile::rock_Id)
    {
        rockCount++;
    }
    if (level->getTile(x + 1, y, z) == Tile::rock_Id)
    ...
    if (level->getTile(x, y, z - 1) == Tile::rock_Id)
    {
        rockCount++;
    }
    if (level->getTile(x, y, z + 1) == Tile::rock_Id)
