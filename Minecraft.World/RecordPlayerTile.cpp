...
bool RecordPlayerTile::use(
    Level *level, int x, int y, int z, shared_ptr<Player> player,
                                 ...
                                 int record)
{
    ...
    shared_ptr<RecordPlayerTile::Entity> rte =
...
void RecordPlayerTile::dropRecording(Level *level, int x, int y, int z)
{
    ...
    shared_ptr<ItemEntity> item = shared_ptr<ItemEntity>(new ItemEntity(
        level, x + xo, y + yo, z + zo,
        shared_ptr<ItemInstance>(new ItemInstance(oldRecord, 1, 0))));
    item->throwTime = 10;
