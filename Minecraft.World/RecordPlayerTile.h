  ...
  public:
    class Entity : public TileEntity
    {
      public:
        eINSTANCEOF GetType()
        {
            return eTYPE_RECORDPLAYERTILE;
        }
        static TileEntity *create()
        {
            return new RecordPlayerTile::Entity();
        }
      ...
      public:
        ...
        virtual void save(CompoundTag *tag)
        {
            ...
            if (record > 0)
            {
                tag->putInt(L"Record", record);
            }
            // return true;	// 4J - TODO, in java there is no return type here but
            // we can't change our derived class member function to not have one -
            // investigate 4J Jev, took out the return statement, TileEntity::save is
            // now virtual. Think this fixes above.
        }

        // 4J Added
        shared_ptr<TileEntity> clone()
        {
            shared_ptr<RecordPlayerTile::Entity> result =
                shared_ptr<RecordPlayerTile::Entity>(new RecordPlayerTile::Entity());
