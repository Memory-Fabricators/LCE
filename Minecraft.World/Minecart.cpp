...
{
    entityData->define(DATA_ID_FUEL, 0);
...
void Minecart::readAdditionalSaveData(CompoundTag *base)
{
    ...
    else if (type == Minecart::CHEST)
    {
        ...
        for (int i = 0; i < inventoryList->size(); i++)
        {
            ...
            {
                (*items)[slot] = shared_ptr<ItemInstance>(ItemInstance::fromTag(tag));
            }
        }
    }
}
...
bool Minecart::interact(shared_ptr<Player> player)
{
    ...
    {
        if (rider.lock() != NULL && dynamic_pointer_cast<Player>(rider.lock()) != NULL && rider.lock() != player)
        {
            return true;
        }
        if (!level->isClientSide)
...
void Minecart::setHasFuel(bool fuel)
{
    if (fuel)
    {
        entityData->set(DATA_ID_FUEL, (entityData->getByte(DATA_ID_FUEL) | 1));
    }
    else
    {
        entityData->set(DATA_ID_FUEL, (entityData->getByte(DATA_ID_FUEL) & ~1));
