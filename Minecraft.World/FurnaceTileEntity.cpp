...
void FurnaceTileEntity::save(CompoundTag *base)
{
    ...
    for (unsigned int i = 0; i < items->length; i++)
    {
        if ((*items)[i] != NULL)
        {
            ...
            tag->putByte(L"Slot", (unsigned char)i);
            (*items)[i]->save(tag);
            listTag->add(tag);
        }
    }
        ...
        if (litTime == 0 && canBurn())
        {
            ...
            if (litTime > 0)
            {
                ...
                if ((*items)[FUEL_SLOT] != NULL)
                {
                    ...
                    if ((*items)[FUEL_SLOT]->getItem()->id == Item::coal_Id && (*items)[FUEL_SLOT]->getAuxValue() == CoalItem::CHAR_COAL)
                    {
                        m_charcoalUsed = true;
                    }
...
bool FurnaceTileEntity::canBurn()
{
    ...
    if (!(*items)[RESULT_SLOT]->sameItem_not_shared(burnResult))
    {
        return false;
    }
    if ((*items)[RESULT_SLOT]->count < getMaxStackSize() && (*items)[RESULT_SLOT]->count < (*items)[RESULT_SLOT]->getMaxStackSize())
    {
        return true;
    }
...
void FurnaceTileEntity::burn()
{
    ...
    if ((*items)[INPUT_SLOT]->count <= 0)
    {
        (*items)[INPUT_SLOT] = nullptr;
    }
}
