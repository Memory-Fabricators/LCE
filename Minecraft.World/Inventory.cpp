...
int Inventory::getSlot(int tileId, int data)
{
    ...
    {
        if (items[i] != NULL && items[i]->id == tileId && items[i]->getAuxValue() == data)
        {
            return i;
        }
    }
...
int Inventory::getSlotWithRemainingSpace(shared_ptr<ItemInstance> item)
{
    for (unsigned int i = 0; i < items.length; i++)
    {
        if (items[i] != NULL && items[i]->id == item->id && items[i]->isStackable() && items[i]->count < items[i]->getMaxStackSize() && items[i]->count < getMaxStackSize() && (!items[i]->isStackedByData() || items[i]->getAuxValue() == item->getAuxValue()) && ItemInstance::tagMatches(items[i], item))
        {
            return i;
        }
    }
...
bool Inventory::contains(shared_ptr<ItemInstance> itemInstance)
{
    for (unsigned int i = 0; i < armor.length; i++)
    {
        if (armor[i] != NULL && armor[i]->equals(itemInstance))
        {
            return true;
        }
    }
    for (unsigned int i = 0; i < items.length; i++)
    {
        if (items[i] != NULL && items[i]->equals(itemInstance))
        {
            return true;
        }
    }
