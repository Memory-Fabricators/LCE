...
class CompoundTag : public Tag
{
  ...
  public:
    ...
    void putByte(const wchar_t *name, unsigned char value)
    {
    ...
    void putLong(const wchar_t *name, std::int64_t value)
    {
    ...
    {
        putByte(string, val ? (unsigned char)1 : 0);
    }
    ...
    unsigned char getByte(const wchar_t *name)
    {
    ...
    std::int64_t getLong(const wchar_t *name)
    {
        if (tags.find(name) == tags.end())
        {
