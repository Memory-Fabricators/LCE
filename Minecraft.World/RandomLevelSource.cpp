...
RandomLevelSource::RandomLevelSource(Level *level, std::int64_t seed, bool generateStructures) : generateStructures(generateStructures)
{
...
void RandomLevelSource::buildSurfaces(int xOffs, int zOffs, byteArray blocks, BiomeArray biomes)
{
    ...
    for (int x = 0; x < 16; x++)
    {
        for (int z = 0; z < 16; z++)
        {
            ...
            auto top = b->topMaterial;
            auto material = b->material;
...
LevelChunk *RandomLevelSource::getChunk(int xOffs, int zOffs)
{
    ...
    auto *tileData = (unsigned char *)XPhysicalAlloc(blocksSize, MAXULONG_PTR, 4096, PAGE_READWRITE);
