...
class BiomeSource
{
  ...
  public:
#ifdef __PSVITA__
    static std::int64_t findSeed(LevelType *generator, bool *pServerRunning); // MGH - added pRunning, so we can early out of this on Vita as it can take up to 60 secs							// 4J added
#else
  ...
  public:
    vector<Biome *> getPlayerSpawnBiomes()
    {
        return playerSpawnBiomes;
    }
