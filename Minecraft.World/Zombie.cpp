...
bool Zombie::hurt(DamageSource *source, float dmg)
{
    if (Monster::hurt(source, dmg))
    {
        ...
        if ((target == NULL) && getAttackTarget() != NULL && getAttackTarget()->instanceof(eTYPE_LIVINGENTITY))
        {
            target = dynamic_pointer_cast<LivingEntity>(getAttackTarget());
        }
        if ((target == NULL) && source->getEntity() != NULL && source->getEntity()->instanceof(eTYPE_LIVINGENTITY))
        {
