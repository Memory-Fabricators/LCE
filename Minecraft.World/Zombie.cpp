...
void Zombie::defineSynchedData()
{
    ...
    getEntityData()->define(DATA_BABY_ID, 0);
    getEntityData()->define(DATA_VILLAGER_ID, 0);
    getEntityData()->define(DATA_CONVERTING_ID, 0);
}
...
bool Zombie::isBaby()
{
    return getEntityData()->getByte(DATA_BABY_ID) == 1;
}

void Zombie::setBaby(bool baby)
{
    getEntityData()->set(DATA_BABY_ID, 1);
    updateSize(isBaby());
}

bool Zombie::isVillager()
{
    return getEntityData()->getByte(DATA_VILLAGER_ID) == 1;
}

void Zombie::setVillager(bool villager)
{
    getEntityData()->set(DATA_VILLAGER_ID, (villager ? 1 : 0));
}
...
void Zombie::killed(shared_ptr<Mob> mob)
{
    ...
    {
        if (!level->canCreateMore(GetType(), Level::eSpawnType_Egg))
        {
            return;
        }
...
void Zombie::startConverting(int time)
{
    ...
    getEntityData()->set(DATA_CONVERTING_ID, 1);
...
void Zombie::handleEntityEvent(unsigned char id)
{
...
{
    return getEntityData()->getByte(DATA_CONVERTING_ID) == 1;
}
