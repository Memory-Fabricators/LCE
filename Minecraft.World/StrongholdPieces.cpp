...
void StrongholdPieces::Straight::addChildren(StructurePiece *startPiece, list<StructurePiece *> *pieces, Random *random)
{
    ...
    if (leftChild)
    {
        generateSmallDoorChildLeft((StartPiece *)startPiece, pieces, random, 1, 2);
    }
    if (rightChild)
    {
        generateSmallDoorChildRight((StartPiece *)startPiece, pieces, random, 1, 2);
    }
}
...
void StrongholdPieces::FiveCrossing::addChildren(StructurePiece *startPiece, list<StructurePiece *> *pieces, Random *random)
{
    ...
    if (leftLow)
    {
        generateSmallDoorChildLeft((StartPiece *)startPiece, pieces, random, zOffA, 1);
    }
    if (leftHigh)
    {
        generateSmallDoorChildLeft((StartPiece *)startPiece, pieces, random, zOffB, 7);
    }
    if (rightLow)
    {
        generateSmallDoorChildRight((StartPiece *)startPiece, pieces, random, zOffA, 1);
    }
    if (rightHigh)
    {
        generateSmallDoorChildRight((StartPiece *)startPiece, pieces, random, zOffB, 7);
    }
}
...
bool StrongholdPieces::PortalRoom::postProcess(Level *level, Random *random, BoundingBox *chunkBB)
{
    ...
    if (!hasPlacedMobSpawner)
    {
        ...
        if (chunkBB->isInside(x, y, z))
        {
            ...
            if (entity != NULL)
            {
                entity->setEntityId(L"Silverfish");
            }
        }
    }
