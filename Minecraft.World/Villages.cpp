...
void Villages::createDoorInfo(int x, int y, int z)
{
    ...
    if (dir == 0 || dir == 2)
    {
        ...
        if (canSeeX != 0)
        {
            unclustered.push_back(shared_ptr<DoorInfo>(new DoorInfo(x, y, z, canSeeX > 0 ? -2 : 2, 0, _tick)));
        }
    }
    else
    {
        ...
        if (canSeeZ != 0)
        {
            unclustered.push_back(shared_ptr<DoorInfo>(new DoorInfo(x, y, z, 0, canSeeZ > 0 ? -2 : 2, _tick)));
        }
    }
}
