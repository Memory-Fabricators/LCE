...
RepairMenu::RepairMenu(shared_ptr<Inventory> inventory, Level *level, int xt,
                       int yt, int zt, shared_ptr<Player> player)
{
    ...
    addSlot(new RepairResultSlot(this, xt, yt, zt, resultSlots, RESULT_SLOT, 134,
                                 43 + 4));
...
void RepairMenu::createResult()
{
    ...
    else
    {
        ...
        if (addition != NULL)
        {
            usingBook = addition->id == Item::enchantedBook_Id &&
                ...
                Item::items[result->id]->isValidRepairItem(input, addition))
            {
                ...
                else
                {
                    ...
                    repairItemCountCost = count;
                     ...
                     (result->id != addition->id || !result->isDamageableItem()))
            {
                resultSlots->setItem(0, nullptr);
                cost = 0;
                return;
            ...
            else
            {
                     ...
                     it != additionalEnchantments->end(); ++it)
                {
                    ...
                    for (AUTO_VAR(it2, enchantments->begin()); it2 != enchantments->end();
                         ++it2)
                    {
                        int other = it2->first;
                        if (other != id && !enchantment->isCompatibleWith(
                                               Enchantment::enchantments[other]))
                        {
                            ...
                            if (DEBUG_COST)
                            {
                                app.DebugPrintf("Enchantment incompatibility fee; price is now "
                                                "%d (went up by %d)\n",
                                                price, extra);
                            }
                        }
                    ...
                    {
                        app.DebugPrintf(
                            "Enchantment increase fee; price is now %d (went up by %d)\n",
                            price, fee * extra);
                    }
                }
            ...
            itemName.length() > 0)
        {
            ...
            if (DEBUG_COST)
            {
                app.DebugPrintf("Naming cost; price is now %d (went up by %d)", price,
                                namingCost);
            }
            ...
            if (input->hasCustomHoverName())
            {
                ...
                if (DEBUG_COST)
                {
                    app.DebugPrintf("Already-named tax; tax is now %d (went up by %d)",
                                    tax, (namingCost / 2));
                }
            }
        ...
        for (AUTO_VAR(it, enchantments->begin()); it != enchantments->end(); ++it)
        {
            ...
            if (DEBUG_COST)
            {
                app.DebugPrintf("Enchantment tax; tax is now %d (went up by %d)", tax,
                                (count + level * fee));
            }
        }
        ...
        if (namingCost == price && namingCost > 0 && cost >= 40)
        {
            ...
            app.DebugPrintf("Naming an item only, cost too high; giving discount to "
                            "cap cost to 39 levels");
            cost = 39;
        }
...
shared_ptr<ItemInstance> RepairMenu::quickMoveStack(shared_ptr<Player> player,
                                                    int slotIndex)
{
