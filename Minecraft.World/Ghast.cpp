...
void Ghast::defineSynchedData()
{
    ...
    entityData->define(DATA_IS_CHARGING, 0);
}
...
{
    FlyingMob::tick();
    unsigned char current = entityData->getByte(DATA_IS_CHARGING);
...
void Ghast::serverAiStep()
{
    ...
    {
        unsigned char old = entityData->getByte(DATA_IS_CHARGING);
        unsigned char current = (charge > 10 ? 1 : 0);
        if (old != current)
        {
