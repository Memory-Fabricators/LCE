        ...
        if (onGround)
        {
            ...
            if (level->getTile(xt, yt, zt) != Tile::pistonMovingPiece_Id)
            {
                ...
                else
                {
                    ...
                    {
                        spawnAtLocation(shared_ptr<ItemInstance>(new ItemInstance(tile, 1, Tile::tiles[tile]->getSpawnResourcesAuxValue(data))), 0);
                    }
                }
            }
        }
...
{
    tag->putByte(L"Tile", (unsigned char)tile);
    tag->putByte(L"Data", (unsigned char)data);
    tag->putByte(L"Time", (unsigned char)time);
