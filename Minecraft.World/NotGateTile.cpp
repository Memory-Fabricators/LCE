...
bool NotGateTile::isToggledTooFrequently(Level *level, int x, int y, int z, bool add)
{
    ...
    if (add)
    {
        recentToggles[level]->push_back(Toggle(x, y, z, level->getTime()));
    }
    int count = 0;
...
bool NotGateTile::hasNeighborSignal(Level *level, int x, int y, int z)
{
    ...
    if (dir == 5 && level->getSignal(x, y - 1, z, 0))
    {
        return true;
    }
    if (dir == 3 && level->getSignal(x, y, z - 1, 2))
    {
        return true;
    }
    if (dir == 4 && level->getSignal(x, y, z + 1, 3))
    {
        return true;
    }
    if (dir == 1 && level->getSignal(x - 1, y, z, 4))
    {
        return true;
    }
    if (dir == 2 && level->getSignal(x + 1, y, z, 5))
    {
        return true;
    }
    return false;
...
void NotGateTile::levelTimeChanged(Level *level, std::int64_t delta, std::int64_t newTime)
{
    deque<Toggle> *toggles = recentToggles[level];
