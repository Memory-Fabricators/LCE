...
bool Level::setTileNoUpdate(int x, int y, int z, int tile)
{
    ...
    this->checkLight(x, y, z);
    if (result && (isClientSide || c->seenByPlayer))
    {
        sendTileUpdated(x, y, z);
    }
    return result;
...
bool Level::setDataNoUpdate(int x, int y, int z, int data)
{
    ...
    if (result && (isClientSide || (c->seenByPlayer && sendTileData && maskedBitsChanged)))
    {
        sendTileUpdated(x, y, z);
    }
    return result;
...
bool Level::isUnobstructed(AABB *aabb, shared_ptr<Entity> ignore)
{
    ...
    for (AUTO_VAR(it, ents->begin()); it != itEnd; it++)
    {
        ...
        if (!e->removed && e->blocksBuilding && e != ignore)
        {
            return false;
        }
    }
...
bool Level::isTopSolidBlocking(int x, int y, int z)
{
    ...
    {
        return (getData(x, y, z) & TopSnowTile::HEIGHT_MASK) == TopSnowTile::MAX_HEIGHT + 1;
    }
    return false;
}
...
int Level::getExpectedSkyColor(lightCache_t *cache, int oc, int x, int y, int z, int ct, int block)
{
    ...
    if (block == 255)
    {
        return 0; // 4J added as optimisation
    }
...
int Level::getExpectedBlockColor(lightCache_t *cache, int oc, int x, int y, int z, int ct, int block, bool propagatedOnly)
{
    ...
    if (block >= 15)
    {
        return expected; // 4J added as optimisation
    }
            ...
            if (expected > c)
            {
                ...
                if (xd + yd + zd < 17 && tcc < (32 * 32 * 32) - 6) // 4J - 32 * 32 * 32 was toCheck.length
                {
                    ...
                    if ((x - 1) >= minXZ)
                    {
                        ...
                        {
                            toCheck[tcc++] = (((x - 1 - xc) + 32)) + (((y - yc) + 32) << 6) + (((z - zc) + 32) << 12);
                        }
                    }
                    if ((x + 1) <= maxXZ)
                    {
                    ...
                    if ((y - 1) >= 0)
                    {
                        ...
                        {
                            toCheck[tcc++] = (((x - xc) + 32)) + (((y - 1 - yc) + 32) << 6) + (((z - zc) + 32) << 12);
                        }
                    }
                    ...
                    if ((z - 1) >= minXZ)
                    {
                        ...
                        {
                            toCheck[tcc++] = (((x - xc) + 32)) + (((y - yc) + 32) << 6) + (((z - 1 - zc) + 32) << 12);
                        }
                    }
                    if ((z + 1) <= maxXZ)
                    {
...
bool Level::mayPlace(int tileId, int x, int y, int z, bool ignoreEntities, int face, shared_ptr<Entity> ignoreEntity)
{
    ...
    if (aabb != NULL && !isUnobstructed(aabb, ignoreEntity))
    {
        return false;
    }
    if (targetTile != NULL &&
    ...
    if (targetTile != NULL && targetTile->material == Material::decoration && tile == Tile::anvil)
    {
        return true;
    }
