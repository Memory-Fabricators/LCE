...
{
    setMaxStackSize(16); // 4J-PB brought forward. It is 64 on PC, but we'll never
                         // be able to place that many
...
int MonsterPlacerItem::getColor(shared_ptr<ItemInstance> item,
                                int spriteLayer)
{
...
shared_ptr<Entity> MonsterPlacerItem::canSpawn(int iAuxVal, Level *level,
                                               int *piResult)
{
    ...
    if (newEntity != NULL)
    {
        ...
        default:
            ...
            {
                // 4J-PB - check if the player is trying to spawn an enemy in peaceful
                // mode
...
bool MonsterPlacerItem::useOn(shared_ptr<ItemInstance> itemInstance,
                              shared_ptr<Player> player, Level *level, int x,
...
shared_ptr<Entity> MonsterPlacerItem::spawnMobAt(Level *level, int mobId,
    ...
    for (int i = 0; i < SPAWN_COUNT; i++)
    {
        ...
        if (mob)
        {
            newEntity->moveTo(x, y, z,
                              Mth::wrapDegrees(level->random->nextFloat() * 360), 0);
            newEntity->setDespawnProtected(); // 4J added, default to being protected
