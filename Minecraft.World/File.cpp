...
{
#if defined _SDL3
    int result = unlink(wstringtofilename(getPath())) == 0;
...
{
#if defined _SDL3
    return ::mkdir(wstringtofilename(getPath()), 0755) == 0;
#elif defined _UNICODE
...
#if defined _SDL3
        struct stat st;
        if (stat(wstringtofilename(pathToHere), &st) != 0)
        {
            if (::mkdir(wstringtofilename(pathToHere), 0755) != 0)
            {
                return false;
            }
        }
...
#if defined _SDL3
    struct stat st;
    return stat(wstringtofilename(getPath()), &st) == 0;
#elif defined _UNICODE
...
#if defined _SDL3
    int result = rename(sourcePath.c_str(), destPath) == 0;
#elif defined _DURANGO
    __debugbreak(); // TODO
    BOOL result = false;
#else
...
vector<File *> *File::listFiles() const
{
    ...
    if (dir)
    {
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            ...
            {
                vOutput->push_back(new File(*this, filenametowstring(entry->d_name)));
            }
        }
        closedir(dir);
    }
...
vector<File *> *File::listFiles(FileFilter *filter) const
{
    ...
    if (dir)
    {
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            if (strcmp(entry->d_name, ".") != 0 && strcmp(entry->d_name, "..") != 0)
            {
                File thisFile = File(*this, filenametowstring(entry->d_name));
                if (filter->accept(&thisFile))
                {
                    File storageFile = thisFile;
                    vOutput->push_back(&storageFile);
...
#if defined _SDL3
    struct stat st;
    return stat(wstringtofilename(getPath()), &st) == 0 && S_ISDIR(st.st_mode);
#elif defined _UNICODE
...
#elif defined _SDL3
    struct stat st;
    if (stat(wstringtofilename(getPath()), &st) == 0)
    {
