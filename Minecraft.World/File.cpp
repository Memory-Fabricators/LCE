...
File::File(const wstring &parent,
           const wstring &child) //: m_abstractPathName( child  )
{
    m_abstractPathName =
...
bool File::renameTo(File dest)
{
    ...
    // sourcePath and destPath having the same value. The solution here is to make
    // a copy of the sourcePath by storing it in a std::string
...
vector<File *> *File::listFiles() const
{
    ...
    uint32_t count = 0;
    err =
        cellFsGetDirectoryEntries(fd, &de, sizeof(CellFsDirectoryEntry), &count);
    if (count != 0)
    {
        do
        {
...
#ifdef _UNICODE
    BOOL result = GetFileAttributesEx(getPath().c_str(),     // file or directory name
