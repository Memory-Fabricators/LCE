...
int LevelChunk::setBlocksAndData(byteArray data, int x0, int y0, int z0, int x1, int y1, int z1, int p, bool includeLighting /* = true*/)
{
    ...
    if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += lowerBlocks->setDataRegion(data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p, includeLighting ? NULL : tileUpdatedCallback, this, 0);
    }
    if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += upperBlocks->setDataRegion(data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p, includeLighting ? NULL : tileUpdatedCallback, this, Level::COMPRESSED_CHUNK_SECTION_HEIGHT);
    }
    /*
    for (int x = x0; x < x1; x++)
        for (int z = z0; z < z1; z++)
    ...
    if (y0 < Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += lowerData->setDataRegion(data, x0, y0, z0, x1, min(compressedHeight, y1), z1, p, includeLighting ? NULL : tileUpdatedCallback, this, 0);
    }
    if (y1 > Level::COMPRESSED_CHUNK_SECTION_HEIGHT)
    {
        p += upperData->setDataRegion(data, x0, max(y0 - compressedHeight, 0), z0, x1, y1 - Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z1, p, includeLighting ? NULL : tileUpdatedCallback, this, Level::COMPRESSED_CHUNK_SECTION_HEIGHT);
    }
...
int LevelChunk::getHighestNonEmptyY()
{
    ...
    if (upperBlocks)
    {
        highestNonEmptyY = upperBlocks->getHighestNonEmptyY() + Level::COMPRESSED_CHUNK_SECTION_HEIGHT;
    }
