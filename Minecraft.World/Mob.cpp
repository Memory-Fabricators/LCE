...
bool Mob::hurt(DamageSource *source, int dmg)
{
    ...
    if (level->isClientSide && dynamic_cast<EntityDamageSource *>(source) == NULL)
    {
        return false;
    }
    noActionTime = 0;
    ...
    if (sound)
    {
        ...
        if (source != DamageSource::drown && source != DamageSource::controlledExplosion)
        {
            markHurt();
        }
        if (sourceEntity != NULL)
...
{
    if (health < Short::MIN_VALUE)
    {
        health = Short::MIN_VALUE;
    }
    health = tag->getShort(L"Health");
    if (!tag->contains(L"Health"))
    {
        health = getMaxHealth();
    }
...
void Mob::handleEntityEvent(unsigned char id)
{
